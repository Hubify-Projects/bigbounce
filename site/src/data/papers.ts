...
export const researchPrograms: ResearchProgram[] = [
  ...
  },
  {
    ...
    result:
      "P1N (v1N.0.9) merges P1A and P1C into one gr-qc/CQG paper: the derived axial spin-spin contact term reduces exactly to Po\u0142awski's torsion-bounce (Hehl\u2013Datta) operator as \u03b3\u2192\u221e (the positive result, now stated as an operator identification only \u2014 see limitation), while the same algebraic elimination closes four candidate dark-energy routes (the negative result, now stated without the withdrawn Po\u0142awski-rebuttal claim). \u2014 SIGN-OFF HOLD RETAINED: both ESSENTIAL findings from the 2026-09-22 CONFIRM board were confirmed real and closed by WITHDRAWING the claims they targeted, not by restoring them.",
    limitation:
      "A dedicated derivation lane (bb-LS13-p1n-essentials, 2026-09-22) confirmed both CONFIRM-board findings as real: the contact term's equation-of-state assumption (w=-1) contradicts its own density parametrization and forces the corrected w=+1, which \u2014 taken with the manuscript's own stated configuration \u2014 makes the term gravitationally ATTRACTIVE (the published Kerlick 1975/O'Connell 1977 ECSK result for the Dirac field), so the independent repulsive-sign argument is withdrawn (DP1N-60); and the claimed 'direct, quantitative rebuttal' of Po\u0142awski's dark-energy proposal evaluated the wrong physical quantity by ~74 orders of magnitude against a misdescribed source and is withdrawn with no replacement rebuttal (DP1N-61). Neither withdrawal touches the \u03b3\u2192\u221e operator identification, the perturbation-transparency theorem, the barrier catalog, or the NJL gap-equation closure. Readiness dropped 95\u219285 COMPUTED. A separate lane (v1N.0.9) independently verified the eq:fierz_row scalar sign that drives G_s<0 (the NJL no-condensate closure) is CORRECT \u2014 confirmed by exact rational arithmetic and a blind adjudicator; no science moved, readiness held. P1A and P1C remain on disk, frozen and unedited, as this paper's archived lineage \u2014 not separate live submission targets.",
    leadSlug: "paper-1n",
    supportSlugs: [],
    status: "One closed-line paper, readiness 85 COMPUTED, SIGN-OFF HOLD RETAINED (2026-09-22, unchanged at v1N.0.9): DP1N-60/61 closed by withdrawing the repulsive-sign bridge and the Po\u0142awski-rebuttal claim, not by restoring them. The Fierz-row sign question is now resolved (CORRECT, v1N.0.9). A fresh exact-version confirmation board remains before readiness can move again.",
  },
  {
...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    version: "v1N.0.9",
    lastUpdated: "2026-09-22",
    tldr: "Merges P1A and P1C into a single gr-qc/CQG paper. The derived axial spin-spin contact term reduces exactly to Popławski's torsion-bounce (Hehl–Datta) operator as γ→∞ — now stated as an operator identification only — while the same algebraic elimination closes four candidate dark-energy routes (the negative result, now stated without the withdrawn rebuttal claim). SIGN-OFF HOLD RETAINED (2026-09-22, unchanged from v1N.0.8): DP1N-60/61 closed by WITHDRAWAL, not restoration. DP1N-60: the contact term's equation of state is w=+1 (stiff), not w=-1 as stated; taken with the manuscript's own stated configuration, the corrected EOS makes the term gravitationally ATTRACTIVE — the published Kerlick (1975)/O'Connell (1977) ECSK result for the Dirac field — so the independent repulsive-sign argument is withdrawn. DP1N-61: the claimed 'direct, quantitative rebuttal' of Popławski's dark-energy proposal evaluated the wrong physical quantity by ~74 orders of magnitude against a misdescribed source and is withdrawn, with no replacement rebuttal manufactured. v1N.0.9 independently verified the Fierz-row scalar sign that drives G_s<0 (the separate NJL no-condensate closure) is CORRECT — confirmed by exact rational arithmetic and a blind adjudicator — and qualified an over-broad 'unique' claim; no science moved, readiness held. Readiness 95→85 COMPUTED (unchanged at v1N.0.9).",
    path: publicationPath({}),
    pages: "12",
    refs: "—",
    readiness: 85,
    ...
    description:
      "Track B of the 2026-09-02 portfolio restructure (directive R3): P1A's algebraic Cartan elimination (spin-sourced axial contact term, zero-spin scalar branch) merged with P1C's 14-entry structural no-go survey (Route-2/Route-3 amplitude-budget closures, six-member dimension-4 parity-odd operator list) into one Note that states both readings of the same result — what minimal ECH gravity does for the bounce, and what it cannot do for dark energy. As of v1N.0.8, the Introduction and Discussion identify the derived contact term with Popławski's spin-spin bounce mechanism (arXiv:1007.0587; arXiv:1102.5667) as an operator identification only (exact as γ→∞) — the paper does not independently establish the sign of pressure at finite γ. As of v1N.0.9, the separate NJL scalar-projection sign (G_s<0, driving the no-condensate closure) has been independently verified rather than merely presumed; see the v1N.0.9 changelog entry.",
    keyResults: [
      "Minimal ECH Cartan elimination yields the axial contact term −(3κ/16)[γ²/(1+γ²)]J₅² in the stated convention",
      "The derived contact term is the same operator as Popławski's torsion-bounce mechanism (Hehl–Datta term, exact as γ→∞) — the positive result; the paper does not independently establish the sign of pressure at finite γ (v1N.0.8)",
    ...
    remainingWork: [
      "SIGN-OFF HOLD RETAINED (2026-09-22, unchanged at v1N.0.9): DP1N-60/61 CLOSED-BY-WITHDRAWAL, not restored — the repulsive-sign bridge (§II) and the Popławski-rebuttal claim (§VII.D + abstract) are withdrawn with no replacement claim manufactured; readiness 95→85 COMPUTED",
      "A fresh exact-version confirmation board on v1N.0.9 is needed (spends a new directive-R2 round) to re-verify the withdrawal reads cleanly to an independent referee before readiness can climb again",
      "The Fierz row's scalar-channel sign (drives G_s, the NJL no-condensate result) has now been independently verified CORRECT as of v1N.0.9 (lane bb-LS15-fierz-sign) — this item is closed and takes readiness no lower, but does not itself raise it",
      ...
      "CQG submission kit assembled: SSOT/CQG_SUBMISSION_KIT_P1N_2026-09-18.md (abstract, categories, cover letter, DAS) — do not submit until the sign-off hold lifts and a fresh board confirms",
      "arXiv tarball rebuilt + standalone-smoke-tested: SSOT/arxiv_tarballs/paper1bc_ech_note_arxiv_v1N.0.9.tar.gz",
      "Houston sign-off has not been sought and should not be until a fresh board confirms v1N.0.9",
    ],
    preprintId: "HUBIFY-2026-001N",
    pdfMeta: "PDF · 12 pp · v1N.0.9 · created Sep 22, 2026 · md5 adc34e683bbc1f0c00b5f937fb63cc42 — SIGN-OFF HOLD RETAINED: DP1N-60/61 closed by withdrawal (readiness 95→85 COMPUTED), not restored.",
    changelog: [
      "v1N.0.9: lane bb-L3e-p1n-fierz-qualifier applied the bb-LS15-fierz-sign derivation's verification record. The eq:fierz_row scalar-channel sign (which drives G_s<0, the separate NJL no-condensate closure) was independently re-derived from scratch — row (S,V,T,A,P)=(1,½,0,½,−1), exact rational arithmetic, agreeing across 3 Dirac representations × 2 signatures × 2 γ⁵ conventions, F²=1 to 6.7e-16 — and found CORRECT, confirmed against Popławski arXiv:1005.0893's own (54 meV)⁴ (reproduced to 53.71 meV⁴; the opposite sign would give negative ρ_Λ), and independently CONFIRMED by a blind adjudicator (opus tier — the fable tier was out of usage credit, recorded honestly rather than silently substituted). Every G_s-dependent claim in the paper (abstract item (i), eq:Gs, eq:gap, §VII.D, Conclusions) is now independently verified; no claim moves. One MINOR wording item closed: 'unique' qualified to the declared direct-channel projection at colour/flavour multiplicity N_cN_f>1 (operator rank 3 at N=1, rank 5 at N≥2, per scripts/multispecies_rank.py — matches eq:gap's own N_cN_f/(4π²) assumption). NOT claimed: the eq:4fermi prefactor was not re-derived here (reported as verified-elsewhere, ech_torsion_onshell_2026_08_08.py) — left open, named, low-priority. Directive-G hygiene: 4-pass recompile, 0 undef refs, 12pp unchanged, pre-existing 4.5pt overfull hbox unchanged; three-way byte-identical mirror; /artifact-link-verify 54/54 URIs resolve (unchanged set); /bib-tarball-rebuild 32==32==32, tarball rebuilt + standalone-smoke-tested. Readiness HOLDS at 85 COMPUTED — this item takes P1N no lower and does not raise it either; SIGN-OFF HOLD RETAINED (unrelated to DP1N-60/61).",
      "v1N.0.8: a dedicated derivation lane (bb-LS13-p1n-essentials, independently re-verified by a blind adjudicator) confirmed both v1N.0.7 ESSENTIAL findings as REAL and closed them by WITHDRAWAL. DP1N-60: the manuscript's w=-1 equation of state contradicts its own density parametrization; the corrected w=+1 (stiff), taken with the manuscript's own stated configuration, makes the term gravitationally ATTRACTIVE — the published Kerlick (1975)/O'Connell (1977) ECSK result for the Dirac field — so the independent repulsive-sign argument is withdrawn from §II, the Introduction, Discussion, and Conclusions (which had independently restated it); the γ→∞ operator identification is untouched and is what the title's 'does for the bounce' half now rests on alone. DP1N-61: the 'direct, quantitative rebuttal' of Popławski's dark-energy proposal evaluated the wrong physical quantity (~74 orders of magnitude off, against a misdescribed source) and is withdrawn from §VII.D and the abstract with NO replacement rebuttal manufactured — §VII.D now states plainly that Popławski's condensate-based mechanism lies outside the Route-1 channel this paper closes. Two references added (Kerlick1975, OConnell1977). Directive-G hygiene: 4-pass recompile, 0 undef refs, 12pp (up from 11, real content), pre-existing 4.5pt overfull hbox unchanged; three-way byte-identical mirror; /artifact-link-verify 54/54 URIs resolve; /bib-tarball-rebuild 32==32==32, tarball rebuilt + standalone-smoke-tested. Readiness 95→85 COMPUTED; SIGN-OFF HOLD RETAINED. New flagged item (not yet a disposition): the Fierz-row scalar-channel sign driving G_s was not checked and is separately being derived (bb-LS15-fierz-sign).",
    ...
    artifacts: [
      { label: "Read PDF", href: "/papers/paper1bc_ech_note_v1N.0.9.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/paper1bc_ech_note_v1N.0.9.pdf", kind: "secondary", download: true },
      {
        label: "LaTeX source",
  ...
  },
  {
    ...
    version: "v3M.0.32",
    lastUpdated: "2026-09-22",
    tldr: "DA3M-VI-01 closed: two now-false statements in Sec. VI (wide-angle corrections 'not applied', only '2 of 5' imaging-systematics splits run) are corrected against the lab's own LEDGER4_RESULT_v4/v5 artifacts -- wide-angle is a genuine null (sources only odd multipoles; the official window uses ell=0,2,4 only) and all five splits were run. The real reason the QSO reproduction's sigma=25 exceeds the published 9.0: the response lever b1-p is 0.649 at p=1.6 vs 1.249 at p=1.0 (1.9x weaker constraint), the published number combines LRG+QSO while this fit is QSO-only, and only b1/f_NL are free against DESI's full nuisance marginalisation. Ledger row 4 (LRG channel) opened in the same bundle: f_NL^loc = -3.4+-5.7 (p=1.0, three official z-bins), AGREES with QSO at T=-0.159, all 15 systematics rows null, WEIGHT_SYS (QSO's dominant -3.05 sigma systematic) null in every LRG bin -- still cannot separate -35/16 from -35/8 (0.38 sigma apart), printed as a null with equal prominence. Board on this exact PDF, authorized by D-A3-15, follows in the same lane.",
    path: publicationPath({}),
    pages: "25",
    refs: "\u2014",
    readiness: 75,
    ...
    pdfMeta: "PDF \u00b7 25 pp \u00b7 v3M.0.32 \u00b7 created Sep 22, 2026 \u00b7 md5 62153f02dcd5cd3158db2e69b2634f4f \u2014 DA3M-VI-01: two now-false Sec. VI statements (wide-angle 'not applied', '2 of 5' splits) corrected against LEDGER4_RESULT_v4/v5 (genuine null; all five splits run). Ledger row 4 (LRG channel) opened: f_NL^loc=-3.4\u00b15.7 (p=1.0), AGREES with QSO at T=-0.159, all 15 systematics rows null, still cannot separate -35/16 from -35/8 (0.38 sigma). Readiness 75 COMPUTED; the D-A3-15-authorized confirmation board runs on this exact PDF next.",
    changelog: [
      "v3M.0.32: lane L1f-a3m-lrg-board closed DA3M-VI-01 [DEFECT] and opened ledger row 4, both sourced from lane LS11-row4-lrg's PROPAGATION_NOTE.md but independently re-verified against the underlying committed artifacts before landing (not taken on the note's word). Sec. VI previously attributed the QSO reproduction's sigma=25 (vs. published 9.0) to wide-angle corrections 'not applied' and only '2 of 5' systematics splits run -- both false: LEDGER4_RESULT_v4 implements and runs the wide-angle correction and finds it a genuine null (sources only odd multipoles; the official window matrix carries ell=0,2,4 only, confirmed by source inspection, a runtime ValueError guard, and explicit numeric construction to machine precision); LEDGER4_RESULT_v5 closes the remaining two of five splits (WEIGHT_SYS, Galactic latitude), so all five have been run. The real explanation, independently recomputed from the committed fit-point JSONs (fnl_official_p16_point.json / p10_point.json), not copied from the note: b1=2.24932 in both fits, giving the response lever b1-p=0.649 at p=1.6 vs 1.249 at p=1.0 (ratio 1.925, matching the claimed 1.9x to the fit's own digits); the published number combines LRG+QSO while this fit is QSO-only; and only b1,f_NL are free (n_shot=0 fixed) against DESI's full nuisance marginalisation. Ledger row 4 (LRG channel) opened as a science addition: DESI DR1 LRG, official window+measured P_ell+EZmock covariance, three z-bins -- f_NL^loc=-3.4+-5.7 (p=1.0, three-bin headline; -1.1+-6.9 restricted to the published 0.6<z<1.1 sample). AGREES with QSO at T=-0.159 (0.16 sigma). Linear biases rise monotonically with z (1.87, 2.05, 2.21) without being imposed. All 15 systematics rows null (closest: E(B-V) at 0.8<z<1.1, -0.995 sigma after the disclosed sqrt2 correction, named not buried). WEIGHT_SYS -- QSO's dominant systematic at -3.05 sigma -- is a null in every LRG bin. Still cannot discriminate -35/16 from -35/8 (0.38 sigma apart at p=1.0): printed as a null for the flagship question with equal prominence, not softened. Directive-I6 figure sweep: neither of the paper's two figures (sigw_nhz_from_lab_spectrum, pbh_compaction_fnl) bakes a Sec. VI number; no regeneration needed. Reproducibility statement updated: QSO manifest citation corrected from the superseded LEDGER4_RESULT_v3 to LEDGER4_RESULT_v5 (v3 kept as record), new LRG citation added (research/desi_png_reproduction/lrg_channel_2026_09_22/, manifest ledger4-desi-dr1-lrg-fnl-channel.json). Directive G: 4-pass, 0 errors, 0 undef refs/citations, 24->25 pp, two small pre-existing overfull hboxes unchanged (3.90pt, 2.16pt, neither touched by this bundle), /latex-audit visual PASS (pp 14/15/21/25). Three-way byte-identical mirror (source/site/public/papers/public/papers, md5 62153f02dcd5cd3158db2e69b2634f4f, sha256 563aaf399474e34314bb61b5a14fe1c9c1b2cd9037a4b9e72fc4d88aac3c2120), confirmed against an isolated fresh 4-pass recompile (pdftotext-content-identical). Convex paperVersions:bump UNAVAILABLE (spending limit) -- queued to CONVEX_BACKFILL_QUEUE_2026-09-21.md. Precondition met for D-A3-15's authorized confirmation board, which runs on this exact PDF next in the same lane. Readiness held at the COMPUTED cap 75.",
      "v3M.0.31: D-A3-15 -- lane bb-L1e-a3m-pbh-rescope applied lane LS12-pbh-perturbativity's pre-registered pointwise perturbativity computation (pre-registration committed alone at b289aa0f before any number existed) to main.tex, closing A3M's last open R12 ESSENTIAL item (DA3M-R12-06) BY COMPUTATION rather than honest disclosure. Every number verified against the committed results.json before use. The diagnostic 1.2|f_NL|sigma_r was re-integrated from the compaction script's own Eq.-53 variance integral at each of the 144 headline points' own required amplitude (five validation gates, three exact to machine zero: G2 all-255-point gamma_cr reproduction, G3 the three committed calibration rows bit-for-bit, G4 the 27-point grid's gamma_cr; G1 wide-vs-narrow integrator 5.7e-16; G5 headline population exact). Result: the criterion (eps<=1 at both legs) holds at only 62 of 144 points -- the -35/8 leg fails at 82 points (all 48 at C_th=0.6), the -35/16 leg at 1. The Channel II required-amplitude ratio is RE-SCOPED, not withdrawn: 1.84+-0.03 (n=144) -> 1.81+-0.02 (n=62) in the abstract, Eq. (eq:pbh_ratio), Sec. V C, the Regime-of-validity passage, and Table V's caption. What separates perturbative from non-perturbative is NOT gamma_cr (P and H\\P span the identical gamma_cr range) -- it is the formation threshold C_th. Table V's caption and dagger footnote also corrected an INVERTED non-perturbative-branch label: measured pointwise, the diagnostic RISES with gamma_cr, so the headline window is the MORE controlled of the two, the opposite of the prior caption's implication. Reported with equal prominence against the paper: no point of the 144 reaches eps<=0.5 (population marginal throughout); the ratio correlates with the diagnostic at Pearson +0.83, so the subset value is threshold-conditional (1.798 at a cut of 0.9 to 1.837 at 1.4), not cut-independent. Channel II's scientific conclusion is UNCHANGED: still 7.0 dex short, f_PBH=0, a NULL (ledger row 11 / A3-1b untouched). Directive-I6 figure sweep: both includegraphics in main.tex inspected via their generator scripts -- neither bakes the superseded value; the row11_gammacr_extension.png the propagation note names as baking a stale '1.7-1.9' band is confirmed real in its own generator script but is not included anywhere in main.tex, a finding stated plainly rather than silently dropped; no figure regeneration needed. Directive G: 4-pass, 0 errors, 0 undef refs/citations, 24 pp (grew from 23), max overfull hbox 3.90pt (unchanged pre-existing; a new 118pt table-footer overflow from this bundle's first-draft caption text was caught by /latex-audit and fixed by shortening + scriptsize before commit), visual PASS on pages 1/11-12/17/21/23 at 130dpi. Three-way md5 d669a73cd1ec8e330e212963a5930c89, sha256 6f84f2600154d17e528184672bbd4eab7c29c85855e960de1868009abad3f21b, verified served == fresh-compile on all three paths. Convex paperVersions:bump QUEUED to CONVEX_BACKFILL_QUEUE_2026-09-21.md (Convex disabled, spending limit). No review board run this bundle (out of scope); directive R2 is re-armed by D-A3-15 -- one confirmation board on the exact v3M.0.31 PDF is now authorized, not spent, the director's call. Readiness held at the COMPUTED cap 75.",
    ...
    artifacts: [
      { label: "Read PDF", href: "/papers/a3_multichannel_arxiv_v3M.0.32.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/a3_multichannel_arxiv_v3M.0.32.pdf", kind: "secondary", download: true },
      {
        label: "LaTeX source",
  ...
  },
  {
    ...
    lastUpdated: "2026-09-22",
    tldr:
      "Publishes a provenance-sealed candidate catalogue of 1,244 spectroscopically unusual DESI DR1 objects (the top 5.7×10⁻⁵ of 21,793,550 science-target spectra), now retitled to carry the null result explicitly. The anomaly score is uncorrelated with exposure quality/brightness, and is almost entirely explained by the blue spectrograph arm's own residual (full-model R²=0.78, collapsing to R²=0.01 without it; the blue arm alone, univariately, explains R²=0.20). A known-object recovery benchmark clears no reference class at its pre-declared bar. The taxonomy was corrected 2026-09-22 (spherical sky embedding fixes an RA-wrap artifact, 25→23 clusters/8→9 families) and a directive-R2 board on that exact PDF closed 9 further real findings, most consequentially removing leaked internal review-process prose the manuscript had carried into its own body and abstract — leaving the honest verdict unchanged: 'refutes one of four, cannot decide the other three.'",
    path: publicationPath({}),
    pages: "19",
    refs: "—",
    readiness: 80,
    ...
    keyResults: [
      ...
      "Anomaly score uncorrelated with exposure quality/brightness (Spearman |ρₛ|≤0.10 against 5 independent measures); a committed variance-decomposition script shows the blue spectrograph arm's own residual explains the full-model R²=0.78 of the score's variance, collapsing to R²=0.01 without it (94.9% of released objects have it as their worst-fit camera), while the blue arm alone, univariately, explains R²=0.20 — a distinction the 2026-09-22 confirmation board found conflated in the summary text — evidence favoring a fixed-wavelength calibration origin, not yet closed",
      "23-cluster/9-family descriptive taxonomy (built over 675 no-SIMBAD/NED-counterpart objects, corrected 2026-09-22 to a spherical sky embedding that eliminates an RA-wrap artifact) is structured but not separated in the model's own latent space — a stratification of the candidate list, not physical classes",
    ...
    figures: [
      ...
      "Mollweide sky map",
      "Per-camera residual diagnostics + blue-arm variance decomposition (full-model R²=0.78→0.01; univariate R²=0.20)",
      "V12 latent-space silhouette vs. permutation null",
    ],
    remainingWork: [
      "12 findings (DAF-41 through DAF-52) are genuinely real per independent verification but need new computation, a literature search this lane cannot responsibly attach, or exceed a single lane's scope: an abstract-drift sweep across five load-bearing sentences, the undocumented wavelength-to-bin calibration, an uncited f_NL=-35/16 value, a recovery-benchmark match-radius asymmetry (1.5″ vs. 3″), a frame-concentration test with no null, an overstated 'five independent measures' claim, a stellar-mass-systematic citation/extrapolation, Table IV schema inconsistencies, under-specified follow-up-tier rules, an unspecified OT-1 template source, an uncited footprint-area comparison, and a missing external recovery-rate benchmark — see DISPOSITIONS/AF.md for the full ledger",
      ...
      "Directive R2: board budget spent again this wave (the DAF-19 closure was the intervening-science decision that authorized it); readiness 80 COMPUTED — no further board without another intervening science/scope decision",
    ],
    preprintId: "HUBIFY-2026-AF",
    pdfMeta: "PDF · 19 pp · vAF.0.8 · created Sep 22, 2026 · md5 795bba62d57aada93bdf9428388c06bd — directive-R2 board closed 9 real findings on the DAF-19-corrected taxonomy, most consequentially removing leaked internal review-process prose.",
    changelog: [
      "vAF.0.8: directive-R2-authorized board (lane LAF5) on the exact vAF.0.7 PDF — Grok API grok-4.3 (REJECT), Gemini API gemini-3.1-pro-preview (ACCEPT WITH MINOR CORRECTIONS), one Claude-opus verdict-blind referee (MAJOR REVISIONS, 17 ESSENTIAL/17 MAJOR/27 MINOR, the most exhaustive single-leg report this paper has received; Fable was out of usage credit, opus substituted). All three legs converged on the manuscript's own leaked internal review-process prose. 9 genuinely-new-real findings closed with real edits: removed a literal '2026-09-22 confirmation board flagged...' sentence from §V.C and an open bibliography-audit trail from the Acknowledgments; removed the abstract's comparison to an unpublished 'earlier construction'; resolved a genuine self-contradiction in §VI.A about which 'manifest' recorded the taxonomy hyperparameters; rewrote Table VIII's caption to define every column and explain how two families can share a printed summary while remaining distinct; disclosed the corrected taxonomy's separate provenance record (with its real SHA-256) outside the original sealed selection chain; disclosed feature standardisation (StandardScaler, verified against the committed script — also FALSIFIED a claim that the pipeline crashes as described); bumped the latent-space permutation null from 50 to 10,000 draws at zero marginal cost, sharpening the empirical p from <1/50 to 0.0041; reconciled §VI.A/§VI.B's descriptions of the taxonomy's structure; fixed three broken cross-references and one PDF-rendering kerning artifact. Two more cheap fixes folded in: Fig. 6's caption falsely claimed the score was uncorrelated with two other cameras when one is significant at p≈5×10⁻⁷; a phrase inferring a training-set size from a model filename was reworded. 5 further findings independently FALSIFIED against source (Grok repeated a backwards-date claim a prior round already caught; two claims that abstract statements are missing when they are already present verbatim; a claim that Table IV's placeholder columns need marking when they already are). 12 further genuinely-real findings (DAF-41 through DAF-52) named honestly for future dedicated closure lanes rather than closed under time pressure. A few editorial/genre-reframing asks (reject an honestly-scoped null result; AAS house-style abstract length) dispositioned OPINION/OUT-OF-SCOPE. Directive-G: 4-pass compile, 0 errors, 0 undefined refs, 0 overfull hboxes >10pt, 19pp (up from 18, real disclosure content); visual audit of every touched page clean; 37 cited = 37 bibitems; standalone tarball-style compile text-identical to the served build; 5-way md5-verified mirror. Directive R2's board budget spent again — no further board without another intervening science/scope decision.",
      "vAF.0.7: DAF-19 CLOSED (lane LAF5, co-director decision) — adopted lane LAF4's corrected re-clustering (sky position embedded on the unit sphere instead of raw RA/Dec degrees, every hyperparameter and the seed reused verbatim, a (survey,programme)-only baseline) as the paper's taxonomy, replacing the published Table VIII/IX rather than citing it as a disclosed robustness check. 25→23 clusters, 8→9 families, Adjusted Rand Index 0.60/Adjusted Mutual Information 0.63 against the published partition (moderate restructuring — no family reproduced unchanged), ARI/AMI 0.26/0.26 against the survey/programme-only baseline (not a relabelling), largest wrap-aware R.A. span 350.4°→331.6° — the not-a-compact-sky-region conclusion is reinforced, not undermined. Every downstream number was re-derived by rerunning the real assembly pipeline against the corrected taxonomy, not hand-edited: family evidence, family-sky spans, the V6 taxonomy-completeness check (made dynamic instead of hardcoded 25/8), the V12 latent-space silhouette test recomputed on the corrected family labels (−0.0162→−0.0183, still structured-but-weak), the 'two families worth starting with' narrative, and the FT-B/FT-C/FT-E follow-up references (30 distinct objects/33 tier rows, was 32/35). Two propagation defects were caught and fixed before adoption: reusing the shared RA-span computation unmodified would have reintroduced the same wrap defect into the corrected tables (it is a naive max−min span, not wrap-aware); and the taxonomy's own per-family descriptor text can name the wrong dominant survey/programme cell (a marginal-mode-vs-joint-mode labelling bug), so the verified empirical joint mode was used throughout instead. Directive-G: 4-pass compile, 0 errors, 0 undefined refs, 0 overfull hboxes >10pt, 18pp (unchanged); visual audit of all touched pages clean; 37 cited = 37 bibitems; standalone tarball-style compile text-identical to the served build. New committed script: taxonomy_adoption_2026_09_22/.",
      "vAF.0.6: R2 confirmation board (lane LAF3, Grok API + Gemini API + Claude-opus verdict-blind referee, ApJS-CATALOG profile) ran on the exact vAF.0.5 PDF — the version R1 never actually reviewed, since lane LAF2 closed real items and bumped twice after R1 without an intervening board. Verdicts REJECT/MAJOR REVISIONS/MAJOR REVISIONS, all diagnostic only. Most consequential (Claude-opus only): the abstract/§I/§XI claim 'the blue arm alone accounts for R²=0.78' conflated the full three-camera regression's R² with the blue arm's own univariate R² — independently computed fresh at R²=0.20, roughly 4× lower; all three summary locations now state both numbers explicitly and distinguish them, with no change to the underlying conclusion. 8 further real items closed: a flux-to-magnitude arithmetic error (≈20.6→≈20.9 AB mag), 3 missing bibliography citations (LoVerde 2008, Eisenstein & Hu 1998, Planck 2018, named in prose but uncited), leftover version-history language ('we no longer report one of them as supported'), an incomplete morphology census (441+316+406=1,163 vs. the released 1,244 — the missing 81 are EXP/SER/DEV), a manuscript passage addressing 'a referee' directly, a bibliography-verification arithmetic error in Appendix A (4+26=30 vs. the actual 37 entries), and an abstract reproducibility claim tightened to scope precisely what is and isn't reproducible. Also fixed: a real rendering defect (not a source-text defect) where 7 description-list labels rendered with doubled punctuation. Several reviewer claims were independently FALSIFIED against source (the null result's effect size was already stated; per-class upper limits already reported; a claimed date inconsistency was backwards). 2 new items honestly left open (a blue-arm cut quantification, a Gunn-Peterson citation), joining the 3 pre-existing disclosed-open items. Directive-G: 4-pass compile, 0 errors, 0 undefined refs, 0 overfull hboxes >10pt, 18pp (unchanged); /latex-audit visual PASS on all 9 touched pages; /artifact-link-verify 6/6; standalone tarball-style compile reproduces the served build byte-for-byte in text. Readiness 77→80. Directive R2: round 2 of 2 spent — no further board without an intervening science/scope decision.",
    ...
    artifacts: [
      { label: "Read PDF", href: "/papers/anomaly_flagship_vAF.0.8.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/anomaly_flagship_vAF.0.8.pdf", kind: "secondary", download: true },
      {
        label: "LaTeX source",
