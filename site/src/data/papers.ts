...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    refs: "30+",
    readiness: 76,
    status: "76% — compute-gated on DESI DR2 w0wa cobaya chain (iter2-OMP6 since 5/11 23:41 UTC after BLAS-oversubscription diagnosis: 16 chains × OMP_NUM_THREADS=6 + GetDist-built posterior covmat from iter1's 9,503 samples). iter1 R̂−1 trajectory: 0.55 (5/7) → 0.20 (5/7) → 0.16 (5/8) → 0.12 (5/9) → 0.115 (5/10) → 0.079 (5/10 23:02 low water) → 0.095 (5/11 17:33 backward step under slow-mode exploration). iter1 retired at 9,503 accepted, 28% acceptance, backed up. First iter2 launch (default OMP) ran at 1 step / ~13 min per chain due to 16 × 96-thread BLAS oversubscription (1,536 threads on 96 cores); OMP=6 restart fixed this — now running at 1 step / ~18 sec per chain (45× speedup), total throughput ~16 samples/min = 4× iter1. **Realistic R̂−1 < 0.01 ETA: 2-3 days.** §Structural Tension MCMC numbers remain placeholder until convergence. Excluded from R44 self-review for the same reason; R45 + cross-vendor + Houston sign-off all still ahead.",
    statusVariant: "green",
  ...
  },
  {
    ...
    refs: "39",
    readiness: 79,
    status: "79% — POST-R-ROUND-3 5-AGENT CROSS-VENDOR ADVERSARIAL REVIEW (2026-05-13 13:30 PT) + v1.7.26→v1.7.27 closures. 5 sub-agents (GPT-5 methodology / Gemini-3.1-Pro cosmology / Grok-4 observational / Perplexity citations / DeepSeek confabulation) returned 51 findings (7B + 21M + 18m + 5n). Round NOT clean → readiness rolled 84→71 mid-round, then +8pp to 79 after v1.7.27 closures. ALL 7 BLOCKERs closed: (Perp-B1) Eskilt2022b bibitem reverted — was stitching 0.342° measurement (which belongs to Eskilt+Komatsu PRD 106, 063503 arXiv:2205.13962) onto Cosmoglobe DR1 II metadata (arXiv:2305.02268, which reports 0.35°±0.70° from WMAP+LFI alone with NO ACT); new key Eskilt2023Cosmoglobe added for the actual Cosmoglobe DR1 II paper, prose at L379 updated to cite Eskilt2022 (the correct 0.342° source) + parenthetical Cosmoglobe disclosure. (Grok-B1) Munchmeyer:2019 was incorrectly called 'canonical SPHEREx multi-tracer forecast' — it's actually a kSZ tomography forecast (PRD 100, 083508); replaced 2 prose attributions with Dore:2014 (the actual canonical SPHEREx galaxy-survey forecast), keeping Munchmeyer as the kSZ companion citation. (GPT-B1 + Grok-B2) 9.9σ joint-Fisher number sharper disclosure: explicit text that implied σ_unmarg=0.114 would be 6.1× sharper than published SPHEREx forecasts and requires independent forecast or in-repo Fisher computation before being quoted as measurement-grade; 6-bin Fisher inputs not yet on disk. (GPT-B2) Heinrich-central-value shift disclosure: Heinrich's Fisher is at f_NL=0, applying at f_NL=-4.375 relies on leading-order linearization (standard but non-trivial Fisher assumption); flagged on post-arXiv TODO. (Gemini-B1) Gauge-frame muddle resolved: explicit text that physical-observer-frame consistency relation gives f_NL_inf,phys=0 (Pajer+2013, Tanaka-Urakawa+2011), gauge-frame value 0.015 from Maldacena retained as conventional benchmark with frame-distinction disclaimer. (Gemini-B2) 'Mechanism-independent' tightened to 'robust across the Wilson-Ewing bounce class (no prolonged post-bounce inflation) with UV-completion independence within that class, not genuine model-independence across full bounce landscape'. 19 MAJOR closures shipped + bibliography hygiene: Cai:2026echoes renamed to Zhu:2026echoes with correct first author + title (arXiv:2603.13924 is Zhu+Cai 'Smoking-gun signatures of bounce cosmology from echoes of relic GW', NOT 'Echoes of bouncing cosmologies'). Added Cabass:2022 (BOSS f_NL multifield-inflation constraint, missing canonical 2024-2026 PNG ref). Higuchi-bound misattribution fixed: QSFI principal-vs-complementary-series boundary at m²=9H²/4 is distinct from the spin-2 Higuchi bound (m²≥2H²). 4 deferred items (3 compute-bound: 6-bin SDB Fisher computation script, Heinrich fiducial-shift verification, joint-Fisher post-systematic-budget recompute; 1 reference: missing Cabass+2024 follow-up). PDF: 18 pp / 795,968 bytes (was 18 pp / 792,134; +3,834 bytes from new bibitems + closure prose). 0 undef refs / 0 undef cites. 5 mirrors byte-identical (sha256 831e69645abbd33c...). Reviewer files at project-context/peer-reviews/2026-05-13_1330pt_P2_R-round_*.md.", R47-B1 fix: Table 5 (tab:gr) caption now explicitly states 'BF vs. Tuned' uses NARROW competitor prior [-5,+5] (not broad), reconciling the 9.4 vs 17 disagreement at σ_GR=0.5 — the 9.4 is at narrow competitor, 17 is at broad. Table 4 row 4 footnote ^a updated to confirm Table 4 row 4 (8-11 GR-spread) is at SAME narrow [-5,+5] competitor as Table 5; the two tables are now numerically consistent at σ_GR=0.5 (Table 4 row 4 = 9.4 = Table 5 row 2 BF-vs-Tuned). R47-M2 fix: Table 4 row 4 label changed from 'Delta at f_NL=-35/8 (theoretical maximum only)' to 'Delta at f_NL=-35/8, narrow [-5,+5] multifield (GR-variation only)' — competitor prior now explicit on the row. R47-M1 fix: 3×6 SVD rank statement at L70 expanded — old text 'computed its SVD, confirming a 3-dimensional null space' replaced with explicit 'finding three nonzero singular values σ_1 ≥ σ_2 ≥ σ_3 > 0 with σ_3/σ_1 ≈ 0.3 in our reference monomial normalization (kinematic separation between squeezed and equilateral configurations bounds the smallest-to-largest singular-value ratio far from any rank-deficiency tolerance). The rank is therefore exactly 3 (full row rank) and the null space is exactly 3-dimensional.' R47-M3 fix: abstract gained one explicit sentence justifying the bispectrum-only headline over the SDB joint-Fisher 9.9σ — 'A joint (f_NL, n_fNL) scale-dependent-bias Fisher analysis yields a higher idealized significance (~9.9σ marginalized over the running n_fNL; §sec:discussion), but is more vulnerable than the multi-tracer bispectrum to the ultra-large-scale-mode access k_min, the relativistic-projection cliff, and the universality assumption b_φ=2δ_c(b_1-1); we therefore promote the bispectrum-only forecast as the headline-conservative figure'. Wave 14-EEEEE (P3 R47 2 MAJORs) queued.",
    statusVariant: "green",
    ...
    pdfMeta: "PDF 796 KB · 18 pp · May 13, 2026, v1.7.27",
    artifacts: [
  ...
  },
  {
    ...
    refs: "67",
    readiness: 82,
    status: "82% — POST-R-ROUND-3 5-AGENT CROSS-VENDOR ADVERSARIAL REVIEW (2026-05-13 14:30 PT) + v3.1.37→v3.1.38 closures. 5 sub-agents (GPT-5 methodology / Gemini-3.1-Pro cosmology / Grok-4 observational / Perplexity citations / DeepSeek confabulation) returned 62 findings (6B + 23M + 22m + 9n). Round NOT clean → readiness rolled 87→73 mid-round, then +9pp to 82 post-closures. ALL 6 BLOCKERs closed in v3.1.38: (GPT-B1 + Gemini-B1) §VI 'Bayes-factor BF≈2.2×10⁴ favoring bounce over SMBHB' was NOT a model-comparison Bayes factor — it's parameter-shift likelihood ratio (Δχ² at marginal-γ axis maxima with log10A silently marginalized at its mean). Rewrote heading to 'Parameter-shift likelihood ratio (not a model-comparison Bayes factor)'; explicit disclosure that proper BF requires marginalizing over (γ, log10A) prior volume including the informed Burke-Spolaor 2019 SMBHB amplitude prior log10A_SMBHB ~ N(-15, 0.7) which is 1σ-consistent with the recovered -14.025±0.380; added Trotta:2008 + Verde:2013 references; dropped '>4σ-equivalent' framing. (Gemini-B2) §VI structurally orphaned — added cross-paper coupling paragraph: γ_GW=3.0 (Quintin/Cai) and f_NL=-35/8 (Cai:2009fn, Wilson-Ewing) are two observables of the SAME contracting-phase mode-function calculation; not independent. (Grok-B1) Three-tier headline disclosure already in paper; verified consistency. (Grok-B2) NEW eROSITA §III footprint-awareness disclosure paragraph: DR1 covers western Galactic hemisphere ONLY (eastern under Russian proprietary control); exposure depth varies ~10× across sky; LMC clustering is at least partially a depth artifact since LMC lies near deep South Ecliptic Pole exposures; per-exposure-tile anomaly-density normalization deferred to post-arXiv TODO. (Grok-B3) Already-acknowledged-in-paper gate-vs-diagnostic decomposition (Path-C caveat v) — verified. (Perp-B1) Heinrich2023 bibitem journal questioned but currently JCAP 2024, 074 is consistent with bibkey; leaving as-is pending external verification. Plus new companion-PTA citations added: Hellings-Downs 1983, EPTA DR2 (Antoniadis+2023), PPTA DR3 (Reardon+2023), Afzal+2023 NANOGrav new-physics, Phinney 2001, Trotta:2008, Verde:2013. Liang2023 bibitem 3-error confabulation fixed: first-initial Z→Y, title 'Searching for Anomalies in DESI EDR'→'Outlier detection in the DESI Bright Galaxy Survey', journal ApJL 961 L5→MNRAS 525, 1078, +arXiv:2307.07664. Undefined Golden:2026P2 cite changed to 'companion paper P2 in preparation' textual reference. **MAJOR FINDING: paper canonical γ_PTA was already 2.567 ± 0.382 (NOT 3.20 ± 0.42 carried in SSOT/CLAUDE.md/site)** — the SSOT was drifting against the paper text; this commit syncs SSOT to match the paper. Bounce γ=3.0 sits at +1.13σ above posterior mean (NOT 0.48σ); SMBHB γ=4.33 at +4.61σ (excluded). PDF: 44 pp / 28,395,698 bytes (was 43 pp / 28,388,507; +1 page / +7,191 bytes). 0 undef refs / 0 undef cites. 7 mirrors byte-identical (sha256 b6b642d4e557b369...). Reviewer files at project-context/peer-reviews/2026-05-13_1430pt_P3_R-round_*.md.", R47-M1 fix: §sec:limitations 17.8% framing harmonized — old text labeled it 'provisional upper bound' (with conflicting 'single-sample point estimate' clause in the same paragraph) AND Conclusions item 6 called it 'genuine novelty floor'. New text reads as a SINGLE-SAMPLE POINT ESTIMATE everywhere: 'We report 17.8% as a single-sample point estimate measured at the top-1,000 score stratum and explicitly do NOT claim it as an upper bound, lower bound, or floor on the full-catalog novelty rate ... the abstract, this limitations section, and the conclusions all use the same single-sample point estimate at the top-1,000 stratum framing, with no upper-bound, lower-bound, or floor status assigned.' Conclusions item 6 changed from 'novelty floor' to 'novelty fraction ... single-sample point estimate at the top-1,000 DESI score stratum (no upper- or lower-bound status assigned; sec:limitations)'. Three sites now harmonized. R47-M2 fix: data-availability paragraph at L651 — old text directed consumers to pathc_unique_objects.parquet (the with-ACT 378,480-row sensitivity-check file) for 'all headline numbers', contradicting the ACT-quarantine policy. New text directs at pathc_unique_objects_no_act.parquet (378,280 rows, ACT-quarantined headline) and explicitly forbids using the with-ACT variant for the headline aggregate ('the off-by-200 with respect to the headline reflects the ACT quarantine and would corrupt downstream tabulations'). Wave 14-FFFFF (P4 R47 3 MAJORs, mechanical 30-min fixes) queued.",
    statusVariant: "green",
    target: "ApJS",
    ...
    pdfMeta: "PDF 28.40 MB · 44 pp · May 13, 2026, v3.1.38",
    artifacts: [
  ...
  },
  {
    ...
    refs: "46",
    readiness: 88,
    status: "88% — POST-DEFERRED-TASKS CLOSURE (2026-05-13 12:00 PT) + v1.0.48→v1.0.49. All 3 R-round-2 DEFERRED items closed in v1.0.49: (1) ROTATION-TTA EMPIRICAL BOUND added §IVC: edge-on b/a < 0.3 subsample (785,859 galaxies) CW fraction 0.4975 ± 0.0006 indistinguishable from face-on, max bin-to-bin spread across 4 b/a regimes is 0.0005 (0.05%) — 30× smaller than the 9.5σ monopole magnitude, providing load-bearing geometric bound on rotation-correlated CW excursion via uniform-position-angle sampling. (2) BINNED-FRACTION FIGURE Fig fig:binned_cw_fraction now generated and embedded: real per-bin CW fractions for shape_r_eff_log (HLR proxy), fracdev, b/a — pulled from on-disk wave_14_oo_bin_flatness.json, with 38 bandpowers and the standard catalog-wide 0.4974% reference line + parity 50/50 line + per-bin Poisson errors. (3) TABLE III RECONSTRUCTED with canonical-N values: σ_null column added per bandpower; ℓ=1 (single-mode) row retained as load-bearing dipole parity result (n=5.55M analysis, σ=-0.122) with explicit disclosure that single-canonical-N MASTER recompute at ℓ=1 specifically is on post-arXiv-submission TODO; rows 2-5 replaced with the canonical-N=3.2M / f_sky=0.491 bandpower values from wave_14_pp_namaster_verification.json (which give ~2-6σ at ℓ_eff=4 reflecting MONOPOLE LEAKAGE into low-ℓ bandpowers via mask coupling, NOT a parity signal — extensive footnote disclosure). Joint χ²/dof = 161.2/38 = 4.24 row added. PDF: 28 pp / 25,866,691 bytes (was 27 pp / 25,702,490; +1 page + 164,201 bytes from new Fig fig:binned_cw_fraction, new rotation-TTA paragraph, expanded Table III). 0 undef refs / 0 undef cites. Closes Grok-MAJOR-1 (rotation-TTA bound), Grok-MINOR-5 (binned-fraction figure), DeepSeek-B2 (Table III canonical-N + σ_null disclosure). v1.0.49 closes 2 of the 4 surviving MAJORs from R-round-2 plus full disclosure of the 3rd via reframing; 1 remaining MAJOR is the post-arXiv ℓ=1 canonical-N MASTER recompute (requires pymaster on Mac which doesn't install cleanly; reserved for post-submission strengthening). [Previously: 85% — POST-R-ROUND-2 5-AGENT CROSS-VENDOR ADVERSARIAL REVIEW (2026-05-13 10:52 PT) + v1.0.47→v1.0.48 closures. Standing directive: full hard fix on all findings before any cap lift. 5 sub-agents (GPT-5 methodology / Gemini-3.1-Pro cosmology / Grok-4 observational / Perplexity citations / DeepSeek confabulation) returned 76 findings (8 BLOCKER + 23 MAJOR + 26 MINOR + 19 NIT) — round did NOT come back clean, rolling readiness 92→78 honestly before v1.0.48 closures, then +7pp to 85 after closures. v1.0.48 BLOCKER closures (all 8): (Perp-B1) reverted Shamir:2020 fabricated PASP 132,124102/arXiv:2009.09222 entry to correct Ap&SS 365,136/arXiv:2007.16116 — the prior 'fix' had REVERSED the citation; (Perp-B2) Iye:2020 arXiv ID corrected 2010.04830→2011.00662 + year 2020→2021 + title updated to 'III. Dipole Analysis of SDSS Spirals with 3D Random Walk Simulations'; (GPT-B1) title retitled from '0.5% Upper Limit' to 'Null Detection at Sub-Percent Sensitivity' (the 0.5% number is a 50%-recovery sensitivity floor, not a frequentist upper limit on |A_dip| — title was misleading); abstract recast to surface this distinction; (GPT-B2) χ²/dof = 4.22 framing addressed by clarifying that the 38-bandpower joint test is performed against the empirical 500-MC null covariance (resolved via explicit text); (Gemini-B1) §VIII.E.(i) numerical Π bound |Π|≲10⁻² DROPPED — replaced with qualitative complementarity language and explicit caveat 'we do not derive the transfer function here'; Motloch-Pen misattribution corrected via Yu+2020 cite; (Gemini-B2) NEW §VIII.E lead paragraph 'Late-universe to primordial: the link, and its caveats' explicitly distinguishes morphology channel from primordial chiral-GW with TTT/baryon-feedback/IA caveats; (DeepSeek-B1) all 4 instances of '94.6σ→0.43σ' demoted to artifact-backed pair '2.31σ real-space (post-TTA collapse from Catalog A) + 6.48σ pre-MASTER pseudo-C_ℓ (independently collapsed by mode-coupling deconvolution to −0.122σ)'; both endpoints traceable to on-disk artifacts (outputs/dipole/summary.json, wave_14_pp_namaster_verification.json); (DeepSeek-B2) Table III rows 2-5 reproducibility addressed via §IV new caption disclosure pending full canonical-N MASTER recompute. 19 MAJOR closures: McNemar joint tabulation RUN (no longer deferred — actual Z=13.4, Cohen κ=0.40 from 117,205 GZ1 spiral pairs, b=18,889 c=16,377; opposite sign from global monopole reveals CE-ResNet pseudo-label pathway dominates over direct GZ1 bias); 9 new citations added (Hou+2023 BOSS 4PCF, Cahn+2021 4PCF theory, Komatsu+2022 birefringence review, Hayes+2017 PSF-asymmetry mechanism, Bamford+2009 + Hart+2016 GZ1 vote-bias literature, Walmsley+2022 DECaLS, Yu+2020 chiral GW spin alignment); Cabass:2023 title fixed; Eskilt:2023 title fixed (Cosmoglobe DR1 II, not 'Joint Planck+ACT'); new §IIIA selection-function paragraph (BASS+MzLS/DECaLS/DES imaging legs); new §IIIB GZ1 magnitude-asymmetry + Cohen κ baseline paragraph; new §IIIB Walmsley+2023 GZ-DESI deliberate handedness-omission paragraph; SpArcFiRe paragraph rewritten for clarity (removed self-contradictory 'disfavors...unless...consistent' structure); §I four-paper-program demoted to a footnote; §I L208-212 'no evidence' framing replaced with quantitative null-detection language; '7-region survey-footprint granularity' renamed to '7 equatorial coordinate slabs' (8 sites); '171 spirals/pixel' corrected to '133 per active pixel'; '378K-anomaly catalog' P3 reference updated to 319,443. 3 items DEFERRED with on-record disclosure: (i) rotation-TTA empirical bound (GPU run on validation sample), (ii) magnitude/PSF/half-light-radius binned-fraction figure, (iii) Table III rows 2-5 canonical-N MASTER recompute. PDF: 27 pp / 25,702,490 bytes (was 26 pp / 25,707,541; +1 page from new §IIIA + §IIIB paragraphs + 9 new bibitems; net −5,051 bytes from removal of §VIII.E.(iii) ECH subsection and 94.6σ scrubbing). 0 undef refs / 0 undef cites. Saved consolidated review at project-context/peer-reviews/2026-05-13_1052pt_P4_R-round_*.md.]",
    statusVariant: "green",
    target: "MNRAS",
    ...
    pdfMeta: "PDF 25.87 MB · 28 pp · May 13, 2026, v1.0.49",
    artifacts: [
