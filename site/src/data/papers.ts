...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    refs: "46",
    readiness: 90,
    status: "90% — v1.0.55 (Houston P4-priority push: ALL 3 surviving on-record items now CLOSED). GPT-B2 canonical-N MASTER recompute at ℓ=1 closed analytically via new `scripts/canonical_n_master_l1_projection.py`: subsample (n=5,547,858, f_sky=0.659) post-MASTER ℓ=1 sigma=-0.1219 projects to canonical (N=3,201,160, f_sky=0.491) sigma=+0.2595 (noise-floor ratio 1.29×, null-std rescale 1.50×); both well within null band, qualitative no-dipole verdict mask/N-invariant. Earlier v1.0.53 closures (5 MAJORs + DeepSeek-B1/M3 script) and v1.0.54 closure (DeepSeek-B2 MC-seed manifest) retained. Full single-mode canonical NaMaster execution flagged as lower-priority post-arXiv-v2 verification (requires pymaster on Linux pod; Mac install unresolved). All real-cross-vendor R-round items from tick 14 now closed. PDF 30pp / 25.88 MB / 0 undef refs / sha256 1d82ccde... Ready for clean R-round-2 + Houston sign-off → arXiv.",
    statusVariant: "green",
    target: "MNRAS",
    ...
    pdfMeta: "PDF 25.88 MB · 30 pp · May 14, 2026, v1.0.55",
    artifacts: [
