...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    statusVariant: "green",
    target: "ApJS",
    description: "The multi-survey anomaly catalog. 378,280 unique anomalies catalogued across 7 surveys from 37.3 million sources via a unified BigAE autoencoder architecture. 17.8% genuine novelty rate against 20 all-sky catalogs (CDS X-Match), with NANOGrav 15yr free-spectrum gamma = 3.20 +/- 0.42 (0.48 sigma from bounce prediction gamma=3.0) and an empirical sigma(f_NL) = 8.27 +/- 2.37 from the multi-tracer DESI forecast at empirical alpha = 0.19 +/- 0.65 (Wave 14-VVV calibration on full 5,384 QSO candidates, replacing the prior fiducial-alpha placeholder). All 65 R42 cross-model peer-review findings closed.",
    keyResults: [
      ...
      "Empirical σ(f_NL) = 8.27 ± 2.37 at α = 0.19 ± 0.65 (Wave 14-VVV calibration; replaces fiducial-α placeholder); 16.4% improvement at α=0.15 (DESI+SDSS); SPHEREx forecast 4.38σ for f_NL = -35/8",
    ...
    pdfMeta: "PDF 28.36 MB · 41 pp · May 6, 2026, v3.1.21",
    artifacts: [
  ...
  },
  {
    ...
    statusVariant: "green",
    target: "MNRAS",
    description: "The galaxy chirality catalog. 8.47M galaxies classified for CW/CCW handedness via a ViT-Small ensemble with rotational-equivariance correction (3.86x asymmetry suppression factor). Hemisphere look-elsewhere null at p_LEE = 9.999e-5 (Wave 12 GPU N_MC=10,000) plus dipole MC injection-recovery establishing >=0.5% empirical detection threshold (catalog-wide sigma=0.43, p=0.30) refute Shamir 2020's 3% claim by a factor of 9. Wave 14-QQQ angular power reconcile: ℓ=1 row corrected from buggy 2.75σ (pre-N_spiral) to canonical -0.12σ (MASTER-deconvolved). All 65 R42 cross-model peer-review findings closed.",
    keyResults: [
    ...
    pdfMeta: "PDF 25.65 MB · 21 pp · May 5, 2026, v1.0.30",
    artifacts: [
