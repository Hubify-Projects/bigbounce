...
export const researchPrograms: ResearchProgram[] = [
  {
    ...
    supportingLinks: [
      ...
      },
      {
        ...
        description:
          "A systematic survey of 7 foundation mechanism classes and 6 observational branches, extracted from the retired P1U barrier catalog, giving 14 distinct mechanism-class constraints across 14 catalog entries and closing four candidate dark-energy routes (R1-R4). Draft v1C.0.13, compiled 23 pp, 0 undefined refs, 0 overfull boxes — closes the R10 correctness-convergence board on the exact v1C.0.12 PDF (Claude INT MAJOR REVISION with ZERO computational errors, 2 MAJOR/7 MINOR, 5 correctness-grade, plus 4 low-DPI false positives the reviewing leg raised and self-withdrew after re-rendering; Grok REJECT; Gemini MINOR REVISIONS — the first sub-major verdict this paper has received; Perplexity leg failed). Both MAJORs were claim-scoping defects and are closed by scoping, never by weakening the science or inventing coverage. MAJOR-1: Appendix D proves the perturbation-transparency theorem (B14) for canonical scalar matter and expressly excludes fermion sources, so B14 cannot subsume B8 (a fermionic (J5)^2 statement) nor constrain R1 (the fermionic NJL channel). B14's route tag is narrowed from [R1-R4] to [R2-R4, zero-spin branch] with its honest content stated — it removes those routes' CLASSICAL zero-spin perturbative baseline, not their quantum or fermionic content — B8 is restored as an independent constraint, and the headline count is RECOUNTED from 13 to 14 distinct mechanism-class constraints at every site (abstract, introduction, Sec. III preamble, Fig. 1 caption and its in-figure edge label, Table I caption, App. A, Sec. VI, Sec. VII). This also resolves Grok's '14 entries (13 distinct)' wording complaint. MAJOR-2: Eq. (1) is a dimension-5 operator on a light pseudoscalar theta_NY carried by a dimension-(-1) coefficient with an extra derivative — three independent reasons it lies outside the Sec. V dimension-4 construction rule — and its ~H0^2 background is precisely the new light scale App. A names as able to EVADE the single-scale NDA bound. The false delegation is removed and replaced with the in-scope argument extracted faithfully from the frozen monolith: for a constant Nieh-Yan coefficient the operator vanishes and the surviving Holst/Nieh-Yan content is O1/O2, exact total derivatives (Tier-I, inside the list); for a dynamical one the completion is non-minimal and closes only at the R4-class naturalness level (Tier-II), reinforced by B7. Route 2's dark-energy leg is therefore stated at REDUCED strength in Sec. IV A, Table II, Sec. VI and Sec. VII, and the 'all four channels close' sentence is re-scoped by closure mode. Also closed: the beta(gamma) 'could only suppress' claim (corrected to a net -1.5 orders stated as a pair), Mercuri [8] dropped from the RG citation, B9's [R2] tag motivated, remaining {O1-O6} 'basis' language moved to 'spanning list', the metric signature and epsilon normalization now stated outright in Sec. II, the abstract's parity-odd label qualified, the version/date stamp removed from the printed title block, beta(gamma) defined at first use, defensive companion phrasing consolidated, Gemini's version-history prose removed, and the R4 standalone-reader gap closed by stating its two checkable steps in-text. Falsified with receipts: Gemini's claim that ACT DR6 ref [13] carries a placeholder arXiv ID (2509.13654 is the real record, verified in R7). Deferred-genuine: the frozen-release Zenodo DOI for this survey's own scripts (P-round packaging, already disclosed in the paper). DRAFT: R-phase NOT converged at R10 — two correctness-grade MAJORs were found and closed, so R11 on the exact v1C.0.13 PDF is the next correctness-convergence check; no readiness score has been computed and no venue/Zenodo kit exists yet.",
        href: "/papers/paper1c_nogo_survey_v1C.0.13.pdf",
        external: true,
      },
    ],
