...
export const researchPrograms: ResearchProgram[] = [
  {
    ...
    supportingLinks: [
      ...
      },
      {
        ...
        description:
          "A systematic survey of 7 foundation mechanism classes and 6 observational branches, extracted from the retired P1U barrier catalog, giving 14 distinct mechanism-class constraints across 14 catalog entries and closing four candidate dark-energy routes (R1-R4). Draft v1C.0.14, compiled 24 pp, 0 undefined refs, 0 overfull boxes — closes the R11 correctness-convergence board on the exact v1C.0.13 PDF (Claude Opus INT MAJOR REVISION with, for the second consecutive round, ZERO computational errors across 30 independently recomputed displayed relations and numerical claims — full Fierz involution, Benedetti-Speziale flow integration, O4/O5 tensor reductions, every Appendix A and E order-of-magnitude figure — turning up only one rounding slip in a parenthetical; Grok REJECT on scope/self-containment/length, not computation; Gemini MAJOR REVISIONS; Perplexity leg failed). All four MAJORs were internal-consistency defects left in the seams of the v1C.0.13 revision — the document disagreeing with its own table, its own explicit non-claims, or one of its own released artifacts — none requiring new physics, new computation, or weakened results. MAJOR-1: Table II carried a second Tier-(I) marker (Route 2's re-homed dark-energy leg) though six text sites and the table's own caption say there is exactly one, and the grading did not meet the paper's stated Tier-I bar since 'minimal Route 2 sources no dark energy' is the disclosed-as-asserted-not-proved spanning claim, not a statement about O1/O2 alone; the leg is regraded (I) -> (II), the genuinely Tier-I ingredient (O1/O2 as exact total derivatives) is named as such, and Table II now carries exactly one Tier-(I) marker matching its own caption. MAJOR-2: Sec. IV B's summary paragraph still delegated the parity-odd operator to the single-scale NDA bound the same revision explicitly disclaims twice on pp. 6 and 8 ('we do not claim the NDA bound covers it') — the R10 sweep caught three of four instances and missed the summary line a skimming referee reads first; replaced with the accurate statement that the birefringence amplitude is bounded by the explicit Eq. (2) budget, plus one sibling correction. MAJOR-3: the abstract's 'each closing one or more of the four routes' universally quantified over all fourteen catalog entries, falsified by the paper's own B14 ('not... a closure of the fermionic or one-loop content of any route') and B9 ('never used as a stand-alone closure'); the abstract now states the joint claim the catalog actually supports and names both non-closure entries. MAJOR-4: Appendix C claimed the Grassmann derivation gives 'the unique solution for identical fields', inverting the cited artifact (research/theory_audit/fierz_adjudication_2026_08_05), which proves uniqueness for four distinct fields and explicitly denies it for identical fields (rank-3 span, two linear relations); the appendix now states the artifact's actual result with its rank-3 caveat, and G_s = -3*kappa/16 is unchanged. Also closed: the beta(gamma) arithmetic slip corrected from 'roughly 1.5' to the actual 1.7 orders (-2.2 + 0.5); Appendix D's invertible-tetrad kernel lemma now stated in components and cited to Hehl et al. (1976) rather than asserted bare; Appendix D's tensor-sector 'deferral' removed since the tensor conclusion is an immediate corollary of its own Statement; the 7+6->14 branch-to-entry multiplicity made explicit; eight orphaned labels resolved; a stale Fig. 1 source comment fixed; Gemini's inline repository paths removed from the main text (retained correctly in Data and Code Availability), the abstract's algebraic/zero-derivative qualifier added, Fig. 1's caption version-history prose removed, and the Lagrangian-density dimension wording made field-theoretically precise. Falsified with receipts: Gemini's claim that the reduced Planck mass symbol is overloaded with the full Planck mass — verified at 300 DPI the printed symbol carries its overline (Mbar_Pl vs M_Pl), the fifth member of the R3/R5/R7/R8/R9 rasterization-artifact family the standing >=300 DPI re-render protocol continues to catch. A new stdlib-only linter, tools/p1c_consistency_check.py, now runs before every version bump and independently rediscovered MAJOR-1/2/3 with no reviewer in the loop, exiting clean on this version. Page count moved 23 -> 24 because the four claim-scoping closures added required text, reported rather than smoothed over; Grok's <=12 pp target remains unmet and unreachable without deleting catalog content. Deferred-genuine: the frozen-release Zenodo DOI for this survey's own verification scripts (P-round packaging, already disclosed in the paper). DRAFT: R-phase NOT converged at R11 — four correctness-grade MAJORs were found and closed, so R12 on the exact v1C.0.14 PDF is the next correctness-convergence check; no readiness score has been computed and no venue/Zenodo kit exists yet.",
        href: "/papers/paper1c_nogo_survey_v1C.0.14.pdf",
        external: true,
      },
    ],
