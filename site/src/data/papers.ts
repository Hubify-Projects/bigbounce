...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    version: "v1B.0.28",
    tldr: "v1B.0.28 (cron fire #86 tick 208): 🎯 FIRST REAL EXTERNAL 5-VENDOR R-ROUND via OpenRouter after weekly cap reset ($0/$50 fresh budget). R26 dispatched DeepSeek-V4-Pro + Gemini-3.1-Pro + GPT-5.5 + Grok-4.3 + Perplexity-Sonar-Pro in parallel. **4/5 reviewers COMPLETELY CLEAN** (DeepSeek+Gemini+GPT-5+Perplexity = 0/0/0/0 each); only Grok-4.3 raised findings (0 BLK + 3 MAJ + 2 min + 1 nit), mostly stylistic scope disagreements with the Technical Verification Companion framing. GRO-B1 (Table 1B +4.3σ in-cell display invites misinterpretation despite caveat footnote) closed by softening to \"(marg.-tail, +4.3σ)\" + expanded fn:wcaveat. GRO-B2/B3/B4 (NaMaster/ALP/ΔNeff \"not ECH-specific\") push-back-with-citation: disclaimers already present in paper as Verification Companion. GRO-B5 push-back. GRO-B6 (strip audit-trail %-comments pre-arXiv) DEFERRED to pre-submission. External 99%-gate MOSTLY-cleared on P1B; final gate Houston sign-off. PDF 11pp/706KB/0 overfull/0 undef.",
    blockingItems: [
      "Houston personal sign-off (the final 1%)",
      "R26 5-vendor confirmation (BLOCKED on OpenRouter per-key cap)",
