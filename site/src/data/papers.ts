...
export const papers: Paper[] = [
  {
    ...
    refs: "70+",
    readiness: 90,
    status: "89% — Wave 14-NNNN: M4 D_inf prefactor justification shipped (Eq. (eq:Dinf) now has full first-principles derivation: exp(-3N_tot) from a^{-3} contorsion dilution between bounce and reheating + (T_reh/M_GUT)^{3/2} matching coefficient from fermion density-of-states + parity-odd phase-space factor; the prefactor is O(0.01-0.1) under any T_reh within an order of magnitude of the GUT scale and does not contribute to the fine-tuning hierarchy at leading order); M5 orphan-label check confirmed clean (Wave 14-IIII multi-label consolidation intentional, no broken \\ref calls). All P1A R44 MAJORs now closed. R45 self-review + cross-vendor non-Anthropic round + Houston sign-off pending.",
    statusVariant: "green",
  ...
  },
  {
    ...
    refs: "31+",
    readiness: 85,
    status: "84% — R44 BLOCKERs B1+B2 closed Wave 14-HHHH; R44 MAJORs M1+M2+M3+M5 + Maldacena:2003 cite all closed Wave 14-MMMM (Bayes factor 8-17 reconciled with explicit headline BF~8 + delta-prior max BF~17 + GR-marg 8-11 spread; curvaton prior [-5,+5] reframed as more physical baseline; Heinrich Eq./Fig reference made explicit; DBI extended with axion-curvaton + QSFI alternatives, +1 Chen:2009zp bibitem; Maldacena:2003 → Maldacena:2002vr key fix). R45 self-review + cross-vendor non-Anthropic round + Houston sign-off pending.",
    statusVariant: "green",
  ...
  },
  {
    ...
    refs: "60+",
    readiness: 89,
    status: "88% — fNL deferral closed (Wave 14-VVV empirical α calibration + Wave 14-KKKK high-confidence Path-B); R44 BLOCKERs all closed; R44 MAJORs M1+M2+M4+M5 + nit1 shipped; R43-M5 anomaly-window-randoms methodology paragraph still open as minor; HuggingFace dataset visibility flip pending (Houston manual); R45 self-review + cross-vendor non-Anthropic round + Houston sign-off pending",
    statusVariant: "green",
    target: "ApJS",
  ...
  },
  {
    ...
    refs: "30+",
    readiness: 89,
    status: "88% — R44 MAJORs M1+M2+M3+M4+M6+M7 + nit2 shipped (Wave 14-GGGG closed M1/M2/M3/nit2: p_LEE upper-bound, Platt 6 sig figs, McNemar paired test, PUSHBACK token cleanup; Wave 14-LLLL closes M4 recall-asymmetry decomposition (1.2 pp = ~1% GZ1 prior ⊕ ~0.5% Cat-C residual), M6 MDD N_eff inflation note, M7 morphology-bin flatness disclosure in abstract); 1 R44 MAJOR still open: M5 deep-MLP RA/Dec ablation (pod RTX A5000 too slow for 8.47M deep-MLP retrain — needs separate H200 spin-up); R45 self-review + cross-vendor non-Anthropic round + Houston sign-off pending",
    statusVariant: "green",
    target: "MNRAS",
