...
export const papers: Paper[] = [
  {
    ...
    refs: "72",
    readiness: 81,
    status: "81% — v1A.0.20 (R-round-3 closures, 4B + 12M). Stale L1075 footnote rewrite, structural-tension reframe, four-route operator-level disclosure, Eskilt2022b bib confab fix. 4 items deferred (theory + compute). PDF 17pp / 789 KB / 0 undef refs.",
    statusVariant: "green",
    ...
    pdfMeta: "PDF 789 KB · 17 pp · May 13, 2026, v1A.0.20",
    artifacts: [
  ...
  },
  {
    ...
    refs: "32",
    readiness: 71,
    status: "71% — v1B.0.4 (R-round-3 closures, 5B + 16M). Phantom 114,992 sample-count removed (real total 309,789), stale ~109/1-3-days footnote propagation fix, ΔNeff proxy reframed as bounce-class compatibility check, β=0.30° citation fix (Diego-Palazuelos+2022). Compute-gated on DESI DR2 w0wa convergence (R̂−1=0.023 at 5/14 02:40 UTC, ETA <0.01 in ~6h). PDF 8pp / 672 KB / 0 undef refs.",
    statusVariant: "green",
    ...
    pdfMeta: "PDF 672 KB · 8 pp · May 13, 2026, v1B.0.4",
    artifacts: [
  ...
  },
  {
    ...
    refs: "39",
    readiness: 79,
    status: "79% — v1.7.27 (R-round-3 closures, 7B + 19M). Eskilt2022b bib confab reverted, Munchmeyer:2019 vs SPHEREx attribution fixed (now Dore:2014), 9.9σ joint-Fisher provenance disclosure, Heinrich-fiducial-shift disclosure, gauge-frame muddle resolved, mechanism-independent tightened, Cai/Zhu:2026echoes author+title fix, Cabass:2022 BOSS f_NL added. 4 deferred (compute). PDF 18pp / 796 KB / 0 undef refs.",
    statusVariant: "green",
  ...
  },
  {
    ...
    refs: "67",
    readiness: 82,
    status: "82% — v3.1.38 (R-round-3 closures, 6B + 19M). \"Bayes factor\" reframed as parameter-shift likelihood ratio (added Trotta:2008 + Verde:2013), §VI cross-paper coupling paragraph, eROSITA western-Galactic + 10×-depth-variation + LMC-depth-artifact disclosure, PTA-companion citations added (Hellings-Downs, EPTA-DR2, PPTA-DR3, Afzal-NewPhys), Liang2023 3-error confab fix. γ_PTA SSOT-drift caught (was 3.20±0.42, paper already at 2.567±0.382). PDF 44pp / 28.4 MB / 0 undef refs.",
    statusVariant: "green",
    target: "ApJS",
  ...
  },
  {
    ...
    refs: "46",
    readiness: 84,
    status: "84% — v1.0.50 (R-round-3 closures, 7B + 18M). \"30× smaller\" arithmetic error fix (actual 5.2×), 3 bibitem title fixes (Hart, Yu, Hayes — LLM-fabrication pattern), §VIII.E null-falsifiability disclosure paragraph, §VIII.E.(ii) orthogonal-projections fix, CW excess → deficit wording fix. McNemar joint tabulation measurement-grade (Z=13.4, Cohen κ=0.40). 5 deferred (compute: PA Rayleigh test, per-imaging-leg CW table, pymaster monopole-leakage, sixth Table III column, ALP-Cabass-Philcox cross-covariance). PDF 28pp / 25.87 MB / 0 undef refs / sha256 5b0c663b...",
    statusVariant: "green",
    target: "MNRAS",
    ...
    pdfMeta: "PDF 25.87 MB · 28 pp · May 13, 2026, v1.0.50",
    artifacts: [
