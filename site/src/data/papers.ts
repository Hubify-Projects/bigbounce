...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    version: "v3M.0.29",
    lastUpdated: "2026-09-22",
    tldr: "Row-9b science decision (LQC/poly Bardeen extension) propagated -- the directive-R2 intervening decision authorizing one confirmation board (R12, pending). The Bardeen continuation, previously established only on the Quintin-type background, now extends to LQC and poly, which cross rho+p=0 smoothly: the continuation is a one-parameter self-adjoint-extension prescription (NOT unique, a retraction from the source lane's own earlier claim), and its time-symmetric member gives linear-transfer amplitudes -1.094 (LQC) / -1.140 (poly) against S1's factor of 6.25 -- only 2.00x / 2.67x on these two backgrounds, background-specific not universal. The cubic term is uncomputed there, so f_NL^after is reported only as a bracket, never a single value. AGAINST the paper, with equal prominence: the tensor no-go strengthens on both (r_after 96.0 / 170.6 vs S1's 24.0). Separately, R11's ESSENTIAL evaluation-window item closes by computation (uniform eta_*/eta_B=15, stationary at every k) rather than disclosure; the headline -1.25 survives. No headline number changed on Quintin-type.",
    path: publicationPath({}),
    pages: "23",
    refs: "\u2014",
    readiness: 75,
    ...
    pdfMeta: "PDF \u00b7 23 pp \u00b7 v3M.0.29 \u00b7 created Sep 22, 2026 \u00b7 md5 8ee1f13bd4e596c87655a9347c0c0918 \u2014 row-9b (LQC/poly Bardeen extension) science-decision propagation: the intervening R2 decision unlocking the authorized R12 confirmation board. No headline number changed on the Quintin-type background; the LQC/poly S1 rows are superseded by a Bardeen continuation (linear-transfer values plus a bracket, cubic term uncomputed) and r_after strengthens on both (96.0, 170.6). Readiness 75 COMPUTED \u2014 R12 board pending.",
    changelog: [
    ...
    artifacts: [
      { label: "Read PDF", href: "/papers/a3_multichannel_arxiv_v3M.0.29.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/a3_multichannel_arxiv_v3M.0.29.pdf", kind: "secondary", download: true },
      {
        label: "LaTeX source",
  ...
  },
  {
    ...
    remainingWork: [
      "HOLD CORRECTION v4P.0.11: the v4P.0.10 dilution bound (D≤0.717±0.005, A₉₅ᵖʰʸˢ≥1.37%) was withdrawn — measured on out-of-domain images (a positive control reproduces released labels at only 43-45% on those images vs. 99.99% in-domain); on hold pending an in-domain re-measurement on the classifier's own Smith42/galaxies images (bounded, disk-only work)",
      "A new sky-dependence dipole systematic (11σ, 23% of A₉₅ on the strict-887,472 support) is now disclosed; row-16(ii-b)'s other closures (P-1–P-4) are unaffected",
      "A fresh INT confirmation board on the exact v4P.0.11 PDF is now due (directive R2 round budget refreshed again) — not run in this lane",
      "arXiv tarball assembled: SSOT/arxiv_tarballs/paper4prime_chirality_test_arxiv_v4P.0.11.tar.gz",
    ...
    artifacts: [
      { label: "Read PDF", href: "/papers/paper4prime_chirality_test_v4P.0.11.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/paper4prime_chirality_test_v4P.0.11.pdf", kind: "secondary", download: true },
      {
        label: "LaTeX source",
