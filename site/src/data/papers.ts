...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    refs: "30+",
    readiness: 76,
    status: "76% — compute-gated on DESI DR2 w0wa cobaya chain (iter2 since 5/11 22:35 PT: 16 chains + GetDist-built posterior covmat from iter1's 9,503 samples, expected 4–8× wall-clock speedup). iter1 R̂−1 trajectory: 0.55 (5/7) → 0.20 (5/7) → 0.16 (5/8) → 0.12 (5/9) → 0.115 (5/10) → 0.079 (5/10 23:02 low water) → 0.095 (5/11 17:33 backward step under slow-mode exploration). iter1 retired at 9,503 accepted, 28% acceptance, backed up at chains_w0wa_dr2_backup_2026-05-11_pre-iter2/. iter2 = same physics config + 4× more chains + posterior-tuned proposal → expected R̂−1 < 0.01 ETA 2–5 days (down from iter1's projected 5–15 days). §Structural Tension MCMC numbers remain placeholder until convergence. Excluded from R44 self-review for the same reason; R45 + cross-vendor + Houston sign-off all still ahead.",
    statusVariant: "green",
