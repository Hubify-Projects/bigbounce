...
export const researchPrograms: ResearchProgram[] = [
  {
    ...
    supportingLinks: [
      ...
      },
      {
        ...
        description:
          "A systematic survey of 7 foundation mechanism classes and 6 observational branches, extracted from the retired P1U barrier catalog, collapsing to 13 distinct mechanism-class constraints across 14 catalog entries and closing four candidate dark-energy routes (R1-R4). Draft v1C.0.11, compiled 20 pp, 0 undefined refs, 0 overfull boxes — closes the R8 confirmation board on the exact v1C.0.10 PDF (Claude INT MINOR REVISIONS, 0 MAJOR/7 MINOR with an 18-item verification log that independently recomputed every checkable equation with zero numeric errors; Grok REJECT; Gemini MAJOR REVISIONS; Perplexity leg failed). The R8 board introduced the correctness/presentation classification: every genuinely-new item is classed correctness-grade (wrong math/number/attribution/claim) or presentation-grade (length, repetition, layout, style). Four genuinely-new items closed — two correctness-grade attribution/citation fixes (the Benedetti-Speziale Eq.-7 pointer bound explicitly to the proceedings paper, companion to the full JHEP analysis; B12's gamma ~ 0.274 now cites the primary Ghosh-Mitra state-counting, PLB 616, 114 (2005), Crossref-verified) and two presentation-grade numeric-notation fixes (the Eq.-(3) integration relabeled Delta-ln-gamma with the exponentiation noted immaterial; the App. A hierarchy quotient prints 1.2209e19 GeV to match the quoted 8.7e122). Claude's headline MINOR-1 (spurious (1+gamma^2)^2 power in the printed |Omega44/alpha4|) FALSIFIED against the 200-DPI render — the exact PDF prints the correct one-power form; Grok's inputs-absent and no-commit-hash claims falsified against the printed values and pin; Gemini's filename-spaces claim falsified as a text-extraction artifact. Zero correctness-grade findings survive the audit on the R8 board's own evidence; the residual presentation-grade items (abstract length, tier-disclaimer repetition) route to the D-round. DRAFT: R9 is the correctness-convergence check on the exact v1C.0.11 PDF; no readiness score has been computed and no venue/Zenodo kit exists yet.",
        href: "/papers/paper1c_nogo_survey_v1C.0.11.pdf",
        external: true,
      },
    ],
