...
export const papers: Paper[] = [
  {
    ...
    pdfMeta: "PDF 777 KB · 16 pp · May 9, 2026, v1A.0.11",
    artifacts: [
  ...
  },
  {
    ...
    refs: "31+",
    readiness: 82,
    status: "82% — R46 forward closure (Wave 14-XXXX-r46): +6pp from 76 after closing R46 BLOCKER B1 (QSFI parenthetical fix: super-squeezed not equilateral) + 4 MAJORs M1 (SDB-Fisher r=0.84 exemption stated explicitly: SDB constrains linear-bias amplitude not bispectrum shape), M2 (13% polynomial null-space scatter added to abstract systematic budget), M3 (Table 4 explicit BF~17 row added retiring caption-note deflection), M4 (curvaton-natural BF~6 added to abstract clause). Per Wave 14-UUUU/RRRR: nit2 parameter-free softened, R45 BLOCKERs B1 (\\ref{sec:gr} fixed) + B2 (9.9σ joint-Fisher reframed) + 6 MAJORs (release tag, Bayes table caption, QSFI sign-convention, curvaton headline, halving range, polynomial null-space). Wave 14-YYYY (P3) queued.",
    statusVariant: "green",
    ...
    pdfMeta: "PDF 780 KB · 17 pp · May 9, 2026, v1.7.18",
    artifacts: [
  ...
  },
  {
    ...
    refs: "60+",
    readiness: 88,
    status: "88% — R46 forward closure (Wave 14-YYYY): +8pp from 80 after closing R46 BLOCKER B1 (sub-tier 264,938/113,342 decomposition DROPPED entirely per R46 reviewer's option (a) — replaced with point-source 378,080 + Planck CMB-patch 200 stratification + approximate ~113,000/~265,000 LAMOST-vs-other split with explicit pointer to released cluster manifest pathc_multi_survey_matches.parquet for exact per-cluster attribution; the false logical implication that LAMOST has zero cross-survey overlaps is retired) + 4 MAJORs M1 (17.8% downgraded to single-sample point estimate with explicit converse-hypothesis caveat across abstract+§simbad+limitations+conclusions), M2 (b-ratio numerical: §sec:fnl line 551 corrected from b_GS/b_full,jk=1.19±0.65 to b_GS/b_full,jk=2.83±2.03 matching α_GS,jk=+1.83), M3 (PTA sign convention propagated to §sec:nanograv body and §sec:bounce_implications: −1.13σ→+1.13σ above, −4.6σ→+4.61σ above), M4 (α_GS 0.90σ-from-null added to abstract). Wave 14-ZZZZ (P4) queued.",
    statusVariant: "green",
    target: "ApJS",
    ...
    pdfMeta: "PDF 28.38 MB · 42 pp · May 9, 2026, v3.1.29",
    artifacts: [
  ...
  },
  {
    ...
    refs: "30+",
    readiness: 85,
    status: "85% — R46 forward closure (Wave 14-ZZZZ): +5pp from 80 after closing all 4 P4 R46 MAJORs M1 (§IV.C self-contradiction recast: \"convergence at the v2 starting point is neutral with respect to whether CE-ResNet carries residual systematic chirality bias at the calibration level — the optimizer's failure to move proves that the loss surface is flat at chance accuracy on GZ1 binary labels, not that CE-ResNet is unbiased\"), M2 (quadrature bound widened: \"[1.12, 1.5]pp under non-negative correlation; the negative-correlation case ρ<0 is empirically possible but produces |1.0-0.5|=0.5pp at ρ=-1, ~0.7pp under partial cancellation\"), M3 (factor-of-9 propagated to §I + §VIII.A + 2 abstract sites as consistent \"factor of ~6-12 (Shamir 2-4% comparator range; central ~9)\"), M4 (McNemar b+c=7,812 reframed as modeling point estimate not measured tabulation: \"under the working modeling assumption that the joint contingency is consistent with marginal CW fractions and moderate Cat-C/GZ1 correlation, we adopt b+c≈7,812 as the modeling point estimate; exact per-galaxy joint label tabulation pod-side-only; the Z=6.77 figure is the assumed-discordance result, not measurement-grade\"). Wave 14-AAAAA (R46 minors+nits sweep) queued. Per Wave 14-TTTT: closed R45 BLOCKER B1 (Platt 6-sig-fig claim downgraded to honest match of artifact: L-BFGS converged at the v2 starting point A_0=1/4.65=0.21505, B_0=-1.58, calibration accuracy at chance 0.519, Brier dominated by deterministic logits — interpretation: GZ1 binary labels do not provide bias-independent recalibration leverage at this 46,017-galaxy match scale, existing Platt is consistent within rounding precision) + 5 of 6 MAJORs M1 (McNemar Z=6.77 reframed as point-estimate at adopted-discordance b+c=7,812; range bounded [3.94, ∞] depending on realised contingency), M2 (abstract orthogonality reframed empirical-via-dipole-null), M3 (quadrature-add reframed sufficiency-check not precision-check; observed 1.2pp falls in [1.12, 1.5]pp under any correlation), M5 (Table 5 N caveat: factor 6-12 range over Shamir's reported 2-4%), M6 (HC-spiral n=471,049 vs HC-broad n=949,584 distinguished). M4 N_eff histogram requires pod compute (carried). Wave 14-UUUU minors+nits next.",
    statusVariant: "green",
    target: "MNRAS",
