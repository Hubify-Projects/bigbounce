...
export const papers: Paper[] = [
  {
    ...
    refs: "70+",
    readiness: 85,
    status: "85% — R46 forward closure (Wave 14-AAAAA, R46 minors+nits sweep): +1pp from 84 after closing P1A R46 m3 (Table II w0wa column asymmetry fix: matter bounce ✗ / Cuscuton ✗ / Ekpyrotic ✗ / slow-roll ✗ all converted to 'not tested‡' with new ‡ footnote pointing at Paper 1B §VII.H zero free-w0-wa-sample disclosure; quintom-B 'consistent†' retained — asymmetry now framed as theoretical-accommodation vs not-computationally-tested rather than ✗ vs ✓), n1 (abstract '14 distinct' → '14 mechanism-class constraints (one of which, B8, is a special case of B14: the parity-even-interaction barrier is the observational consequence of the perturbation-transparency theorem, retained in the catalog for historical mechanism-class completeness)'), m1 self-resolved by v1A.0.12 date stamp 2026-05-09. R46 m2 (8 OOM / 17.5 OOM) confirmed n/a — no '8 OOM' anchor in body; existing α/M arithmetic at L580-582 computes M_Pl·(α/M) ~ 10^-2 (2 OOM gap) which is the actual quoted figure. R46 n2 (35 OOM m_θ qualifier) confirmed n/a — m_θ ~ H_0 regime already explicit at L657-678. Wave 14-BBBBB (R47 launch — second convergence test) queued. Per Wave 14-WWWW: R46 BLOCKER B1 (Route 2 §IV.B Δθ_one-loop/Δθ_obs ~ 10^-58 dimensionless ratio) + 3 MAJORs M1 (R4 CC-tuning closure), M2 ('would plausibly be erased' propagation), M3 (D_inf §XII downgrade) all closed. Per Wave 14-QQQQ: both R45 BLOCKERs B1+B2 + 4 MAJORs M1-M4 closed.",
    statusVariant: "green",
    ...
    pdfMeta: "PDF 781 KB · 17 pp · May 9, 2026, v1A.0.12",
    artifacts: [
  ...
  },
  {
    ...
    refs: "31+",
    readiness: 83,
    status: "83% — R46 forward closure (Wave 14-AAAAA, R46 minors+nits sweep): +1pp from 82 after closing P2 R46 m1 ('exactly half the full polynomial' softened in abstract to 'approximately half ... per-config ratios 0.500 ± 0.001 at equilateral, folded, and squeezed; reported individually in §sec:assumptions and Table~tab:benchmarks'), m2 (600,000 MC count rhetorical-decoration converted to '6×10⁵ realizations' with explicit one-line clarification that this is the convergence-stability sample size, not a tightening of the Heinrich+2023 σ(f_NL)=0.7 forecast), m3 (Table 'Corrected (10% residual)' relabeled '...sanity row' with caption footnote leading 'by construction equal to Ideal at this template-overlap order; included as a no-op sanity row, not as an independent configuration'), n1 (release tag pinning v1.7.18-paper2 → v1.7.19-paper2 across both URL and release-tag display via replace_all), n2 (Maldacena 0.015 §I gauge-frame caveat closed: '(gauge-frame value; conformal-Fermi-frame equivalent differs by O(slow-roll) corrections per Pajer:2013 + TanakaUrakawa:2011 and is not the bounce-discriminating quantity here)' — abstract + §VII already had the caveat). R46 m4 (Cai polynomial '3 valid solutions' attribution) confirmed n/a — paper frames the underdetermined coefficient system as a 3D null space not as 3 polynomial roots; existing footnote at L70 already cites Cai:2009fn and adopts the -35/8 root canonically. Wave 14-BBBBB (R47 launch) queued. Per Wave 14-XXXX-r46: R46 BLOCKER B1 + 4 MAJORs M1-M4 all closed.",
    statusVariant: "green",
    ...
    pdfMeta: "PDF 781 KB · 17 pp · May 9, 2026, v1.7.19",
    artifacts: [
  ...
  },
  {
    ...
    refs: "60+",
    readiness: 89,
    status: "89% — R46 forward closure (Wave 14-AAAAA, R46 minors+nits sweep): +1pp from 88 after closing P3 R46 m1 (9.6× α-ratio not b-ratio relabel in abstract: 'central value is 9.6× higher as an α-ratio (NOT a b-ratio: α_GS,jk/α_full,jk = +1.83/0.19 = 9.63, while underlying bias-ratio shift is the more modest b_GS/b_full,jk ≈ 2.83 ± 2.03)'), m4 (9.6× central-value shift physical-plausibility narrative added: two sentences tying shift to Gold+Silver P>0.9 confidence cuts preserving bounce f_NL=-35/8 shape signal that low-confidence tracers dilute, and connecting direction to independent Pipeline-1 1.58× clustering-bias enhancement), m2 (319,443 dropped from title — title now '378,280 Path-C Unique Anomalies and Native-Trained Novelty Rates...'; 319,443 retained in abstract/body for before/after baseline narrative), m3 (γ rounding harmonized: '3.20 ± 0.42' canonical with raw-fit '3.193 ± 0.423' parenthetical at first mention §6 + §App.cross-checks; subsequent mentions use rounded 3.20), n1 (sec:limitations added 'prior-fiducial' qualifier to fiducial bias enhancement α=0.15), n3 (orphan ◊ footnote in tab:survey_summary removed — symbol existed in caption block but no marker existed in table body; redundant with caption + ‖ footnote). R46 n2 (4.6σ vs 4.61σ harmonization) confirmed n/a — all 4 occurrences L557/614/633/949 already read +4.61σ. Wave 14-BBBBB (R47 launch) queued.",
    statusVariant: "green",
    target: "ApJS",
    ...
    pdfMeta: "PDF 28.38 MB · 43 pp · May 9, 2026, v3.1.30",
    artifacts: [
  ...
  },
  {
    ...
    refs: "30+",
    readiness: 86,
    status: "86% — R46 forward closure (Wave 14-AAAAA, R46 minors+nits sweep): +1pp from 85 after closing all 5 P4 R46 minors+nits — m2 (bootstrap 28.80σ 'external peer-review-required uncertainty' framing softened at L990: replaced with 'self-consistency check on the raw-catalog asymmetry baseline (the 28.80σ figure is the bootstrap-stability metric of the chirality-fraction estimator, not an external-validation σ; an independent non-Anthropic cross-vendor R-round is required for that)'), m3 (§VII.B confidence stratification: HC-spiral N=471,049 spiral-only P>0.9 vs HC-broad N=949,584 max-prob>0.6 disambiguated with explicit N counts + cluster-distinguishing paragraph), n1 ('structurally parity-asymmetric' hangover at L397-398 replaced with neutral 'exhibits a residual CW/CCW asymmetry at the sub-percent level'), n2 (Footnote 4 condensed from 14 lines to 5: retained core caveat — canonical computation method + supersession of pre-v1.0.6 3,321,795 figure; ~9 lines saved). R46 m1 (3.32M vs 3.20M canonical N denominator) confirmed verified — body uses 3.20M everywhere; sole remaining 3.32M reference at L1924 is the explicit canonical-N qualifier in the diagnostic-baseline context which is correct. Wave 14-BBBBB (R47 launch) queued. Per Wave 14-ZZZZ: all 4 P4 R46 MAJORs M1-M4 closed. Per Wave 14-TTTT: R45 BLOCKER B1 (honest GZ1 Platt downgrade) + 5 of 6 R45 MAJORs closed (M4 N_eff histogram pod-compute-gated, carried).",
    statusVariant: "green",
    target: "MNRAS",
    ...
    pdfMeta: "PDF 25.66 MB · 23 pp · May 9, 2026, v1.0.38",
    artifacts: [
