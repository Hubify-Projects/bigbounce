...
export const researchPrograms: ResearchProgram[] = [
  {
    ...
    supportingLinks: [
      ...
      },
      {
        ...
        description:
          "A systematic survey of 7 foundation mechanism classes and 6 observational branches, extracted from the retired P1U barrier catalog, collapsing to 13 distinct mechanism-class constraints across 14 catalog entries and closing four candidate dark-energy routes (R1-R4). Draft v1C.0.10, compiled 20 pp, 0 undefined refs, 0 overfull boxes — closes the R7 confirmation board on the exact v1C.0.9 PDF (Claude INT MINOR REVISIONS, 0 MAJOR/8 MINOR with every recomputable equation verified; Grok REJECT; Gemini MAJOR REVISIONS; Perplexity leg failed): seven genuinely-new items closed, all wording/notation/presentation-grade — the B1 tuning ratio un-inverted (cancellation to one part in (M_Pl/H_0)^2 ~ 1e122, the literal delta-m^2/m^2 form was inherited inverted from the monolith), the Sec. V four-fermion contact operator no longer mislabeled parity-odd (its Fierz image is parity-even per App. B), the Omega44/alpha4 range floor corrected to O(3)-O(5) (bounded below by 378/120), the Route-2 one-loop numerator exhibited as an explicit intermediate display (Hubble-time accumulation; dropped conservative factors named), the 3.6/2.9-sigma values qualified as not directly comparable (different datasets and null procedures), the Fig. 1 caption now states the entries' mixed evidentiary status with a Table II pointer, and the Data & Code artifact paths set so none breaks mid-filename. Gemini's space-before-colon claim falsified against the 300-DPI render (italic correction; extraction artifact); the ACT DR6 citation verified against the live arXiv record (exact match). DRAFT: R7 found genuinely-new items, so an R8 confirmation board is required; no readiness score has been computed and no venue/Zenodo kit exists yet.",
        href: "/papers/paper1c_nogo_survey_v1C.0.10.pdf",
        external: true,
      },
    ],
