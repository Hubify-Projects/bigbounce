...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    refs: "46",
    readiness: 94,
    status: "94% — v1.0.63 (12th real-vendor R-round on v1.0.62 closed). R-round caught GPT-B1 (abstract/Table III/Fig caption still cited v1.0.55 +0.26σ analytic projection while only §Conclusions had been updated to v1.0.62 +1.85σ direct-MC) + 5 GPT MAJORs (1.85σ over-interpretation as 'rejection', unsupported 0.6%-half-modulation extrapolation, estimator-table N-convention ambiguity, null_distribution.npy provenance, half-modulation/full-amplitude floor convention) + DeepSeek B1 (null_distribution.npy not in repo) + 3 DeepSeek MAJORs (sensitivity-floor inequality, hemisphere-LEE depth-correlation argument, manifest provenance) + Perplexity 2 MAJORs (closure-paragraph reconciliation language, deferrals-paragraph audit-log tone). Gemini-2.5-Pro: 0 BLOCKERs (6th consecutive endorsement-class verdict on v1.0.62). All BLOCKERs + MAJORs closed in v1.0.63 bundled wave: (a) v1.0.62 direct-MC value +1.85σ propagated to abstract + Table III caption + Fig multipoles caption + §IX conclusions item-2; (b) §Conclusions paragraph rewritten to remove the unsupported 0.6%-half-mod extrapolation, reframe 'rejection' as 'mild canonical-mask excess', add empirical-rank p_MC=0.030, expand the estimator table with explicit N_spiral and N_pixels-used columns, add physical justification for full-sky and strict-superset robustness, restructure honest-interpretation into a (i)/(ii)/(iii) enumerated list; (c) null_distribution.npy pulled from pod via base64-over-SSH and committed to outputs/canonical_provenance/canonical_n_master_l1_direct_null_distribution.npy (sha256 4812778e...); (d) mc_seed_manifest.json extended with v1_0_62_addendum recording all sha256s + reproduce command + interpretation paragraph. PDF 32pp / 25.90 MB / 0 undef refs / sha256 df1390af... Readiness 93→94 (+1pp) per feedback_readiness_oscillation recovery after substantive closure; awaiting fresh 13th vendor R-round on v1.0.63 to confirm clean state + Houston sign-off (gate b) to lift to 99.",
    statusVariant: "green",
    target: "MNRAS",
    ...
    pdfMeta: "PDF 25.90 MB · 32 pp · May 14, 2026, v1.0.63",
    artifacts: [
