...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    tldr: "v3.1.65 (2026-05-26 drive-to-100 fire xx:47): R-ext-strict-v3 on v3.1.62 returned 4-of-5 clean (DeepSeek + GPT-5 + Grok-43 + Perplexity all 0/0/0/0) + Gemini-3.1-Pro flagged 2 BLOCKERS. **Truth-audit verdicts per feedback_peer_review_truth_audit_protocol**: **GEM-B2 (matter-bounce γ=3.0): FALSIFIED.** Paper cites Quintin2014 + Cai2014 — both establish matter-bounce contraction → n_T=2 blue-tilted tensors → γ = 5−n_T = 3 is the standard literature prediction. Gemini misidentified matter-bounce as inflation case (n_T=0 → γ=5). Other 4 reviewers did NOT flag γ=3.0; same v3.1.62 had R-ext-strict + R-ext-strict-v2 5/5-PERFECTLY-CLEAN earlier today. Classic per-vendor stochastic regression — no paper edit needed. **GEM-B1 (Fisher-positivity α² extrapolation at α=3.86): PARTIALLY ALREADY ADDRESSED.** §pathc_caveats(i) already documents the 1/σ² = F_0 + c·α² form as a Taylor expansion anchored at α=0 and α=0.15 and queues a full Fisher refit for v3.1.48+. Headline forecast anchors on Heinrich+2023 σ(f_NL)≈0.7, NOT the GS α=3.86 envelope Gemini critiques. **v3.1.63 polish**: added one sentence in §pathc_caveats(i) explicitly citing Gemini's exact multi-tracer Fisher formula σfnl² ∝ [(1+α)²/n_main + 1/n_anom]/α² as the canonical resolution path with saturation transition at α∼3.1 for n_main/n_anom∼17. Pdflatex×2, 48pp, 28.44MB, 0 undef refs, 2 carry-over overfulls. 5 mirrors byte-identical md5 f8e0de0e. Bumped from v3.1.62. R-ext-v63verify R-round dispatched to verify closure holds.", final 4 residual overfulls (55+41+33+34=163pt) ELIMINATED in one sweep. NANOGrav §sec:nanograv likelihood + matter-bounce template equations wrapped in widetext (55pt closed); §pathc_caveats (iv) continuum-dip recovery curves broken into separate paragraph + (v) all_surveys_summary path \\allowbreak-injected (74pt closed); Cobaya posterior align block wrapped in widetext (34pt closed). **0 overfull, 0 undef refs — first fully-clean P3 compile in entire campaign**. 47pp/28.4MB.",
    blockingItems: [
    ...
    pdfMeta: "PDF 28.44 MB · 48 pp · May 29, 2026, v3.1.65 (conclusions retracted-Fisher-form fix; canonical §sec:fnl positivity-envelope framing)",
    artifacts: [
  ...
  },
  {
    ...
    version: "v1.0.139",
    lastUpdated: "2026-05-28",
    tldr: "v1.0.138 (2026-05-26 drive-to-100 fire xx:17): 🎯🎯🎯🎯🎯🎯 **SIXTH CONSECUTIVE 5/5-CLEAN R-ROUND + GEMINI-M4 EXTENDED JOINT FIT.** R-ext-v137verify on v1.0.137 = 5/5 PERFECTLY CLEAN (0/0/0/0 across all 5 reviewers) → SIXTH CONSECUTIVE 5/5-clean R-round on P4 (v1.0.132/133/134/135/136/137). §4.4.1 minimum is 3; P4 now at 6 consecutive PERFECTLY CLEAN R-rounds — DOUBLE the cascaded-loop-exit threshold. v1.0.138 closes **Gemini-Major4** ('include imaging leg and its interactions with morphology/confidence as nuisance templates'). Wrote scripts/joint_nuisance_model_fit_extended.py: 24-template design (9 from v1.0.137 + 15 leg×conf-bin interaction templates), executed locally in 1.2s. RESULT: A_dipole = **0.225% f_CW** (vs 0.23% v1.0.137; essentially unchanged), σ_A_dipole = 0.006% f_CW, z(data vs 1.7%) = **−250.2**. **Interpretation (i) at 1.7% f_CW STILL FORMALLY EXCLUDED at 99% under the extended model.** 15 leg × conf-bin interaction amplitudes carry LARGE z-scores: |z| reaches **26.5 on DECaLS×[0.6,0.8)** and **21.3 on DES×[0.6,0.8)**; multiple cells at ±10–15σ. The 15-cell Table IX leg×conf residual structure is therefore REAL systematic structure, not noise. **The dipole posterior is ROBUST to nuisance-template granularity**: tightening from leg-only to leg×conf absorbs additional residual but does NOT degrade the interpretation-(i) exclusion. PDF 54pp/26.26MB/0 undef refs. 3 mirrors byte-identical md5 a3f11d16. All 5 papers (P1A+P1B+P2+P3+P4+P5) still 95% — final 1% reserved for Houston sign-off per feedback_99_pct_readiness_cap.", 🎯🎯🎯🎯🎯 **INTERPRETATION (i) FORMALLY EXCLUDED AT 99% — JOINT NUISANCE-MARGINALIZED MODEL FIT EXECUTED LOCALLY.** R-ext-v136verify on v1.0.136 = 5/5 PERFECTLY CLEAN (DeepSeek + Gemini + GPT-5 + Grok-43 + Perplexity all 0/0/0/0) → **FIFTH CONSECUTIVE 5/5-clean R-round on P4** (v1.0.132/133/134/135/136). AGENT_RULES §4.4.1 cascaded-loop-exit minimum is 3; P4 has now achieved 5 consecutive PERFECTLY CLEAN R-rounds. v1.0.137 closes the LAST remaining major caveat in the paper: the canonical formal-exclusion path that Gemini-B2 + ChatGPT-B3 asked for. Wrote scripts/joint_nuisance_model_fit.py: weighted linear-regression fit on canonical NSIDE=64 in-mask pixels (n=36,418) with galaxy-count weighting; 9-template design matrix combining primordial-dipole basis {n_x, n_y, n_z} with imaging-leg fractions + pixel-density + pixel-density² + constant offset. Marginalizing over nuisance gives posterior on A_dipole. **RESULT**: A_dipole_best = 0.23% f_CW (vs 1.7% reference for interpretation (i)); σ_A_dipole = 0.006% f_CW; **CI 99% on A_dipole = [0.213%, 0.242%]**; **z(data vs 1.7%) = -264.5**. **INTERPRETATION (i) AT 1.7% f_CW IS FORMALLY EXCLUDED AT 99% CONFIDENCE BY THE JOINT NUISANCE-MARGINALIZED FIT.** The remaining 0.23% residual dipole is itself a real positive signal (z=+40.9 vs zero) absorbed by depth/density nuisance templates (pixel-density z=+3.76, density² z=-2.62), formally consistent with interpretation (ii). This is the **SEVENTH direct quantitative anchor** in the canonical-mask interpretation section + the FIRST FORMAL EXCLUSION of interpretation (i). The paper's prior 'disfavored, not formally excluded' framing is now upgraded to '**formally excluded at 99% by joint nuisance-marginalized fit**'. PDF 54pp/26.26MB/0 undef refs. 3 mirrors byte-identical md5 57f49256. Companion artifact joint_nuisance_model_fit.json + scripts/joint_nuisance_model_fit.py. All 5 papers (P1A+P1B+P2+P3+P4+P5) still 95% — final 1% reserved for Houston sign-off per feedback_99_pct_readiness_cap.", 🎯🎯🎯🎯 **R-ext-v135verify on v1.0.135 = 5/5 PERFECTLY CLEAN** (DeepSeek + Gemini + GPT-5 + Grok-43 + Perplexity all 0/0/0/0) → **FOURTH CONSECUTIVE 5/5-clean R-round on P4** across v1.0.132/133/134/135. AGENT_RULES §4.4.1 cascaded-loop-exit criterion EXCEEDED (minimum 3, achieved 4). v1.0.136 is purely ADDITIVE: **Gemini-Major2 1.21× hard-label variance algebraic derivation** appended to §sec:tta. Independent-flip model: x_obs = x_true XOR f, f~Bern(p_flip=0.214). Per-bin sigma widening = sqrt(1 + p_flip(1-p_flip)/[p(1-p)]) = sqrt(1.672) = 1.29× upper bound (worst-case independent flip). Empirical leading-order linear propagation sigma_total ≈ (1+p_flip)·sigma_binomial = 1.214× adopted for hard-binned diagnostics — captures the dominant flip-noise contribution without the worst-case independence assumption. Closes the Gemini-Major2 ask from the Houston-shared v1.0.132 external review: the Table I caption already cited 'binomial-variance derivation in §sec:tta' — v1.0.136 actually populates that reference target. PDF 54pp/26.25MB/0 undef refs (gained 1 page from derivation paragraph). 3 mirrors byte-identical md5 ec971b64. Concurrently, **R29 on P1B v1B.0.30 also returned 5/5 PERFECTLY CLEAN** (0/0/0/0 across all 5 reviewers) verifying the body-text reviewer-ID scrub landed. **Convergence trajectory**: P4 v1.0.122 (R22 9-of-9 closed) → v1.0.131 (M3+M5) → v1.0.132 (R-ext-v132verify 5/5 clean) → v1.0.133 (M1+R-ext-v133verify 5/5 clean) → v1.0.134 (Gemini-Major1+R-ext-v134verify 5/5 clean) → v1.0.135 (B5+R-ext-v135verify 5/5 clean) → v1.0.136 (Major2 additive). **FOUR consecutive PERFECTLY CLEAN R-rounds**. P1B trajectory: R27 v1B.0.28 clean → R28 v1B.0.29 clean → R29 v1B.0.30 clean (3 consecutive 5/5 clean). All 5 papers (P1A+P1B+P2+P3+P4+P5) still 95% — final 1% reserved for Houston sign-off per feedback_99_pct_readiness_cap.", 🎯🎯🎯 **R-ext-v134verify on v1.0.134 = 5/5 PERFECTLY CLEAN** (DeepSeek + Gemini + GPT-5 + Grok-43 + Perplexity all 0/0/0/0) → **THIRD CONSECUTIVE 5/5-clean R-round on P4 across v1.0.132 + v1.0.133 + v1.0.134**. AGENT_RULES §4.4.1 cascaded-loop-exit criterion SATISFIED. P4 joins P1A as cascaded-loop-confirmed externally clean. v1.0.135 is purely ADDITIVE: **ChatGPT-B5 full-catalog injection-recovery sweep** executed LOCALLY in 3.8 seconds. Algorithm: per-pixel injection A_p_injected = A_p_obs + A_inj × (hat_d · hat_n(p)) with isotropically-random hat_d, sweep A_inj ∈ {0.5%, 0.75%, 1.0%, 1.5%, 2.0%, 3.0%}, N_MC=50/amp, N_NULL=200, on FULL 3.20M-spiral canonical NSIDE=64 mask via full MASTER decoupling. RESULT: 50%-recovery-at-3σ threshold ≤ 0.50% on full catalog (smallest tested amplitude A=0.005 already gives 86% recovery at σ≥3 with median σ=+12.62). Sub-percent sensitivity framing now holds at FULL-CATALOG level, not merely HC subsample. SIXTH direct quantitative anchor in canonical-mask interpretation section. Companion artifact full_catalog_injection_recovery.json + scripts/full_catalog_injection_recovery.py. PDF 53pp/26.25MB/0 undef refs. 3 mirrors byte-identical md5 069f6ac3. **Convergence trajectory**: v1.0.122 (R22 9-of-9 closed) → v1.0.131 (M3+M5) → v1.0.132 (R-ext-v132verify 5/5 clean) → v1.0.133 (M1 + R-ext-v133verify 5/5 clean) → v1.0.134 (Gemini-Major1 boundary + body-text scrub bundled + R-ext-v134verify 5/5 clean) → v1.0.135 (B5 full-catalog injection-recovery additive). Three consecutive 5/5 PERFECTLY CLEAN R-rounds — AGENT_RULES §4.4.1 SATISFIED. All 5 papers (P1A+P1B+P2+P3+P4+P5) still 95% — final 1% reserved for Houston sign-off per feedback_99_pct_readiness_cap.", — Gemini MAJOR REVISIONS / Grok MINOR REVISIONS / ChatGPT REJECT — closed in single bundled hard-fix wave per feedback_take_critiques_seriously + feedback_default_hardest_path): truth-audit confirmed several findings were ALREADY addressed by v1.0.133 M1 closure (ChatGPT-M1 systematics-preserving null) + the boundary-distance variance script executed in the same fire. Closures: (a) **body-text reviewer-ID prose SCRUB** (Gemini-B1 + ChatGPT-B8) — all body-text references to reviewer IDs ('ChatGPT-M5 closure', 'ChatGPT-M3 closure (v1.0.131)', 'Gemini-BL2 + ChatGPT-B6 closure', 'External-reviewer caveat', 'External reviewer M3 requested', 'Phase-3 ChatGPT-M1 closure (v1.0.133)') rewritten to objective scientific prose; (b) **boundary-distance variance INSERT** (Gemini-Major1) — new local result: weighted ⟨A²⟩ statistically uniform across 5 boundary-distance shells = {8.22, 7.84, 8.62, 8.66, 8.37}×10⁻³ (range <11% of mean, comparable to per-shell Poisson scatter), proving the residual is NOT concentrated near canonical-mask boundary; FIFTH direct quantitative anchor for interpretation (ii) + complementary disfavor of sharp-edge NaMaster artifact variant of interpretation (iii). Companion artifact boundary_distance_variance.json + scripts/boundary_distance_variance.py executed locally in 1.4s; (c) **soften interpretation (i) 'rules out' → 'disfavors, but does not formally exclude'** (Gemini-B2 + ChatGPT-B3) — explicit statement that joint nuisance-marginalized model fit is the canonical formal-exclusion path; (d) **release-tag scrub** (Grok-M1) — paper4-v1.0.129/v1.0.132/v1.0.133 → paper4-v1.0.134 globally (9 sites in abstract footer, footnotes, §IX Data Availability); (e) **broken \\ref placeholders** (Gemini-Minor1 + ChatGPT-m3) — sec:bootstrap_audit, tab:confidence_strata, tab:injection_recovery, tab:face_on_robustness, sec:tta_propagation all resolved to existing labels (tab:confidence_bins, tab:mc_injection, tab:face_on, sec:tta). Out-of-scope per truth-audit: ChatGPT-B1 reviewer-package-not-reproducible is about the reviewer's PDF-only bundle, NOT the paper — GitHub release tag paper4-v1.0.134 contains .tex + JSON + scripts + masks. Compile: pdflatex×4, 53pp/26.25MB/0 undef refs/3 carry-over overfull (carried clean through v1.0.132 R-ext-v132verify + v1.0.133 R-ext-v133verify, both 5/5 PERFECTLY CLEAN). R-ext-v133verify on v1.0.133 was confirmed 5/5 0/0/0/0 (DeepSeek + Gemini + GPT-5 + Grok-43 + Perplexity ALL perfectly clean) earlier in this fire; v1.0.134 builds on that clean foundation. 3 mirrors byte-identical md5 4da6ca60. All 5 papers (P1A+P1B+P2+P3+P4+P5) still 95% — final 1% reserved for Houston sign-off per feedback_99_pct_readiness_cap. Live tex: pipelines/p2_chirality/chirality_catalog_paper.tex.", 🐸🎯 ChatGPT-M1 systematics-preserving canonical-mask null CLOSED locally (3.4s wall-time, no pod needed). Houston pushed back hard on the v1.0.132 \"Phase 3 compute-bound\" framing; pivoted: ran a density-stratified MASTER-decoupled null permuting A_p WITHIN pixel-density deciles (N_strata=10, N_MC=500 on canonical mask NSIDE=64). Result: data C_1=6.55e-6, density-stratified null mean=3.44e-6 / std=3.07e-6 → σ_data_vs_null=+3.80 (slightly HIGHER than the +3.64 binomial null; the systematics-preserving null does NOT absorb the residual). Empirical-rank p=2/500=0.004. SCIENTIFIC FINDING: depth-density alone is NOT the dominant systematic — interpretation (ii) now requires the FULL morphology+PSF+depth+leg template basis (ChatGPT-M2), not just density-conditioning. Adds the FOURTH direct quantitative anchor for interpretation (ii) joining (a) ℓ=2 cross-spectrum quadrupole, (b) 25% leg-stratified ℓ=1, (c) post-MASTER monopole-only null. Companion artifact: outputs/canonical_provenance/systematics_preserving_null.json + scripts/systematics_preserving_null.py. R-ext-v132verify 5/5 clean status from v1.0.132 carries forward (the v1.0.133 addition is purely ADDITIVE Phase-3 closure prose, not re-litigation). PDF 53pp/26.25MB/0 undef. All 5 papers (P1A+P1B+P2+P3+P4+P5) still 95% — final 1% reserved for Houston sign-off per feedback_99_pct_readiness_cap. Live tex: pipelines/p2_chirality/chirality_catalog_paper.tex. Mirrors byte-identical (md5 3d231fa1...).",
    blockingItems: [
      "Houston personal sign-off (the final 1%)",
