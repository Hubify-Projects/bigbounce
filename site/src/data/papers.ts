...
export const papers: Paper[] = [
  {
    ...
    refs: "70+",
    readiness: 80,
    status: "80% — R47 launch backward step (Wave 14-BBBBB): −5pp from 85 after CCAI multi-agent R47 self-review found 1 BLOCKER + 1 MAJOR + 3 MINOR + 1 NIT (6 findings; R46 had 9; −33% CONVERGING). The R47 BLOCKER (P1A-R47-B1) is a CROSS-PAPER consistency failure introduced by Wave 14-AAAAA: the new ‡ footnote at L1056 cites 'Paper 1B §VII.H' which does not exist (Paper 1B §VII is 'Cross-Paper Verification Status' with no lettered subsections), AND it claims 'zero free-w0wa samples' in Paper 1B but Paper 1B Table III L481 explicitly lists 'DESI DR2 w0wa (new): ~109 accepted, Running'. Coordinated CROSS-PAPER fix landing in Wave 14-CCCCC: re-anchor P1A footnote + add P1B §VII.H content (or restructure §VII to include explicit free-w0wa chain-status disclosure). R47 MAJOR M1: abstract/intro/conclusions say '7 observational research branches (Branches H–O)' but H–O is 8 letters and Table II shows 6 letter-labelled branches — off-by-one. R47 minors: R2 ratio inputs as written give ~10⁻⁶⁰ vs paper's ~10⁻⁵⁸ (2 OOM); Conclusions still says '14 distinct' while abstract has B8⊂B14; v2.3.0 release tag doesn't exist. R47 nit: Heinrich:2023 bibkey but published 2024. Wave 14-CCCCC (P1A v1A.0.12 → v1A.0.13 R47 B1 cross-paper coordinated fix + 1 MAJOR) next.",
    statusVariant: "green",
  ...
  },
  {
    ...
    refs: "31+",
    readiness: 76,
    status: "76% — R47 launch backward step (Wave 14-BBBBB): −7pp from 83 after CCAI multi-agent R47 self-review found 1 BLOCKER + 3 MAJOR + 3 MINOR + 2 NIT (9 findings; R46 had 11; −18% CONVERGING). R47 BLOCKER P2-R47-B1: Table 4 (tab:bayes) row 5 claims BF~17 at 'delta + broad multifield [-15,+15] + σ_GR=0.5'; Table 5 (tab:gr) BF-vs-Tuned column at σ_GR=0.5 gives 9.4. Tables disagree by 1.8× if 'Tuned' = broad multifield, OR Table 5 caption is silently using a narrower competitor prior — the BF~17 row addition in Wave 14-XXXX-r46 collided with the existing Table 5 framing. R47 MAJORs: M1 (3×6 SVD null-space rank check missing — the 3D null space claim doesn't show numerical SVD evidence), M2 (Table 4 row 4 'theoretical maximum only' contradicts row 5's BF~17), M3 (abstract bispectrum-only 5.25σ headline-vs-joint-Fisher 9.9σ idealized — why is the smaller number the headline?). R47 MINORs: m1 bounce-prior vs competitor-prior axes unclear at L211; m2 σ=0.7 vs σ=0.5–0.7 inconsistency between abstract and §VII.B; m3 missing error bar on birefringence aside. R47 NITs: 2 typos. Wave 14-DDDDD (P2 v1.7.19 → v1.7.20 R47 B1 + 3 MAJORs) queued.",
    statusVariant: "green",
  ...
  },
  {
    ...
    refs: "60+",
    readiness: 84,
    status: "84% — R47 launch backward step (Wave 14-BBBBB): −5pp from 89 after CCAI multi-agent R47 self-review found 0 BLOCKER + 2 MAJOR + 3 MINOR + 2 NIT (7 findings; R46 had 12; −42% CONVERGING — P3 is now BLOCKER-free, the cleanest paper at R47). R47 MAJORs: M1 (§sec:limitations L583 still labels 17.8% as 'provisional upper bound' while abstract uses 'single-sample point estimate' AND Conclusions item 2 calls it 'genuine novelty floor' — three contradictory labels coexist; the R46/M1 closure should have deleted the 'upper bound' framing), M2 (data-availability paragraph L651 directs readers to pathc_unique_objects.parquet — the with-ACT 378,480-row sensitivity-check file — for 'all headline numbers', contradicting the rest of the paper's ACT-quarantine policy; off-by-200 reproducibility hit). R47 MINORs: m1 Conclusions item 4 says 'Three DESI×SDSS cross-matches' without the broader 637-cluster manifest from item 8; m2 §sec:fnl conflates two estimators (b_GS/b_full=3.17 per-bin geomean vs 2.83±2.03 jackknife geomean) without a single-sentence rule; m3 abstract's '3-5σ detection significance' implicitly anchors to Münchmeyer σ(f_NL)≈0.4-0.9 rather than Wave 14-II internal Fisher floor 0.07-0.12, not stated explicitly. R47 NITs: 2 minor rounding/formatting slips. Cluster-manifest pointer + PTA sign convention + 9.6× α-vs-b distinction + γ rounding + eROSITA 298 top-cut + 378,080+200=378,280 stratification all clean. Wave 14-EEEEE (P3 v3.1.30 → v3.1.31 R47 2 MAJORs) queued.",
    statusVariant: "green",
    target: "ApJS",
  ...
  },
  {
    ...
    refs: "30+",
    readiness: 80,
    status: "80% — R47 launch backward step (Wave 14-BBBBB): −6pp from 86 after CCAI multi-agent R47 self-review found 0 BLOCKER + 3 MAJOR + 4 MINOR + 2 NIT (9 findings; R46 had 9; FLAT — propagation failures from ZZZZ+AAAAA waves rather than convergence). R47 MAJORs are all NEW propagation failures: M1 (§X Conclusion #1 still quotes 'marginal 2.75σ' for ℓ=1 — the SUPERSEDED raw-pseudo-C_ℓ buggy-normalization value; canonical is −0.12σ MASTER-deconvolved; paper's own §VII.B explicitly claims 'abstract, intro, conclusions all quote the high-confidence dipole upper bound' — Conclusion #1 violates this self-stated invariant), M2 (§X Conclusion #2 reverts to single-number 'factor of 9' — abstract/§I/§VIII.A all carry the corrected 'factor of ~6-12 (central ~9)' from Wave 14-ZZZZ M3 closure but conclusions weren't updated), M3 (§III.B claims 3.86× suppression on +0.79% → −0.26% but 0.79/0.26 = 3.04 not 3.86 — the 3.86× is the §IX.B raw-to-eq-NS-pool factor on +2.05% → −0.53% where 2.05/0.53 = 3.87; §III.B grafts a different paragraph's factor onto its own number pair). R47 MINORs: m1 simple-dipole p=0.33 vs 0.30, m2 σ_global 0.027% vs 0.028%, m3 per-pixel 4,168 vs √4,326 stale-snapshot residuals, m4 Table 4 bold 'All sky' still shows superseded 3,321,795. R47 NITs: 2 nits anchored on stale 2.75σ. Wave 14-FFFFF (P4 v1.0.38 → v1.0.39 R47 3 MAJORs) queued. Mechanical fixes, ~30 minutes, no new computation needed.",
    statusVariant: "green",
    target: "MNRAS",
