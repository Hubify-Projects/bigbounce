...
export const researchPrograms: ResearchProgram[] = [
  {
    ...
    supportingLinks: [
      ...
      },
      {
        ...
        description:
          "A systematic survey of 7 foundation mechanism classes and 6 observational branches, extracted from the retired P1U barrier catalog, collapsing to 13 distinct mechanism-class constraints across 14 catalog entries and closing four candidate dark-energy routes (R1-R4). Draft v1C.0.6, compiled 18 pp, 0 undefined refs, 0 overfull boxes — closes the R3 confirmation board on the exact v1C.0.5 PDF (Claude INT minor-revisions, Grok REJECT, Gemini MAJOR; Perplexity leg failed): 8 genuinely-new items closed, headlined by the honest downgrade of the operator-basis completeness framing to exactly what the released script verifies (Sec V retitled 'The Operator-Basis Argument'; the script's overclaiming docstring corrected in the same commit; a real mechanized enumeration recorded as deferred-genuine rather than claimed), plus the strict kappa = 8piG = M_Pl^-2 contradiction resolved with an exact reduced-mass definition, honest 61-67-order endpoint labeling (derived flow vs pessimistic chiral-count bound), explicit alpha_em/4pi rounding, a Table-III 'Final' column showing O4 = O5 -> kappa(J5.J5), and the R4 anchor's two-sentence algebraic origin carried from P1A. Three findings falsified with recomputation receipts (Omega-ratio exponent; Fierz (F_c)_13 = 1/2 with F^2 = identity verified; the no-operator-table claim). DRAFT: R3 found genuinely-new items, so an R4 confirmation board is required; no readiness score has been computed and no venue/Zenodo kit exists yet.",
        href: "/papers/paper1c_nogo_survey_v1C.0.6.pdf",
        external: true,
      },
    ],
