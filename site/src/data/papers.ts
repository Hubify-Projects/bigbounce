...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    refs: "46",
    readiness: 92,
    status: "92% — v1.0.58 (7th REAL cross-vendor verification round on v1.0.57 — Gemini-2.5-Pro EXPLICITLY ENDORSES: 'No blocker-grade findings. The paper is exceptionally rigorous, transparent, and self-critical. The reframe of the look-elsewhere effect rejection as a systematic artifact is correctly and consistently applied. ... The paper is at the external-review-gate state.'). 2 surviving BLOCKERs (down from 6 in tick 19) both closed: (GPT-B1) §VIII.B 'statistical fluctuation' + §IX 'no signal exceeding 3σ after LEE' framings still contradicted the LEE reframe at 2 downstream sites — rewrote as 'documented systematic-floor artifact (random-label null REJECTED at p_LEE≤10⁻⁴; below empirical detection threshold |A_dipole|>0.5% under systematic-inclusive injection-recovery)'; (DeepSeek-B1) added explicit canonical results-file citation in abstract for σ_dipole=0.43 p=0.30 figure (outputs/dipole/summary.json + outputs/canonical_provenance/mc_seed_manifest.json). 3/5 vendors fully clean on v1.0.57: Gemini (ENDORSES external-review-gate state), Grok (no BLOCKERs), Perplexity (no BLOCKERs/MAJORs). PDF 31pp / 25.89 MB / 0 undef refs / sha256 526a07bc... Cumulative P4 today: 79→84→87→90→91→89→92 across 7 ticks, +13pp net.",
    statusVariant: "green",
    target: "MNRAS",
    ...
    pdfMeta: "PDF 25.89 MB · 31 pp · May 14, 2026, v1.0.58",
    artifacts: [
