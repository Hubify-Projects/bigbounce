...
export const papers: Paper[] = [
  {
    ...
    pdfMeta: "PDF · 63 pp · v1U.0.18 · updated Jul 12, 2026 · md5 7b162d4b — v1U.0.18 (NJ6 INT-Claude MINOR closed, editorial-consistency only): the single residual 'far sub-critical' left by the v1U.0.17 tempering pass at the App. njl_gap leg-(B) intro (App. njl_gap:5133) — internally inconsistent with the already-tempered abstract, main-text mirror, and appendix verdict, all of which read 'comfortably'/'sub-critical' — is now 'comfortably sub-critical'. Wave-2/2 recompute-verified all NJL numbers against njl_gap_equation_route1_results.json; no numeric or physics change. v1U.0.17 (NJ4 INT-Claude MINOR-REVISIONS closed, presentation-only): the Route-1 NJL magnitude leg (B) now explicitly covers the attractive axial (AA) channel — the worst scanned |G_AA|/G_crit = 2×0.156 = 0.31 (exact factor G_AA/|G_scalar|=2) is stated in App. njl_gap and the main-text mirror rather than relying on the PP-only |G_PP|=|G_scalar| equality; a physical point is added that AA (Lorentz-violating axial-vector) and PP (parity-breaking) vacua are not the scalar σ-condensate at issue, so the leg-(A) sign exclusion is already decisive for the object under exclusion; blanket 'far sub-critical' in the abstract + discussion aligned to plain 'sub-critical' to match the tempered appendix worst-case (0.156/0.31). Conclusion unchanged. v1U.0.16 (INT-Claude genuinely-new MINOR closed): the Route-1 NJL sign-exclusion (leg A) is now explicitly scoped to the scalar (SS/χSB) channel; the axial (AA) and pseudoscalar (PP) channels are attractive by the same Fierz lemma (G_AA=+3/32κ, G_PP=+3/64κ per eq:AAdecomp) and are excluded not by the sign leg but by the magnitude leg (|G_PP|=|G_scalar|, equally sub-critical); 'far sub-critical' tempered to 'comfortably sub-critical' (worst scanned ratio 0.156). Conclusion unchanged. NJ1 closure (v1U.0.15): INT-Claude verified the new NJL-exclusion appendix (app:njl_gap) SOUND and caught Holst-dressed NJL factor stated as ∼30× in body when the paper's own committed script arxiv/scripts/njl_gap_equation_route1.py gives holst_factor=0.0698=γ²/(γ²+1) → 14.3× ≈ ∼14×; corrected at 2 body sites + header comment; conservative direction preserved, conclusion unchanged. EXT Grok MAJOR + ChatGPT REJECT re-flagged pre-existing ledger classes (DP1U-06/-07/-08/-11/-12 etc.) without engaging the new appendix. INT OpenAI/Grok/Gemini did not engage the new appendix; only INT-Claude verified the new content. R4 spectator-ALP framing sharpened (no number changed, nothing fabricated). The Route-4 closure now states explicitly what is ECH-specific versus generic: the m_θ~H₀ ultralight-mass tuning is acknowledged as the same tuning that afflicts every quintessence/ultralight-axion dark-energy model (not a distinctive ECH barrier), while the ECH-specific content is (i) the rigid one-loop-coupling amplitude overshoot of ρ_θ over ρ_Λ by 22–36 orders of magnitude, and (ii) that once the coupling is floated to escape that overshoot, minimal ECH supplies no first-principles reason for m_θ~H₀, so the predictive route is closed. G15 closure (v1U.0.9, Jul 10): reframed the stock-CAMB MCMC as an upper-bound baseline envelope check (not an ECH test) per Gemini, restructured the Eq.6→Eq.7 hand-off to lead with the dimension-4 operator basis over the Case II curvature-dressing heuristic, and closed Grok/ChatGPT minors (Eq.17 margin note, transparency-theorem all-orders/FLRW clause, M_Pl/κ convention footnote, Holst-topological terminology). Presentation-only, no verified number changed.",
    changelog: [
    ...
    artifacts: [
      { label: "Read PDF", href: "/papers/paper1_unified_v1U.0.18.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/paper1_unified_v1U.0.18.pdf", kind: "secondary", download: true },
      {
        label: "LaTeX source",
  ...
  },
  {
    slug: "paper-1b",
    number: "1B",
    title: "[MERGED INTO PAPER 1] Technical Verification Companion to the ECH Spin-Torsion Program: ΛCDM+ΔN_eff MCMC Proxy, NaMaster Pipeline Recovery, and a Birefringence Consistency Check with a Spectator-ALP Model",
    version: "v1B.0.104 (merged)",
    lastUpdated: "2026-07-08",
    tldr: "MERGED INTO PAPER 1 (2026-07-08, reviewer-consensus, Houston-approved) — no standalone submission. Formerly the technical companion to Paper 1A (309K-sample ΛCDM+ΔN_eff MCMC null test, NaMaster pipeline validation, DESI DR2 w0wa chain, spectator-ALP birefringence check); now folded into the unified Paper 1 as appendices. This card is retained for the record; the standalone PDF link is preserved below.",
    path: publicationPath({}),
    pages: "22",
    refs: "32",
    readiness: 56,
    statusVariant: "amber",
    target: "Merged into Paper 1 — no standalone submission",
    description: "MERGED INTO PAPER 1 (2026-07-08, unanimous reviewer recommendation, Houston-approved): this content now ships as appendices of the unified Paper 1 (v1U.0.1), which kills the companion-reliance and standalone-scope reviewer objection classes. Card retained for the record. Formerly a technical verification companion to Paper 1A, documenting three analyses: a 309,189-sample Cobaya ΛCDM+ΔN_eff MCMC that recovers ΛCDM (H0 = 67.68 ± 1.06 km/s/Mpc, ΔN_eff consistent with zero); a NaMaster pipeline validation on synthetic ΛCDM polarization skies with an ACT-like mask (inject β=0.27°, recover 0.238°, bias −0.032° — a pipeline-validation figure, not a sky measurement); and a spectator-ALP consistency check against the published Planck+ACT β = 0.342° ± 0.094° (3.6σ).",
    keyResults: [
    ...
    remainingWork: [
      "MERGED INTO PAPER 1 (2026-07-08, reviewer-consensus, Houston-approved) — no standalone submission; all follow-up work now tracked under the unified Paper 1",
    ...
    preprintId: "HUBIFY-2026-001B (merged into HUBIFY-2026-001)",
    pdfMeta: "MERGED INTO PAPER 1 · standalone PDF retained for the record · 22 pp · v1B.0.104 · md5 76dce947",
    artifacts: [
  ...
  },
  {
    ...
    pdfMeta: "PDF · 37 pp · v1.7.116 · updated Jul 12, 2026 · md5 abfdbf70 — directive-M presentation restructure (ZERO content change): consolidated the repeated scope/caveat/proxy/illustrative statements the REJECT/minor raws named (DP2-30) to canonical homes + cross-refs, relegated the cosmic-birefringence auxiliary paragraph to a new Appendix, tightened the Caveats→Scope-and-limitations register. Freeze held: every number byte-identical (one redundant body-prose 10^-3 disclosure consolidated to a cross-ref; 6 canonical disclosures remain), −35/16 quadruple-certification untouched (App-A+tables cert count 20→20), abstract still states all four certification methods. INT re-test: OpenAI REJECT / Grok MAJOR / Gemini MAJOR / Claude ABSENT (session-blocked → GAP); 0 genuinely-new editable findings (all map to standing DP2 classes; the 'far too long' item is the exact class this restructure actions), 0 regressions; residual REJECT/MAJOR = the documented LLM harsh-referee floor (Cov_B unavailable, venue/Houston-gated). Prior science (v1.7.115) md5 1f425220 — INT-Claude genuinely-new MAJOR closed by RE-COMPUTE: the c15 channel-native Fisher built the GR leg ∂B/∂A_GR without the M123 transfer product that the f_NL primordial leg carries, leaving the GR template in potential space while f_NL was in the observed galaxy-density basis; contracted against the density-space covariance this collapsed F[2,2]~1e-18 and FAKED ρ(f_NL,A_GR)≈−0.001 orthogonality. FIX (Dg = b·b·b·M123·S_GR — the same promotion cross_fisher_alpha already applied) + re-run (231s, CAMB 1.6.0): CORRECTED ρ(f_NL,A_GR)=−0.42 (2×2) / −0.49 (3×3) — the GR channel is MODERATELY correlated with f_NL, less degenerate than the −0.868 SDB proxy or |ρ|≈0.95 shape-cosine (both overstated the loss) but NOT orthogonal; ρ(f_NL,b_φ)=+0.99. b_φ-30%-prior σ_marg(f_NL^bounce)=0.94 → 2.32σ for −35/16 (local self-consistency σ_local=0.94). Load-bearing conclusion SURVIVES: channel-native floor 2.32σ still HIGHER than the 1.30σ retained proxy floor → proxy still the conservative quoted endpoint. cross-Fisher α=0.992 unchanged. −35/16 unchanged, nothing fabricated.",
    changelog: [
    ...
    artifacts: [
      { label: "Read PDF", href: "/papers/02_full_draft_v1.7.116.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/02_full_draft_v1.7.116.pdf", kind: "secondary", download: true },
      {
        label: "LaTeX source",
  ...
  },
  {
    ...
    statusVariant: "amber",
    target: "ApJS",
    description: "The multi-survey anomaly catalog: 378,280 unique anomalies from 37.3 million sources across 7 surveys via one BigAE autoencoder architecture, with a 17.8% novelty rate at the top-1,000 stratum against 20 all-sky catalogs. A NANOGrav 15-yr free-spectrum re-fit gives γ = 2.567 ± 0.382 — matter-bounce γ=3.0 is consistent (+1.14σ) while SMBHB γ=4.33 is excluded (+4.63σ). Multi-tracer forecast: σ(f_NL) = 8.14 envelope [3.92, 8.98].",
    keyResults: [
      ...
      "NANOGrav 15-yr KDE free-spectrum γ = 2.567 ± 0.382 — bounce γ=3.0 consistent (+1.14σ), SMBHB γ=4.33 excluded (+4.63σ)",
    ...
    pdfMeta: "PDF · 37 pp · v3.1.155 · updated Jul 11, 2026 · md5 ebd4bfd1",
    artifacts: [
      { label: "Read PDF", href: "/papers/paper3_draft_v3.1.155.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/paper3_draft_v3.1.155.pdf", kind: "secondary", download: true },
      {
        label: "LaTeX source",
  ...
  },
  {
    ...
    path: [
      ...
      },
      {
        label: "External journal-style review",
        state: "active",
        note: "NOT passed — R9 board (Jul 9): Grok MINOR · Gemini 'Accept w/ minor' · ChatGPT MAJOR; two clean minor-lists from a double-accept",
      },
      {
        label: "Reviewer ACCEPT",
        state: "pending",
    ...
    pdfMeta: "PDF · 36 pp · v1.0.240 · updated Jul 13, 2026 · md5 7dcf5eaf · 34 MB — M24-EXT DP4-22 close: the edge-on Appendix-E sensitivity penalty is corrected from the Fisher-CRB sqrt scaling (1−δ)^−1/2−1=8.98% to the LINEAR (1−δ)^−1−1=18.8% that the naive per-pixel f_CW estimator (which RETAINS edge-on systems in N_spiral) actually incurs; the sqrt value would apply only to an optimal estimator that excludes/down-weights the zero-information edge-on population. Both call sites + the b/a-threshold sweep updated to the conservative linear value; the artifact JSON regenerated from the parquet (script-reproducible). Neither value changes the null verdict — injection-recovery floors are measured on the same edge-on-contaminated observed-label field, and the penalty is largely subsumed by the g=2a−1 GZ1-accuracy dilution. GENUINELY-NEW correctness item (ChatGPT-EXT #10), VERIFIED REAL by independent re-derivation; clean-wave streak resets. Prior — INT re-test of the v1.0.238 e2e fold (OpenAI MAJOR / Gemini ACCEPT-with-minor / Grok REJECT, all native-PDF): closes the one genuinely-new correctness item OpenAI-INT raised (P4-E7) in the newly-folded Sec. VI B paragraph — the prior wording 'exactly parity-antisymmetric … T_eq=0.9997 with maximum antisymmetry deviation 0.0' conflated two quantities and is corrected to distinguish the probability-level antisymmetry (exact by Z2-TTA construction, max deviation 0.0) from the argmax-label flip-recovery rate (T_eq=0.9997, residual 2.6×10⁻⁴ = argmax ties p_CW^eq=p_CCW^eq, not a physical asymmetry), per the e2e_transfer_function_full.json note. No number changed, nothing fabricated; Gemini softened to ACCEPT-with-minor and Grok's 4 majors + OpenAI E1–E6/E9/M1–M8 remain source-cited standing re-flags (DISPOSITIONS/P4.md). Prior — Directive-L compute closer FOLDED (v1.0.238): the full image-level end-to-end mirror-flip injection that Sec. VI B previously flagged as 'the operative next step and do not perform here' is now PERFORMED on the full 8.47M-galaxy image source through the production ViT (bamfai/galaxy-chirality-v2, val_acc 0.9369; artifact e2e_fullrun/e2e_transfer_function_full.json, 192/192 shards). Real result: raw image-level transfer T_raw=0.2303±0.0002, stable across confidence bins (0.207–0.261) and N/S strata (0.218 vs 0.251); the production Z2-TTA labeling is exactly parity-antisymmetric at the image level (T_eq=0.9997, antisym maxdev 0.0) verified image-by-image on all 8.47M galaxies — the net-handedness channel a spatially-varying confusion would need to seed a dipole is absent by construction and now empirically confirmed. Honestly scoped: does NOT close the finer per-pixel depth/PSF/morphology confusion model; g=0.398 GZ1-accuracy dilution factor unchanged. Directly answers ChatGPT DP4-15 / Gemini image-level pseudo-label-independence MAJOR. Prior — H17 INT re-test closure (v1.0.234): closed the three genuinely-new Claude-subagent MINORs on the new v1.0.233 sky/confidence-stratified confusion analysis (Appendix B, Table tab:gz1_stratified). (1) softened the Sec IV C narrative 'cannot manufacture a dipole' phrasing to 'corroborates but does not fully close the differential-error channel at the sub-percent level', with the CI half-widths (≲0.6 pp science-cut) stated as the operative bound that does not exclude a sub-percent differential asymmetry; (2) added an explicit two-cell (dec≷+32°) scope caveat that a differential-error dipole not aligned with the leg-split axis is bounded only through the coarser overall-stratum CI (±0.56 pp), not resolved; (3) added a one-line note that the stratified accuracies (0.912 overall, 0.961 science-cut) are the confident-spiral ∩ classifier-CW/CCW accuracies, not a revision of the headline conservative GZ1 chirality-accuracy floor (0.6991, κ=0.40) that feeds g=2a−1. Presentation/scoping only, no verified number changed. Shamir factor-of-2 (A_ref=0.017, z≈−7.6) intact from v1.0.231.",
    changelog: [
    ...
    artifacts: [
      { label: "Read PDF", href: "/papers/chirality_catalog_paper_v1.0.240.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/chirality_catalog_paper_v1.0.240.pdf", kind: "secondary", download: true },
      {
        label: "LaTeX source",
  ...
  },
  {
    ...
    pdfMeta: "PDF · 46 pp · v0.1.123-2026-07-12 · updated Jul 12, 2026 · md5 186d985e — v0.1.123 (INT-Claude + EXT-ChatGPT genuinely-new): the RSD Zel'dovich reconstruction bound is now correctly scoped — it was applied to the UNRESTRICTED (secondary) void contrast (void bin vs all z≤0.24 non-void spirals, no footprint mask), NOT the primary footprint-restricted estimand; the text now states this and frames the unrestricted, larger-sample contrast as the more conservative coherent-outflow probe. Honest disclosure added that reconstruction reassigns void membership substantially (n_void 57,058→42,864, −25%, as LOS-elongated redshift-space holes contract in real space) yet the Δf_CW shift stays tiny (0.024pp, +0.069→+0.045pp, ~40× under the ≈0.9pp envelope) because the reassigned galaxies are parity-symmetric; sign convention corrected (−0.069→+0.069pp) and quadrature term-sum updated (√0.898=0.94pp). Null preserved, nothing fabricated. v0.1.122 (DP5-12): RSD MAJOR CLOSED-BY-COMPUTE via the first-order Zel'dovich bound (scripts/26 membership re-run). DESIVAST dual-primary reconciliation + three companion closures (no verified number changed, nothing fabricated). The abstract now leads with the single footprint-restricted primary estimand (Δf_CW = +0.0018) and demotes the unrestricted contrast (+0.0007) to a secondary sensitivity check; a 'footprint ≠ selection function' distinction is added at the primary designation (the angular-disc footprint is a geometric construction, not the published DESIVAST/BGS completeness mask); the ≈0.9pp systematic envelope is justified with the explicit quadrature term-sum (√(0.44²+0.37²+0.60²+0.37²+0.11²+0.24²+0.02²)=0.94pp); and the simultaneous Bonferroni-5 bound across all five void estimators is computed (widest per-test 99%-level CI ⇒ no void definition admits |Δf_CW|≳1.1pp at family-wise 95%). G15 closure (v0.1.113, Jul 10): softened the post-hoc 'designated primary' framing so the family-wise Bonferroni-5 null leads from the outset (Table IV disclosure retained), quoted the de-attenuated physical-chirality bound ≈2.26pp (= 0.9pp / 0.3982, from the paper's own 2a−1 attenuation factor) alongside the classifier-label 0.9pp bound in the abstract/§XIII/App A, and hardened the Appendix B toy-EFT operator with an explicit non-covariant/out-of-scope caveat. Presentation-only, no verified number changed.",
    artifacts: [
      { label: "Read PDF", href: "/papers/p5_desi_chirality_v0.1.123-2026-07-12.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/p5_desi_chirality_v0.1.123-2026-07-12.pdf", kind: "secondary", download: true },
      {
