...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    refs: "46",
    readiness: 87,
    status: "87% — v1.0.54 (Houston P4-priority push continues; 2nd of 3 surviving on-record items CLOSED). DeepSeek-B2 MC-seed deferral CLOSED via new `scripts/dump_mc_seed_manifest.py` → emits `outputs/canonical_provenance/mc_seed_manifest.json` documenting: Wave 12 hemisphere LEE (SEED=42, N_MC=10000, BATCH=100, NSIDE_DIR=8, script + null-distribution + results-JSON sha256s); pre-TTA 2.31σ + post-TTA 0.43σ real-space dipole (N_MC=1000, NSIDE=64, rebuild-from-log caveat documented); post-MASTER -0.122σ at ℓ=1 (NSIDE=64, f_sky=0.4928, sha256 + canonical-N MASTER recompute disclosure). Earlier v1.0.53 closures retained: 5 MAJORs (Shamir factor-6-12 amplitude reframe; Jia 2023 SOTA baseline; 0.5% empirical primary sensitivity floor; TTA soft-prob symmetrization correction) + DeepSeek-B1/M3 reproducibility script (verify_canonical_numbers.py). Surviving on-record (1 item): GPT-B2 canonical-N MASTER recompute (compute-bound, pymaster on Mac unresolved). PDF 30pp / 25.88 MB / 0 undef refs / sha256 2e3ed24e...",
    statusVariant: "green",
    target: "MNRAS",
    ...
    pdfMeta: "PDF 25.88 MB · 30 pp · May 14, 2026, v1.0.54",
    artifacts: [
