...
export const researchPrograms: ResearchProgram[] = [
  {
    ...
    supportingLinks: [
      ...
      },
      {
        ...
        description:
          "A systematic survey of 7 foundation mechanism classes and 6 observational branches, extracted from the retired P1U barrier catalog, collapsing to 13 distinct mechanism-class constraints across 14 catalog entries and closing four candidate dark-energy routes (R1-R4). Draft v1C.0.12, compiled 22 pp, 0 undefined refs, 0 overfull boxes — closes the R9 correctness-convergence board on the exact v1C.0.11 PDF (Claude INT MAJOR REVISION, 4 MAJOR/11 MINOR with 8 findings self-classed correctness-grade; Grok REJECT; Gemini MAJOR REVISIONS; Perplexity leg failed). The round was adjudicated by an independent symbolic computation committed at 1130b7c5 (research/theory_audit/operator_basis_adjudication_2026_08_07.py), which re-derived O1-O6 from the Cartan structure equations in exact rational arithmetic. Three adjudication-driven changes landed: (a) Sec. V now calls {O1-O6} a spanning list / generating set, not a basis — computed rank 4 (nullity 2; rank 2 modulo total derivatives), with both exact relations stated (O1 - O6 = 0 and 2*O1 + 2*O2 - O4 = 0, i.e. O1 = (1/2)O4 - O2) and the Nieh-Yan form-vs-density normalization now fixed explicitly, which is what had made the referee's literal coefficients come out a factor 2 off; (b) Table III's O1 row keeps Final = 0 but its reason is now branch-scoped (0 on the torsion-free branch by Bianchi/Check A; equal to -NY, an exact total derivative, on the T = kappa S branch) — the referee's claimed 'internal contradiction' was FALSIFIED by the computation; (c) a new correctness item neither party raised: Table III's O4 row, its caption, and the App. A 1 'O4 = O5' chain were wrong as printed — Check D's identity concerns the epsilon-free square T_abc T^abc, whereas the epsilon-contracted O4 vanishes identically under the purely axial Cartan torsion. All three sites corrected; this STRENGTHENS the no-go (an operator contributing nothing is a stronger disposal than one contributing a Planck-suppressed contact term) and the physics conclusion is unchanged. Other closures: the Route-3 '61-67 orders' statement now has a displayed mass-dimension scaling relation with the reference budget defined as rho_Lambda,obs and the Hubble symbol made uniformly H0; the Route-2 section now states plainly that Sec. IV A closes the birefringence channel while Route 2's dark-energy closure is inherited from the Sec. V / App. A bound, and the App.-A bridge sentence no longer mis-describes Eq. (1) as the dimension-(+1) operator. Falsified with receipts: Grok's Eq.-(2) over-suppression and typesetting-slip claims, Grok's '3.6 vs 3.9e-69 never reconciled' claim (the paper reconciles it in two places), and Gemini's 'gauge-invariaut' typo (a text-extraction artifact; the PDF prints 'gauge-invariant'). DRAFT: R-phase NOT converged at R9 — a structural correctness item was found and fixed, so R10 on the exact v1C.0.12 PDF is the next correctness-convergence check; no readiness score has been computed and no venue/Zenodo kit exists yet.",
        href: "/papers/paper1c_nogo_survey_v1C.0.12.pdf",
        external: true,
      },
    ],
