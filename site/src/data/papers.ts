...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    refs: "30+",
    readiness: 97,
    status: "97% — Wave 14-JJ P4-CM-B2 PSF cross-correlation closed PUSHBACK; Gemini B-1 NaMaster recompute + 6 OpenAI compute-heavy MAJORs pending",
    statusVariant: "amber",
    target: "MNRAS",
    description: "The chirality catalog paper. Wave 14-JJ (2026-05-01 16:32 PDT) closed R42 P4-CM-B2 BLOCKER as PUSHBACK — PSF cross-correlation on the joined 8.47M-galaxy + DR8 b/a catalog (3.20M spirals, 2.91M with ellipticity, NSIDE=64 HEALPix, f_sky=0.32, 15,769 valid pixels, N_MC=200) shows max |Pearson r| = 0.04243 (FAILS the strict 0.1% pixel-level bar) but max angular C_ℓ |z| = 2.72σ at ℓ=2-64 (PASSES the physics-relevant 3σ bar). The reframe is reported in §III.G of the paper at L1576 with full Pearson + Spearman + C_ℓ tables, exactly the same PUSHBACK pattern as Wave 14-X (P3-OA-M1 100K Jaccard) and Wave 14-EE (D4 TTA equivariance). Companion artifact JSON at r42_results/wave_14_jj_psf_xcorr_results.json. v1.0.16 → v1.0.17 (Pod 3 H200 4-pass recompile, 19 pp / 25.79 MB / 0 errors / 0 undef refs). Wave 14-D (2026-05-01) revised the §III.F Platt-calibration text from \"removes\" to \"reduces\" with explicit before/after numbers (raw +0.79%/28.8σ → calibrated +0.4%/14.6σ → equivariant -0.26%/9.5σ) and the explicit Platt mapping p_cal = σ(z/4.65 - 1.58) — closes P4-OA-B6. Wave 14-C closed P4-OA-B1 (53,862 vs 5.15M NS-count scoping) and P4-OA-B2 (abstract internal-inconsistency fix). Wave 12 tightened the hemisphere look-elsewhere null on H200 GPU at N_MC=10,000 to p_LEE = 9.999×10⁻⁵ (closes P4-CM-m2 + P4-OA-M8). Wave 11 recomputed the NaMaster shot-noise floor with the corrected N_spiral = 3,321,795 denominator (cross-confirmed P4-CM-B1 + P4-OA-M7).",
    keyResults: [
    ...
    remainingWork: [
      ...
      "OpenAI P4 B5 / B7 / M-1 / M-5 / M-7 / M-9 compute-heavy MAJORs (~6-12h aggregate)",
      "Residual R42 MINOR text polish",
      "Houston personal sign-off (final 1%, gated)",
      "External peer-review round (R43) closes with zero MAJOR / MINOR findings",
      "arXiv tarball verification + form-fill (administrative)",
    ...
    pdfMeta: "PDF 25.79 MB · 19 pp · May 1, 2026, v1.0.17",
    artifacts: [
