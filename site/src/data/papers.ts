...
export const papers: Paper[] = [
  {
    ...
    refs: "70+",
    readiness: 78,
    status: "78% — R45 backward step (Wave 14-PPPP): −12pp from 90 because R45 found 2 BLOCKERs inside the new four-route no-go appendix (§IV.B Route 2 dimensional inconsistency between rotation rate β̇ in eV vs angle uncertainty σ(β); §IV.D Route 4 amplitude-bound numerics 8 vs 35 orders-of-magnitude contradiction) + 4 MAJORs (D_inf prefactor hand-wave, B8/B14 non-independence, inflation-fNL tension under-derived, LiteBIRD detection-vs-discrimination conflation). Wave 14-QQQQ queued to close. Cycle continues: backward step now visible.",
    statusVariant: "green",
    ...
    pdfMeta: "PDF 772 KB · 16 pp · May 9, 2026, v1A.0.8",
    artifacts: [
  ...
  },
  {
    ...
    refs: "31+",
    readiness: 73,
    status: "73% — R45 backward step (Wave 14-PPPP): −12pp from 85 because R45 found 2 BLOCKERs (abstract `\\ref{sec:gr}` undefined produces `??` in PDF; §VII 9.9σ vs 3-5σ headline still semantically conflated — they come from different Fisher matrices, not pre/post systematic budget) + 6 MAJORs (stale v1.7.14 release tag, Bayes factor table missing 17, QSFI sign-convention swapped Δ vs Δ-3/2, abstract-vs-§V curvaton headline mismatch, halving range 5.2-2.75σ asymmetric, polynomial-coefficient null-space ~15% systematic absent). Wave 14-RRRR queued.",
    statusVariant: "green",
    ...
    pdfMeta: "PDF 777 KB · 16 pp · May 9, 2026, v1.7.15",
    artifacts: [
  ...
  },
  {
    ...
    refs: "60+",
    readiness: 80,
    status: "80% — R45 backward step (Wave 14-PPPP): −9pp from 89 because R45 found 1 BLOCKER (catalog-grade 264,938 + exploratory 113,342 = 378,280, NOT 378,080 as repeatedly stated in abstract + 8 other public surfaces) + 5 MAJORs (§6 Limitations didn't propagate Wave 14-OOOO 0.06σ wording, 141× scale claim violates the abstract's own no-quote rule, 17.8% upper bound asserted not argued, Wave 14-KKKK α_GS missing from abstract+conclusions, PTA consistency framing asymmetric). Wave 14-SSSS queued.",
    statusVariant: "green",
    target: "ApJS",
    ...
    pdfMeta: "PDF 28.38 MB · 42 pp · May 9, 2026, v3.1.26",
    artifacts: [
  ...
  },
  {
    ...
    refs: "30+",
    readiness: 76,
    status: "76% — R45 backward step (Wave 14-PPPP): −13pp from 89 because R45 found 1 BLOCKER (the GZ1 Platt L-BFGS recalibration parameters A=0.215143, B=−1.581205 cited in §IV are NOT in the on-disk artifact `wave_14_fff_gz1_platt_recal.json` — that artifact shows placeholder values A=0.21505, B=−1.58, accuracy 0.5194 chance, Brier=NaN; the 6-sig-fig agreement claim is unverifiable from cited source) + 6 MAJORs (McNemar contingency b=4205,c=3607 has no artifact, abstract overstates morphology-bin orthogonality as structural not empirical, GZ1+Cat-C quadrature add not justified for non-independent components, N_eff 5-15% claim unsupported by histogram, factor-of-9 Shamir uses superseded N denominator, two distinct \"high-confidence > 0.6\" subsamples 471K vs 949K). Wave 14-TTTT queued.",
    statusVariant: "green",
    target: "MNRAS",
