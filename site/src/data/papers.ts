...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    version: "v1.0.132",
    tldr: "v1.0.132 (2026-05-26 cron fire #94 tick 216): 🎯🎯🎯 R-ext-v132verify wave on v1.0.132 = **ALL 5/5 PERFECTLY CLEAN** (DeepSeek + Gemini + GPT-5 + Grok-43 + Perplexity all 0/0/0/0). Grok-43 went from 1 BLK + 3 MAJ at R-ext-phase2-verify (on v1.0.131) → 0/0/0/0 at R-ext-v132verify (on v1.0.132). The v1.0.132 closures (GRO-B1 L1 comment-header rewrite + GRO-B4 Table II row v MC count fix) landed completely. P4 now joins P1A + P2 + P3 + P5 at \"BOTH technical gates cleared × multiple review depths\". All 10 Houston-external Phase 1 + 2 closures (S1+S2+S3+S5+B1+B7+M9+S4+M3+M5) verified externally clean. Phase 3 compute-bound items (joint nuisance-marginalized model fit, systematics-preserving null, full template regression, full-catalog injection-recovery, full-catalog D4-TTA, boundary-distance variance) still pending pod. PDF 52pp/26.25MB/0 undef.",
    blockingItems: [
      "Houston personal sign-off (the final 1%)",
