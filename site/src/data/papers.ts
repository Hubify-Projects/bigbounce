...
function publicationPath(overrides: {
  ...
  return [
    ...
    },
    {
      ...
      state: "active",
      note: "Exact P1B v109/P4 v245 boards are complete; truth audits found open major defects",
    },
    overrides.external ?? {
...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    tldr: "Standalone P1B is restored alongside the retained P1U history, but the exact v1B.0.109 board is MAJOR/MINOR/ACCEPT and its truth audit found that the synthetic-polarization driver passed D_ell-scale template values to a C_ell interface. The algebraic window/operator test survives; the physical sensitivity, scatter, and bias claims require a full rerun with physical CAMB EE/BB spectra. Readiness holds 56.",
    path: publicationPath({}),
    pages: "20",
    refs: "32",
    readiness: 56,
    ...
    description: "Standalone technical verification companion to Paper 1A. It documents a 309,189-sample Cobaya ΛCDM+ΔN_eff proxy analysis and an ALP prior-predictive diagnostic reported before and after Ω_a<0.01. The current NaMaster artifact remains useful only as an algebraic window/operator validation: its sky generator used nonphysical polarization spectra, so its Planck-EE/10 μK-arcmin sensitivity interpretation is withdrawn pending rerun. Frozen metadata also shows the chains used CAMB 1.6.5's default BBN predictor, not the later YAML's claimed PArthENoPE pin. These are not evidence, posterior probabilities, or a sky detection.",
    keyResults: [
      ...
      "The current 500-MC NaMaster artifact verifies algebraic window/operator recovery only; its physical sensitivity, scatter, and bias interpretation is invalid pending rerun with raw CAMB EE and physical BB spectra",
      "ALP prior-predictive accounting uses 100,000 seed-1234 trajectories per arm: 34,947 satisfy Ω_a<0.01; within that spectator-conditioned subset, 863/34,947 broad-coupling draws lie in the signed 1σ band and 0/34,947 fixed-C_aγ=8 draws do (95% Wilson upper bound 0.011%). These are fixed-background Monte Carlo frequencies, not evidence or posterior probabilities",
    ...
    remainingWork: [
      "Rerun the full 500-MC and robustness suite with physical raw CAMB EE/BB spectra; regenerate every affected figure and sensitivity/bias number",
      "Correct the frozen-run BBN provenance to CAMB 1.6.5's executed default, recompute the S8 overlay with the declared 30% burn-in, and close the manifest/release gate",
      "Exact board: Codex subscription MAJOR, Gemini direct MINOR, Grok direct ACCEPT; human review, immutable archive/DOI, venue disposition, and submission metadata remain open",
    ],
    preprintId: "HUBIFY-2026-001B",
    pdfMeta: "Standalone companion · 20 pp · v1B.0.109 · sha256 36b8fc984b5be164f5ece1e2f0c3f661dfb49c9f99faa76e2b050e2bd0674a78 · exact board MAJOR/MINOR/ACCEPT; physical-spectrum rerun required; readiness holds 56",
    artifacts: [
  ...
  },
  {
    ...
    version: "v1.0.245",
    lastUpdated: "2026-07-15",
    tldr: "Classifies 8.47M galaxies for spiral handedness and finds a null HC real-space chirality dipole under the primary fixed-occupancy galaxy-label randomization (z=0.7053, one-sided p=0.22468). The exact v1.0.245 board is MAJOR/MINOR/MINOR: the narrow null is supported, but release availability, training provenance, and spatially varying symmetric calibration error remain open major gates. Readiness holds 80.",
    path: publicationPath({}),
    pages: "26",
    refs: "46",
    readiness: 80,
    ...
    description: "The galaxy chirality catalog: 8.47M galaxies classified CW/CCW by a ViT-Small ensemble with rotational-equivariance correction. The primary HC real-space statistic is consistent with isotropy under a fixed-occupancy galaxy-label randomization (z=0.7053, one-sided rank p=0.22468); the former pixel-asymmetry permutation is robustness only. The result concerns observed classifier labels, not true three-dimensional galaxy spins or a primordial parity bound. Submission is blocked until exact release payloads are public, conflicting training records are reconciled, and the spatially varying symmetric-error channel is propagated or bounded.",
    keyResults: [
      "8.47M galaxies classified (1,687,069 CW / 1,634,726 CCW / 5,152,736 NOT_SPIRAL)",
      "Primary HC real-space null: fixed-occupancy galaxy-label randomization gives z=0.7053169638 and one-sided empirical-rank p=0.2246775322; the old pixel-asymmetry permutation is robustness only (z=0.5491201934, p=0.2651734827)",
      "Confidence-cut profile: the parent selection has N_selected=949,584 and the estimator uses N_support=947,326; its fixed-occupancy label-randomization result is z=+0.7053, while the former pixel-permutation result z=+0.5491 is robustness only",
    ...
    remainingWork: [
      "Exact board: Codex subscription MAJOR, Gemini direct MINOR, Grok direct MINOR; close the verified training-provenance and symmetric-error-channel majors",
      "Publish the exact safe/quarantine/null products, checksums, manifest, reproducer inputs, immutable tag, catalog/source/PDF archive, and real DOI before using release language",
      "Propagate z=0.705317 through every primary-result anchor; retain supported GZ1 N/mask provenance and distinguish N_selected from N_support",
      "Author arXiv endorsement and journal-submission decision",
    ],
    preprintId: "HUBIFY-2026-004",
    pdfMeta: "PDF · 26 pp · v1.0.245 · updated Jul 15, 2026 · sha256 e37d0af7 — exact board MAJOR/MINOR/MINOR; narrow fixed-occupancy null supported, but release/provenance/calibration majors remain; readiness holds 80.",
    changelog: [
    ...
    artifacts: [
      { label: "Read PDF", href: "/papers/chirality_catalog_paper_v1.0.245.pdf", kind: "primary", external: true },
      { label: "Download PDF", href: "/papers/chirality_catalog_paper_v1.0.245.pdf", kind: "secondary", download: true },
      {
        label: "LaTeX source",
