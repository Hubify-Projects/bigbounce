...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    version: "v1.0.132",
    tldr: "v1.0.132 (2026-05-26 cron fire #93 tick 215): R-ext-phase2-verify wave on v1.0.131 = 4/5 CLEAN (DeepSeek + Gemini + GPT-5 + Perplexity all 0/0/0/0); only Grok-43 raised 1 BLK + 3 MAJ + 1 min + 1 nit. Truth-audit closure: (a) GRO-B1 (\"No Evidence for Large-Scale Parity Violation\" title/abstract mismatch) PARTIALLY FALSIFIED — the actual paper title (`\\title{}`) is the v1.0.129 reframed version; \"parity violation\" framing existed ONLY in the L1 `%-comment header` which is TeX-only/invisible-in-PDF. Closed by rewriting the L1 comment header to match the actual title. (b) GRO-B4 (Table II row v MC=500 vs body N=10,000) CONFIRMED REAL — updated row (v) MC count to canonical 10,000. (c) GRO-B2/B3 scope-rewrite recommendations PUSH-BACK with citation to existing scope-narrowing disclaimers. (d) GRO-B5 Fisher vs empirical floor confusion DEFERRED to v1.0.133+ polish. Three previous Houston-external Phase 1+2 closures all hold. PDF 52pp/26.25MB/0 undef.",
    blockingItems: [
      "Houston personal sign-off (the final 1%)",
