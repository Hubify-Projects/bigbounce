...
export const liveStatus: LiveStatus = {
  ...
  lastUpdatedDisplay: "May 15, 2026 · 6:00 AM PT",
  headline:
    "**P4 v1.0.66 -> v1.0.67 -- HOUSTON EXTERNAL 4-VENDOR REVIEW caught BLOCKERs missed by 8 internal Gemini endorsements; readiness 95 -> 85 (-10pp) honest oscillation.** Houston ran external adversarial review on ChatGPT/GPT-5, Gemini Deep Research, Grok, Gemini using public PDF + artifacts. Unanimous NO-GO for arXiv + NO-GO for journal. **External BLOCKERs (4-vendor convergent)**: (1) LLM/agent log artifacts in body (DeepSeek-B1, GPT-B2 closures, v1.0.5X version archaeology) -- Gemini called instant desk-reject; (2) cited outputs/dipole/summary.json is PRE-TTA 2.31sigma not headline 0.43sigma post-TTA; (3) Shamir-2022 ~16x larger comparison wrong (Shamir 2022 DESI used ~1.3M spirals not 200k, true ratio ~2.5x); (4) HF dataset card contradicts paper (ResNet/GalaxyMNIST/D4 vs paper ViT/2-fold flip); (5) HF dataset viewer CastError schema mismatch; (6) GitHub release tag paper4-v1.0 does not exist; (7) independent GZ1 accuracy 69.91% buried under 93.7% internal distillation headline; (8) hemisphere pLEE<=10^-4 dismissed without formal monopole+mask leakage null; (9) reject-at-any-amplitude overclaim (P(sigma>2)=0.18 at A=0.5% does NOT reject); (10) Table V stale 3,321,795 snapshot; (11) title overclaims No Parity Violation despite 9.5sigma monopole + 3.05sigma hemisphere + 1.85sigma canonical; (12) Fig 8 caption admits showing OLD 2.75sigma data; (13) training arithmetic 26,626 vs 26,636 components do not sum. **Closed in v1.0.67 single bundled wave (text-level)**: (a) SCRUBBED all LLM/agent log artifacts from Section VII Conclusions + figure captions + body; (b) created canonical_provenance/catalog_c_post_tta_dipole_summary.json as canonical post-TTA 0.43sigma anchor + updated abstract citation; (c) softened title to No Evidence for Large-Scale Parity-Violating Dipoles (explicit Dipoles vs implicit monopole-included Parity Violation); (d) Shamir 16x corrected to ~2.5x with explicit Shamir 2022 published abstract reference; (e) abstract reframed to lead with independent GZ1 69.91% (Cohen kappa=0.40); 93.7% demoted to training self-consistency check; (f) reject-at-any-amplitude removed; replaced with empirical 50% recovery threshold above 0.5% per actual injection table; (g) Table V 3,321,795 snapshot row deleted, defensive footnote removed; (h) Fig 8 caption rewritten -- low-ell +2-to-+6sigma bandpowers attributed to monopole-mask leakage, not parity dipoles; (i) Fig 5 caption rewritten -- close to 50/50 but formally inconsistent at 9.5sigma; (j) training arithmetic 26,626 -> 26,636 corrected throughout; (k) added Catalog Usage Limitations paragraph in Data Availability warning not-ground-truth + 0.5% empirical floor. **Deferred on-record to v1.0.68** (compute-bound or external-artifact-bound): per-imaging-leg systematics (BASS+MzLS / DECaLS / DES); PSF-ellipticity 2D scatter plot; controlled monopole+mask leakage null simulation; HF dataset card rewrite (Houston push); HF dataset viewer schema fix; GitHub release tag paper4-v1.0 + Zenodo DOI ceremony. **PDF 33pp / 25,899,597 bytes / 0 undef refs / sha256 9f6c4d30b55e...** (was 34pp / 25,905,601; -1 page net from LLM-log scrub + Table V row drop). 6 mirrors byte-identical. **Readiness P4 95 -> 85 (-10pp)** honest oscillation per feedback_readiness_oscillation -- the v1.0.66 status was overstated because internal R-rounds missed all 13 BLOCKERs Houston external review caught in 1 round. v1.0.67 closes 11 of 13 text-level BLOCKERs; remaining 6 deferred items must close in v1.0.68 + v1.0.69 before P4 is genuinely arXiv-ready. P4 NO LONGER PUBLISH-READY pending external-review closures. Houston external review = de facto Gate (a) that the 8 internal Gemini endorsements failed to anticipate. Per-paper: P1A 74 (v1A.0.23), P1B 66 (v1B.0.7), P2 82 (v1.7.30), P3 85 (v3.1.41), **P4 85 (v1.0.67, -10pp honest)**. Cobaya unchanged: 59,832 / R-hat-1=0.01945 / 22:53 UTC last flush.",
  summary:
  ...
  papers: [
    ...
    },
    {
      ...
      readiness: 85,
      pendingWork:
        "v1.0.67 -- HOUSTON EXTERNAL 4-VENDOR REVIEW (ChatGPT/GPT-5, Gemini Deep Research, Grok, Gemini) caught 13 BLOCKERs missed by 8 internal Gemini endorsements; readiness 95 -> 85 honest oscillation per feedback_readiness_oscillation. Unanimous NO-GO for arXiv + NO-GO for journal until external-review closures land. **Closed in v1.0.67 (text-level, locally closable)**: (a) SCRUBBED all LLM/agent log artifacts from Conclusions + body + captions (Gemini called this instant desk-reject); (b) created canonical_provenance/catalog_c_post_tta_dipole_summary.json as canonical 0.43sigma anchor; (c) softened title to No Evidence for Large-Scale Parity-Violating DIPOLES; (d) Shamir 16x -> 2.5x with Shamir 2022 DESI published abstract reference; (e) abstract leads with 69.91% independent GZ1 instead of 93.7% internal distillation; (f) reject-at-any-amplitude removed; replaced with empirical >0.5% recovery threshold; (g) Table V 3,321,795 snapshot row deleted; (h) Fig 8 caption rewritten -- low-ell bandpowers explicitly attributed to monopole-mask leakage; (i) Fig 5 caption rewritten -- close to 50/50 but formally inconsistent at 9.5sigma; (j) training arithmetic 26,626 -> 26,636; (k) added Usage Limitations paragraph -- labels NOT ground-truth, 0.5% empirical floor. **Deferred on-record to v1.0.68** (compute-bound or external-artifact-bound): per-imaging-leg systematics; PSF-ellipticity calibration plot; monopole+mask leakage null sim; HF card rewrite; HF schema fix; GitHub release tag + Zenodo DOI. PDF 33pp / 25.90 MB / 0 undef refs / sha 9f6c4d30b55e... 6 mirrors byte-identical. P4 NO LONGER PUBLISH-READY pending external-flagged closures.",
    },
  ],
  blockerTally: {
    closed: 208,
