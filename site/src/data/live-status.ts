...
export const liveStatus: LiveStatus = {
  lastUpdatedISO: "2026-07-11T16:43:00Z",
  lastUpdatedDisplay: "July 11, 2026 · 9:43 AM PT",
  headline:
    "P3 FR4 (Jul 11) — FIRST CLEAN WAVE on the DP3-19 fix → P3 clean-wave streak REBUILDS 0→1. The FR3 v3.1.155 correction (matter-bounce parameter-shift +1.13σ→+1.14σ, made consistent with the v3.1.154 SMBHB +4.63σ fix) is VERIFIED LANDED at all 7 reader-visible sites with 0 stray +1.13/+4.61 in body and no regression (the recompute Claude INT re-test independently confirms matter_bounce_3p0=1.13543→+1.14σ, smbhb_13_3=4.6274→+4.63σ, F₀=0.01240 all reproduce against savage_dickey JSON). FR4 surfaced 0 genuinely-new editable findings: INT openai/grok/gemini/claude = REJECT/REJECT/REJECT/MINOR; EXT Grok MAJOR (all re-flag DP3-07/-08/-01/-09/-15/-10); ChatGPT EXT FAILED-dead (FR4b retry in flight — chart GAP, not a verdict). Claude's display-vs-full-precision note is a PROCESS-NIT under DP3-19 (values correct + internally consistent). No version bump (v3.1.155 stands). P3 still OFF the exit set — needs one more clean wave for the two-clean-waves bar. Current streaks: P1U 2 · P2 3 · P3 1 · P4 4 · P5 2. Venue/human-referee decision remains Houston-gated.",
  summary:
  ...
  currentlyRunning: [
    "P3 FR4 adjudicated (Jul 11) — FIRST CLEAN WAVE on the DP3-19 fix → P3 streak REBUILDS 0→1. The special verify PASSED: the +1.13σ→+1.14σ matter-bounce parameter-shift correction is landed at all 7 reader-visible sites (abstract, §nanograv ×3, §disc, contributions, Table VIII footnote), 0 stray +1.13/+4.61 in body, no regression — the recompute Claude INT leg confirms every σ-shift + F₀ reproduces against the committed savage_dickey chain. FR4 = 0 genuinely-new editable findings: INT REJECT/MAJOR/REJECT/MINOR (openai/grok/gemini/claude), EXT Grok MAJOR (4 MAJOR + 2 MINOR, all re-flag DP3-07/-08/-01/-09/-15/-10), ChatGPT EXT FAILED-dead (FR4b retry in flight — chart GAP). Claude's display-vs-full-precision note = PROCESS-NIT under DP3-19 (values correct + internally consistent, optional polish only). No bump (v3.1.155 stands). P3 still OFF the exit set; one more clean wave for the bar. Convex EXT cap 56.",
  ...
  papers: [
    ...
    },
    {
      ...
      version: "v3.1.155",
      readiness: 56,
      pendingWork: "FR4 (2026-07-11T16:43Z) — FIRST CLEAN WAVE on the DP3-19 fix → P3 clean-wave streak REBUILDS 0→1 (still OFF the exit set; needs one more clean wave for the two-clean-waves bar). The FR3 v3.1.155 correction is VERIFIED LANDED: matter-bounce parameter-shift +1.13σ→+1.14σ present at all 7 reader-visible sites (abstract, §nanograv ×3, §disc, contributions, Table VIII footnote), 0 stray +1.13/+4.61 in body, no regression — the recompute Claude INT re-test independently confirms matter_bounce_3p0=1.13543→+1.14σ, smbhb_13_3=4.6274→+4.63σ, F₀=0.01240 all reproduce against savage_dickey JSON. FR4 = 0 genuinely-new editable findings. INT verdicts openai/grok/gemini/claude = REJECT/REJECT/REJECT/MINOR; EXT Grok MAJOR (4 MAJOR + 2 MINOR, all re-flag DP3-07 process-volume, DP3-08 eROSITA-excision, DP3-01/-09 validation-heterogeneity, DP3-15 pod-blocked reproducibility, DP3-10 §V-null); ChatGPT EXT FAILED-dead (FR4b retry in flight — recorded as chart GAP, not a verdict). Claude's display-vs-full-precision note = PROCESS-NIT under DP3-19 (the +1.14σ/+4.63σ values are arithmetically correct + internally consistent; residual is intrinsic display-rounding, optional polish only). No version bump (v3.1.155 stands, served md5 ebd4bfd13962b0ee8d14e5393a9bd2c9). Convex EXT cap 56 (Grok MAJOR + ChatGPT REJECT-carryover + Gemini REJECT). Venue question (PRD vs ApJS/MNRAS) Houston-gated.",
    },
    {
      slug: "paper-4",
      number: "4",
