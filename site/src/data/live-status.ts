...
export interface NeedsHoustonItem {
  ...
  why: string;         // why ONLY Houston can unblock this
  blockedPaper?: string; // e.g. "P1A", "P4", "P5" — which paper this gates
  ask: string;         // exact action Houston needs to take
}
...
export interface LiveStatus {
  ...
  needsHouston: NeedsHoustonItem[]; // ONLY items truly blocked on Houston (API keys, SSH, pod launch, personal sign-off, arXiv submission)
...
export const liveStatus: LiveStatus = {
  lastUpdatedISO: "2026-05-21T18:50:00Z",
  lastUpdatedDisplay: "May 21, 2026 · 11:50 AM PT",
  headline:
    "🎯 **CRON FIRE #2 / R25 + P5 v0.1.0 + 3-CONSECUTIVE CASCADED-LOOP EXIT ON ALL 5 REVIEWABLE PAPERS.** Cron fire #2 (xx:47 slot, 11:50 AM PT 2026-05-21) fired R25 in parallel on P1B/P2/P3/P4 — 20/20 reviewers returned 0/0/0/0. Combined with R24, **all 5 reviewable papers now satisfy AGENT_RULES §4.4.1 cascaded-loop exit (3+ consecutive 5/5 clean R-rounds)**: P1A R15+R16+R24, P1B+P2+P3+P4 each R23+R24+R25. **P5 Phase 2 sensitivity sweep completed** (9/9 cells, 18min wall, max CW-fraction range across env classes 0.22pp — environmental-independence headline is robust). **P5 paper LaTeX expanded from 9KB scaffold to v0.1.0 first-draft** (6pp / 332KB / 0 undef refs; full sections: intro + V-Web algorithm + Phase 2 sweep + systematics + discussion + bibliography). First P5 R-round attempted but ALL 5 reviewers returned **HTTP 403 'Key limit exceeded (weekly limit)' from OpenRouter** — weekly bucket on the API key emptied after R23+R24+R25 (70 successful calls this session). New needsHouston[0] item: Houston either raises the OpenRouter weekly limit or autonomous cron continues non-API work for ~7 days until reset. **Readiness**: P1A 90 CAP, P1B 90, P2 95, P3 95, P4 95 CAP, **P5 30 → 55** (+25pp for sensitivity sweep + first compiled draft). Next cron fire xx:17 will fix 20.2pt overfull in P5 Table I, retry R26 with graceful 403 handling, push HF model card v1.0.122. The autonomous loop is fully functional; Houston sign-off on P1A + P4 + OpenRouter limit raise remain the ONLY blockers.\n\n[Legacy R24] 🎯 **CRON FIRE #1 / R24 5-VENDOR WAVE: 25 OF 25 REVIEWERS RETURN 0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT ACROSS ALL 5 REVIEWABLE PAPERS — first perfect 25/25 convergence in campaign history.** Autonomous 30-min cron fired at 11:10 AM PT 2026-05-21; R24 launched in parallel on P1A v1A.0.34 / P1B v1B.0.20 / P2 v1.7.30 / P3 v3.1.56 / P4 v1.0.122 via OpenRouter against DeepSeekV4Pro + Gemini-3.1-Pro + GPT-5 + Grok-4.3 + Perplexity-Sonar-Pro. **No findings to close on any paper.** Cascaded-loop convergence: P1A confirmed (R15+R16+R24 = 3 consecutive 5/5 clean rounds with no audit-falsified BLOCKERs); P1B + P2 + P3 + P4 each now at 2 consecutive 5/5 clean (R23+R24) — one more clean R-round per paper gives all 5 cascaded-loop exit. Readiness rolled forward: P1A 90 (CAP, Houston sign-off gate), P1B 67 → 90 (+23pp), P2 82 → 95 (+13pp), P3 86 → 95 (+9pp), P4 95 CAP. P5 Phase 2 sensitivity sweep launched in background — 9 cells {R_s ∈ {10,25,50} Mpc/h × N=256 × λ_th ∈ {0.0,0.1,0.3}}; patched 01_compute_vweb.py to honor --config flag (was hardcoded ignore). ETA ~15 min. Next cron fire xx:47. The autonomous loop is functional; Houston sign-off on P1A v1A.0.34 + P4 v1.0.122 remains the ONLY blocker.\n\n[Legacy] 🎯 **R23 5-VENDOR WAVE: 24 OF 25 REVIEWERS RETURN 0 BLOCKER / 0 MAJOR ACROSS ALL 5 PAPERS.** Fired in parallel on P1A v1A.0.33 / P1B v1B.0.20 / P2 v1.7.30 / P3 v3.1.56 / P4 v1.0.122 via OpenRouter against DeepSeekV4Pro + Gemini-3.1-Pro + GPT-5 + Grok-4.3 + Perplexity-Sonar-Pro. **P1B, P2, P3, P4 ALL returned full 5-of-5 reviewers clean** (first such convergence in campaign history for P2 + P3 + P4). P1A had Gemini return 1 BLOCKER (audit-FALSIFIED — Gemini was checking the R23 prompt's claim of a NaMaster paragraph at §VI L427 against P1A; that paragraph lives in deprecated long-form arxiv/main.tex / P1B not in P1A which is the structural-closure no-go theorem and correctly has no NaMaster MCMC methodology) + 1 MAJOR + 1 minor + 1 nit — ALL closed in v1A.0.34 bundled text-level wave (M1 Mercuri-Capozziello conflation fix; m1 §XII.A dilution-factor physical-vs-mathematical reminder; n1 K^μ Chern-Simons 4-current inline definition) + 3 pre-existing undef-ref closures (sec:route2 → sec:r2_oneloop; eq:route2_ratio prose softened; sec:lqc → Ashtekar2011 cite). P1A v1A.0.34 PDF 20pp / 813KB / 0 overfull >20pt / 0 undef refs / 0 undef cites. **Aggregate post-R23: P1A + P1B + P2 + P3 + P4 are all external-review-ready; only Houston sign-off + arXiv submission gates remain.** P5 (DESI environmental chirality) remains at 30% with Phase 1 V-Web env_finder landed; Phase 2 sensitivity sweep + paper expansion queued autonomous. **NEEDS HOUSTON section** now pinned to top of homepage listing only the 4 truly-blocked items (P1A sign-off, P4 sign-off, arXiv endorsement, optional DESI env-VAC). Per-paper: P1A 90 (v1A.0.34), P1B 67→pending bump (v1B.0.20), P2 82→pending bump (v1.7.30), P3 86→pending bump (v3.1.56), P4 95 CAP (v1.0.122), P5 30 (bootstrap-2026-05-15).\n\n[Legacy] 🎯 **P1A v1A.0.33 LANDS THE CASCADED-LOOP EXIT MILESTONE — first paper in the campaign to satisfy AGENT_RULES §4.4.1.** R16 5-vendor cross-vendor returned 0 BLOCKER + 0 MAJOR across ALL 5 of 5 reviewers (DeepSeek-V4-Pro, Gemini-3.1-Pro, GPT-5, Grok-4.3, Perplexity-Sonar-Pro); 9th-consecutive Gemini-cosmology 0-BLOCKER; 2nd-consecutive 5-vendor clean round (R15+R16 both 0/0). External-review-ready, capped at 90% / 99% pending Houston sign-off. P1B v1B.0.20 R16 audit had Grok-only B1+B2 BLOCKERs FALSIFIED via stale-comment direct-file inspection (4 of 5 reviewers 0/0). P3 v3.1.56 closed the 6-round-deferred 9,576-object dedup-shortfall MAJOR via existing on-disk artifact (10,213 = 637 multi-survey + 9,576 intra-survey union-find collapses). P4 v1.0.116 trajectory v1.0.68 → v1.0.116 includes external review wave + 11 internal R-rounds + multi-null battery + cross-spectrum smoking gun + paper-wide +1.85σ → +3.64σ convention. R17/R21 cross-vendor cron blocked on OpenRouter top-up ($500.52/$500 verified). HF dataset README pushed to Hub at v1.0.116 (tick 106). All 4 paper PDFs verified byte-identical Vercel ↔ local at tick 108. Per-paper: P1A 90 (v1A.0.33, loop-exit), P1B 67 (v1B.0.20), P2 82 (v1.7.30), P3 86 (v3.1.56), P4 95 CAP (v1.0.116). Long-form legacy headline preserved for audit trail below.\n\n[Legacy May 15 headline] **P4 v1.0.67 -> v1.0.68 -- cycle-3 polish round on Houston-external-4-vendor-review findings; 7 additional text-level closures; readiness 85 -> 87 (+2pp).** **Closed in v1.0.68 single bundled wave**: (a) abstract rewritten ~900 -> ~520 words 4-paragraph structure (Gemini-DR-I-1 / Gemini-I-1); (b) 9.5sigma monopole causal-language softened to working-hypothesis-pending-independent-ground-truth + explicit N_eff caveat (block bootstrap inflates per-pixel variance via 2-point correlation) (ChatGPT-B-3 + ChatGPT-D-5); (c) first-published multi-test overclaim qualified to to our knowledge one of the most extensive (ChatGPT-F-3); (d) multi-survey consensus softened to independent lines of evidence that do not reproduce (ChatGPT-F-4); (e) projected-morphology-vs-3D-spin scope statement added at Introduction + cosmology-transfer-function caveat (ChatGPT-A-4 + Gemini-DR-B-4); (f) bounce-cosmology framing minimized -- four-paper companion footnote removed; §VI.F What does the present null falsify -> What does the present null constrain (ChatGPT-I-7 + Grok-H-1 + Gemini-DR-B-4); (g) Iye photometric-duplication critique added to Shamir-context (ChatGPT-F-4); (h) Sec VI.G redshift-check raw-Catalog-A caveat made explicit (ChatGPT-A-3). **Remaining deferred to v1.0.69+ (compute-bound or external-artifact-bound)**: per-imaging-leg systematics; PSF-ellipticity calibration plot; controlled monopole+mask leakage null simulation (pymaster did not build locally; needs RunPod pod); HF dataset card rewrite (Houston push); HF dataset viewer schema fix; GitHub release tag + Zenodo DOI. **PDF 32pp / 25,887,836 bytes / 0 undef refs / sha256 26989c9e7f40...** (was 33pp / 25,899,597 v1.0.67; -1 page net from abstract shortening + cosmology-framing trim). 6 mirrors byte-identical. **Readiness P4 85 -> 87 (+2pp)** -- modest gain from 7 text-level reviewer-finding closures; compute-bound items still open. Cap 95% pending v1.0.69+ + Houston sign-off. Per-paper: P1A 74 (v1A.0.23), P1B 66 (v1B.0.7), P2 82 (v1.7.30), P3 85 (v3.1.41), **P4 87 (v1.0.68, +2pp)**. Cobaya unchanged 59,832/0.01945.",
  summary:
  ...
  needsHouston: [
    ...
    {
      title: "arXiv endorsement + submission credentials (astro-ph.CO)",
      blockedPaper: "all",
      why: "Only Houston has the arXiv account + the astro-ph.CO endorser relationships. Agents cannot create arXiv accounts or get endorsed.",
    ...
    {
      title: "DESI environmental VAC ('187 DESI-derived attributes' catalog)",
      blockedPaper: "P5",
      why: "Exhaustive sub-agent search (2026-05-15, reconfirmed tick 114) cannot locate this file in the repo or via any DESI public release. We have built a workaround (V-Web env_finder Phase 1 MVP, 104s laptop run on 14.6M spectro galaxies, headline result intact) but a published VAC is the gold-standard reference for the paper. If you do not have access to one, the V-Web workaround is sufficient and we proceed; if you do, share the path/DOI.",
