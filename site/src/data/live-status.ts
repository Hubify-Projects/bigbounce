...
export const liveStatus: LiveStatus = {
  lastUpdatedISO: "2026-05-15T06:20:00Z",
  lastUpdatedDisplay: "May 14, 2026 · 11:20 PM PT",
  headline:
    "**P2 v1.7.28 → v1.7.29 — 1st REAL cross-vendor R-round on P2 closed in single bundled wave; substantive findings landed.** Rotation moved from P4 (at 95% cap) to P2 v1.7.28 (81%) per Houston standing protocol. 1st real-vendor R-round dispatched via tools/real_cross_vendor_review.py against the v1.7.28 .tex. **Per-vendor verdicts**: GPT-5.5 1 BLOCKER + 5 MAJORs (load-bearing items: **sign-arithmetic error** on f_NL(ε) = -35/8 + κ_1(ε-3/2) consistency-relation — sign should be MINUS so the Planck-n_s correction gives the quoted [-4.35,-4.02] range; 9.9-sigma Fisher unverifiable; HDM ECH decoupling caveat doesn't close gap); Gemini-2.5-Pro 1 BLOCKER (mechanism-independence overclaim) + 2 MAJORs (κ_1 order-of-magnitude uncertainty vs 'tightly determined'; QSFI BF gap); DeepSeek-V3.2 1 BLOCKER (physical-frame f_NL_inf=0 no traceable provenance) + 3 MAJORs (9.9σ + r-ranges + BF analytic provenance); Perplexity Sonar Pro **2 MAJORs verifying CaiBrandenberger:2014 arXiv ID 1404.6968 is wrong** — direct lookup shows that ID maps to a 'Computed-torque orthosis' robotic medical-physics paper, NOT cosmology (same fused-metadata pattern as the prior Shamir/Jia catches); Grok-4 502 capacity FAIL (4th consecutive). **All real findings closed in v1.7.29**: (a) **GPT-M1 sign-arithmetic fix** on κ_1(ε-3/2) consistency-relation; (b) **Mechanism-independence narrowing** — added Assumption (f) explicitly excluding fermion-sourced torsion during contracting phase; (c) **CFC physical-frame tempering** — bounce-vs-inflation discrimination reframed as dual-pronged (gauge-frame ~290× contrast + CFC physical-frame complementary discriminator with explicit clarification SPHEREx measures the conventional gauge-frame quantity); (d) **CaiBrandenberger:2014 bib fix** — eprint=1404.6968 removed with inline note (verified medical-physics paper, not cosmology), DOI 10.1103/PhysRevD.90.023534 retained; (e) **9.9-sigma demoted from abstract** pending Fisher-input release; (f) **r-range unification** — all sites now [0.829, 0.876]; (g) **'tightly determined' tempered** — reframed as 'minimally parameterized at zeroth order; κ_1 ∈ [5.6, 80] carries substantial theoretical uncertainty'; (h) **Suyama-Yamaguchi 'saturates' → 'satisfies'** (inequality not equality since bounce bispectrum is not exactly local); (i) fixed a latex double-subscript bug n_\\fnl → n_{\\fnl} discovered during compile. GPT-B1 Appendix A convention split + GPT-M3 error-budget table deferred to v1.7.30 (require Appendix A structural rewrites). **PDF 19pp / 815,236 bytes / 0 undef refs / sha256 a381c29c70506b97...** (was 813,163 / sha 0e504915bc86; +2,073 bytes). 4 mirrors byte-identical. **Readiness P2 81 (unchanged)** — this was the FIRST real cross-vendor round on P2, so 81% reflected the pre-real-vendor confidence; substantial closures in v1.7.29 keep readiness at 81 pending the 2nd vendor round to confirm clean state and move 82-85. Gate (a) of feedback_99_pct_readiness_cap requires successive clean rounds; 95% cap pending Houston sign-off; final 1pp Houston-only. **Per-paper readiness**: P1A 73, P1B 64, **P2 81 (v1.7.29 closed substantive R-round findings)**, P3 84, **P4 95** (at cap, awaiting Houston sign-off). Cobaya iter2-OMP6 still stalled at R̂−1 = 0.01775 / 53,736 samples (no progress in ~8h; investigation deferred since P1B is compute-gated and non-blocking for P4).",
  summary:
