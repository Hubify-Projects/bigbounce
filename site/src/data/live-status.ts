...
export const liveStatus: LiveStatus = {
  ...
  currentlyRunning: [
    "Cobaya 16-chain MPI run on RTX A5000 pod ijzftpy3klystt (iter2; relaunched 5/11 22:35 PT after Houston-approved speedup). 4-chain iter1 retired with 9,503 accepted (covmat extracted via GetDist 30%-burn-in posterior), backed up at /workspace/quintom_dr2/chains_w0wa_dr2_backup_2026-05-11_pre-iter2/. iter2 = 16 ranks × fresh GetDist-built proposal covmat (w0wa_dr2_iter2.covmat, 17×17); same physics config (DESI DR2 + Planck NPIPE + Pantheon+ + DES-Y5; w0-wa CPL PPF). Expected wall-clock to R̂−1<0.01: 2–5 days (4–8× speedup vs iter1 trajectory) from the 4× more parallel chains + posterior-tuned proposal.",
  ...
  etaToCompletion:
    "Compute: cobaya R̂−1 < 0.01 ETA 2–5 days after the 5/11 22:35 PT iter2 speedup (16 chains + GetDist-built posterior covmat; iter1 honest ETA was 5–15 days at R̂−1=0.095/9,127 accepted; expected 4–8× wall-clock improvement); P4 deep-MLP RA/Dec ablation needs an H200 spin-up. Each paper carries a backward step on R45 launch (CCAI multi-agent self-review will open new findings) and a larger backward step on cross-vendor non-Anthropic round (GPT-5/Gemini/Grok/Perplexity). True 99% is gated on BOTH a clean CCAI round AND a clean cross-vendor round AND Houston sign-off; the final 1% is Houston manually triggering arXiv submission.",
  pods: [
    {
      name: "ijzftpy3klystt (cobaya-r43-v2, RTX A5000 SECURE)",
      state: "active",
      note:
        "96 vCPU, 100 GB container disk; **mpirun -n 16** cobaya-run (iter2 since 5/11 22:35 PT, up from -n 4) on Planck NPIPE + DESI DR2 BAO + Pantheon+ + DES-Y5 in tmux 'chains'. GetDist-built posterior covmat from iter1's 9,503 samples. ~$0.27/hr.",
    },
  ],
};
