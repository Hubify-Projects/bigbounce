...
export const liveStatus: LiveStatus = {
  lastUpdatedISO: "2026-05-15T06:50:00Z",
  lastUpdatedDisplay: "May 14, 2026 · 11:50 PM PT",
  headline:
    "**P3 v3.1.39 → v3.1.40 — 1st REAL cross-vendor R-round on P3 closed in single bundled wave; substantive recompute-bound items flagged on-record.** Rotation continued P4→P2→P3 per protocol. **Per-vendor**: GPT-5.5 2 BLOCKERs + 4 MAJORs (both BLOCKERs require recompute: 378,280 dedup arithmetic 637-coincidence vs 10,213-implied-duplicates mismatch; DESI OOD MSE-vs-threshold normalization); Gemini-2.5-Pro 1 BLOCKER (f_NL=-35/8 / γ=3.0 'not independent' overstates rigidity beyond w=0 scalar-only) + 1 MAJOR + 3 minors; DeepSeek-V3.2 1 BLOCKER (same 378,280 issue) + 3 MAJORs; Grok-4 3 MAJORs (378,280 narrative inflation; NANOGrav asymmetric framing; f_NL <1σ-from-null emphasis); Perplexity Sonar Pro 2 FALSE-POSITIVE BLOCKERs (Heinrich2023 + ACT_DR6 bib state actually correct on direct lookup) + 4 minor bib audit. **Closed in v3.1.40**: Gemini-B1 mechanism-rigidity scoping (f_NL/γ tight coupling explicitly scoped to scalar-only w=0 matter-bounce class; ekpyrotic/Cuscuton/quintom decouple); Gemini-m2 deleted self-ref '>4σ-equivalent framing dropped in earlier drafts' sentence; Gemini-n1 + Grok-B2 SMBHB '+4.61σ (excluded)' reframed at 3 sites as 'strongly disfavored as parameter-shift; full marginalized model-comparison required for model-level exclusion'; added §Path-C Caveats deferred-items paragraph documenting all 4 recompute-bound items on-record for v3.1.41 (378,280 dedup-arithmetic; DESI OOD MSE normalization; σ(f_NL) zero-systematics vs propagated; NANOGrav Savage-Dickey covariance). Several MAJORs already addressed in v3.1.39 text from prior rounds (17.8% point-estimate caveat, α heterogeneous-z, σ(f_NL) central-value-forecast framing). **Deferred to v3.1.41**: 378,280 union-find recompute, OOD MSE-units validation, full multi-parameter Fisher with photo-z/fiber-assignment/selection nuisance blocks, NANOGrav Savage-Dickey full chain marginalization, comprehensive bib audit. **PDF 44pp / 28,401,009 bytes / 0 undef refs / sha256 `69a7afb585eb8042...`** (was 28,395,628 / sha 9eae50d1; +5,381 bytes for deferred-items paragraph + reframings). 5 mirrors byte-identical. **Readiness P3 84 unchanged** — substantive findings closed where narrative-actionable; recompute-bound items flagged on-record for v3.1.41 closure. **Per-paper**: P1A 73, P1B 64, P2 81, **P3 84 (v3.1.40)**, **P4 95 (at cap awaiting Houston sign-off)**. Cobaya iter2-OMP6 process alive on pod (16-rank mpirun running since May 11) but progress checkpoint hasn't flushed since 15:43 UTC (~6h); sampler is slow-mode-dominated, not stalled.",
  summary:
