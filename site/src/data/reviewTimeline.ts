...
export const skillsSeries: SkillsPoint[] = [
  ...
  { id: "debias-exec-ext-hardening-2026-06-30", dateISO: "2026-06-30", patterns: 64, promptRules: 26, note: "Session 2026-06-28→30 skill upgrades, all validated by the 3-round grind: (1) the deferred EXT-prompt DE-BIAS was EXECUTED — severity-steering struck from the external referee prompt (ExternalReviewPanel L55-58); the de-biased prompt then caught 2 genuine self-favoring items (P1A 'logically-independent', P3 'catalog-grade') the biased prompt buried = rule 25. (2) EXT-sweep VERDICT-line anti-inflation gate — every browser leg must record the explicit 'Recommendation:' line ('could be accepted after revisions' ≠ ACCEPT); added after a sweep-worker manifest over-counted ACCEPTs and was caught + corrected = rule 26. Plus operational hardening (per-leg poll-cap anti-stall on browser sweeps; reviewTimeline.ts same-commit directive re-enforced after the /reviews page was found stale). Patterns unchanged at 064." },
];
