...
export const publicationMap: readonly PublicationMapGroup[] = [
  {
    ...
    rows: [
      ...
      { name: "Paper-af · Track C2 anomaly-score catalogue", role: "Data-release paper (Track C2)", status: "vAF.0.4 (2026-09-19) — R1 INT board (Claude opus 6 BLOCKER/16 MAJOR, Grok REJECT, Gemini MAJOR REVISIONS, all convergent) closed 17/20 canonical findings; most consequential: the abstract's 'supports one' quasar candidate independently re-verified as sitting inside a disclosed photometric-join defect, downgraded Supported->Undecidable. New blue-arm variance-decomposition script shows the score is R^2=0.78 explained by the blue arm alone. 17pp, title now states the null result directly; ledger #8 framing held (data release, pre-declared discovery condition NOT met). Readiness 75 COMPUTED. Directive R2: round 1 of 2 spent", destination: "Venue chosen after a bounce-vs-inflation discriminator is earned per candidate channel", dependency: "A scope decision before any further review round", nextGate: "Close the 3 honestly-disclosed open items or advance packaging", href: "/papers/paper-af" },
