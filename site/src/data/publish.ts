  ...
  decisions: [
    {
      ...
      detail:
        "A1 · P2′'s exact-amplitude theory (ledger #1 CLOSED) is now folded into A3 (paper-a3m, v3M.0.25), the flagship submission candidate combining the exact amplitude with multi-channel consistency checks (NANOGrav γ, PBH abundance, SPHEREx/MegaMapper). Ledger row 19 (D-A3-14) closed 2026-09-04, lifting the R2 rounds-stop; R9 closed 3 MAJOR + 1 MAJOR-lite + 20 minor/nit on 2026-09-19 → v3M.0.25 (reproducibility pin re-pointed, Eq. (15) re-attributed, the 10^11 f_NL figure bounded by a committed perturbativity floor); readiness 75 COMPUTED; an R10 confirmation board is in progress. A2 (transmission through an explicit bounce) remains a research brief in progress.",
    },
    {
...
export const publicationMap: readonly PublicationMapGroup[] = [
  {
    ...
    rows: [
      { name: "A3 · Track A flagship multi-channel paper", role: "Flagship submission candidate (A3 skeleton + P2′ theory folded in)", status: "v3M.0.25 — R9 closed 3 MAJOR + 1 MAJOR-lite + 20 minor/nit (reproducibility pin re-pointed, Eq. (15) re-attributed, 10^11 f_NL figure bounded by a committed perturbativity floor); readiness 75 COMPUTED; R10 confirmation board in progress. Was: v3M.0.24 — A2 monopole gap reconciled", destination: "Physical Review D (regular article)", dependency: "Houston final author review and sign-off for readiness 100", nextGate: "Houston visual approval + independent review, then submission", href: "/papers/paper-a3m" },
      ...
      { name: "Early-universe anomaly map", role: "Discovery paper (Track C2)", status: "Data-release + taxonomy full first draft vAF.0.2 (2026-09-19): 15pp, 9 figures, 11 tables, every number sourced from a committed artifact; corrected science-target parent (21,793,550 unique TARGETIDs, not 27,547,223 all-fibre), executed FT-A Lyman-break follow-up test (refutes 1 candidate, supports 1, 2 undecidable on schema gap); ledger #8 framing held (data release, pre-declared discovery condition NOT met); no INT/EXT review board run yet", destination: "Venue chosen after a bounce-vs-inflation discriminator is earned per candidate channel", dependency: "INT/EXT review board on the vAF.0.2 draft", nextGate: "Run the first review round on the full draft", href: "https://github.com/Hubify-Projects/bigbounce/blob/main/pipelines/p1_highz_tracers/anomaly_flagship_draft", external: true },
