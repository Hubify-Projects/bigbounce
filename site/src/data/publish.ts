  ...
  decisions: [
    {
      ...
      detail:
        "A1 · P2′'s exact-amplitude theory (ledger #1 CLOSED) is now folded into A3 (paper-a3m, v3M.0.5), the flagship submission candidate combining the exact amplitude with multi-channel consistency checks (NANOGrav γ, PBH abundance, SPHEREx/MegaMapper). v3M.0.5: R2 verification closed (Fable minor / Grok reject / Gemini major → 16 items closed incl. a real 30-bin injection test); automated review converged, rounds stopped (R2 budget 2/2); final author review pending. A2 (transmission through an explicit bounce) remains a research brief in progress.",
    },
    {
...
export const publicationMap: readonly PublicationMapGroup[] = [
  {
    ...
    rows: [
      { name: "A3 · Track A flagship multi-channel paper", role: "Flagship submission candidate (A3 skeleton + P2′ theory folded in)", status: "Converged (v3M.0.5) — R2 verification closed (16 items incl. real 30-bin injection test), automated review converged, rounds stopped (R2 budget 2/2), final author review pending", destination: "Physical Review D (regular article)", dependency: "Final author review, then Houston sign-off for readiness 100", nextGate: "Houston visual approval + independent review, then submission", href: "/papers/paper-a3m" },
