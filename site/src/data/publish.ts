  ...
  decisions: [
    {
      ...
      detail:
        "A1 · P2′'s exact-amplitude theory (ledger #1 CLOSED) is now folded into A3 (paper-a3m, v3M.0.2), the flagship submission candidate combining the exact amplitude with multi-channel consistency checks (NANOGrav γ, PBH abundance, SPHEREx/MegaMapper). A3 is a draft — PBH compaction-function row pending, one INT board pending. A2 (transmission through an explicit bounce) remains a research brief in progress.",
    },
    {
...
export const publicationMap: readonly PublicationMapGroup[] = [
  {
    ...
    rows: [
      { name: "A3 · Track A flagship multi-channel paper", role: "Flagship submission candidate (A3 skeleton + P2′ theory folded in)", status: "Draft (v3M.0.2) — PBH compaction-function row pending, one INT board pending", destination: "Physical Review D (regular article)", dependency: "PBH compaction-function computation; INT review board", nextGate: "INT board, then Houston visual approval + independent review", href: "/papers/paper-a3m" },
