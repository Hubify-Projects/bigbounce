  ...
  decisions: [
    {
      ...
      detail:
        "A1 · P2′'s exact-amplitude theory (ledger #1 CLOSED) is now folded into A3 (paper-a3m, v3M.0.3), the flagship submission candidate combining the exact amplitude with multi-channel consistency checks (NANOGrav γ, PBH abundance, SPHEREx/MegaMapper). v3M.0.3 integrates the real PBH compaction-function channel (item A3-1); R1 INT board dispatched (Fable + Grok + Gemini), verdicts pending. A2 (transmission through an explicit bounce) remains a research brief in progress.",
    },
    {
...
export const publicationMap: readonly PublicationMapGroup[] = [
  {
    ...
    rows: [
      { name: "A3 · Track A flagship multi-channel paper", role: "Flagship submission candidate (A3 skeleton + P2′ theory folded in)", status: "Draft (v3M.0.3) — PBH compaction-function channel integrated, R1 INT board dispatched", destination: "Physical Review D (regular article)", dependency: "R1 INT board verdicts (Fable + Grok + Gemini)", nextGate: "INT board verdicts, then Houston visual approval + independent review", href: "/papers/paper-a3m" },
