...
Current constraints from Planck (CMB bispectrum, $\fnl = -0.9 \pm 5.1$~\cite{Planck:2019fnl}) can be recast onto the bounce template using Eq.~(\ref{eq:projection}). Recasting the Planck constraint with the template mismatch factor $r = 0.90$ gives $\fnl^{\rm bounce} = -1.0 \pm 5.7$, which is 0.6$\sigma$ from the bounce prediction and 0.2$\sigma$ from zero---fully consistent with both. We note that DESI DR1 has not published an independent $\fnl$ constraint from scale-dependent bias as of this writing; once such a constraint is available, combining it with the Planck bispectrum measurement will tighten the current bound. Current data cannot discriminate between the bounce and inflation.
