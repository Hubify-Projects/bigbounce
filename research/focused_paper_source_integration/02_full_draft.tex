...
\begin{equation}
n_s = 8\epsilon - 11\,, \qquad \fnl(\epsilon) = -\frac{35}{8} + c_1(\epsilon - \tfrac{3}{2}) + \mathcal{O}(\epsilon - \tfrac{3}{2})^2\,,
\end{equation}
where $c_1$ depends on both the explicit $\epsilon$-prefactors in the cubic action and the mode-function growth rate, which changes significantly near the singular point $\epsilon = 3/2$.
\textit{Linearization note.} The relation $n_s = 8\epsilon - 11$ is a linearization in the slow-roll parameter $\epsilon$ around exact matter domination; the exact expression from the growing-mode solution is $n_s - 1 = 2(2\epsilon - \eta)$, where $\eta \equiv -\dot{\epsilon}/(\epsilon H)$ is the second slow-roll parameter. For exact matter domination ($\epsilon = 3/2$, $\eta \approx \epsilon$) these agree to first order; small deviations from $\eta = \epsilon$ (controlled by the quasi-dust equation of state $w = -0.003$) produce a correction of order $|\eta - \epsilon| / \epsilon \sim 10^{-3}$, entirely negligible relative to the $1$--$8\%$ $\epsilon$-correction uncertainty in $\fnl$. The coefficient $c_1$ is bounded: explicit prefactor scaling gives $c_1 \approx 2$ (lower bound), while including the mode-function amplitude change gives $c_1 \approx 18$ (upper bound). Eliminating $\epsilon$, the consistency relation takes the form
\begin{equation}
...
\item \textbf{Alternative convention} (used by Li \& Brandenberger): $c = 1$, giving $\fnl = -35/4$. In this convention, $\fnl(c{=}1) = 2 \times \fnl(c{=}2) = 2 \times (-35/8) = -35/4$. Li \& Brandenberger~\cite{LiBrandenberger:2014} report $\fnl = -35/16$; the additional factor of two beyond the $c$-convention difference arises from how permutation factors from Wick contractions are absorbed into the total shape function $A_T$ (see main text, Sec.~\ref{sec:assumptions}).
\end{itemize}
The \emph{physical} bispectrum amplitude $B_\zeta$ is identical in both conventions: $2 \times (-35/8) = 1 \times (-35/4) = -35/4$. The detection significance $|\fnl|/\sigma(\fnl)$ is also convention-independent, since $\sigma(\fnl)$ scales inversely with $c$.
