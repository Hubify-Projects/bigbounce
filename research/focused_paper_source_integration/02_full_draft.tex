...
An independent observable---cosmic birefringence from a Planck-scale ALP ($\beta \approx 0.27^\circ$)---provides a complementary test of bounce-motivated physics in the published literature, with the prediction $\beta\approx 0.27^\circ$ depending on the ALP coupling $g_{\phi\gamma}$ and ALP mass $m_a$ being consistent with sub-eV/super-Planck-coupled spectator-ALP windows (so the test is bounce-motivated rather than parameter-free in the Maldacena-consistency-relation sense): the $3.6\sigma$ Eskilt \etal~\cite{Eskilt2022} joint Planck analysis and the $2.9\sigma$ ACT~DR6 measurement of Diego-Palazuelos \etal~\cite{DiegoPalazuelos2025} are both consistent with the bounce ALP prediction. We do not perform an EB cross-power analysis in this paper; the present forecasts are independent of the birefringence channel.
...
All analysis code, Monte Carlo scripts, and shape-function evaluation routines are available at \url{https://github.com/Hubify-Projects/bigbounce/tree/v1.7.19-paper2/research/} (pinned to release tag \texttt{v1.7.19-paper2}). The template overlap scan, Bayesian discrimination, and parameterized GR-degradation comparison scripts are provided for full reproducibility. No new observational data are introduced; all forecast sensitivities are adopted from published analyses~\cite{Heinrich:2023,Schlegel:2022}.
