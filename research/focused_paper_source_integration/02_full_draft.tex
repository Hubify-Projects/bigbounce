...
Standard single-field slow-roll inflation predicts $\fnl = (5/12)(1-n_s) \approx +0.015$~\cite{Maldacena:2002vr}---differing by $|\fnl^{\rm bounce}|/|\fnl^{\rm inf}| \approx 400$ in absolute value and opposite in sign. Non-canonical single-field models (DBI, etc.) produce equilateral-shape $\fnl$, not local.
...
To make the factor chain explicit: (1)~the Planck $c=2$ convention converts Li~\&~Brandenberger's in-calculation value $-35/16$ by a factor of $1/c = 1/2$, giving $-35/8$; (2)~additionally, the Li~\&~Brandenberger permutation convention introduces a further factor of $1/2$ relative to the in-in commutator doubling used by Cai \textit{et~al.}, accounting for the remaining discrepancy. The net effect is that the four quantities $\{-35/4,\,-35/8,\,-35/16,\,-35/8\}$---corresponding to $c=1$, $c=2$, Li \& Brandenberger, and Cai \textit{et~al.}\ after full convention resolution---all describe the same physical bispectrum.
