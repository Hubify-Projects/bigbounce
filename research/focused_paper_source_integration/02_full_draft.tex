...
Bounce vs.\ tuned multifield [$-15,+15$] & 8--11 & 87--96\% \\
\bottomrule
\end{tabular}
\caption{Bayesian model comparison for a mock detection at $\fnl = -4.375$ with $\sigma = 0.7$ and a delta-function bounce prior. SSFSR = standard single-field slow-roll inflation. The range 8--11 reflects different GR treatment scenarios (Table~\ref{tab:gr}). Broadening the bounce prior to a Gaussian with $\sigma_{\rm theory} = 1.0$ reduces these to $\sim 8$--17 (abstract headline).}
\label{tab:bayes}
\end{table}
