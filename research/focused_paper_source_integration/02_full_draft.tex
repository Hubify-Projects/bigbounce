...
\date{May 1, 2026, 07:30 PDT --- v1.7.6}
...
\bottomrule
\end{tabular}
\caption{Bayesian model comparison for a mock detection at $\fnl = -4.375$ with $\sigma = 0.7$ and a delta-function bounce prior, evaluated at the four GR-treatment scenarios of Table~\ref{tab:gr}; the 8--11 range is the GR-marginalization spread at \emph{fixed} delta prior. The headline envelope $\sim 8$--$17$ in the abstract spans the recommended broadened-prior baseline ($\sigma_{\rm theory}=1.0$, lower bound $\sim 8$) up to the delta-prior maximum at the broad multifield competitor prior $[-15,+15]$ (upper bound $\sim 17$); broader bounce priors give \emph{smaller} Bayes factors, never larger.}
\label{tab:bayes}
\end{table}

Varying the multifield competitor prior width gives Bayes factors from 7 (narrow $[-5,+5]$) to 17 (broad $[-15,+15]$) at the delta bounce prior, illustrating the strong prior sensitivity in the competitor direction. The abstract envelope $\sim 8$--$17$ then brackets, at the broad multifield competitor prior $[-15,+15]$, the recommended broadened bounce prior ($\sigma_{\rm theory}=1.0$, lower bound $\sim 8$) up to the delta-prior maximum (upper bound $\sim 17$). The sense of the prior dependence is fixed: the delta prior is the maximum, every finite-width broadening reduces the Bayes factor, and the abstract, Table~\ref{tab:bayes}, and the bullet list above are now numerically aligned in that convention.
