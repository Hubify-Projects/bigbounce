---
title: "Paper 3: Multi-Survey Anomaly Catalog — POINTER"
type: entity
tags: [paper, anomaly, desi, sdss, erosita, lamost, fnl, nanograv, pointer]
last_updated: 2026-04-17
superseded_by: project-context/SSOT/paper-3/status.md
---

# Paper 3: Multi-Survey Anomaly Catalog — POINTER ONLY

> **This wiki entry is a pointer.** The canonical status lives in the SSOT.
> Do not read status claims from this file — read them from the SSOT.

## Canonical sources

- **Paper-3 SSOT status:** [`project-context/SSOT/paper-3/status.md`](../../project-context/SSOT/paper-3/status.md)
- **Cross-paper dashboard:** [`project-context/SSOT/index.md`](../../project-context/SSOT/index.md)
- **Task queue:** [`project-context/SSOT/queue.md`](../../project-context/SSOT/queue.md)
- **Manuscript source:** `pipelines/p3_anomaly_engine/paper3_draft.tex`

## Connections

- Pipeline: [[pipeline-b-desi-anomaly]]
- f_NL theory: [[fnl-prediction]]
- Related paper: [[paper-4-chirality]]
- Downstream: [[anomaly-detection-methodology]]
