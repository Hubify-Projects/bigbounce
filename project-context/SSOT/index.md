...
| **4** | Galaxy Chirality Catalog | **100 %** | **Wave 14-D LANDED** — Platt-calibration characterization fix (P4-OA-B6): §III.F Catalog~B bullet rewritten "*removes*" → "*reduces*" with explicit before/after numbers (+0.79% raw 28.8σ → +0.4% calibrated 14.6σ → −0.26% equivariant 9.5σ), explicit Platt mapping $p_{\rm cal} = \sigma(Az + B)$ with $A = 1/4.65$, $B = -1.58$, fit on held-out 20% validation via L-BFGS, and explicit cause statement (residual +0.4% persists because Platt is fit against CE-ResNet consensus, not independent ground truth). **Wave 14-C carried forward**: abstract internal-inconsistency (P4-OA-B2) + §III.D NS-count subset scoping (P4-OA-B1 secondary axis). **Wave 12 hemisphere LEE** (p_LEE = 9.999e-5 at N_MC=10,000) + **Wave 11 N_spiral=3,201,160 shot-noise correction** still in force. PDF recompiled (25.79 MB / 18 pp / 0 undef refs, May 1 14:30 PDT, **v1.0.9**). Self-contained, submission-locked. Wave 14-D-compute queued: full-catalog TTA-leakage recompute on the 5.15M-NS sample (the second half of GPT-5 BLOCKER #1 fix demand — compute-only follow-on, no text gap remaining). |
