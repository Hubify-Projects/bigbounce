...
Last authoritative update: 2026-04-20 (drive-to-100 fire #119 — P3-PATHC-INJECTION-RECOVERY-ALL-SURVEYS criterion #6 advanced 70 → 80 % via NEOWISE mask injection-recovery `pipelines/p3_anomaly_engine/pathc_neowise_ecliptic/neowise_mask_injection_recovery.py` (~160 lines, local CPU < 2 s). Two-test protocol: (A) specificity — 100K isotropic-sphere sample → 1.51 % observed rejection vs 1.52 % theory (|Δ| = 0.01 %, PASS); (B) sensitivity — 1,000 polar-cap-injected anomalies per band at `|β_ecl| > {85°, 82°, 80.5°}` → 1000/1000 = 100.0 % rejection at all three bands (PASS). **Gate PASS** — mask is both specific (no over-rejection of clean catalog) and sensitive (polar-cap artifact population fully caught), validating fire #84's decision to mask rather than retrain. Criterion #6 now covers 4 surveys: SDSS (continuum-dip 64 % @ 5σ PASS), LAMOST (9.7× over emission-line), CMB (100 % @ 5σ PASS), NEOWISE (100 % at polar bands PASS); eROSITA + Gaia + DESI deferred. SDSS rescore 140 → 140 batch UNCHANGED vs fire #118 (flush-boundary artifact, LAMOST progressed 420 → 420 nights batch 40 → 41 + 4.40 M → 4.50 M scored + 613 → 619 anomalies). Budget ~$57.00/$400.)
