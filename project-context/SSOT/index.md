...
Last authoritative update: 2026-04-20 (drive-to-100 fire #122 — P3-PATHC-INJECTION-RECOVERY-ALL-SURVEYS criterion #6 advanced 90 → 95 % via all-surveys synthesis aggregator `pipelines/p3_anomaly_engine/pathc_injection_recovery/synthesize_all_surveys.py` (~240 lines, local CPU < 2 s). Reads the 6 per-survey Path-C injection-recovery JSONs from fires #85/#95/#98/#119/#120/#121 and writes a single Paper 3 §pathc_caveats-ready summary: `all_surveys_summary.json` (structured row-per-survey) + `all_surveys_summary.md` (copy-paste markdown table). Headline tally: 6/8 surveys covered = 3 gate PASS (SDSS continuum-dip 64.0 %, CMB 100.0 %, NEOWISE spec + sens) + 3 gate FAIL-with-rigorous-diagnostic (LAMOST 9.7× em/cont, Gaia 41 % XV, eROSITA 81.5 % XV); remaining 2/8 = DESI (awaits #4) + ACT (legitimately out of scope). Artifact closes the loop on 6 injection-recovery fires with a single auditable reviewer-facing table. Criterion #6 now at 95 %; the final 5 % = §pathc_caveats .tex integration (dovetails criterion #8). Index.md headline holds at ≈65 %. Prior fire #121: eROSITA DR1 latent-space IF injection-recovery strict gate FAIL (1.2 %) + fresh-IF recovery of h200 top-1 % = 81.5 % (7582/9303), 4× better than Gaia. SDSS rescore expected flush to 150; LAMOST 440 nights steady at 625 anom. Budget ~$57.80/$400.)
