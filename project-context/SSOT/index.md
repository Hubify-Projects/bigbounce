...
Last authoritative update: 2026-04-20 (drive-to-100 fire #118 — P3-PATHC-NEOWISE-ECLIPTIC-MASK criterion #5 advanced 85 → 95 % via reviewer-evidence figure `pipelines/p3_anomaly_engine/figures/fig_pathc_neowise_ecliptic.{png,pdf}` (plot script at `pathc_neowise_ecliptic/plot_ecliptic_latitude_distribution.py`, ~110 lines, local CPU ~5 s). Figure overlays (a) `|β_ecl|` histogram of all 436 raw anomalies, (b) 17 Path-C rejected objects in red, (c) uniform-sphere `cos(b)` null density, (d) `|b_ecl| = 80°` cut line, (e) annotation box computing observed 3.90 % vs uniform-null 1.52 % → **2.57× pole excess** (matches canonical 2.566× from `pathc_ecliptic_summary.json` to 2 dp). Math bug caught mid-fire: uniform null is `1 − sin(80°)` (spherical cap area fraction with `|b| > 80°`), not `1 − cos(80°)`. Figure at non-paper-body path → intentionally NOT coupled to criterion #9 recompile. SDSS rescore flushed 135 → 140/471 (ETA 35.3h); LAMOST 420 nights + one decompression error logged (ETA holds 26.3h). Budget ~$56.60/$400.)
