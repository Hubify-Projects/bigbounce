...
Last authoritative update: 2026-04-20 (drive-to-100 fire #120 — P3-PATHC-INJECTION-RECOVERY-ALL-SURVEYS criterion #6 advanced 80 → 85 % via Gaia DR3 IsolationForest injection-recovery `pipelines/p3_anomaly_engine/pathc_injection_recovery/gaia_injection_recovery.py` (~170 lines, local CPU ~20 s). Fresh IF fit on 494,500 clean Gaia sources (22-dim variability feature vector); strict Path-C 5σ gate FAIL (5.2 %) with companion diagnostic fresh-IF-recovery of h200 published top-1 % set = 41.0 % (2048/5000). Methodology finding parallel to fire #85: 22-dim IF dominated by non-variability axes (mean mag / BP-RP / num_selected) → pure-variability-axis injection moves along a subspace and cannot force joint point past isolation boundary. Physics-relevant signal is the 41 % cross-validation (motivates methodology-footnote caveat on Gaia Table 1 row that detection is training-sample-conditioned, like DESI-k-fold plan). Criterion #6 now covers 5 surveys (SDSS continuum-dip PASS + LAMOST 9.7× + CMB 100 % + NEOWISE 100 % + Gaia FAIL-with-diagnostic); eROSITA + DESI remaining. SDSS rescore expected flush to 145; LAMOST 420 nights steady at 619 anom. Budget ~$57.40/$400.)
