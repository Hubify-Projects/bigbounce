...
Last authoritative update: 2026-04-17 (drive-to-100 fire #9 — Docker TeX Live compile burst: all 4 papers recompiled from source with 0 undefined citations, fresh bbl for Paper 1, §IV corner figure rendered, Paper 3 cross-refs + Siemens/Rosado bibitems rendered, Paper 4 LSST projection line rendered, Paper 1 tarball rebuilt clean + smoke-tested. All 4 PDFs mirrored to `public/papers/`)
...
| **3** | **Multi-Survey Anomaly Catalog** | 🟠 **PATH-C REBUILD IN FLIGHT (≈55 %)** post-2026-04-19 novelty-integrity pushback. Prior 2026-04-16 lock (319,443 anomalies / 58.8 % novel / NANOGrav γ = 3.20 ± 0.42) preserved as §7 before-after baseline. Path-C progress (fire #100, 2026-04-20): criterion #3 CMB native retrain **CLOSED** (val_loss 0.4437 gate PASS, 500/500 = 100 % injection-recovery at 5× noise, 200K-patch full re-score complete); criteria #1/#2 SDSS+LAMOST native re-scores in flight (55/471 batches + 150/1177 nights, ETA 32-44 h); #5 NEOWISE ecliptic mask 85 %; #6 injection-recovery 60 % (continuum-dip variant SDSS native **gate PASS 64 % at 5σ** vs 7.2 % emission-line); #7 8-way dedup 70 % (5/8 surveys loaded, 197,246 → 191,432 unique physical objects); #8 §2.4 + §7 Path-C methodology subsections landed 70 %; #10 HF-REBUILD 45 %; #12 site-sync 70 % (all 4 primary pages carry Path-C amber banners). Pre-Path-C compile state: 28 MB PDF / 0 undef refs at `public/papers/paper3_anomaly_catalog.pdf`; post-Path-C recompile (criterion #9) gated on natives. | **Rebuild ≈55 %** | 12 Path-C exit criteria — 1 CLOSED (#3 CMB) + 8 in progress (#1/#2/#5-8/#10/#12) + 3 not-started (#4 DESI k-fold, #9 recompile, #11 P1-PDF-V3). See `SSOT/drive-to-100.md` Phase-2 Path-C block + fire-by-fire Loop log. | `pipelines/p3_anomaly_engine/paper3_draft.tex` | [paper-3/status.md](paper-3/status.md) |
