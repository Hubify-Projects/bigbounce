...
|---|---|---|---|---:|---|
| 1 | **Document class conversion.** Rewrite line 1 from `[a4paper,11pt]{article}` + natbib to `[aps,prd,twocolumn,superscriptaddress,showpacs,preprintnumbers,nofootinbib,longbibliography,floatfix]{revtex4-2}`. Remove `\bibliographystyle{unsrtnat}` and natbib preamble. | `P2-REVTEX4-2-CONVERT` | agent | 6 % | [ ] |
| 2 | **Bibliography resolution.** Replace all `\citep{}` with `\cite{}`. Embed `\bibitem` entries from `03_references.bib` + `focused_paper_refs.bib` (or link cleanly). Ensure every cited label resolves — zero `[?]` in output. | `P2-BIB-RESOLVE` | agent | 4 % | [ ] |
...
| 4 | **Cross-reference audit.** Confirm Paper 1 `\citep{Golden:2026framework}` is the right handle; scan §4/§5 for Paper 3 implicit reference and add explicit `\cite{}` if present; verify no Paper 4 dependency. | `P2-XREF-AUDIT` | agent | 1 % | [ ] |
...
| 6 | **Wiki pointer rewrite.** `wiki/entities/paper-2-fnl-forecast.md` → pointer-only to this SSOT. Drop the stale 2026-04-04 status line. | `P2-WIKI-POINTER` | agent | 0.3 % | [ ] |
| 7 | **`CURRENT_STATUS.md` row update.** Paper 2 row currently says "v1.3.0 · Ready for submission" — update to v1.6.0 · 85 %-ready-pending-revtex4-2. | `P2-CURRENT-STATUS-SYNC` | agent | 0.2 % | [ ] |
...
| 9 | **arXiv tarball.** `tar -czf paper2_arxiv_submission.tar.gz 02_full_draft.tex 02_full_draft.bbl fig*.png bphi_sensitivity.pdf` → smoke-test build from tarball alone. | `P2-TARBALL` | agent | 0.2 % | [ ] |
