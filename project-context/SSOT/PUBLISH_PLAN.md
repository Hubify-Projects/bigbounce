...
| C5. P1B Caγ continuous grid MCMC [4,60] | P1B (#A3-5) | ~1-4h (NOT 1d) | original prior [1,30] truncates data-required [9,51] | **RUNNING 2026-06-09 19:26 UTC** — 4 MPI chains, cobaya 3.6.1, ground truth run2_extended.yaml replicated verbatim (alp_theory/alp_ode/birefringence_lk reused, only prior widened). Gaussian-summary β likelihood (Eskilt PR4+ACT-DR6 β=0.342°±0.094°) — no Planck download needed, hence hours not days. NEW TEXT FINDING: P1B appendix says "Planck PR4 + ACT DR6 EB-spectrum likelihoods" but implementation is the Gaussian summary on Eskilt's combined β — fix wording in v1B.0.49 bundle. |
