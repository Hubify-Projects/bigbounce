...
**🎯 Last authoritative update: 2026-06-11 (PDT) — P3 v3.1.84 → v3.1.85 — R26conf NOT CLEAN but ZERO ARITHMETIC ERRORS; 12 textual closures.** R26conf cross-vendor round on v3.1.84: **zero arithmetic errors across the round**; 12 textual closures landed in v3.1.85, headline items: (a) **cluster accounting made EXACT from the dedup artifact** — the cluster-collapse bookkeeping now reproduces the committed dedup artifact exactly; (b) **NANOGrav Eq. E1 claim FALSIFIED by rederivation** — the reviewer accusation did not survive a from-scratch rederivation. PDF 26 pp / 0 errors / 0 undef refs / md5 b096e82c, mirrored to site/public/papers/paper3_anomaly_catalog_v3.1.85.pdf. **Readiness 91 → 92** (zero-arithmetic-error textual wave; not a clean round). **Close-the-gap: 1 ESS queued (tabular feature-scaling spec recovery, pod-side) + 2 MAJOR recomputes → R27conf cross-vendor round clean on v3.1.85 → Houston external round + sign-off → HF visibility flip → arXiv (tarball rebuilding as paper3_arxiv_v3.1.85.tar.gz — supersedes the v3.1.84 build).** Live tex: `pipelines/p3_anomaly_engine/paper3_draft.tex` v3.1.85.
