...
Last authoritative update: 2026-04-18 (drive-to-100 fire #14) — pod deploy delivered. RunPod H100 SXM pod `uyl9w5oo37uf06` (AP-IN-1 · $2.69/hr) provisioned, two jobs ran in parallel tmux sessions. **P3-FISHER-FULL core result landed**: σ(log₁₀A)=0.135, **σ(γ)=0.506**, ρ=−0.794, **2.24σ tension vs SMBHB γ=13/3** — ready for Paper 3 §6. Future-PTA scaling panel has a real bug (uniform `cov_scale` cancels in the Fisher trace); filed `P3-FISHER-FULL-FIX` and rerunning on same pod with signal/noise covariance decomposition. **P3-A**: TESS returned 0 products for TIC 374313355 — filed `P3-A-ALT` (TESS-FFI cutout + Kepler + K2 by resolved coords, same pod). Both fixes in flight via background sub-agent `a0506c5415378292a`. Files pulled local: `pipelines/p3_anomaly_engine/fisher_full/` + `pipelines/p3_anomaly_engine/p3a_tess_374313355_lomb_scargle/`. Pod will self-terminate after fix sub-agent completes. Prior fire #13 HF-upload-extend state unchanged (5 of 8 HF blocks, 61.7 % coverage).
...
| `P3-A` | TIC 374313355 periodicity analysis (TESS archival light curve + Lomb-Scargle) | pod | P3 | 0.05 % | [~] | PARTIAL 2026-04-18 drive-to-100 fire #14 — pod `uyl9w5oo37uf06` ran `lightkurve.search_lightcurve("TIC 374313355")`: returned **0 products**. No TESS SPOC data for that TIC. Likely causes: typo in internal note, or no SPOC pipeline delivery. Results note at `pipelines/p3_anomaly_engine/p3a_tess_374313355_lomb_scargle/RESULTS.md`. Follow-up `P3-A-ALT` filed below (TESS-FFI cutout + Kepler + K2 by coordinates). Paper 3 §7.3 annotation: "target resolution pending" until alt-ID search lands |
| `P3-A-ALT` | TIC 374313355 fallback search: `lightkurve.search_tesscut` FFI cutout + Kepler + K2 by resolved coordinates | pod | P3 | 0.05 % | [~] | In flight 2026-04-18 fire #14 via same pod. Sub-agent `a0506c5415378292a`. Uses same Lomb-Scargle pipeline on whichever archive yields data |
...
| `P3-FISHER-FULL` | Full Fisher-matrix calculation over the NANOGrav free-spectrum covariance to replace the scaling-only forecast with a properly marginalized σ(γ) projection | pod | P3 | 0.05 % | [~] | PARTIAL 2026-04-18 drive-to-100 fire #14 — pod `uyl9w5oo37uf06` produced **core NG15-current result: σ(log₁₀A)=0.135 · σ(γ)=0.506 · ρ=−0.794 · 2.24σ tension vs SMBHB γ=13/3**. Written to `pipelines/p3_anomaly_engine/fisher_full/fisher_result.json` + `sigma_gamma_forecast.png` + `RESULTS.md`. Ready for Paper 3 §6 insertion. Future-PTA scaling panel is **buggy** (uniform `cov_scale` cancels in Fisher trace — `F=½Tr[C⁻¹ ∂C C⁻¹ ∂C]` is invariant under C→αC) — filed `P3-FISHER-FULL-FIX` below |
| `P3-FISHER-FULL-FIX` | Correct future-PTA scaling: decompose C = C_signal(A,γ) + C_noise; scale only C_noise per scenario | pod | P3 | 0.02 % | [~] | In flight 2026-04-18 fire #14 via same pod. Sub-agent `a0506c5415378292a`. Core NG15 result stands; only the NG20/CPTA/SKA forecast columns need re-derivation |
