...
Last authoritative update: 2026-04-18 (drive-to-100 fire #13) — `P3-HF-UPLOAD-EXTEND` advanced three more blocks in one commit: **NEOWISE (436) + Planck CMB (200) + Gaia DR3 (500)** uploaded to private HF dataset `bamfai/bigbounce-anomaly-catalog`, each exactly matching Paper 3 Table 1 via explicit top-N-by-`anomaly_score` cut (not `is_top1pct` — that column is noisy: 444 for NEOWISE, 5,000 for Gaia, wouldn't match paper). Coverage now **DESI + ACT + NEOWISE + Planck + Gaia = 197,165 / 319,443 rows (61.7 %)**. Dataset card refreshed to 5-block table with per-survey status + honest note that SDSS / LAMOST / eROSITA source files in the 2026-04-08 H200 snapshot show `data_source: "synthetic"` (aggregate score-dist only; no row-level table with RA/Dec/score). Those 3 surveys now flagged as requiring pod regeneration from source — retargeted to `P3-HF-UPLOAD-EXTEND-POD`. Script: `pipelines/p3_anomaly_engine/hf_upload_extend.py`. Simultaneously: background sub-agent deploying RunPod for P3-A + P3-FISHER-FULL (per Houston's 2026-04-18 guidance to deploy pods rather than defer).
