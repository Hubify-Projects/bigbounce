...
- **Readiness (post Wave 14-OOO P1-split):** **P1A 85 %**, **P1B 75 %** (both rolled back 2026-05-08 22:30 PT from a stuck-at-99% reading per Houston status-honesty directive; see SSOT/index.md banner of same date). P1A: R44 BLOCKERs B1+B2+B3 (Wave 14-EEEE) + four-route no-go appendix scaffold (Wave 14-IIII, NJL Hehl-Datta + one-loop EA Mercuri-Capozziello + Immirzi Date-Kaul-Sengupta + parity-CMB Lue-Wang-Kamionkowski; +5 standard bibitems, +2 pp) + cross-paper Golden2026P{1b,2,3,4} bibitems (Wave 14-JJJJ) shipped. Residual P1A R44 MAJORs: M4 D_inf prefactor justification + M5 orphan section labels. P1B: compute-gated on cobaya R-1 < 0.01 (currently 0.076 at 18:27 PT 5/8); §Structural Tension MCMC numbers placeholder until convergence; not yet through R44 self-review. **R45 + cross-vendor non-Anthropic round + Houston sign-off all still pending for both.** Cap stays at 95% until BOTH a clean CCAI R-round AND a clean cross-vendor round have passed; only then can rise to 99%. The final 1 % to 100 % is Houston pulling the arXiv trigger only.
