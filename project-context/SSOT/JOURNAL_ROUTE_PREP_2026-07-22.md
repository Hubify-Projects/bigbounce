...
- [x] Compiled PDF exists and is current: `arxiv/paper1b_namaster_proof.pdf`, v2B.0.14.
...
**MISSING (must close before the submission click)**
- [ ] **ORCID not embedded in the manuscript.** `\author{Houston Golden\\Independent researcher}` (line 68) carries no ORCID. Houston's confirmed ORCID is `0009-0008-5616-5994` (per `project-context/PUBLICATION_DECISION_BRIEF_2026-07-20.md`, D5) but it does not appear anywhere in `arxiv/paper1b_namaster_proof.tex`. JORS doesn't state ORCID is mandatory (UNVERIFIED requirement either way), but the portal's own author-metadata fields will very likely prompt for it — cheap to add now.
- [ ] **Template conformance unverified.** The manuscript is a plain `\documentclass[11pt]{article}` (not JORS's own .docx/LaTeX template). JORS's submissions page states metapapers "must conform to provided templates," but does not clarify whether that's enforced at initial submission or only reformatted at camera-ready. **UNVERIFIED** — needs a direct check of JORS's downloadable template (linked from the submissions page but not independently fetched in this pass) before the actual submission.
...
**Verdict:** No hard blocker in the repo/manuscript content itself. The two closeable items (ORCID in the author block; confirming template conformance) are small; the APC and account creation are Houston's own next actions, not blockers to preparing the submission package.
...
| | JORS / P1B | ApJS / P3 |
|---|---|---|
| Correct format already in repo? | Plain `article` class — JORS template conformance **UNVERIFIED** | ✅ `aastex701` (current AASTeX v7.0.1) — no conversion needed |
...
| ORCID embedded in manuscript? | ❌ Missing (known value 0009-0008-5616-5994) | ❌ Missing (same) |
| Persistent software/data identifier? | ✅ Zenodo DOI 10.5281/zenodo.21481753 | ✅ Zenodo DOI 10.5281/zenodo.21461888 |
| Concrete content blocker? | None found | **Abstract ~415 words vs. 250-word AAS limit** |
