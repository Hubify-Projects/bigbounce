...
- [x] **L9: Hierarchy refactor — Projects-as-threads, Papers as siblings, M:M wiring** (commit `8613533`, BUILD_READINESS loop 2026-04-09) — Replaced `.project-cards` flat list with `.project-threads` expandable accordion. Each `.pt-thread` wrapper contains a `.pt-head` (click → toggleProjectThread) and a `.pt-body` that opens inline on toggle. Header row: dot · title · time badge · "open →" link (stopPropagation → openProjectDetail full view) · chevron (rotates 90° when open via `.pt-head.open .pt-chevron`). Body shows experiments + papers as siblings (M:M wiring): each `.pt-item` row has type tag (exp/paper) · name · status pill. Status pills: pass (green) · running (amber) · queued (gray) · primary (green) · supporting (gray). Real data for all 3 projects: P1 (3 exp EXP-053/052/056 + Paper 3 primary + Paper 2 supporting), P3 (3 exp EXP-050/054/055 + Paper 3 primary + Paper 1 supporting), P4 (1 exp EXP-051 + Paper 1 primary). `toggleProjectThread(id)` JS function added (7 lines). 25 new CSS rules under `.project-threads` namespace. All existing `openProjectDetail` call sites preserved — sidebar + view-pipeline links still work. Chevron animation via CSS transform only (no JS).
...
**L status:** 12/12 = **100% ✅** — L1 (sidepeek kill iter 1) + L2 (detail breathing room) + L3 (dead sidepeek DOM+CSS deleted) + L4 (rename pass) + L5 (multi-tab support) + L6 (Director rebuild) + L7 (papers full pages) + L8 (sidebar drastic reduction) + L9 (projects-as-threads, commit `8613533`) + L10 (operational consolidation) + L11 (Lab Dossier 10-section) + L12 (Chats scope-organized) all shipped. Category L COMPLETE.
...
| **L. UX rebuild (kill sidepeeks · director · papers · dossier)** | **12** | **12** | **100% ✅** |
| **OVERALL** | **162** | **172** | **94%** |

**Translation:** **9 of 12 categories at 100% ✅** (B · C · D · E · F · G · J · K · L all locked end-to-end). **Category L just hit 100%** — L9 (Projects-as-threads accordion, commit `8613533`) completed the final L item. **Remaining autonomous work:** H.8 (test-lab pre-validation, ~1 day Convex backend build) + quality polish passes. All other remaining items (A gate · A.53 · H.7 · H.9 · I.1-I.7) are Houston-sign-off-blocked. Overall: 162/172 = 94%.
