...
|----|---------|-------------|
| META-M1 (mask dilation insufficient for kernel support) | **VERIFIED → QUEUED** | Cube-connected/≥3-iter dilation rerun is compute-class. Interior-buffer + NSIDE-64 controls already published bound the boundary sensitivity. Queue (with pre-existing Queue #16 randoms-weighted rebuild). |
...
| META-M4 (zero-padding integral constraint) | **STALE/QUEUED** | Same compute family as R24conf META-M1/Queue #16 (randoms-weighted δ rebuild). |
