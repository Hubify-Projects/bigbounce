...
## H17H SUB-ROUND ADDENDUM (2026-07-10, EXT re-test vs v0.1.118) — unmatched-finding adjudication

Raws read + verified before any verdict: `H17H/P5_grok_H17H.md`, `H17H/P5_chatgpt_H17H.md`. Pre-triage (`tools/ledger_match.py`) left 1 Grok + 3 ChatGPT UNMATCHED; all four Opus-adjudicated.

- **Grok = MINOR REVISIONS** (from H17F ACCEPT@v0.1.117). 3 real minors on the NEW v0.1.118 void text + prior presentational asks → DP5-09 (dedicated env-relabeling caveat paragraph, already §XII B/App A), DP5-13 (move exploratory/post-hoc-quotable line into abstract, already §V B + abstract), DP5-11 (footnote the √0.885=0.94pp quadrature formula, term list already §VIII). UNMATCHED#1 "revisions the paper should be accepted" (0.00) = **parser noise**, tail fragment of "With minor revisions the paper should be accepted." **Fresh-content minors on the new void integration = expected + cheap, NOT oscillation** (ACCEPT→MINOR is Grok reacting to the v0.1.118 additions). 0 genuinely-new editable.
- **ChatGPT = REJECT** (maximal-harsh floor). 3 UNMATCHED all RE-FLAG:
  - #2 (0.29) "Adjustment in lieu of covariate regression: no matching/IPW/regression on primary DESIVAST" → RE-FLAG DP5-06 + DP5-19; §VIII B l.3273-3293 already states the logistic/IPW form, the program-split+systematics adjustment set, 0.001pp residual leakage, and the DR2-deferred full regression verbatim (the reviewer quotes the paper's own paragraph title).
  - #3 (0.25) "monopole cancels algebraically only for additive bias; void confusion [−0.057,+0.022] ≫ 0.9pp" → RE-FLAG DP5-08 + DP5-09; l.4803-4809 scopes cancellation to the catalog-wide monopole *amplitude*, and [−0.057,+0.022] is the paper's OWN measured void error-asymmetry diff (DP5-08 v0.1.118) which corroborates-but-cannot-exclude, de-attenuation caveat DP5-09 stays.
  - #13 MINOR (0.17) "Tempel/T-Web/ASTRA concordance uses independent z-tests; paired covariance required" → RE-FLAG DP5-15 + DP5-19 OPINION; these checks are labeled `descriptive`/`supporting not load-bearing` (analysis tree l.1894-1895; l.4300-4308) and enter no headline. 0 genuinely-new editable.

**Ledger maintenance:** added the missing fingerprint keywords to DP5-09, DP5-15, DP5-19 so the matcher catches these re-flags next round. **post_verdict.sh:** Grok minor-revisions + ChatGPT reject recorded (H17H labels). **No bump; v0.1.118 stands; directive_g.sh not run.** No ACCEPT faked, no finding dismissed without a source-cited verdict, no math fabricated.

---

## W3-EXT ADJUDICATION ADDENDUM (2026-07-11, EXT re-test closing wave W3; reviewed v0.1.119, paper now v0.1.120)

Raws read + verified before any verdict: `W3/P5_grok_W3.md` (**MINOR REVISIONS**, 5 minors) + `.png`, `W3/P5_chatgpt_W3.md` (**REJECT**, 10 MAJOR + 3 MINOR) + `.png`. The v0.1.120 delta vs the reviewed v0.1.119 is a changelog-block comment only — a PROCESS-NIT (not reader-visible in the compiled PDF) per the 2026-07-11 spec rule. Pre-triage `tools/ledger_match.py`: Grok 5/7 matched (UNMATCHED #1, #6), ChatGPT 12/13 matched (UNMATCHED #11). All UNMATCHED Opus-adjudicated below (verdict-first, source-cited).

- **Grok UNMATCHED#1 (score 0.00) "REVISIONS ISSUES:"** → **PARSER NOISE** (verdict-header fragment, not a finding).
- **Grok UNMATCHED#6 (score 0.12) "overall presentation: extreme length, repeated caveats, defensive tone; condense §VI–VII; separate headline family-wise result from supporting cross-checks"** → **RE-FLAG of DP5-22** (standing editorial-length / readability / presentation D-round class). No load-bearing claim; matcher score below threshold only from prose dilution.
- **ChatGPT UNMATCHED#11 MINOR (score 0.12) "§III C angular cross-match does not quantify false associations / one-to-one ambiguities; report shifted-coordinate control, conflict statistics, and match-radius effect on the primary Δf_CW (not only the global fraction)"** → **RE-FLAG of DP5-22 (cross-match/match-radius robustness class).** Source-verified against the §Cross-match method block (tex l.1339-1367): nearest-neighbour separations, median 0.30″ ≪ 1.0″ acceptance, shared-astrometric-provenance explanation of the 0.0066″ seam, and acceptance-radius sensitivity at {0.5,1.0,2.0,3.0,5.0}″ labeled "mild" are all present; the match-radius systematic (0.02pp) is a tabulated row in `tab:systematic_budget` (l.3160, l.3211) folded into the primary Δf_CW envelope. The "effect on the primary contrast" + shifted-coordinate/conflict-statistics asks are further variants of the same disclosed cross-match robustness check — cosmetic-robustness D-round class, no new defect.

The 12 MATCHED ChatGPT items and the 4 other Grok minors map 1:1 to the standing dispositions (DP5-07 same-footprint-estimand/sample-membership, DP5-04 Bonferroni-5 family, DP5-11 0.9pp envelope, DP5-09 de-attenuation, DP5-12 RSD, DP5-14 T-Web, DP5-10 spatial-covariance OPEN-COMPUTE, DP5-21 Paper-IV venue, DP5-20 EFT/physical-interpretation, DP5-03 program-interaction, DP5-13/-07 exploratory-primary, DP5-17 editorial length) — same disclosed-content mapping as W2/H17H/H17F.

**Outcome:** **0 genuinely-new reader-visible editable findings on the EXT leg → wave W3 completes clean of reader-visible findings.** No bump — v0.1.120 stands (only W3 delta was the W3-INT PROCESS-NIT changelog block, comment-only, already directive-G-mirrored md5=f5f8a8f3be133be829d2022ab34df21c / 45 pages / 8 paths). **Streak:** the W3-INT row is left as-recorded (genuinelyNew=1 / streak 0 for DP5-23, a comment-only PROCESS-NIT closed BEFORE the 2026-07-11 no-reset-for-process-nits rule); the W3-EXT row is posted genuinelyNew=0 / cleanWaveStreak=1 — the first clean wave measured under the PROCESS-NIT rule — honestly noting the pre-rule W3-INT reset stays as-recorded. **Integrity:** Grok MINOR + ChatGPT REJECT recorded as-is (post_verdict.sh, labels "Grok (W3)"/"ChatGPT (W3)"); no ACCEPT faked, no finding dismissed without a source-cited verdict, no math fabricated; DP5-22 fingerprint extended (presentation-length + angular-cross-match/match-radius).

---

## W4-INT ADJUDICATION ADDENDUM (2026-07-11, INT confirm wave 2/2 vs v0.1.120) — P5 CROSSES THE BAR (streak 2)

Raws read + verified before any verdict: `INT_api/H17_2026-07-10/intwave_P5_claude_0022.md` (Claude INT = **MINOR**, pre-matched 100%), `INT_v3/ROUND_2026-07-09/API_P5_openai.md` (OpenAI gpt-5.5 = **MAJOR REVISIONS**, native-PDF v0.1.120, UTC 2026-07-11T07:22:06Z), `.../API_P5_grok.md` (Grok grok-4.3 = **MINOR**, native-PDF v0.1.120, same UTC). Pre-triage `tools/ledger_match.py`: OpenAI 8 real UNMATCHED (+2 parser-noise), Grok 2 UNMATCHED (both parser noise). All Opus-adjudicated verdict-first + source-cited in `DISPOSITIONS/P5.md` (W4-INT block).

- **HEADLINE — OpenAI moved REJECT → MAJOR REVISIONS on P5, its FIRST non-REJECT verdict on this paper.** Verified against the cited raw l.6/l.12 (`VERDICT: MAJOR REVISIONS`), native-PDF, reviewed v0.1.120, UTC 2026-07-11T07:22:06.295144Z. Honest tier-lift on unchanged reader-visible content (its l.52 concedes the central narrow claim "is broadly supported by the tabulated counts"), NOT continuation signal.
- **All 8 OpenAI real UNMATCHED = source-cited RE-FLAGs:** #5 footprint≠selection-mask → DP5-06 (§VIII B l.3033); #10 2a−1/under-powered void arm → DP5-08+DP5-09 (void stratum measured v0.1.118, ±3.7pp corroborate-not-exclude); #12 duplicate-TARGETID/T-Web unique-object → DP5-14+DP5-06 (primary is one-row-per-TARGETID join l.1346-1348, T-Web secondary + unique-galaxy recomputes disclosed); #13 fragmented multiplicity hierarchy → DP5-04+DP5-19 (`tab:bonferroni5_family` l.3454 + analysis tree l.1848); #15 abstract length + #16 Fig 6/8 legibility + #18 N_MC precision → DP5-22 (D-round); #19 DOI-at-acceptance → DP5-18+DP5-21 (DAS added v0.1.114, Zenodo-DOI OPEN-VENUE Houston-gated). **0 genuinely-new editable.**
- **Grok INT MINOR** — 3 minors all pre-matched (DP5-13 post-hoc primary, DP5-09 de-attenuation, DP5-12 RSD); closing sentence l.19 concedes the bounded null is supported. 2 UNMATCHED (0.00) = parser noise. **0 genuinely-new.**
- **Claude INT MINOR** — pre-matched 100%; 2 D-ids spot-checked: primary Δf_CW=+0.0018/z=+0.78 from `29_ext3…json` (DP5-01/-02 ✓), Bonferroni-5 GALZONE Δ=−0.0037/|z|=1.25/p=0.21 from `30_ext4…json` (DP5-04 ✓). Residual minors → DP5-22/-21/-09 + a stale-empty-`.bbl` PROCESS-NIT (bibliography inlined, citations resolve — close, no reset). Raw l.20 verbatim: "Wave 2/2 on v0.1.120 confirmed: comment-only changelog, 0 genuinely-new reader-visible findings — a clean wave."
- **NET: 0 genuinely-new reader-visible editable findings across all three INT legs** (13 source-cited re-flags + 4 parser-noise fragments). No bump — v0.1.120 stands (comment-only; served md5=f5f8a8f3be133be829d2022ab34df21c / 45 pages / 8 paths already mirrored+Convex-verified). directive_g.sh not re-run.
- **STREAK → 2. P5 CROSSES the two-clean-waves convergence bar (directive-K). MILESTONE: ALL FIVE PAPERS (P1U 2, P2 3, P3 4, P4 4, P5 2) are now past the bar — the edit-loop program exits; wave-1 arXiv kit + human-referee handoff are Houston-gated.**
- **Integrity:** OpenAI MAJOR / Grok MINOR / Claude MINOR recorded as-is (recordWave); no ACCEPT faked, no finding dismissed without a source-cited verdict, no math fabricated. The OpenAI REJECT→MAJOR move is recorded honestly (its first on P5) and does not itself drive the streak — the 0-genuinely-new count does.

---

## W4-EXT ADJUDICATION ADDENDUM (2026-07-11T07:42Z, EXT confirm half of exit wave vs v0.1.120) — EXIT CONFIRMED (streak 2 HELD)

Raws read + verified before any verdict: `W4/P5_grok_W4.md` (**MINOR REVISIONS**, 1 MAJOR-tagged post-hoc item + 4 MINOR) + `.png`, `W4/P5_chatgpt_W4.md` (**MAJOR REVISIONS**, 10 MAJOR + 2 MINOR — ChatGPT's move UP from REJECT) + `.png`. Pre-triage `tools/ledger_match.py`: Grok 5/6 matched (UNMATCHED #1), ChatGPT 10/13 matched (UNMATCHED #1, #3, #12). All UNMATCHED Opus-adjudicated below, verdict-first + source-cited. The v0.1.120 paper is unchanged since W4-INT (comment-only changelog delta already established); this EXT half is a pure confirm of the exit crossing.

- **Grok UNMATCHED#1 (score 0.00) "REVISIONS ISSUES:"** → **PARSER NOISE** (verdict-header fragment, not a finding). Grok's 5 real findings all MATCHED: the [MAJOR]-tagged post-hoc/pre-registration item → DP5-13; headline-attenuated-proxy-not-in-abstract → DP5-09; redshift-space-not-caveated-in-title → DP5-12; Paper IV placeholder → DP5-21; DESIVAST correlated-family effective-K → DP5-04. **0 genuinely-new.**

- **ChatGPT UNMATCHED#1 (score 0.00) "REVISIONS (2) ISSUES:"** → **PARSER NOISE** (verdict-header fragment).
- **ChatGPT UNMATCHED#3 MAJOR (score 0.29, matcher→DP5-05) "footprint-restricted primary control is an author-constructed union of projected hole-sphere discs, not the DESIVAST angular mask / completeness / veto / randoms; sharing gross angular–radial support does not balance fibre completeness, imaging depth, redshift, magnitude, inclination, morphology, classifier confidence; requires official mask/randoms + matched or covariate-adjusted control"** → **RE-FLAG of DP5-06 + DP5-19** (matcher mis-pointed at DP5-05, the *GALZONE catalog-invalid-pooling* code item, on weak overlap). Source-verified: §VIII B "Footprint ≠ selection function" paragraph (tex l.3072–3090) states verbatim the footprint is a *purely geometric* union of the 101,863 hole-sphere angular discs (NSIDE=64) ∩ radial span and is *not* the published DESIVAST/BGS completeness mask, vetoes, or a DESI random-catalog estimate; that matching this support "does *not* guarantee matched fibre-assignment completeness, imaging depth, or radial selection between the two classes"; and that "a fully selection-function-matched control would require the DESIVAST/BGS mask and DESI randoms … which we do not construct here," with the residual mismatch folded into `tab:systematic_budget` (l.3090). The covariate/IPW-adjusted-control ask is the disclosed DR2-deferred robustness item at §VIII (l.3308, "The full logistic/IPW-weighted control selection functions…"). Disclosed limitation + disclosed future robustness, not a new defect. **0 genuinely-new editable.**
- **ChatGPT UNMATCHED#12 MINOR (score 0.22, matcher→DP5-10) "non-rejection should not be called evidence for 'environment independence' except within a formally defined equivalence interval; Jeffreys/frequentist/permutation/heuristic-2σ mixed; Table XVI residuals use a monopole estimated from the same observations without covariance → not standard-normal z-scores"** → **RE-FLAG of DP5-19 + DP5-13.** Source-verified: (a) the equivalence-framing concern is the paper's OWN stated stance — "a null is not positive evidence; we report it as a controlled-sample non-detection of the environment-dependence hypothesis on this dataset" (tex l.3228–3229; echoed l.1212) — the paper explicitly does NOT claim proven independence; (b) the Table (`tab:p4_monopole_residual`) z-score convention is disclosed verbatim at l.3664–3672: the residuals subtract the *P5 matched-sample* monopole f_CW^P5=0.4972 estimated from the same observations, are "computed from the exact integer CW/total counts," and are labeled diagnostic/descriptive supporting statistics — the headline inference is carried by the two-proportion contrast + contingency test, not these residual z's (analysis tree l.1848–1849). Disclosed convention + honestly non-load-bearing diagnostic, not a new defect. **0 genuinely-new editable.**

The 10 MATCHED ChatGPT MAJORs + 1 MATCHED MINOR and the 5 Grok findings map 1:1 to the standing dispositions (DP5-06 footprint≠selection-mask + edge-void membership; DP5-10 spatial-covariance/design-effect OPEN-COMPUTE; DP5-04 Bonferroni-family estimand plurality; DP5-11 0.9pp quadrature envelope; DP5-08/-09 2a−1 de-attenuation + void-stratified matrix [void stratum measured v0.1.118, ±3.7pp corroborate-not-exclude]; DP5-12 RSD membership-only MC; DP5-14 T-Web selection-dominated; DP5-20 EFT parity-even-operator/non-derived; DP5-21 Paper-IV placeholder-arXiv venue; DP5-13 post-hoc primary; DP5-17 abstract-length presentation) — same disclosed-content mapping as W2/W3/H17H/H17F and this round's W4-INT half.

**Outcome:** **0 genuinely-new reader-visible editable findings on the EXT leg → the W4-EXT half CONFIRMS the exit crossing.** P5's clean-wave **streak HOLDS at 2** (no reset — no PROCESS-NIT and no genuinely-new item this half). No bump — v0.1.120 stands (served md5=f5f8a8f3be133be829d2022ab34df21c / 45 pages / 8 paths, already mirrored+Convex-verified at W4-INT). `directive_g.sh` NOT run. P5 remains PAST the two-clean-waves bar (directive-K); all five papers stay past the bar; the edit-loop program exit HOLDS.

- **MILESTONE (posted):** ChatGPT's EXT verdict moved **REJECT → MAJOR REVISIONS** on P5 on the exit wave — its first non-REJECT EXT verdict on this paper — verified against raw `W4/P5_chatgpt_W4.md` l.1 (`VERDICT: MAJOR REVISIONS`). An honest EXT verdict-gap tier-lift on unchanged reader-visible content; it does NOT drive the streak (the 0-genuinely-new count does).
- **Integrity:** Grok MINOR + ChatGPT MAJOR recorded as-is (post_verdict.sh, labels "Grok (W4)"/"ChatGPT (W4)"; recordWave paper-5/P5 W4-EXT); no ACCEPT faked, no finding dismissed without a source-cited verdict, no math fabricated; DP5-06 + DP5-19 fingerprints extended so these re-flags pre-match next round.
