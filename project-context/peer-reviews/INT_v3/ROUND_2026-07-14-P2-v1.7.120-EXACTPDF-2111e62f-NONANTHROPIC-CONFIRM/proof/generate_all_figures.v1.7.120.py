...
def fig5_inflation_comparison():
    ...
    # (Appendix A), half the erroneous -35/8. Display the imported Heinrich
    # local-template uncertainty in bounce-amplitude coordinates using Eq. (7):
    # sigma_bounce = sigma_local/r. This is distinct from the separate
    # shape-matched surrogate-covariance result.
    FNL = -35/16  # -2.1875
    R_RECAST = 0.84
    SIGMA_LOCAL = 0.7
    SIGMA_BOUNCE = SIGMA_LOCAL / R_RECAST
    ...
    # SPHEREx error bar in bounce-amplitude coordinates (Eq. 7).
    ax.errorbar(FNL, 4, xerr=SIGMA_BOUNCE, fmt='none', ecolor=CB_BLUE,
                capsize=6, lw=2,
                label=(r'SPHEREx recast: '
                       r'$\sigma_{\rm bounce}=0.7/0.84=0.83$'))
