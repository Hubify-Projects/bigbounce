...
\newcommand{\paperVersion}{v1A.0.116}
...
%   to the declared single-species, standard mean-field setup.
...
single-species, hard-cutoff, standard mean-field Nambu--Jona-Lasinio (NJL)
treatment, the scalar Fierz projection is repulsive,
$G_{\rm scalar}=-3\kappa/64$, and the magnitude remains subcritical over the
declared cutoff/flavor scan (largest scalar-channel ratio $0.156$; the larger
axial-channel ratio remains $0.31$).  These statements constrain this
specific contact channel; they do not exclude beyond-mean-field dynamics,
...
from the frozen Cobaya chains (\texttt{reproducibility/cosmology/chains}), the
NaMaster $EB$ validation (\texttt{reproducibility/p1\_namaster\_500mc}), and the
galaxy-chirality and multi-survey anomaly catalogs
(\texttt{pipelines/p2\_chirality}, \texttt{pipelines/p3\_anomaly\_engine}).
...
+ S_{\rm D}[e,\omega,\psi],
\end{align}
where $\kappa=8\pi G$, $R^{IJ}(\omega)=d\omega^{IJ}+\omega^I{}_K\wedge
\omega^{KJ}$, and $e^I$ and $\omega^{IJ}$ are varied independently.  There
...
Thus the finite-density expectation is roughly $70$ orders below
$\rho_\Lambda$ even under the deliberately high density choice, while the
...
single-species convention the scalar projection is
$G_{\rm scalar}=-3\kappa/64<0$, hence repulsive.  Ignoring the sign and testing
only its magnitude gives $|G_{\rm scalar}|/G_{\rm crit}=0.156$ at the most
aggressive point of the stated scan; the axial projection is twice as large
and remains subcritical at $0.31$.  The machine-checkable calculation is
\artifact{arxiv/scripts/njl_gap_equation_route1.py}.
...
condense in the declared standard mean-field NJL check.  Separately, canonical
...
These are the complete claims of this paper.  They do not constitute an
operator-complete no-go or an ECH explanation of dark energy.  The unresolved
...
\artifact{arxiv/scripts/fierz_lemma_check.py} and
\artifact{arxiv/scripts/njl_gap_equation_route1.py}.  The repository and
...
common $\kappa=M_{\rm Pl}^{-2}$ prefactor.  The matrix, involution, and channel
...
G_{\rm crit}=\frac{\pi^2}{N_fN_c\Lambda^2}
\end{equation}
...
=\frac{3N_fN_c}{64\pi^2}\,\frac{\Lambda^2}{M_{\rm Pl}^2}.
\end{equation}
The committed scan includes $\Lambda=M_{\rm Pl}$ and a formal above-Planck
cutoff-sensitivity point $\Lambda=M_{\rm Pl}/\sqrt{\gamma}$, together with the
declared flavor/color multiplicities.  The above-Planck point is a stress test,
not a claim that the contact EFT remains controlled there.  Its largest
scalar-channel value is $0.156$.
The attractive axial projection has twice this magnitude and reaches $0.31$,
still below unity; the pseudoscalar projection equals the scalar magnitude.

The result is reproducible with
\artifact{arxiv/scripts/njl_gap_equation_route1.py}.  Its interpretation is
