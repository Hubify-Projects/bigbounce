...
\newcommand{\paperVersion}{v1A.0.117}
...
order $M_{\rm Pl}^{-2}$.  At late-time fermion densities its contribution is
negligible compared with the observed dark-energy density.  Within a
...
\subsection{Late-time amplitude and coherent mean field}
\label{sec:r1_njl}
...
fermion densities $n_\psi$ obeys the coefficient-independent conservative
bound $|\rho_{4f}|\lesssim\kappa n_\psi^2=8\pi n_\psi^2/M_{\rm Pl}^2$.
...
The finite-density amplitude is negligible at cosmologically relevant
baryon/electron density.
A naive order-of-magnitude estimate using dense ISM-like number
densities $n_\psi\sim\mathcal{O}(10^2)\,\text{cm}^{-3}$ --- adopted here
as an elevated homogeneous-density benchmark, not the
...
No equation of state is inferred from $\langle J^5\rangle\approx0$:
a vanishing one-point current does not determine the state-dependent stress
tensor of the composite $\langle J^5J^5\rangle$.  The late-acceleration
conclusion here rests only on the explicit finite-density amplitude bound.
...
The vanishing one-point axial current does not itself exclude a vacuum
condensate.  We therefore Fierz-project the axial--axial operator and solve a
...
interaction after algebraic torsion is integrated out.  That interaction is
irrelevant at late-time fermion densities, and its scalar projection does not
condense in the declared standard mean-field NJL check because its coupling is
repulsive.  This conclusion does not use a blanket magnitude-subcriticality
...
These are the complete claims of this paper.  In the specified minimal
semiclassical state and EFT, the derived contact interaction and classical
Holst sector supply neither late acceleration nor a bounce observable.  This
does not constitute an operator-complete no-go or an ECH explanation of dark
energy.  The unresolved
...
not a derived axial-vector condensation threshold.}
\begin{ruledtabular}
\begin{tabular}{cccc}
