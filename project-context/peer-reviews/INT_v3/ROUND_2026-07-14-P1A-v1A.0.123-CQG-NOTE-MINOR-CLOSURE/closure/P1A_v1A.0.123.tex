...
\newcommand{\paperVersion}{v1A.0.123}
\newcommand{\paperTimestamp}{July 14, 2026, 1:59 PDT}
...
one convention-audited derivation.  Section~\ref{sec:theory} states the
...
For completeness, let $E_I$ be the frame dual to $e^I$ and define the one-form
$t\equiv\iota_{E_I}T^I$ (summed over $I$).  Contracting the last equation with
$\iota_{E_I}$ in four dimensions gives
$T^J+e^J\wedge t=0$.  A second contraction yields
$t=\iota_{E_J}T^J=-3t$, hence $t=0$ and therefore $T^I=0$.  Thus the kernel is
trivial for an invertible tetrad.  The Einstein--Cartan limit
...
\subsection{Standard mean-field NJL check}
...
homogeneous scalar gap equation shows that this negative coupling has no
nonzero solution.  The sign conclusion is logically separate from the
...
This result is deliberately conditional.  It closes the scalar condensate in
the standard mean-field NJL model with the stated regulator, field content,
and projection convention.  It does not remove the known Fierz ambiguity of
