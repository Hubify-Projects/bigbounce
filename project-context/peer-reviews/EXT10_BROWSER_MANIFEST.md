# EXT10 Browser Submission Manifest

**Round:** EXT10  
**Goal:** Verify path to 18/18 ACCEPT after EXT9-closure-wave (6 same-day closures)  
**Submitted:** 2026-06-13 ~13:45–14:25 PDT  
**Harvest ETA:** ≥30 min from last submission = 14:55 PDT or later  
...
- Live site versions were behind (e.g., P1A showing v1A.0.70 not v1A.0.71)
- Per skill protocol: used local `site/public/` mirror PDFs with matching md5s

## PDF Verification (Phase 1)

| Paper | File | md5 (prefix 8) | Expected | Match |
|-------|------|----------------|----------|-------|
| P1A | paper1a_ech_nogo_v1A.0.71.pdf | ea008466 | ea008466 | ✅ |
| P1B | paper1b_mcmc_companion_v1B.0.68.pdf | 06410d7c | 06410d7c | ✅ |
| P2 | paper2_fnl_forecast_v1.7.62.pdf | 406af9f8 | 406af9f8 | ✅ |
| P3 | paper3_anomaly_catalog_v3.1.105.pdf | 8533c5a7 | 8533c5a7 | ✅ |
| P4 | chirality_catalog_paper_v1.0.185.pdf | 414048af | 414048af | ✅ |
| P5 | p5_desi_chirality_v0.1.74.pdf | 43619245 | 43619245 | ✅ |

Staged at: `/private/tmp/bigbounce-ext-review/EXT10/` and `~/Downloads/bigbounce-ext-review/EXT10/`
...
| Provider | Submitted | Model/Effort | All 6 confirmed |
|----------|-----------|--------------|-----------------|
| ChatGPT | 6/6 | Pro Extended | ✅ |
| Grok | 6/6 | Heavy | ✅ |
| Gemini | 6/6 | 3.5 Thinking (/u/0/) | ✅ (growth-confirmed) |
| **Total** | **18/18** | | ✅ |

**No chat refusals. No upload size rejections (P4 34MB accepted by all 3 providers).**

---

## Skill Update Notes (for SKILL.md)

- Gemini model picker at EXT10: options are "3.5 Flash", **"3.5 Thinking"** (Solves complex problems), "2.1 Pro". NOT "2.5 Thinking" as the screenshot may suggest — text extraction gives the correct "3.5 Thinking" label.
- Gemini account: `/u/0/` confirmed correct at EXT10 (consistent with EXT4+ behavior).
- Gemini native dialog: osascript Cmd+Shift+G → keystroke path → key code 36 → key code 36 works reliably when executed as separate `-e` calls (not inline heredoc). The heredoc approach had a syntax error.
- After `$B goto` the ref cache is invalidated — always run `$B snapshot -i` before any `$B click @eN` calls.

---

## Harvest Instructions

Wait until **14:55 PDT or later** (≥30 min from last submission at 14:25).

For each row: open the chat URL, confirm no streaming indicator, copy the complete referee report, save to:
`project-context/peer-reviews/EXT10_<paper>_<Provider>.md`

Header format:
```
# EXT10 Harvest — <Paper> — <Provider>
- Provider: <ChatGPT/Grok/Gemini>
- Model/Effort: <Pro Extended / Heavy / 3.5 Thinking>
- Chat URL: <url>
- PDF md5: <hash>
