...
|-----|-----|-----------|--------|-----------|
| **Pod 1** (frail_tomato_koi) | `root@<pod-ip> -p <port>` | P1-M3 NaMaster 500MC + P3-M1 UMAP | ✅ **DONE 2026-04-29 05:31 PDT** — clean 2.7h compute. Results: `pipelines/h200_results/pod1_namaster_umap_2026-04-29/` (commit 5d54fbc). | **IDLE ~5h** at $3.59/hr ≈ $18 |
| **Pod 2** (regular_green_pig) | `root@<pod-ip> -p <port>` | P4 chirality suite + WT-5 PDF compile | ❌ **CRASHED 2026-04-29 02:53 PDT** — HF 401 on `bamfai/galaxy-chirality-v2` + missing `/workspace/external_catalogs/pre_desi.fits`. Salvaged prior `/root/results` snapshot (16 JSON files, commit 6a2bd2f). | **IDLE ~7.5h** at $3.59/hr ≈ $27 |
...
- **Pod 1** (o76k3jfzbfh25e): Connection refused (SSH port <port> @ <pod-ip>). Pod may have expired/stopped. NaMaster 500MC birefringence status UNKNOWN — need to check RunPod dashboard.
...
**Pod 1: frail_tomato_koi** (H200, SSH: root@<pod-ip> -p <port>)
...
**Pod 2: regular_green_pig** (H200, SSH: root@<pod-ip> -p <port>)
