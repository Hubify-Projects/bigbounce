...
Q1 per-Rs dilation Phase-2 rerun **CLOSED-LOCAL 2026-06-10** (`scripts/26_r27conf_ess_recomputes.py` → `outputs/26_r27conf_ess_recomputes.json`: production `01_compute_vweb.py` already scales dilation per sweep cell — ⌈Rs/cell⌉+1 = 3 at Rs=50 verified against the published mask (3,416,333 vs 3,416,329 cells, vol fracs <1e-6); reviewer's fixed-2 premise quantified as the counterfactual: ≤0.82 pp volume shift, 99.75% spiral-assignment agreement, ≤0.27 pp per-class f_CW — closing sentences added to tex §Phase-2 sweep + methods bullet) · Q2 FoG-compressed DESIVAST membership **CLOSED-LOCAL 2026-06-10** (same artifact: σ=5 Mpc/h LOS Gaussian MC ×200 (scale disclosed; audit row specified none) reassigns ~4.4e4 memberships/realization, n_void 57,081→76,490±161, yet Δf_CW stays within [−0.34,+0.37] pp, coherent ±5 Mpc/h shifts −0.16/+0.03 pp, all two-sample |z|≤1.9 — null unchanged; closing sentences added to tex RSD paragraph) · Q3 TARGETID-level permutation + inflated-null residual benchmark · Q4 full-dipole logistic refit · Q5 normalized-λth sweep · Q6 trilinear label assignment · Q7 mock validation (or limitations paragraph) · Q8 Table IX NSIDE 32/64.
