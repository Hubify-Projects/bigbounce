...
1. **Public site** — `seti.hubify.app` (or final). Houston-quality design, mobile-first, fast. Auto-generated from `hubify-lab-default` template per PRD §53, then heavily vibe-coded for press-ready polish. The `site-worker` agent + `outreach-lead` collaborate on site updates.
