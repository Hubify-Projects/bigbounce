...
- [ ] **AUG-014 — Anomaly flagship follow-on validation/taxonomy/manuscript:** turn the completed clean rerun into the defensible selected sample, validation contract, taxonomy, and manuscript draft. Execution gate: B2 structurally retains the full corpus, but no BigBounce RunPod workspace is running (two stopped pods, one network volume). On explicit downstream-compute authorization, mount the retained corpus, run `build_flagship_sample.py --describe`, select from the observed distribution without historical-count tuning, then cross-match and taxonomy.
- [x] **AUG-015 — 2026-08-26 backup/compute receipt re-verification:** completed as a scope-bounded structural-retention audit. Local summary/comparison and sealed-manifest byte hashes verify; semantic contract bindings are internally consistent. Authenticated B2 lists exactly 36,634 receipt objects and 36,634 shard objects; authenticated Hugging Face is a partial private mirror (10,000 receipts plus sealed inputs/checkpoints); RunPod authentication works and the historical pod is absent. This does not claim a fresh multi-TB full-byte download.
- [ ] **AUG-016 — Hubify canonical-lab alignment:** source/registry parity is verified: Hubify's public-truth, offline parity, and live BigBounce Convex parity checks all pass for the six canonical IDs, versions, and PDFs. The pin deliberately remains the last unified science snapshot (`d957c0cd`); later commits are operations/provenance changes, not a new six-paper snapshot. Remaining gap: authenticated Hubify product API verification requires `HUBIFY_TOKEN`; no rows or mutations were guessed.
