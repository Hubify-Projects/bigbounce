...
- [x] **CMUX-001 — Local mirrored launcher and hardened evaluation contract:** commits `e2e2496` and `ec6f48e` in the separate `learning-cmux-with-agents` lab add subscription-authenticated mirrored teams, immutable task/repository snapshot binding, Codex project-trust bypass, private capability-token files, atomic one-way seal/reveal, a transactional four-workspace topology gate, caller-bound provider-auth/short-liveness receipts plus bounded settle, target-repository write denial for every run-owned surface, atomic abort invalidation before rollback, process-group shutdown, deadline/timeout adjudication, fail-closed mutation, and **28 passing tests**. Local commits remain intentionally unpushed because `origin` is IndyDevDan's upstream repository rather than Houston's fork. (sources: 2026-07-13 CMUX prompts and acceptance runs)
