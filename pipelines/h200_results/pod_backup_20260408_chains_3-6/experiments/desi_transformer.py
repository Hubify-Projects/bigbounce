...
# Output directory
# ---------------------------------------------------------------------------
OUTPUT_DIR = "/workspace/results/desi-transformer"
...
def main():
    ...
    transformer.load_state_dict(torch.load(os.path.join(OUTPUT_DIR, "transformer_best.pt"), weights_only=True))
    conv_ae.load_state_dict(torch.load(os.path.join(OUTPUT_DIR, "conv_ae_best.pt"), weights_only=True))
    ...
    top_anomalies_df.to_csv(os.path.join(OUTPUT_DIR, "all_reconstruction_errors.csv"), index=False)
    ...
    t_only_df.to_csv(os.path.join(OUTPUT_DIR, "transformer_only_anomalies.csv"), index=False)

    c_only_df = top_anomalies_df[top_anomalies_df["conv_only"] == 1].copy()
    c_only_df = c_only_df.sort_values("err_conv_ae", ascending=False)
    c_only_df.to_csv(os.path.join(OUTPUT_DIR, "conv_only_anomalies.csv"), index=False)
