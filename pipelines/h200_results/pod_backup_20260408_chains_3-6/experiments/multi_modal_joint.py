...
# Output directory
# ---------------------------------------------------------------------------
OUTPUT_DIR = "/workspace/results/multi-modal-joint"
...
def main():
    ...
    joint_model.load_state_dict(torch.load(os.path.join(OUTPUT_DIR, "joint_best.pt"), weights_only=True))
    spec_model.load_state_dict(torch.load(os.path.join(OUTPUT_DIR, "spectral_best.pt"), weights_only=True))
    photo_model.load_state_dict(torch.load(os.path.join(OUTPUT_DIR, "photometry_best.pt"), weights_only=True))
    ...
    hidden_true.to_csv(os.path.join(OUTPUT_DIR, "hidden_true_anomalies.csv"), index=False)
