...
K_MIN = 1e-3    # h/Mpc — corrected from 1e-4 (caused sigma->0 underflow)
K_MAX = 0.2     # h/Mpc
N_K_BINS = 60
...
def alpha_fnl(k, z):
    ...
    alpha(k, z) = (2/3) * (k*c/H0)^2 * T(k) * D(z) / Omega_m
    k in h/Mpc, c/H0 = 2998 Mpc/h — required dimensional factor.
    """
    T_k = transfer_function(k)
    D_z = growth_factor(z)
