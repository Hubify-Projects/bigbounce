...
# Output directory
# ---------------------------------------------------------------------------
OUTPUT_DIR = "/root/results/sdss-native-autoencoder"
...
def train_model(model, train_loader, val_loader, epochs, model_name):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val:
            ...
            except Exception as _e: print(f"[warn] save: {_e}", flush=True)
...
def main():
    ...
    # [patched] skip load — PyTorch 2.4.1 checkpoint bug
    # [patched] skip load — PyTorch 2.4.1 checkpoint bug
    ...
    try: results_df.to_csv(os.path.join(OUTPUT_DIR, "all_reconstruction_errors.csv"), index=False)
    except Exception as _e: print(f"[warn] csv: {_e}", flush=True)

    sdss_only_df = results_df[results_df["sdss_only"] == 1].sort_values("err_sdss_native", ascending=False)
    try: sdss_only_df.to_csv(os.path.join(OUTPUT_DIR, "sdss_only_anomalies.csv"), index=False)
    except Exception as _e: print(f"[warn] csv: {_e}", flush=True)

    desi_only_df = results_df[results_df["desi_only"] == 1].sort_values("err_desi_transfer", ascending=False)
    try: desi_only_df.to_csv(os.path.join(OUTPUT_DIR, "desi_only_anomalies.csv"), index=False)
    except Exception as _e: print(f"[warn] csv: {_e}", flush=True)
    ...
    try:
        ...
        print(f"[warn] json save: {_e}", flush=True)
