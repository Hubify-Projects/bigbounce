...
\end{equation}
where $\hat{\mathbf{x}} = \BigAE(\mathbf{x})$ and $N$ is the input dimensionality. For spectroscopic surveys, we additionally decompose the score into per-band contributions $r_B$, $r_R$, $r_Z$ computed over the blue (3600--6200~\AA), red (6200--8200~\AA), and near-infrared (8200--9800~\AA) subsets.
...
Table~\ref{tab:survey_summary} presents the summary statistics for all eight surveys. We describe each survey in turn.
...
The DESI-only multi-tracer result is $\sigfnl = 8.43$, a 6.1\% improvement over the standard DESI QSO constraint of $\sigfnl = 8.98$. The improvement is concentrated in the high-redshift bins ($z > 2$) where the AI-selected sample contributes the most additional tracers with the highest bias enhancement.
...
%%  FIGURE PLACEHOLDERS
%% ============================================================
% NOTE: The following figures should be generated and included before submission.
%
% Figure 1: Mollweide sky map of all anomalies across the 8 surveys, color-coded by survey.
%            \includegraphics[width=\textwidth]{fig_skymap_all_surveys.pdf}
%
% Figure 2: BigAE architecture diagram showing encoder-decoder structure with per-survey
%            input/latent dimensions.
%            \includegraphics[width=\columnwidth]{fig_architecture.pdf}
%
% Figure 3: Anomaly score distributions for DESI, SDSS, and LAMOST on log-log scale,
%            with thresholds marked.
%            \includegraphics[width=\columnwidth]{fig_score_distributions.pdf}
%
% Figure 4: Example DESI spectra: (a) normal spectrum, (b) Z-dominant high-z candidate,
%            (c) B-dominant anomaly, (d) multi-band anomaly.
%            \includegraphics[width=\textwidth]{fig_example_spectra.pdf}
%
% Figure 5: UMAP embedding of SDSS top 50K anomalies, colored by HDBSCAN cluster.
%            \includegraphics[width=\columnwidth]{fig_sdss_umap.pdf}
%
% Figure 6: SIMBAD novelty fractions by survey, bar chart.
%            \includegraphics[width=\columnwidth]{fig_novelty_fractions.pdf}
%
% Figure 7: LAMOST blue-excess anomaly concentration: histogram of peak anomaly wavelength.
%            \includegraphics[width=\columnwidth]{fig_lamost_blue_excess.pdf}
%
% Figure 8: Multi-tracer f_NL improvement: sigma(f_NL) per redshift bin, comparing standard
%            DESI QSO vs. multi-tracer with AI-selected anomalies.
%            \includegraphics[width=\columnwidth]{fig_fnl_improvement.pdf}
%
% Figure 9: NANOGrav spectral fit: data points with bounce (gamma=3.0) and SMBHB (gamma=13/3)
%            model curves. Inset: MCMC posterior on gamma.
%            \includegraphics[width=\columnwidth]{fig_nanograv_fit.pdf}
%
% Figure 10: Spatial density map of combined anomalies (HEALPix N_side=64), showing
%             non-uniform distribution uncorrelated with Galactic foregrounds.
%             \includegraphics[width=\textwidth]{fig_density_map.pdf}
%
% Figure 11: eROSITA top anomalies near the LMC, overlaid on X-ray count map.
%             \includegraphics[width=\columnwidth]{fig_erosita_lmc.pdf}
%
% Figure 12: Planck vs ACT anomaly sky positions, demonstrating the null cross-correlation.
%             \includegraphics[width=\columnwidth]{fig_planck_act_null.pdf}
