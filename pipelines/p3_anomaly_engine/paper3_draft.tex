...
\begin{flushleft}
{\footnotesize
$^\dagger$ Path-C native convolutional autoencoder (Section~\ref{sec:planck}): 3 convolutional layers $\rightarrow$ \texttt{Linear}(4096, 128) bottleneck; symmetric \texttt{ConvT} decoder; $1.1\times10^6$ parameters. Training on $2\times10^5$ galactic-plane-masked ($|b|\geq20^\circ$) Planck SMICA patches on A100 GPU reached best validation (val\_loss $0.4437$) at epoch 99 of a 150-epoch schedule; the total training wall-clock for this run was not preserved in the run logs, so no figure is quoted (an earlier draft listed $10.6$~s, which is inconsistent with a ${\sim}100$-epoch convolutional run and has been withdrawn). The 200K-patch full re-score took $25.3$~s on A100, the source of the ${\sim}8{,}000$~patches/s throughput entry.\\
