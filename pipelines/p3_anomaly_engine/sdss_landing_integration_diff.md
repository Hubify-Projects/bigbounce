...
- **1,925,279 SDSS DR18 spectra scored** across 471 batch shards on `best_sdss_native.pt` (val_loss 0.0311 gate PASS fire #80)
- **12 sources with S > 5** vs cross-transfer 77,905 → reduction factor to be computed on post-hoc
- Top-77,905 slice published at S ≥ 0.1060
- Score distribution: median 0.0151, p99 0.2051, p99.9 0.5808, max 13.7705

## Paper body edits — `pipelines/p3_anomaly_engine/paper3_draft.tex`

**§3 lead-in (L164)** — replace prior `7-of-8` → `8-of-8` (all natives in), replace `310,788` → `388,693`, `301,222` → `378,480`:

    Old:  "6-of-8 surveys loaded"  (last substituted fire #132)
    New:  "8-of-8 surveys loaded: DESI + SDSS native + LAMOST native + Gaia + NEOWISE-masked + eROSITA + Planck + ACT DR6 → 388,693 detections → 378,480 unique physical objects, 637 multi-survey matches"
...
**§3.2 SDSS paragraph** — replace the in-flight clause (pattern matches fire #134 LAMOST edit):
    ...
    Replace with: "SDSS native retrain gate PASS (val_loss 0.0311); re-score of 1,925,279 DR18 spectra complete on `best_sdss_native.pt`; 12 sources at S > 5 on the native distribution vs 77,905 cross-transfer — confirms cross-transfer domain-shift was inflating SDSS anomaly rate. Top-77,905 slice at S ≥ 0.1060 supersedes the cross-transfer `sdss_dr18_anomalies.parquet` in Table~\ref{tab:survey_summary}."

**§Conclusions bullet #8** — same 310,788/301,222/7-of-8 → 388,693/378,480/8-of-8 substitution.

**§Data-availability manifest** — same substitution.

**Table 1 SDSS row** — update `N_anom` and add footnote ‡ referencing SDSS native.

**Abstract** — update headline unique-object number 301,222 → 378,480 and multi-survey-match count.
...
- L63 Path-C unique-objects row: 7/8 → 8/8, 310,788 → 388,693, 301,222 → 378,480, multi-survey matches 2 → 637.
- L84 Coverage paragraph: add SDSS native top-77,905 staged note; declare "All Path-C criteria green on HF staging surface".
