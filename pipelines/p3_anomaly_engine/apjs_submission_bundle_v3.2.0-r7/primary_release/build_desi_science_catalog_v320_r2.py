#!/usr/bin/env python3
"""Build the P3 v3.2.0-r2 DESI DR1 science-target candidate catalog.
...
def angular_separation_arcsec(
    ...
    """Great-circle separation using the same stable chord conversion as the join."""
    first = sky_unit_vectors(ra1_deg, dec1_deg)
    second = sky_unit_vectors(ra2_deg, dec2_deg)
    return chord_to_arcsec(np.linalg.norm(first - second, axis=1))
...
def write_dictionary(path: Path, final: pd.DataFrame) -> None:
    descriptions = {
        ...
        "original_member_separation_arcsec": "Great-circle separation between the recovered public DESI target and the canonical original DESI anomaly-member coordinate, in arcseconds; distinct from target-to-cluster match_separation_arcsec.",
...
def build_release(
    ...
    final.insert(
        ...
        angular_separation_arcsec(
            final["target_ra"].to_numpy(np.float64),
            final["target_dec"].to_numpy(np.float64),
            final["original_ra_deg"].to_numpy(np.float64),
            final["original_dec_deg"].to_numpy(np.float64),
        ),
    )
    ...
    qc = {
        ...
        "assertions": {
            ...
            "all_original_member_separations_finite": bool(
                np.isfinite(final["original_member_separation_arcsec"]).all()
            ),
        },
    ...
    build_copy = paths.output_dir / Path(__file__).name
    ...
    if len(multi_member) != 1:
        raise RuntimeError(f"expected exactly one multi-member released cluster, found {len(multi_member)}")
    multi = multi_member.iloc[0]

    readme = f"""# P3 DESI DR1 science-target anomaly candidates v{VERSION}-{REVISION}
...
Exactly one released row has `n_detections > 1`: `{multi['candidate_id']}` (cluster
`{int(multi['cluster_id'])}`, `n_detections={int(multi['n_detections'])}`). Its public target is
`{float(multi['original_member_separation_arcsec']):.6f}` arcsec from the canonical original
