% P3 v3.2.0 structural recovery: focused DESI DR1 candidate-catalog paper.
...
\newcommand{\releaseTag}{p3-v3.2.0-r1}
\newcommand{\releaseRevision}{983209ae606be311d9bda9f0258716d56386ee69}
...
\texttt{ZWARN}=0 leaves 181 unique public \texttt{TARGETID}s.  These 181 rows form the v3.2.0
...
\item a clean v3.2.0 bundle containing only this DESI product and the artifacts required to audit it.
\end{enumerate}
...
not enter the v3.2.0 selection and no non-DESI table is included in the new release.
...
The public identity substrate is \texttt{zall-pix-iron.fits} from DESI DR1.  The local file has
...
\caption{Released v3.2.0 candidates.  (a) DESI-footprint sky distribution colored by descriptive
...
the celestial equator.  All twelve 30-degree right-ascension bins are occupied, while five of
...
inference is part of v3.2.0.  Establishing novelty requires object-by-object literature and archive
...
quality gates differ.  The contribution of v3.2.0 is orthogonal: it converts a historically
...
All 181 rows pass all 18 field comparisons.  The validator separately confirms unique candidate,
...
an ETag of \texttt{6a516695-5356e87c0}, and a last-modified time of 2026-07-10 21:39:33 GMT.  Eight
independently chosen 1~MiB byte ranges, including the first and last ranges, also match between the
local file and live endpoint.  An older local provenance sidecar written in May 2026 records a
...
\begin{enumerate}
\item \nolinkurl{desi_dr1_science_anomaly_candidates_v3.2.0.parquet};
...
\end{enumerate}
The manifest excludes itself to avoid a self-referential hash.  An independent post-generation
...
future rescoring possible, but v3.2.0 does not claim exact reproduction of the historical neural
network output.
...
The v3.2.0 release bundle is published under the immutable correction tag \releaseTag{} in the
public Hugging Face dataset
\url{https://huggingface.co/datasets/bamfai/bigbounce-anomaly-catalog}, path
\texttt{releases/p3-v3.2.0/}.  The tag resolves to commit
\texttt{\releaseRevision}.  All 11 remote files were downloaded from that tag and verified
byte-for-byte against the local release.  The original \texttt{p3-v3.2.0} tag remains immutable
at commit \texttt{f2df8c00284350a7fe84a4ba6eb064\allowbreak68408b1c49}; \releaseTag{} supersedes it only
because the correction adds explicit immutable-input URLs and reproduction commands.
...
\end{verbatim}
The shell joins the two physical hash lines because the first ends in a backslash.  The release
README gives the same copy-paste download, build, and validation commands.  The source tree contains the same
...
This work used public data from the Dark Energy Spectroscopic Instrument.  The author thanks the
DESI collaboration and data-management teams for making DR1 redshift catalogs and checksums
publicly accessible.  The analysis benefited from automated code and manuscript review; all
catalog assertions reported here were checked against machine-readable artifacts, and no
...
Astron.\ J.\ (2025), arXiv:2503.14745.
...
Mon.\ Not.\ R.\ Astron.\ Soc.\ \textbf{547}, 2 (2026), arXiv:2506.17376.
...
The release contains 42 columns after addition of the separation-quality tier.  The authoritative
...
\begin{center}
\textbf{Table \thetable.} Independent v3.2.0 integrity checks
