...
\newcommand{\paperVersion}{v1.0.258}
...
\newcommand{\paperTimestamp}{July 16, 2026, 01:00 PDT}
...
\hline
$C_1$, 500 draws & $7.0113{\times}10^{-6}$ & $(5.2420{\pm}0.9257){\times}10^{-6}$ & $+6.983$ \\
$C_1$, 10,000 draws & $7.0113{\times}10^{-6}$ & $(5.1242{\pm}0.2618){\times}10^{-6}$ & $+7.207$ \\
\end{tabular}
\end{table*}
...
We publish an 8,474,531-object observed-label catalog release and report one primary result: the release-safe HC real-space dipole is null-consistent ($N_{\rm selected}=890{,}069$, $N_{\rm support}=887{,}472$, $z_{\rm mom}=+0.635$, one-sided add-one rank $p=0.23768$). The exact safe catalog, quarantine, retained null array, manifest, schema, and reproducer are public at the commit-pinned release identified in Data Availability. All harmonic, WLS, injection, and transfer calculations are diagnostics. Our main conclusions are:
