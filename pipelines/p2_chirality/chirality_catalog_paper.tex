...
\newcommand{\paperVersion}{v1.0.245}
...
\newcommand{\paperTimestamp}{July 15, 2026, 02:42 PDT}
...
In this paper we present an 8.47-million-object chirality catalog whose novelty relative to CE-ResNet is scale, a dedicated \NS{} class, a machine-readable release contract, and a multi-axis bias-hardening audit. This catalog-and-methods emphasis, including a compact schema, executable selection, checksummed null, and explicit quarantine, is the intended ApJS venue fit. Because 67.5\% of the 26,616 training-pool rows carry CE-ResNet-derived labels, the catalogs are not fully independent and model-output permutation nulls cannot test inherited survey-correlated structure by themselves. Our sole primary result is the HC real-space observed-label null ($N=949{,}584$, $z=+0.71$, one-sided upper-tail $p=0.225$). The WLS and MASTER results are systematics-sensitive diagnostics, not independent detections or physical exclusions. The GZ1 human-only test uses human votes rather than learned chirality labels and corroborates the null at lower statistical power; it does not tighten the HC amplitude constraint.
...
We assemble 26,616 labeled rows: 6,637 Galaxy Zoo~1 high-confidence CW/CCW galaxies, 17,153 CE-ResNet high-confidence spiral classifications, 826 CE-ResNet-selected non-spirals, and 2,000 synthetic \NS{} examples ($n_{\rm train}=21{,}293$, $n_{\rm val}=5{,}323$; Appendix~B). CE-ResNet-derived labels therefore supply 67.5\% of the pool, so validation partly measures agreement with that model. Horizontal flips are generated on the fly for a training-only consistency loss and do not add dataset rows. The full GZ1 overlap gives 69.91\% chirality agreement ($\kappa=0.40$); no retained object-ID manifest establishes exclusion of the 6,637 GZ1 training rows. The human-vote GZ1 dipole test gives $z=-0.54$ at $N=46{,}017$ and corroborates a null, but without calibrated power or a spatial transfer model it does not provide a tighter amplitude bound.
...
\hline
$N_{\rm HC}$ / valid pixels & $949{,}584$ / $23{,}682$ \\
...
the HC ($p_{\rm eq}\!>\!0.6$) real-space dipole collapses to $0.55\sigmaunit$. Generation counts and the projected streaming read are recorded in \artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/fig7\_raw\_vs\_eq\_manifest.json}. This visual
...
$z=+0.71$); the WLS template score in Appendix~D is diagnostic.\label{fig:multipoles}}
\end{figure}
...
(the $+0.71\sigmaunit$ real-space null and the finite observed-label pilot
...
Four gates remain open: a physical morphology transfer function, a matched-footprint independent-estimator comparison, a joint real-space/harmonic/nuisance covariance likelihood, and independent external preregistration or blinding of the primary analysis. The HC real-space estimator is the sole primary result; it is empirically less exposed to the measured harmonic leakage but does not mathematically bypass that channel. The WLS and harmonic analyses remain diagnostics.
...
We release an 8,474,531-object observed-label catalog and report one primary result: the HC real-space dipole is null-consistent ($N=949{,}584$, $z=+0.71$, one-sided upper-tail $p=0.225$ under the declared null convention). All harmonic, WLS, injection, and transfer calculations are diagnostics. Our main conclusions are:
...
\paragraph{Training.}
The model is trained with AdamW optimization (head learning rate $3\!\times\!10^{-4}$, encoder learning rate $2\!\times\!10^{-5}$, weight decay 0.02), batch size~128, and a cosine annealing warm-restart schedule ($T_0 = 10$, $T_{\rm mult} = 2$). Early stopping with patience 15 monitors best validation \emph{accuracy} within 80 epochs; the production model's best checkpoint was at epoch~79. The reported 93.7\% accuracy is the best-epoch \emph{three-class} (CW/CCW/\NS) validation accuracy on an un-augmented random 80/20 split ($n_{\rm val}=5{,}323$ of $26{,}616$); horizontal flips are evaluated on the fly in training batches for the equivariance consistency loss and do not add rows. The historical script did not seed the CE non-spiral sample, synthetic-image generation, PyTorch split, or epoch shuffles, and no exact object/split manifest was retained, so the historical training run is not exactly reproducible from the repository. The released result artifact records 94.9\% CW per-class validation accuracy on that run (CCW $91.3\%$, \NS{} $99.4\%$). For binary CW/CCW discrimination it records 93.2\% accuracy, CW recall 93.8\%, and CCW recall 92.6\%.

\paragraph{Training-data provenance.}
Table~\ref{tab:training_provenance} reconstructs the historical training-label counts from the production script and released result artifact. The 26,616-row pool comprises Galaxy Zoo~1 human labels, CE-ResNet spiral pseudo-labels, 826 CE-ResNet-selected non-spirals, and synthetic hard negatives. It is randomly split into 21,293 training and 5,323 validation rows before training; neither split gains rows from flips. The flip-equivariance term evaluates a horizontally flipped view of each training batch on the fly. Because the historical stochastic operations were not fully seeded and stable object/split IDs were not retained, the table documents the recovered run but is not an exact replay manifest.

\begin{table}[t]
\caption{\label{tab:training_provenance}Recovered historical training-data provenance and split semantics. ``Pseudo-label'' denotes CE-ResNet~\cite{Jia:2023} model predictions; ``human'' denotes Galaxy Zoo~1~\cite{Lintott:2008} $>\!70\%$-vote labels; ``synthetic'' denotes artificial \NS{} hard negatives. Flips are generated on the fly for the training consistency loss and do not add rows.}
\begin{ruledtabular}
\begin{tabular}{lr}
...
CE-ResNet-selected \NS{} labels & $826$ \\
Synthetic \NS{} hard negatives & $2{,}000$ \\
CE-ResNet-derived fraction of pool & $67.5\%$ \\
Train / validation split & $80.0/20.0$ \\
$n_{\rm train}$ / $n_{\rm val}$ & $21{,}293$ / $5{,}323$ \\
