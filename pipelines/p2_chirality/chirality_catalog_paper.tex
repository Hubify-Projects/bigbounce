...
\newcommand{\paperVersion}{v1.0.241}
...
\begin{abstract}
We measure the large-scale chirality dipole of spiral galaxies and find the pre-specified high-confidence real-space estimator consistent with null. The released catalog contains $8{,}474{,}531$ DESI Legacy DR8 galaxies classified by a flip-equivariant Vision Transformer into CW, CCW, and non-spiral classes, yielding $3{,}201{,}160$ spirals; $66.5\%$ of its training-label source manifest consists of CE-ResNet pseudo-labels, so classifier independence is limited and is checked separately with Galaxy Zoo human labels. The primary $p_{\rm eq}>0.6$ sample ($N=949{,}584$) gives $+0.41\sigmaunit$ (moment-$z$ against an isotropic pixel-permutation null; rank $p=0.31$), stable across confidence cuts and a per-galaxy label-shuffle null. A discrete paired-image-to-field surrogate over 20 deterministic axes gives grid-level HC thresholds $A_{50}=0.75\%$ and $A_{95}=1.5\%$, but it is conditional on classifier-defined original/mirror pairs and is not an unconditional physical calibration. The block-bootstrap WLS fit disfavors a clean \emph{observed-label} $A_p=1.7\%$ template at $z\approx-7.6$; after the measured $g=0.398$ dilution, a physical $1.7\%$ signal maps to $0.6766\%$ and is only $1.36$ bootstrap standard deviations above the fit, while the observed-label $A_{95}=1.0$--$1.5\%$ range maps to $2.51$--$3.77\%$ physically. The post-MASTER harmonic channel remains diagnostic: its full $0.695\%$ residual exceeds the like-for-like full-sample $A_{95}=0.63\%$, and a map-level cross-estimator injection changes the HC fitted amplitude, so the residual cannot be dismissed as sub-threshold; its physical attribution and joint real-space$\times$harmonic covariance remain open. The $\ell=1$ observable is parity-even (isotropy-breaking), not a direct parity-violation test.
\end{abstract}
...
\emph{Quantitative forward model and cross-estimator stress test.} The imaging$+$DR8-morphology forward model reproduces $53.0\%$ of the full-sample $\ell=1$ amplitude by the scalar amplitude-ratio definition, leaving $47.0\%$ unmodelled; because the observed and model vectors are not parallel ($\cos\theta=0.84$), direct vector subtraction instead leaves $62.3\%$ of the observed amplitude. These are different remainder definitions and are not interchangeable. The full diagnostic residual is $|a_1|=6.95\times10^{-3}$ ($0.695\%$ in $A_p$), which must be compared with the like-for-like unthresholded full-sample real-space recovery floors, $A_{50}=0.36\%$ and $A_{95}=0.63\%$, rather than the HC floors: $0.695\%$ is \emph{above} the full-sample $A_{95}$, not below it. We therefore retract the prior sub-threshold dismissal. A deterministic map-level cross-estimator stress test injects three pure-$\ell=1$ templates into the canonical HC real-space field: the full observed template changes the fitted amplitude from $0.4597\%$ to $0.6710\%$ ($+46.0\%$); the scalar-$47\%$ remainder template changes it to $0.5533\%$ ($+20.4\%$); and the direct vector-subtraction remainder changes it to $0.5694\%$ ($+23.9\%$). This shows that the residual is not automatically irrelevant to the real-space estimator. It does \emph{not} establish a detection, identify the remainder as cosmological, or provide the missing joint real-space$\times$harmonic covariance likelihood: the injection is deterministic, the estimators share the same catalog and footprint, and no covariance or nuisance posterior is inferred. The eight-anchor morphology/depth/PSF evidence continues to favor a survey-correlated origin, but the physical attribution and joint covariance remain explicit open items. Artifacts: \artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/c16\_r24conf\_pod\_batch.json}, \artifact{pipelines/p2\_chirality/outputs/systematic\_l1\_forward\_model\_dr8morph.json}, and \artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/residual\_template\_cross\_estimator\_injection.json}.
...
canonical HC sample the grid-level paired-surrogate thresholds are
$A_{50}=0.75\%$ and $A_{95}=1.5\%$; the per-axis first-$3\sigma$ crossing
has p16/median/p84 $=0.75/0.75/0.99\%$. On the concordant full pool they are
$A_{50}=0.5\%$, $A_{95}=0.75\%$, with per-axis
p16/median/p84 $=0.5/0.5/0.75\%$. Recovery is visibly axis- and
stratum-dependent: several imaging-leg, edge-on, depth, and PSF strata do
not cross $A_{50}$ or $A_{95}$ within the tested $2\%$ grid, and this
failure is reported rather than extrapolated. The full recovery
