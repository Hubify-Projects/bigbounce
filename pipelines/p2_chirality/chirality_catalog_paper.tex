...
\newcommand{\paperVersion}{v1.0.248}
...
\newcommand{\paperTimestamp}{July 15, 2026, 04:45 PDT}
...
The six-support-consistent diagnostic analysis of the $+3.64\sigmaunit$ \FSC{} residual (apodized-mask robustness, multipole-spectrum coherence, quality-quartile stratification, leg-proxy cross-power, boundary-distance variance, and direct cross-spectrum) is in Appendix~D. Three discriminators disfavor interpretation~(i) (real cosmological dipole at $\sim\!1.7\%$): (a)~$\ell\!=\!2 > \ell\!=\!1$ broadband structure incompatible with a clean dipole; (b)~$p_{\rm eq}$ quality-quartile washout (all four quartiles $|\sigma|<1$); (c)~suggestive cross-spectrum evidence at $\ell\!=\!2$ ($r_{\ell=2}\!=\!-0.65$, $z\!=\!-2.89$ against a 200-realization permutation null). Historical density-null and WLS/bootstrap calculations used a different latitude-cut support and are excluded from this synthesis.
...
\begin{table*}[tbp]
\caption{Compact support audit of diagnostics associated with the $+3.64\sigmaunit$ \FSC{} $\ell\!=\!1$ residual. Rows marked out-of-scope used a different latitude-cut support and are not part of the \FSC{} synthesis.\label{tab:eightanchor_summary}}
\begin{ruledtabular}
\begin{tabular}{llc}
...
\hline
\multicolumn{3}{p{0.95\textwidth}}{\emph{Synthesis: the six \FSC{}-consistent diagnostics favor a survey-correlated systematic; the two different-support rows are excluded rather than harmonized post hoc.}} \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
This appendix documents six diagnostics evaluated on the declared \FSC{} support: (a)~apodized-mask robustness, (b)~multipole-spectrum coherence, (c)~quality-quartile stratification, (d)~leg-proxy cross-power, (e)~boundary-distance variance, and (f)~direct cross-spectrum. Two historical calculations---a density-stratified null and a joint WLS/block-bootstrap fit---used the broader $|b_{\rm gal}|>15^\circ$ support with $N_{\rm total}>0$ (24,187 pixels), not \FSC{} (24,087 pixels), and are retained below only as explicitly out-of-scope provenance. They do not support the \FSC{} synthesis.
...
Stratifying in-mask pixels into five boundary-distance shells: the per-shell weighted variance $\langle A_p^2\rangle$ is statistically uniform (range $<\!11\%$ of the mean). The chirality asymmetry-map variance is NOT concentrated near the \FSC{} boundary, disfavoring the sharp-edge NaMaster artifact variant.
...
The \FSC{} $+3.64\sigmaunit$ residual is \emph{not} a positive detection of a primordial chirality dipole. The retained \FSC{} diagnostics favor a per-pixel-correlated low-$\ell$ systematic (depth/PSF/morphology): the $\ell=2$ cross-spectrum anti-alignment is suggestive ($r_{\ell=2}=-0.65$, $z=-2.89$; 200-MC null), imaging-leg templates account for about 25\% of the $\ell=1$ amplitude, quality quartiles wash out, and boundary-distance variance is uniform. The historical $+3.80$ density-null and WLS/bootstrap numbers are not on \FSC{} and are excluded. The primary scientific result remains the HC real-space $+0.71\sigma$ observed-label null.
