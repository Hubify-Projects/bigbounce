...
\newcommand{\paperVersion}{v1.0.246}
...
\newcommand{\paperTimestamp}{July 15, 2026, 03:44 PDT}
...
\begin{abstract}
We construct an ApJS-oriented release candidate for a catalog of $8{,}474{,}531$ DESI Legacy DR8 galaxies and test the observed-label chirality field; the candidate payload is not yet publicly archived. The single primary estimator is the real-space dipole of the declared high-confidence selection ($p_{\rm eq}>0.6$, $N_{\rm selected}=949{,}584$), of which $N_{\rm support}=947{,}326$ galaxies enter the supported-pixel fit and fixed-occupancy galaxy-label-randomization null. That estimator is null-consistent at $+0.71\sigmaunit$ (one-sided upper-tail rank $p=0.225$). The release-candidate contract removes raw-pass and reconstructed flip-pass scores from the science-facing product and quarantines all 249,066 catalog-wide reconstruction violators (59,515 in HC) as explicitly uncalibrated diagnostics. A paired-image-to-field surrogate over 20 deterministic axes is reported only as a pilot grid: the HC score-crossing counts are $1/20$, $16/20$, $16/20$, $20/20$, and $20/20$ at full amplitudes $A=A_p=0.5\%$, $0.75\%$, $1.0\%$, $1.5\%$, and $2.0\%$. These fractions are not a calibrated population recovery curve or physical upper bound; $A=1.5\%$ corresponds to a $0.75\%$ peak deviation of $f_{\rm CW}$ from one half. WLS and post-MASTER harmonic results are diagnostics on different fields and samples. A spatially resolved confusion/transfer model, matched-footprint independent-estimator comparison, joint real-space$\times$harmonic covariance, an immutable public catalog archive, and independent external preregistration or blinding remain open. The $\ell=1$ observable is parity-even; without a transfer function we claim no primordial parity bound.
\end{abstract}
...
Five gates remain open: a physical morphology transfer function, a matched-footprint independent-estimator comparison, a joint real-space/harmonic/nuisance covariance likelihood, an immutable public archive of the release-candidate payload, and independent external preregistration or blinding of the primary analysis. The HC real-space estimator is the sole primary result; it is empirically less exposed to the measured harmonic leakage but does not mathematically bypass that channel. The WLS and harmonic analyses remain diagnostics.
...
We construct an 8,474,531-object observed-label catalog release candidate and report one primary result: the HC real-space dipole is null-consistent ($N_{\rm selected}=949{,}584$, $N_{\rm support}=947{,}326$, $z=+0.71$, one-sided upper-tail $p=0.225$ under the declared null convention). The release-candidate payload remains local and is not yet an archived public release. All harmonic, WLS, injection, and transfer calculations are diagnostics. Our main conclusions are:
...
\begin{itemize}
\item \textbf{Catalog:} \url{https://huggingface.co/datasets/bamfai/galaxy-chirality-catalog} (CC-BY-4.0, Parquet). The current public source is pinned to revision \texttt{a21eb596fd10edb9af9e7a1bcefb04f87327a724}, 952,115,239 bytes, and SHA-256 \texttt{e8525ba5c98576f6...3563}. It does \emph{not} contain the previously described \texttt{qc\_flip\_identity\_violator} column; users must not rely on that nonexistent public field. The v1.0.244 local release candidate instead provides a science-facing file with raw/reconstructed columns absent and Boolean \texttt{raw\_flip\_qc\_unsafe}, plus a separate provenance-only quarantine containing all 249,066 catalog-wide violators (59,515 in HC), each marked \texttt{do\_not\_use\_for\_science=True}. Safe filters are \texttt{primary\_hc == True} for the declared hard-label primary and \texttt{primary\_hc == True and raw\_flip\_qc\_unsafe == False} for the strict diagnostic. The machine contract is \artifact{pipelines/p2\_chirality/apjs\_release\_schema\_v1\_0\_244.json}; the human dictionary, builder, tests, and minimal primary-null reproducer are \artifact{pipelines/p2\_chirality/CATALOG\_SCHEMA.md}, \artifact{pipelines/p2\_chirality/build\_apjs\_release\_v1\_0\_244.py}, \artifact{pipelines/p2\_chirality/tests/test\_apjs\_release\_v1\_0\_244.py}, and \artifact{pipelines/p2\_chirality/reproduce\_p4\_primary\_null\_v1\_0\_244.py}. The generated Parquet payloads remain local until a content-addressed public upload is completed. No immutable archive or DOI exists yet; that gate remains open and no placeholder DOI is claimed.
