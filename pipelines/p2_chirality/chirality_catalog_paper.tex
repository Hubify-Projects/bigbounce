...
\newcommand{\paperVersion}{v1.0.171}
...
$A_p=(N_{\rm CW}-N_{\rm CCW})/(N_{\rm CW}+N_{\rm CCW})$ at HEALPix
...
\begin{table*}[t]
\caption{Monopole+mask leakage null ($f_{\rm sky}\!=\!0.49005$, seed$\,=\,$42; $N\!=\!500$ binomial-monopole realizations). Pseudo-$C_\ell$ entries in this table are dimensionless band values of the un-monopole-subtracted $f_{\rm CW}$-map convention used by the generative null, and are NOT on the $A_p$-map $\times10^{-6}$\,sr scale of Table~\ref{tab:multipole}; the two tables intentionally use different fields and normalizations. The monopole-only null reproduces $99.32\%$ of the observed pre-MASTER pseudo-$C_\ell^{(\ell=1)}$ power (residual $+1.69\sigmaunit$). Both rows are computed against the monopole-only generative null; the hemisphere row uses its own 768-direction NSIDE${}_{\rm dir}\!=\!8$ scan grid, distinct from the 648-direction $10^\circ$-grid look-elsewhere scan of Appendix~C, and its $z$ is not comparable to the label-shuffle $3.05\sigmaunit$ value quoted there.\label{tab:monopole_mask_null}}
\begin{ruledtabular}
\begin{tabular}{lccc}
...
Repository state for this version: commit \texttt{2a2939b2} (June 2026); a Zenodo DOI snapshot will accompany the journal submission.
