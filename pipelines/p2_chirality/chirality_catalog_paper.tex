...
\newcommand{\paperVersion}{v1.0.240}
...
\newcommand{\paperTimestamp}{July 13, 2026}
...
\title{A Null Chirality Dipole in 8.5 Million DESI Galaxies\\
from Equivariant Deep Learning}
...
\begin{abstract}
We measure the large-scale chirality dipole of spiral galaxies and find it consistent with null. The measurement rests on the largest chirality-labeled galaxy catalog to date: $8{,}474{,}531$ DESI Legacy DR8 galaxies classified by a flip-equivariant Vision Transformer into clockwise (CW), counter-clockwise (CCW), and non-spiral classes, yielding $N_{\rm spiral}=3{,}201{,}160$ spirals, released publicly with model weights and reproducibility scripts. Our primary estimator --- a real-space dipole fit to the pre-specified high-confidence subsample ($p_{\rm eq}>0.6$, $N\approx 9.5\times 10^5$ spirals) --- gives $+0.41\sigmaunit$ (moment-$z$ against an isotropic pixel-permutation null; empirical-rank $p=0.31$), robust across the confidence-cut sweep and under a per-galaxy label-shuffle test; a block-bootstrap WLS template fit disfavors a clean $A_{\rm ref}=0.017$ dipole (the lower end of Shamir's reported $1.7\%$--$4.0\%$ asymmetry range~\cite{Shamir:2020,Shamir:2022DESI}, which maps to our $A_p\!=\!2(f_{\rm CW}-\tfrac12)$ without rescaling) at $z\approx-7.6$. This $\ell=1$ observable is parity-\emph{even} (isotropy-breaking, not a direct parity-violation test). Injection--recovery brackets the real-space $95\%$-recovery amplitude $A_{95}$ in $(1.0\%,1.5\%]$ ($A_{50}\approx0.75\%$), so a genuine Shamir-scale dipole would have been detected; a detection at $\ge\!5\sigmaunit$ with $A\gtrsim A_{95}$ would falsify this null. The MASTER pseudo-$C_\ell$ harmonic channel is treated as a systematics diagnostic, not an independent cosmological null (monopole--mask leakage reproduces $99.32\%$ of raw pre-MASTER $\ell=1$ power; residuals are attributed via an eight-anchor battery, with $\sim\!47\%$ of the post-MASTER amplitude an explicit open item that lies below $A_{95}$ and does not affect the primary null; Sec.~\ref{sec:monopole_mask_null}, Appendix~D). Significances from distinct null procedures are not directly comparable as detection significances (Sec.~\ref{sec:notation}); a leg- and confidence-stratified GZ1 human-label cross-check confirms the classifier's dipole-biasing error asymmetry is consistent with zero in every stratum (Appendix~B).
\end{abstract}
...
\item \emph{Block-bootstrap $z$}: $(A_{\rm dipole}^{\rm best} - A_{\rm ref})/\sigma_{\rm boot}$ with spatial-coherence-corrected $\sigma_{\rm boot}$; the $z\!\approx\!-7.6$ primary exclusion (against $A_{\rm ref}\!=\!0.017$, Shamir's $1.7\%$ asymmetry in the same $A_p$ units) uses this convention only.
\end{itemize}
...
\dtc{Detection-efficiency (recovery) threshold of the primary real-space estimator; not a frequentist upper limit} & \dte{(vii) injection--recovery} & \dts{HC, $p_{\rm eq}\!>\!0.6$ ($949{,}584$)} & \dtn{per-pixel shuffle} & \textsc{calibration} \\
\hline\hline
\end{tabular}
\end{center}
\end{table*}
...
\begin{table}[t]
\caption{\textbf{Primary Results callout.} The two load-bearing claims of this paper (rows P1--P2) and, for reference, the non-primary harmonic diagnostics (rows D1--D2) that are \emph{systematics-attributed, not detections}. Each row names the estimator and the specific null procedure its $\sigma$/$z$ is computed against. \textbf{Only rows P1--P2 carry cosmological weight;} the $\sigma$/$z$ values across rows (including $+0.41\sigma$, $+3.64/+7.28\sigma$, and $z\approx-7.6$) derive from distinct null constructions and are \emph{not} directly comparable as detection significances (Sec.~\ref{sec:notation}).\label{tab:primary_callout}}
\begin{ruledtabular}
\footnotesize
\begin{tabular}{lll}
...
\textbf{P2} clean-$1.7\%$ exclusion & block- & $z\!\approx\!-7.6$ \\
\quad (WLS fit), \emph{primary} & \quad bootstrap & \quad (disfavored) \\
\hline
...
\noindent\emph{Reader's note (Table~\ref{tab:multipole}).} The $\sigma$/$z$ entries in this table are systematics-attributed harmonic \emph{diagnostics}, each against its own null; they are \emph{not} detection significances and must \emph{not} be compared row-to-row, footprint-to-footprint, or with the primary real-space dipole. Only the two primary estimators (HC real-space dipole $+0.41\sigmaunit$; template-fit exclusion $z\!\approx\!-7.6$) carry cosmological weight.
...
\emph{Quantitative forward model of the residual amplitude.} \textbf{Bottom line, stated first: the $\sim\!47\%$ of the $\ell\!=\!1$ residual amplitude that our imaging$+$morphology forward model does not reproduce is below the real-space estimator's current recovery threshold and therefore does not affect our exclusion/sensitivity statements for dipoles above $A_{95}$.} Even if the \emph{entire} $\ell\!=\!1$ residual ($|a_1|\!=\!6.95\!\times\!10^{-3}$, i.e.\ $A_p\!=\!0.695\%$) --- not merely the unmodelled $\sim\!47\%$ ($A_p\!=\!0.32\%$) --- were a clean coherent cosmological dipole, it would sit \emph{below} the real-space $50\%$-recovery floor $A_{50}\!=\!0.75\%$ and $\gtrsim\!3$--$4\times$ below the $95\%$-recovery threshold $A_{95}\!\in\!(1.0\%,1.5\%]$; the direct real-space Catalog~C dipole ($+0.41\sigmaunit$, $p_{\rm 2\text{-}sided}\!=\!0.62$) --- the very estimator that \emph{would} register such a signal --- returns null. The unmodelled remainder is therefore bounded below the recovery threshold (derivation below), so it does not affect the exclusion/sensitivity statements; the primary null does not consume the harmonic channel at all, and the remainder's physical origin (survey systematic vs.\ residual signal) remains an explicit open item. The eight-anchor battery establishes the \emph{direction} of the attribution (survey-correlated systematic, not clean dipole) but does not by itself state what \emph{fraction} of the residual amplitude the available imaging systematics reproduce. We therefore forward-model the residual directly: a galaxy-count-weighted least-squares fit of the canonical-mask $A_p$ field onto the imaging-systematic template basis (imaging-leg fractions, source density $+$ density$^2$, mean PSF FWHM, mean $grz$ depth, and $E(B\!-\!V)$; the same brick-level templates as Appendix~D), with the fitted systematic field then projected onto $\ell\!=\!1$ on the exact canonical mask ($N_{\rm spiral}(p)\!\ge\!10$, $f_{\rm sky}\!=\!0.49005$). (The three imaging-leg fraction templates sum identically to zero on every galaxy-weighted pixel and are therefore exactly collinear with the constant term, making the full design rank-deficient; we drop one leg to obtain a well-conditioned baseline design that reproduces the fitted dipole to machine precision, Table~\ref{tab:wls_fit} and Appendix~D.) The predicted systematic $\ell\!=\!1$ amplitude is $|a_1^{\rm sys}|\!=\!3.75\!\times\!10^{-3}$, i.e.\ $\approx\!54\%$ of the observed $|a_1|\!=\!6.95\!\times\!10^{-3}$ ($\approx\!52\%$ under the wider $|b|\!>\!15^\circ$ mask, so the fraction is mask-stable), aligned with the observed residual dipole vector at $\cos\theta\!=\!+0.83$ (correct direction). In equivalent-significance terms the imaging-systematic prediction alone carries $+0.7\sigmaunit$ against the depth-stratified label-shuffle null used for the observed $+3.64\sigmaunit$. \emph{We make no over-claim here: this forward model accounts for only a \textbf{minority} ($\approx\!52$--$54\%$) of the residual amplitude, and we do \textbf{not} state that the imaging systematic \emph{explains}, or that the $+3.64\sigmaunit$ residual is attributed to, the imaging$+$morphology templates.} The depth/PSF/EBV/leg/density templates forward-model roughly \emph{half} of the residual amplitude in the correct direction; the remaining $\gtrsim\!\sim\!47\%$ is \textbf{not} captured by imaging templates alone and is left as an explicit \textbf{open item} --- it could be residual signal, or a survey-correlated systematic outside the imaging$+$morphology template families we have tested (candidates below). \emph{Statistical upper limit on the cosmological content of the unmodelled remainder.} Although a fully-closed per-pixel attribution of the remainder is deferred to dedicated compute, its \emph{cosmological} (coherent real-space dipole) content is directly bounded now. The harmonic residual amplitude $|a_1|\!=\!6.95\!\times\!10^{-3}$ and the real-space dipole amplitude are in the same $A_p\!=\!2(f_{\rm CW}\!-\!\tfrac12)$ units, so the residual maps to an equivalent real-space dipole amplitude $A_p\!=\!0.695\%$ --- \emph{below} the real-space $50\%$-recovery floor $A_{50}\!=\!0.75\%$ and far below the $95\%$-recovery threshold $A_{95}\!\in\!(1.0\%,1.5\%]$ (Sec.~\ref{sec:sensitivity}). Hence even if the \emph{entire} $\ell\!=\!1$ residual --- not merely the unmodelled $\sim\!47\%$ (amplitude $A_p\!=\!0.32\%$) --- were a genuine coherent cosmological dipole, it would \emph{not be reliably recovered} by the real-space estimator (below the $50\%$-recovery floor) and is below $A_{95}$ (the $95\%$-recovery / detection-efficiency threshold, i.e.\ below the amplitude at which the estimator would reliably recover it --- not a frequentist upper limit); the unmodelled remainder is \emph{a fortiori} $<\!A_{95}$, i.e.\ $\gtrsim\!3$--$4\times$ below the recovery threshold. Independently, the direct real-space Catalog~C dipole null ($+0.41\sigmaunit$, $p_{\rm 2\text{-}sided}\!=\!0.62$; Sec.~\ref{sec:dipole}) is fully consistent with zero cosmological dipole, so the estimator that \emph{would} register such a signal registers none: the remainder therefore lies below the real-space estimator's recovery threshold and does not affect the dipole exclusion/sensitivity statements. Its physical origin remains unresolved, but is attributed on independent grounds to the survey-systematic (mask-coupled pseudo-$C_\ell$) channel, consistent with the modelled $\sim\!53\%$ (\artifact{pipelines/p2\_chirality/scripts/gemini\_v215\_residual\_bound\_edgeon\_coherence.py}). In summary: any cosmological contribution to the unmodelled $\sim\!47\%$ remainder must lie below $A_{50}$ and is therefore consistent with the $z\!=\!+0.41\sigmaunit$ real-space null. The full per-pixel morphology \emph{attribution} of the remainder (not required for this bound) remains pod-deferred. Critically, \emph{the paper's central null-dipole conclusion does not depend on explaining this residual at all}: the primary result rests on the two systematics-independent estimators of Sec.~\ref{sec:dipole} (the HC real-space dipole and the block-bootstrap WLS clean-$1.7\%$-dipole exclusion), and the $+3.64\sigmaunit$ pseudo-$C_\ell$ residual is a non-primary, diagnostic-only quantity throughout. The per-galaxy morphology channel flagged as the leading un-modelled term has now been forward-modelled directly: pulling the real DESI Legacy DR8-sweep morphology for all $3{,}201{,}160$ spirals (100\% \texttt{dr8\_id} match; axis ratio $b/a$ from the ellipticity, \texttt{fracdev}, and effective radius \texttt{shape\_r}), building galaxy-count-weighted per-pixel morphology templates, orthogonalizing them against the imaging basis, and re-fitting the field raises the forward-modelled $\ell\!=\!1$ fraction only from $52.4\%$ to $53.0\%$ ($+0.7$ points; $\cos\theta\!=\!+0.84$) --- i.e.\ it \emph{remains a minority of the amplitude, still leaving $\sim\!47\%$ open}, and does not convert this diagnostic into an explanation of the residual. Per-galaxy morphology, once its imaging-correlated part is removed, therefore adds negligibly to the $\ell\!=\!1$ projection: the $\sim\!47\%$ remainder is \emph{not} a morphology artifact but persists as a genuine open item. \emph{Likely physical origin of the unmodelled remainder.} The physical picture we favor is that the residual is a survey systematic, not a cosmological detection, for three converging reasons. (i)~It lives in the diagnostic pseudo-$C_\ell$ channel, which mixes true sky power with mask-geometric leakage and classifier-bias modulation, and the modelled $\gtrsim\!53\%$ is already aligned in sign and direction with the observed residual ($\cos\theta\!=\!+0.84$), i.e.\ the part we can attribute points the systematic way. (ii)~The eight-anchor battery independently disfavors a clean dipole ($\ell\!=\!2\!>\!\ell\!=\!1$, quartile washout, depth anti-alignment; Table~\ref{tab:eightanchor_summary}), so the remainder is far more plausibly \emph{additional} survey-correlated systematic than hidden signal. (iii)~The leading unmodelled channel is physically identifiable: it is the classifier's confidence-vs-depth response --- a per-pixel modulation of chirality-label reliability by imaging depth/PSF/seeing that our pixel-level template (mean depth, PSF FWHM, $E(B\!-\!V)$, leg fractions) captures only to first order, missing the per-galaxy, morphology-correlated selection whereby the classifier's effective purity varies with source properties across the footprint. Fully attributing this remainder therefore requires a per-pixel classifier confidence-vs-depth response map built from the full DR8-sweep morphology at production scale --- a GPU/pod-bound computation deferred to dedicated compute --- specifically: a full DR8-sweep morphology $\to$ per-pixel classifier-purity map (depth-conditioned calibration, mapping per-galaxy source properties to effective chirality-label reliability across the footprint), followed by re-fitting the $\ell\!=\!1$ projection with this morphology-purity template added to the design basis. This is the planned follow-up approach to absorb the remaining $\sim\!47\%$ selection systematic; we do not fabricate this result here, but the physical expectation is that it is a depth/selection systematic of the same family as the modelled fraction, not a primordial dipole. Critically, this residual does not gate the headline: the primary null-dipole result rests on two estimators \emph{independent} of the $\ell\!=\!1$ harmonic residual --- the real-space HC dipole (Sec.~\ref{sec:dipole}) and the block-bootstrap WLS clean-$1.7\%$-dipole exclusion (Table~\ref{tab:wls_fit}) --- neither of which consumes the pseudo-$C_\ell^{(\ell=1)}$ quantity in which the residual lives, so a fully-closed systematic budget for it is not a precondition for the null, which stands regardless. Artifacts: \artifact{pipelines/p2\_chirality/outputs/systematic\_l1\_forward\_model.json} (imaging-only), \artifact{pipelines/p2\_chirality/outputs/systematic\_l1\_forward\_model\_dr8morph.json} (imaging $+$ real DR8 morphology).
...
Under the present ViT/TTA pipeline, our maximum WLS template amplitude in the full-footprint regional fit is $0.32\%$ in $A_p$ units (i.e.\ $A_p\!=\!0.0032$; equivalently a $0.16\%$ deviation in $f_{\rm CW}$), restricting to the cleanest equal-area partition; the equal-area slab maxima in the 10-slab per-axis decomposition reach $0.46$--$0.56\%$, \artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/c12\_r24conf\_local\_batch.json}) and the $0.41\sigmaunit$ HC ($p_{\rm eq}\!>\!0.6$) simple dipole is well below the $2\text{--}4\sigmaunit$ dipoles reported by Shamir~\cite{Shamir:2012,Shamir:2020,Shamir:2022DESI}. We do \emph{not} claim a frequentist exclusion of Shamir's Ganalyzer estimator: a likelihood-level exclusion requires a matched-footprint Ganalyzer reanalysis under his pipeline + cuts (not performed here). The discrepancy most likely reflects two factors: (1)~Ganalyzer lacks a published bias audit comparable to our 8-test suite; (2)~the monopole-mask leakage channel demonstrated in Sec.~\ref{sec:monopole_mask_null} can generate a comparable pre-MASTER dipole-class artifact under this DESI/ViT-Small pipeline; a matched Ganalyzer reanalysis remains required. Specifically, the monopole--mask leakage channel (Sec.~\ref{sec:monopole_mask_null}, $99.32\%$ pre-MASTER reproduction) and the depth-coupled classifier bias demonstrated here (global monopole artifact, confidence-cut sweep, leg-proxy cross-power) supply plausible mechanisms by which a pipeline lacking equivalent equivariant TTA and an explicit \NS{} class could generate or amplify spurious low-$\ell$ power on a patchy footprint --- providing positive methodological context for why the present null is expected under improved methodology, while still making clear that a matched-footprint reanalysis remains the definitive test. For self-containedness we note where a Shamir-scale signal would sit relative to our sensitivity: the injection-recovery floor is $A_{50}\!\approx\!0.75\%$ with $A_{95}\!\in\!(1.0\%,1.5\%]$ (Sec.~\ref{sec:sensitivity}), so an injected clean dipole at Shamir's lower reported amplitude ($1.7\%$ asymmetry $\equiv A_p\!=\!0.017$) sits above $A_{95}$ and would be recovered by our real-space estimator with $P(\sigma\!>\!3)\!\to\!1$ --- i.e.\ a genuine Shamir-scale real-space dipole would have been detected here; its absence is what the $+0.41\sigmaunit$ null and the $z\!\approx\!-7.6$ template exclusion jointly express, without constituting a frequentist exclusion of Shamir's distinct Ganalyzer estimator. These conclusions corroborate and extend the methodological critique of Iye~\etal~(2021)~\cite{Iye:2021} with $3.2\!\times\!10^{6}$ spirals (a ${\sim}25\times$ sample extension over the $\sim\!1.27\!\times\!10^{5}$-galaxy SDSS sample underlying the critiqued analyses; $3.2\!\times\!10^{6}/1.27\!\times\!10^{5} \approx 25$).
...
\emph{Model-independent cross-check (stated first).} The concern that the $66.5\%$ CE-ResNet pseudo-labels could inherit survey-correlated bias into the null is addressed directly and independently: using the \emph{Galaxy~Zoo~1 human CW/CCW votes themselves as the per-galaxy chirality label} \cite{Lintott:2011} --- no learned model of any kind in the chirality-label chain, hence maximally CE-ResNet-independent --- the dipole returns the same clean null at $z\!=\!-0.54\sigmaunit$ (per-pixel permutation rank-$p\!=\!0.67$; $z\!=\!-0.55\sigmaunit$ per-galaxy binomial, rank-$p\!=\!0.68$; $N_{\rm spiral}^{\rm HC}\!=\!4.60\!\times\!10^{4}$; human-label CW fraction $0.4836$, dipole amplitude $0.0546$; artifact \artifact{pipelines/p2\_chirality/outputs/gz1only\_fullN\_dipole\_result.json}), under the identical HEALPix estimator (NSIDE~$64$, MIN\_PIX\_COUNT~$10$), confidence cut, seed, and $N_{\rm MC}\!=\!10{,}000$ null as the headline (\texttt{run\_dipole\_gz1only\_fullN.py}). This $N$ is $3.08\times$ the earlier GZ1-only-model cross-check ($N\!=\!1.50\!\times\!10^{4}$; the old $N$ was an arbitrary $20{,}000$-galaxy streaming cap, not the true overlap), tightening the statistical-only floor by $\sqrt{3.08}\!\approx\!1.75\times$; the negative $z$ places the observed dipole slightly \emph{below} the isotropic-random mean. \emph{The null holds with the learned model removed entirely from the label chain.} Because the chirality supervision is then fully human and CE-ResNet-independent, no pseudo-label-inherited coherent dipole large enough to register at this sensitivity is present; the remainder of this subsection quantifies the ceiling and the residual template-orthogonal channel. The $66.5\%$ CE-ResNet pseudo-label fraction is thus not the operative independence question: the model-free GZ1-human-only null above is a direct model-independent test, and it holds (at the coarse sensitivity quantified below; it corroborates but does not test the sub-percent inherited structure). For completeness we still state the residual-channel limitation the pseudo-labels impose on the \emph{shuffle} nulls. Because $66.5\%$ of the training labels derive from CE-ResNet predictions (Sec.~\ref{sec:data}), the per-galaxy label-shuffle and per-pixel permutation nulls used throughout randomize this model's own outputs; as stated in the Introduction, they therefore cannot by themselves test independence from large-scale survey-correlated structure potentially \emph{inherited} through the CE-ResNet pseudo-labels. We make explicit here why this limitation does \emph{not} render the canonical-mask attribution circular, and we bound the inherited contribution. (i)~The eight-anchor battery of Appendix~D is not shuffle-based: the joint nuisance-marginalized WLS template fit, the leg-proxy cross-power, the density-stratified null, and the direct $A_p\times n_{\rm total}$ cross-spectrum all regress or cross-correlate the \emph{fixed} chirality field $A_p$ against \emph{external} survey templates (imaging-leg fractions, pixel density, depth/PSF morphology). These diagnostics detect survey-correlated low-$\ell$ structure regardless of whether it originates in the sky or is inherited via the pseudo-labels --- inherited survey structure would manifest as exactly the $A_p\times$template correlation these anchors measure. The one channel this does not automatically cover is a \emph{non-uniform} (spatially-varying) CE-ResNet systematic that is \emph{not} aligned with any of the external templates (imaging-leg fractions, pixel density, depth/PSF morphology) that Appendix~D regresses against: the anchors are sensitive to inherited structure only insofar as it projects onto that fixed template basis, so a hidden inherited pattern orthogonal to all of them could in principle escape the template-regression anchors --- but it would still have to survive the \emph{template-agnostic} block-bootstrap dipole fit and the injection-recovery floor, both of which return a clean-dipole disfavor ($z\!\approx\!-7.6$) and null irrespective of the systematic's template alignment, so such a pattern is bounded by the same ceiling derived above rather than being unconstrained. (ii)~Direction of bias: a survey-correlated inherited bias \emph{adds} spurious low-$\ell$ power, biasing the dipole estimator \emph{away} from null, so the real-space high-confidence null at $+0.41\sigmaunit$ is conservative against it. The only channel by which inheritance could \emph{mask} a real signal is uniform dilution from finite classifier accuracy, which is folded into the injection-recovery floor via $g=2a-1$ (Sec.~\ref{sec:sensitivity}) and is shown not to overturn the null. (iii)~The quality-quartile washout (Appendix~D, anchor c: per-quartile $\ell\!=\!1$ all $|\sigma|\!<\!1$ with no monotone trend in label quality) and the $\ell\!=\!2\!>\!\ell\!=\!1$ broadband structure (anchor b) are the signatures of a label-noise/depth systematic, not of an inherited-then-diluted primordial dipole, which would strengthen with label quality. (iv)~Bound on the inherited handedness: on the confident GZ1 cross-match the human-label CW fraction is $0.4838$ versus the production equivariant catalog's $0.4974$ (artifact \artifact{pipelines/p2\_chirality/r42\_results/wave\_14\_fff\_gz1\_platt\_recal.json}); the inherited handedness is thus a spatially-uniform CW-deficit \emph{monopole}, whose leakage onto the patchy mask is separately quantified in Sec.~\ref{sec:monopole_mask_null} --- a uniform monopole does not by itself generate a real-space dipole. The fully independent check requested by external review has now been performed directly, and at its natural statistical ceiling. Taking the \emph{Galaxy~Zoo~1 Table~2 human CW/CCW votes} \cite{Lintott:2011} \emph{directly as the per-galaxy chirality label} --- no GZ1-trained network, no CE-ResNet, no learned model at any step in the label chain --- the $48{,}414$ confident ($\max(P_{\rm CW},P_{\rm ACW})\!>\!0.6$; GZ1's native anticlockwise column $P_{\rm ACW}$ is the CCW class in our convention) GZ1 spirals cross-match ($1''$ KDTree) to $N\!=\!46{,}017$ DESI-footprint galaxies, whose only role is to supply sky positions. Under the \emph{identical} real-space dipole estimator, confidence cut, seed, and $N_{\rm MC}\!=\!10{,}000$ per-pixel label-permutation null as the headline result (\texttt{run\_dipole\_gz1only\_fullN.py}), this human-label field yields a dipole consistent with null at $z=-0.54\sigmaunit$ (per-pixel-permutation rank-$p=0.67$), corroborated by the per-galaxy binomial null at $z=-0.55\sigmaunit$ (rank-$p=0.68$); human-label CW fraction $0.4836$, dipole amplitude $0.0546$ (artifact \artifact{pipelines/p2\_chirality/outputs/gz1only\_fullN\_dipole\_result.json}). This supersedes the earlier reduced-$N$ GZ1-only-\emph{model} cross-check ($z=-0.04\sigmaunit$, $N=1.50\times10^{4}$; artifact \artifact{pipelines/p2\_chirality/outputs/gz1only\_dipole\_result.json}), whose $N$ was set by an arbitrary $20{,}000$-galaxy streaming cap rather than the true GZ1$\times$DESI overlap, and is $3.08\times$ more powerful. Because the chirality supervision here is entirely human votes with \emph{no learned model at all}, the persistence of the null establishes that the vanishing dipole is \emph{not} an artifact inherited from the pseudo-labels. \emph{Explicit ceiling on undetected inherited large-scale power.} We can bound the maximum inherited coherent large-scale (dipole-class) chirality power that could remain undetected at headline sensitivity directly from the anchor-battery numbers, because any inherited survey-correlated structure enters the same $A_p$ field these anchors constrain. The block-bootstrap WLS template fit (Appendix~D) sets $\sigma_{\rm boot}(A_{\rm dipole})=1.63\times10^{-3}$ in $A_p$ units on the canonical mask; a clean inherited dipole would have to exceed the $95\%$-recovery falsification amplitude $A_{95}\in(1.0\%,1.5\%]$ (Sec.~\ref{sec:sensitivity}) to survive, and one at the interpretation-(i) $1.7\%$ ($A_p$-ref $0.017$, Shamir's reported asymmetry in $A_p$ units) reference is disfavored at $z\!\approx\!-7.6$. Combining these, the ceiling on any inherited coherent dipole amplitude consistent with the observed field is $|A_{\rm inh}|\lesssim A_{95}\lesssim1.5\%$ ($f_{\rm CW}$ units; $\lesssim3\times10^{-2}$ in $A_p$ units) at the $95\%$-recovery bound, and $\lesssim0.75\%$ at the $50\%$-recovery consistency level; below this the inherited power is indistinguishable from the systematic floor. Above the monopole channel (bounded separately in Sec.~\ref{sec:monopole_mask_null}), no inherited coherent dipole larger than this ceiling can be hiding in the pseudo-labels without having already registered in the block-bootstrap and injection-recovery anchors, both of which return null/disfavor. \emph{Statistical power and the natural ceiling of the human-label test.} The GZ1-human-label test is a self-consistent, fully model-independent null cross-check performed \emph{at the natural ceiling} of the independent-human-label sample: its high-confidence spiral sample $N_{\rm spiral}^{\rm HC}\!=\!4.60\times10^{4}$ is the \emph{entire} confident ($P\!>\!0.6$) GZ1 CW/CCW spin catalog that cross-matches to the DESI footprint (out of $667{,}944$ GZ1 rows, $190{,}225$ spirals, $48{,}414$ confident, $46{,}017$ DESI-matched). It is $3.08\times$ the earlier reduced-$N$ cross-check, tightening the statistical-only dipole floor by $\sqrt{3.08}\!\approx\!1.75\times$ (Fisher scaling $\sigma(A)\!\propto\!N^{-1/2}$; Sec.~\ref{sec:sensitivity}). It remains below the headline $N_{\rm spiral}^{\rm HC}\!=\!9.5\times10^{5}$ by a factor $\sim\!21$, so its statistical-only floor is inflated by $\sqrt{9.5\times10^{5}/4.60\times10^{4}}\!\approx\!4.5\times$; \emph{making this transparent as a sensitivity statement}, the same injection--recovery construction applied to this $N\!=\!4.60\times10^{4}$ estimator therefore yields $A_{50}\!\approx\!4.5\times0.75\%\!\approx\!3.4\%$ and $A_{95}\!\approx\!4.5\times(1.0$--$1.5)\%\!\approx\!4.5$--$6.8\%$ (Fisher $\sigma(A)\!\propto\!N^{-1/2}$ scaling of the headline floors), i.e.\ the human-label-only test can \emph{corroborate} the null but is \emph{not} sensitive to the sub-percent amplitudes the headline HC sample constrains; it therefore can only \emph{fail to overturn}, not tighten, the headline null, exactly as its $z\!=\!-0.54\sigmaunit$ result does. Crucially, the reason it does not reach headline $N$ is \emph{exhaustion, not compute}: GZ1 human spin votes at $P\!>\!0.6$ are exhausted at $\sim\!46{,}017$ within the DESI footprint, so this is the ceiling of any GZ1-human-only test, not another arbitrary cap. A larger-$N$ model-independent test is not obtainable from GZ1 human labels alone; a full-catalog match to headline $N$ necessarily reintroduces a learned classifier and so cannot be strictly human-label-independent. The clean null recovery ($z\!=\!-0.54\sigmaunit$) at this $3\times$-larger, fully model-independent sample confirms that a pseudo-label-inherited dipole large enough to survive at $N\!=\!4.60\times10^{4}$ is absent.
...
field and propagates it through (i) the per-pixel map-making, (ii) the
$\ell\!=\!1$ dipole estimator, and (iii) the per-pixel-shuffle null
calibration --- so $A_{50}/A_{95}$ are genuine end-to-end thresholds
\emph{for the observed-label field and the dipole estimator on it}. It
does \emph{not} pass a signal through the upstream image links: the ViT
classifier, the \NS{} triage, the $p_{\rm eq}$ confidence cut, or the
depth-/leg-/seeing-dependent confusion. Consequently $A_{50}\!\approx\!0.75\%$
and $A_{95}\!\in\!(1.0\%,1.5\%]$ are, as stated above, thresholds on the
\emph{observed} $f_{\rm CW}$ field; they are the operative falsification
quantities and we do not claim them as physical morphology-dipole
thresholds. The bridge from observed-label amplitude to underlying
physical amplitude is the single dilution factor $g$, and we make its
value robust to the asymmetric-confusion concern rather than assume
...
the $N$-dependent floor above), not the transfer slope $g$. \emph{What
remains genuinely outside this calibration --- and requires new
simulation, not a table reconciliation --- is a full image-level
end-to-end injection}: applying an image-plane chirality transformation
to the raw galaxy cutouts, re-running the classifier, triage, and
confidence cut, and only then fitting the dipole, with a
\emph{spatially-varying} confusion model conditioned on depth, PSF,
morphology, and imaging leg. That test is the one calibration that would
convert the observed-label thresholds into a spatially-resolved physical
transfer function; it requires per-cutout image transforms at
$\sim\!10^{6}$-galaxy scale and a validated conditional confusion model.
\emph{We now perform the image-level end-to-end mirror-flip injection
directly.} Passing every galaxy image \emph{and} its mirror reflection
through the production ViT (\texttt{bamfai/galaxy-chirality-v2}, validation
accuracy $0.9369$) over the full $8.47\times10^{6}$-galaxy image source
measures the raw image-level chirality transfer function --- the fraction of
the $3.32\times10^{6}$ raw-head CW/CCW-classified spirals whose class flips
under reflection --- at $T_{\rm raw}=0.2303\pm0.0002$, stable across
original-confidence bins ($0.207$--$0.261$ from the $[0.9,0.99)$ to the
$[0,0.7)$ bin) and across the North/South hemisphere split ($T_{\rm
raw}^{N}\!=\!0.218$ vs.\ $T_{\rm raw}^{S}\!=\!0.251$, a $\lesssim\!0.033$
absolute variation). Crucially, the production Z2-equivariant TTA labeling
(\texttt{class\_eq}/$p_{\rm eq}$) is parity-antisymmetric at the image level
\emph{by construction}: the symmetrized probability vector satisfies mirror
antisymmetry exactly (maximum probability-level antisymmetry deviation $0.0$,
verified numerically image-by-image), while the corresponding argmax
flip-recovery rate is $T_{\rm eq}=0.9997$ --- the residual $2.6\times10^{-4}$
shortfall from unity arising solely from argmax ties
($p_{\rm CW}^{\rm eq}\!=\!p_{\rm CCW}^{\rm eq}$), not from any physical
handedness asymmetry --- on all $8.47\times10^{6}$ galaxies (artifact
\artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/e2e\_fullrun/e2e\_transfer\_function\_full.json}).
The net image-level handedness preference that a spatially-varying confusion
would have to organize into a coherent dipole is therefore \emph{absent by
TTA construction and now confirmed empirically image-by-image}, and the raw
transfer function itself carries no strong sky-position dependence (the
$\lesssim\!0.033$ N/S variation is a dilution-\emph{magnitude} effect,
symmetrized by the TTA, not a directional CW$\leftrightarrow$CCW asymmetry).
This converts the previously-flagged ``operative next step'' into a performed,
full-catalog measurement. What the image-level run does \emph{not} yet supply
is the finer per-pixel confusion model jointly conditioned on depth, PSF, and
morphology --- the hemisphere-level and imaging-leg stratifications bound but
do not fully resolve it --- so we continue to hold the operative falsification
claims to the observed hard-label field.
...
(the $+0.41\sigmaunit$ real-space null and the $A_{50}/A_{95}$ observed-label
thresholds), and treat the $g$-based physical-amplitude conversion as an
...
Edge-on galaxy contamination reduces effective sample size: the empirical axis-ratio cross-match of all $3{,}201{,}160$ classified spirals against DR8-sweep morphology finds $f_{\rm edge}\!=\!15.80\%$ ($505{,}889$ spirals with $b/a\!<\!0.3$) mislabelled CW/CCW rather than \NS. Because these flip-symmetric contaminants carry zero mean asymmetry yet are \emph{retained} in $N_{\rm spiral}$ (they are not excluded), the naive per-pixel $f_{\rm CW}$ estimator used here dilutes the recovered dipole amplitude \emph{linearly}, $A_{\rm obs}\!=\!(1-\delta)A_{\rm phys}$, so the physical-amplitude sensitivity floor inflates by $(1-\delta)^{-1}-1\!=\!18.8\%$ for $\delta\!=\!0.158$ (the smaller Fisher-optimal $(1-\delta)^{-1/2}-1\!=\!9.0\%$ would apply only to an estimator that excludes or inverse-variance-down-weights the edge-on population; we adopt the larger, more conservative linear value), replacing the earlier qualitative $5$--$8\%$ bound. The equivariant averaging mitigates but does not eliminate this contamination (it renders it a pure sensitivity dilution, not a directional bias); full empirical edge-on analysis and the threshold sweep are in Appendix~E. LSST extrapolations and spectroscopic-redshift upgrade paths are deferred to a future matched-footprint analysis.

\subsection{Relation to Parity-Violating Sectors}
\label{sec:parity_translation}

The morphological-chirality dipole null constrains the late-universe, projected, morphology-channel observable at $z\!\lesssim\!1$. The $\ell\!=\!1$ dipole observable is parity-\emph{even} (isotropy-breaking axial-vector, not a direct parity-violation test); the parity-odd signal lives in the $\ell\!=\!0$ monopole and even-$\ell$ multipoles. A mapping onto primordial parity-violating tensor amplitudes requires a transfer function from the primordial chiral-tensor signal through galaxy formation to the late-universe projected morphology channel; that transfer function is not derived in this paper and is left to follow-up theory work. Among the early-universe parity-violating scenarios most directly relevant to a late-universe morphology-channel dipole floor at $\sim\!0.75\%$: \emph{cosmic-birefringence / CMB parity-violation} constraints~\cite{LueWangKamionkowski:1999,Eskilt2022,Eskilt2023Cosmoglobe} bound parity-odd physics in the polarization channel (Lue, Wang \& Kamionkowski established the CMB rotation/parity-violation observable; Eskilt \& Komatsu and the Cosmoglobe analyses report the current isotropic cosmic-birefringence limits from WMAP/Planck data), any underlying mechanism for which would generically also break handedness isotropy in the late-universe morphology channel; and \emph{gravitational Chern-Simons} gravity~\cite{Jackiw:2003} modifies the gravitational-wave dispersion relation in a parity-asymmetric way that would preferentially align galactic angular momenta. These mechanisms predict a non-zero morphology-channel dipole in principle; the $0.75\%$ floor established here bounds the amplitude of such signals for these scenarios in the DESI footprint, pending a derived transfer function. The present null is sensitive at the $50\%$-recovery level to models predicting a late-universe morphology-channel dipole $A\!\gtrsim\!0.75\%$ on the DESI Legacy footprint; the $95\%$-recovery threshold (detection efficiency, not a frequentist upper limit) is $A\!\gtrsim\!A_{95}\in(1.0\%,1.5\%]$ (Table~\ref{tab:injection_recovery}). The Shamir $\sim\!2$--$4\%$ amplitude class is in tension at the amplitude level by a factor of $\sim\!3.7$--$8.8$ under the present pipeline, comparing both amplitudes in the same $A_p$ units: our canonical joint nuisance-marginalized WLS best-fit is $0.455\%$ in $A_p$ units (Appendix~\ref{sec:app_systematic}), and Shamir's reported $1.7\%$--$4.0\%$ is already an \emph{asymmetry} $(N_{\rm CW}-N_{\rm CCW})/(N_{\rm CW}+N_{\rm CCW})$, which is identical to our $A_p\!=\!2(f_{\rm CW}\!-\!0.5)$ (a full-count asymmetry, not an $f_{\rm CW}$-deviation), so it maps to $1.7\%$--$4.0\%$ in $A_p$ units directly, without the factor-of-two rescaling. This is an amplitude-level tension under our pipeline, not a frequentist exclusion of Shamir's Ganalyzer estimator (Sec.~\ref{sec:comparison}); a matched-footprint Ganalyzer reanalysis on the same DESI Legacy footprint and cuts remains the cleanest path to a formal likelihood-level exclusion of that estimator.
...
\begin{table*}[t]
\caption{\label{tab:gz1_stratified}GZ1-overlap human-vs-classifier CW/CCW confusion stratified by imaging leg and confidence cut ($N\!=\!40{,}987$ GZ1-confident spirals classified CW or CCW; $1''$ cross-match, $N\!=\!46{,}017$). ``Asymmetry'' $=\mathrm{err}(\mathrm{CW}\!\to\!\mathrm{CCW})-\mathrm{err}(\mathrm{CCW}\!\to\!\mathrm{CW})$ with an unpooled two-proportion $95\%$ CI --- the only channel that biases $f_{\rm CW}/A_p$; a value consistent with zero means the classifier's parity error cannot manufacture a dipole in that stratum. Rates are Wilson-CI point estimates; full arrays in \artifact{pipelines/p2\_chirality/outputs/gz1\_stratified\_confusion.json}.}
\begin{ruledtabular}
\begin{tabular}{lrccc}
...
\emph{Conclusion this licenses (stated precisely).} The CW$\leftrightarrow$CCW error \emph{asymmetry} --- the only confusion channel that biases $f_{\rm CW}/A_p$ --- is consistent with zero in \emph{every} tested stratum: overall $+0.0001\,[-0.0054,+0.0056]$, BASS+MzLS $+0.0042\,[-0.0056,+0.0140]$, DECaLS $-0.0020\,[-0.0086,+0.0046]$, and the science-cut sample $-0.0019\,[-0.0061,+0.0023]$. Because the two imaging legs are the dominant axis of spatial variation in this survey's depth/PSF/system, a leg-symmetric, parity-symmetric error \emph{cannot manufacture a dipole at the $A_p$ scale of the null} (the science-cut asymmetry is bounded to $\lesssim\!0.6$~pp, and even the noisiest single stratum, BASS+MzLS, to $\lesssim\!1.4$~pp; these CI half-widths are the operative bound and do \emph{not} exclude a sub-percent differential asymmetry). What \emph{does} vary spatially is the error \emph{magnitude} (parity-symmetrically): three-class accuracy rises from $0.724$ at $p_{\rm eq}\!\le\!0.6$ to $0.961$ in the science cut, and the leg magnitudes differ significantly ($\mathrm{err}(\mathrm{CW}\!\to\!\mathrm{CCW})$ differs BASS+MzLS vs.\ DECaLS at $z\!=\!4.2$, $p\!=\!3\!\times\!10^{-5}$). That magnitude variation affects only \emph{dilution}, which is already carried conservatively by the calibration-free transfer factor $g=2a-1$ (Sec.~\ref{sec:sensitivity}) --- it does not induce a directional bias, because at fixed parity-symmetry the induced $\Delta f_{\rm CW}$ per stratum is consistent with zero ($+0.0014\,[-0.0037,+0.0066]$ BASS+MzLS; $+0.0033\,[-0.0001,+0.0067]$ DECaLS). This is a leg-granular realization of the spatially-varying confusion model previously flagged as unavailable in Sec.~\ref{sec:sensitivity}; the finer per-pixel conditional confusion model (on depth, PSF, and morphology simultaneously) remains the honest residual gap, which the two-leg split bounds but does not fully close. \emph{Two scope caveats stated explicitly.} First, this is a two-cell ($\mathrm{dec}\!\gtrless\!+32^\circ$) projection, not a full dipole map: a differential-error dipole \emph{not} aligned with the leg-split axis (e.g.\ an RA-varying asymmetry \emph{within} a leg) is bounded only through the coarser overall-stratum CI ($\pm0.56$~pp), not resolved; the two-leg decomposition is the dominant-axis instance of, not a substitute for, the per-pixel model. Second, the high stratified accuracies here ($0.912$ overall, $0.961$ science-cut) are the confident-spiral $\cap$ classifier-CW/CCW accuracies on the leg-stratified GZ1 overlap and are \emph{not} a revision of the headline conservative GZ1 chirality-accuracy floor ($0.6991$, $\kappa\!=\!0.40$; Training Labels, Sec.~\ref{sec:data}) that feeds the dilution factor $g=2a-1$: the two quantities are measured on distinct (nested) subsamples (confident spirals with \NS{} triaged out here vs.\ the full GZ1$\times$catalog match there, of which the here-sample is a high-confidence subset) and the conservative $0.6991$ floor is retained unchanged for all downstream isotropy bounds.
...
Fitting the canonical-mask $A_p$ field by galaxy-count-weighted linear regression to a 9-template design matrix (primordial-dipole basis $\{\hat x,\hat y,\hat z\}$ + imaging-leg fractions + pixel-density + pixel-density$^2$ + constant; Table~\ref{tab:wls_fit}): the joint fit recovers $A_{\rm dipole}^{\rm best}\!=\!4.55\!\times\!10^{-3}$ in $A_p$ units ($0.23\%$ in $f_{\rm CW}$ units). The block-bootstrap at NSIDE$\,=\,8$ ($N_{\rm boot}\!=\!1000$, 440 super-pixels; per iteration the 440 in-mask super-pixels are resampled \emph{with replacement}, the chosen super-pixels' member NSIDE$\,=\,$64 pixels are concatenated with their original galaxy-count weights, and the full 9-template WLS fit is repeated on that resample), which respects spatial coherence, inflates $\sigma(A_{\rm dipole})$ from the naive WLS $1.11\!\times\!10^{-4}$ to $1.63\!\times\!10^{-3}$ ($14.7\times$); against the interpretation~(i) reference amplitude $1.7\%$ --- Shamir's reported \emph{asymmetry} $(N_{\rm CW}-N_{\rm CCW})/(N_{\rm CW}+N_{\rm CCW})$, which is identical to our $A_p$, so $A_{\rm ref}\!=\!0.017$ in $A_p$ units with no factor-of-two rescaling --- \textbf{the primary exclusion statistic is the block-bootstrap $z\!\approx\!-7.6$} (the full block-bootstrap null distribution of $A_{\rm dipole}$, rendered from the committed percentile array, is shown in Fig.~\ref{fig:bootstrap_null}).\footnote{The NSIDE$\,=\,$8 block scale ($\sim\!7^\circ$ pixels, 440 super-pixels in mask) was chosen to preserve spatial coherence on angular scales $\gtrsim\!5^\circ$ characteristic of the imaging-leg systematic structures (BASS+MzLS/DECaLS boundary scales $\sim\!5$--$10^\circ$; PSF variation scale $\sim\!3^\circ$) while maintaining adequate super-pixel statistics for a reliable bootstrap covariance estimate. At NSIDE$\,=\,$4 ($\sim\!15^\circ$ pixels) the number of super-pixels would fall to $\sim\!110$, insufficient for a 9-parameter fit; at NSIDE$\,=\,$16 ($\sim\!3.5^\circ$ pixels) the block scale falls below the PSF coherence length and the inflation factor would underestimate the spatial covariance. The NSIDE$\,=\,$8 choice is therefore the natural block scale for this systematic family. A block-scale sensitivity check at NSIDE$\,\in\!\{4,8,16\}$ was computed on the same catalog and 9-template design ($N_{\rm boot}\!=\!500$ per scale, seed 42): the primary exclusion statistic is $z\!=\!-7.1$ (NSIDE$\,=\!4$, $\sim\!127$ super-pixels), $z\!=\!-7.8$ (NSIDE$\,=\!8$, $\sim\!439$ super-pixels), and $z\!=\!-8.2$ (NSIDE$\,=\!16$, $\sim\!1631$ super-pixels), with inflation factors $15.7\times$, $14.4\times$, and $13.7\times$ respectively. (The NSIDE$\,=\,$8 sensitivity-check value $z\!=\!-7.8$ here differs from the primary $z\!\approx\!-7.6$ only because this cross-scale check used $N_{\rm boot}\!=\!500$ per scale for uniformity, whereas the primary statistic uses $N_{\rm boot}\!=\!1000$; the $\sim\!0.2$ difference is bootstrap-sample-size noise on $\sigma_{\rm boot}$.) The primary exclusion $|z|\!\geq\!7$ is stable across all three block scales (artifact \artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/block\_bootstrap\_nside\_sensitivity.json}).}\footnote{The naive WLS posterior gives $z\!=\!-112$ (9-template fit; $z\!\approx\!-106$ for the extended 24-template fit), but these values ignore spatial coherence in the residuals and are superseded by the block-bootstrap covariance; we do not quote them as exclusion significances.} Interpretation~(i) at $A\!=\!1.7\%$ is strongly disfavored under the spatial-coherence-respecting bootstrap covariance. To be explicit: this $z$ quantifies tension with a clean $1.7\%$ template under the spatial error model validated in Appendix~D and is conservative with respect to classifier dilution; it is not a calibrated frequentist exclusion significance. An extended 24-template fit (adding 15 leg $\times$ confidence-bin interaction templates) yields an essentially unchanged dipole posterior ($A_{\rm dipole}^{\rm best}\!=\!4.51\!\times\!10^{-3}$), confirming robustness to nuisance-template granularity. \emph{Scope of the block-bootstrap error model.} The block bootstrap resamples in-mask super-pixels with replacement, so its $\sigma_{\rm boot}=1.63\times10^{-3}$ propagates the spatial (cosmic-variance-like and depth-systematic) covariance of the \emph{measured} $A_p$ field but does \emph{not} separately inflate the error for per-galaxy classifier-label uncertainty (the finite-accuracy misclassification noise). That channel enters the analysis instead through the empirical injection-recovery floor, where the $g\!=\!2a\!-\!1$ dilution and the full per-pixel binomial label-shuffle null already fold classification noise into the $A_{50}/A_{95}$ thresholds (Sec.~\ref{sec:sensitivity}); it acts to \emph{dilute} any real dipole toward null, so omitting it from $\sigma_{\rm boot}$ makes the $z\!\approx\!-7.6$ disfavor of a clean $1.7\%$ template \emph{conservative} against a diluted true signal, not overstated. \emph{Effective joint bound on the three nuisance channels.} Taken together the paper therefore already brackets the combined effect of the three systematic channels a reviewer would want marginalized simultaneously --- classifier confidence, imaging depth, and morphology --- albeit through two coupled diagnostics rather than a single closed-form nuisance likelihood: the depth and morphology channels are marginalized \emph{jointly and in one fit} here (the 9- and 24-template WLS designs carry pixel-density, density$^2$, and imaging-leg $\times$ confidence-bin morphology-proxy templates simultaneously, with $A_{\rm dipole}^{\rm best}$ stable at $4.5\!\times\!10^{-3}$ across both template granularities), while the classifier-confidence channel enters as the injection-recovery dilution floor and, independently, as the confidence-cut sweep of Sec.~\ref{sec:dipole} (the null verdict is stable across $p_{\rm eq}\!\in\!\{0.6,0.7,0.8\}$). Because each of the three channels is separately shown to move the estimator \emph{toward} null (confidence via dilution; depth/morphology via the toward-null direction of the survey-correlated bias, Sec.~\ref{sec:monopole_mask_null}), their combined worst-case effect cannot manufacture the primary null from a hidden $\gtrsim\!1.7\%$ signal; the bound is therefore conservative under joint variation of all three. \emph{Caveat: what is \underline{not} yet done} is a single simultaneous likelihood that co-varies the cosmological dipole amplitude against confidence-, depth-, and morphology-dependent nuisance parameters within one covariance --- e.g.\ a Gaussian-process spatial likelihood with per-galaxy classifier-noise propagation. That fully-simultaneous marginalization is a genuine extension (it requires the per-galaxy DR8 morphology/depth pull described in Sec.~\ref{sec:conclusions} and a joint MCMC over the coupled nuisance covariance) and is reported as future work; the present coupled-diagnostic bound is an upper envelope on its result, not a substitute for the formal joint posterior. The block-bootstrap $z$ is therefore a template-model-disfavor statistic under the spatial error model, not a calibrated detection significance, and is reported as such throughout (``disfavors,'' not ``excludes at $7.6\sigma$''). A conditioning audit quantifies the leg-template collinearity noted in the Table~\ref{tab:wls_fit} caption: the three centered leg-fraction templates sum identically to zero on every galaxy-weighted pixel, so $X^{T}WX$ is exactly rank-8 (condition number $4.5\!\times\!10^{16}$); the degeneracy is confined to the nuisance subspace, and SVD-pseudoinverse, explicit leg-drop (condition number $1.2\!\times\!10^{4}$), and weighted Gram--Schmidt-orthogonalized-nuisance refits all reproduce $A_{\rm dipole}^{\rm best}\!=\!4.55\!\times\!10^{-3}$ to machine precision (artifact \artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/c12b\_wls\_conditioning.json}).

\begin{table}[t]
\caption{Joint nuisance-marginalized WLS template fit on the canonical-mask $A_p$ field (9-template design; coefficients in $A_p$ units with naive-WLS $1\sigma$ errors; $z=\hat a/\sigma$). The three imaging-leg fraction templates sum identically to zero on every galaxy-weighted pixel, so they are exactly collinear with the constant template: $X^{T}WX$ is rank-deficient (rank-8, condition number $4.5\!\times\!10^{16}$), and the three leg $z$ values ($\sigma\!\approx\!6\times10^{2}$) are \emph{not} individually meaningful. This rank-deficiency is confined to the nuisance subspace and does \emph{not} affect the dipole recovery: an explicit one-leg-drop refit yields a well-conditioned system (condition number $1.2\!\times\!10^{4}$) and reproduces $A_{\rm dipole}^{\rm best}\!=\!4.55\!\times\!10^{-3}$ to machine precision (as do SVD-pseudoinverse and Gram--Schmidt-orthogonalized-nuisance refits; Appendix~D text, artifact \artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/c12b\_wls\_conditioning.json}). The bottom rows give the marginalized dipole-amplitude posterior and the primary block-bootstrap exclusion; the naive-WLS width $\sigma_{\rm naive}=1.11\times10^{-4}$ and exclusion $z=-112$ are listed only so the accompanying footnote is reproducible from the table, and are superseded by the block-bootstrap values.\label{tab:wls_fit}}
\begin{ruledtabular}
\begin{tabular}{lrrr}
...
$z$ vs.\ $A_{\rm ref}=0.017$ & \multicolumn{3}{r}{$-7.6$ (block-boot., primary)} \\
 & \multicolumn{3}{r}{$-112$ (naive WLS, superseded)} \\
\end{tabular}
\end{ruledtabular}
\textsuperscript{$\dagger$}\emph{Convention mapping:} the fitted $A_{\rm dipole}^{\rm best}\!=\!4.55\!\times\!10^{-3}$ is in $A_p\!=\!2(f_{\rm CW}\!-\!\tfrac12)$ units (asymmetry-$A$ convention used in the injection-recovery sections); the equivalent full-amplitude $A$ is identical numerically ($A\!=\!A_p$ for a dipole: $p_{\rm CW}(\hat n)\!=\!\tfrac12(1\!+\!A\cos\theta)$ implies the dipole amplitude in $f_{\rm CW}$ deviation equals $A/2$, while $A_p\!=\!2(f_{\rm CW}\!-\!\tfrac12)$ implies $A_p\!=\!A$ for a pure dipole field centered at $0.5$). The falsification boundary $A_{95}\!\in\!(1.0\%,1.5\%]$ and sensitivity floor $A_{50}\!\approx\!0.75\%$ from Table~\ref{tab:injection_recovery} are therefore directly comparable to $A_{\rm dipole}^{\rm best}\!=\!0.455\%$ in this table: all three are in the same full-amplitude-$A$ units.
\end{table}
...
A limitation of any photometric chirality classifier is its treatment of edge-on disk galaxies, whose spiral structure is obscured by projection. We now quantify this contamination empirically rather than qualitatively. We pull the per-galaxy DESI Legacy DR8-sweep morphology for all $3{,}201{,}160$ classified spirals ($100\%$ \texttt{dr8\_id} match; axis ratio $b/a\!=\!(1-|e|)/(1+|e|)$ with $|e|\!=\!\sqrt{e_1^2+e_2^2}$ from the type-appropriate DEV/EXP shape parameters \texttt{SHAPEDEV\_E1,E2}/\texttt{SHAPEEXP\_E1,E2}, DEV shape for \texttt{TYPE}$\in\{$DEV,COMP,SER$\}$ or \texttt{FRACDEV}$\!\ge\!0.5$ else EXP) and measure the axis-ratio distribution of the sample that actually enters the dipole. The measured edge-on fraction is $f_{\rm edge}\!=\!15.80\%$ ($505{,}889$ of $3{,}201{,}160$ classified spirals with $b/a\!<\!0.3$): $15.8\%$ of the galaxies feeding the dipole are edge-on systems mislabelled CW/CCW rather than \NS. This is the empirical replacement for the previous qualitative $\sim\!5$--$8\%$ estimate. However, the equivariant averaging enforces flip-equivariance of the soft-probability protocol, so for any galaxy whose mirror image is morphologically indistinguishable from the original (as for edge-on disks) the ensemble-mean CW and CCW probabilities are flip-symmetric. The primary effect is therefore a dilution of sensitivity, not a bias; its magnitude, however, depends on the estimator. The primary dipole uses the \emph{naive} per-pixel $f_{\rm CW}$ on the full classified-spiral sample, in which the edge-on systems are \emph{retained} in $N_{\rm spiral}$ (mislabelled CW/CCW, not excluded). A flip-symmetric edge-on fraction $\delta\!=\!f_{\rm edge}\!=\!15.8\%$ carries zero mean CW$-$CCW asymmetry, so it contributes $50/50$ under hard argmax and dilutes the recovered per-pixel amplitude \emph{linearly}: $\mathbb{E}[A_p]\!=\!(1-\delta)A_p^{\rm phys}$. The physical-amplitude sensitivity floor therefore inflates by $(1-\delta)^{-1}-1\!=\!18.8\%$, not by the Fisher-optimal $(1-\delta)^{-1/2}-1\!=\!9.0\%$: the sqrt scaling is the Cram\'er--Rao bound for an \emph{optimal} estimator that identifies and down-weights (or excludes) the zero-information edge-on population, whereas the naive $f_{\rm CW}$ estimator actually used incurs the full linear dilution. We adopt the larger, more conservative linear value. Crucially, \emph{neither} value changes the null verdict: the empirical injection-recovery floors of Sec.~\ref{sec:sensitivity} ($A_{50}\!\approx\!0.75\%$) are measured on the same edge-on-contaminated observed-label field as the observed dipole, so the like-for-like null comparison is unaffected and the penalty quantifies only the observed-to-physical amplitude transfer for the falsification floor (and is in any case largely subsumed by the primary $g\!=\!2a-1$ GZ1-accuracy dilution, whose $69.91\%$ human-label accuracy already reflects edge-on misclassification empirically). The measurement is robust to the edge-on threshold: $f_{\rm edge}\!=\!5.86\%/10.49\%/15.80\%/21.51\%/27.47\%$ at $b/a\!<\!0.20/0.25/0.30/0.35/0.40$, i.e.\ a linear floor inflation of $6.2\%/11.7\%/18.8\%/27.4\%/37.9\%$ across the plausible edge-on-cut range (\artifact{pipelines/p2\_chirality/outputs/edge\_on\_contamination\_metric.json}). \emph{Directional (dipole) bias from edge-on leakage is excluded by the equivariance argument, independent of $f_{\rm edge}$:} because flip-equivariant TTA (Eq.~\eqref{eq:tta}) forces $\langle p_{\rm CW}^{\rm eq}\rangle = \langle p_{\rm CCW}^{\rm eq}\rangle$ for any flip-symmetric morphology, an edge-on contaminant has zero expected CW$-$CCW asymmetry \emph{at every sky position}. A spatially varying edge-on fraction $f_{\rm edge}(\hat n)$ therefore modulates only the local \emph{dilution} (and hence the local noise amplitude $\sigma(A_p)$), not the local \emph{mean} $A_p$, so it can inflate the per-pixel variance but cannot project a coherent signal onto the $\ell\!=\!1$ dipole. The dipole estimator is built from hard argmax CW/CCW counts under this flip-symmetric assignment; the empirical $b/a$ cross-match reported above tightens the dilution magnitude (the measured $18.8\%$ conservative linear floor inflation) but cannot reintroduce a directional bias the equivariance has already symmetrized away. \emph{Caveat on the argmax step.} The equivariance identity $\langle p_{\rm CW}^{\rm eq}\rangle\!=\!\langle p_{\rm CCW}^{\rm eq}\rangle$ holds exactly for the soft probabilities; the hard-argmax operation is not itself linear, so on borderline galaxies (where $p_{\rm CW}^{\rm eq}\!\approx\!p_{\rm CCW}^{\rm eq}\!\approx\!0.4$ and the $Z_2\!\to\!D_4$ argmax flips in $21.4\%$ of cases, Sec.~\ref{sec:tta}) a per-galaxy directional bias could in principle survive symmetrization if the sign of the argmax tie-break were spatially coherent. It is not left unbounded: the per-pixel label-shuffle null (Sec.~\ref{sec:dipole}) preserves the observed per-pixel CW/CCW argmax counts and is therefore \emph{blind} to any spatially coherent argmax bias by construction, but the spatial coherence of the tie-break is now bounded \emph{directly}, not only through the confidence-cut sweep. Measuring the $\ell\!=\!1$ spatial dipole of the borderline tie-break band ($p_{\rm eq}\!\in\![0.5,0.6]$, which contains the argmax-flip population) per imaging leg against an isotropic label-shuffle null, the tie-break decisions are \emph{spatially isotropic} in the BASS$+$MzLS leg ($z\!=\!+0.31$, consistent with zero) and carry coherence only in the DECaLS leg ($z\!=\!+4.72$, family-wise $p\!=\!0.0086$ over the $15$-cell leg$\times$confidence grid). This leg-selectivity is the signature of a depth/imaging-correlated \emph{systematic} --- the same DECaLS-depth channel forward-modelled in Appendix~D --- not of an isotropic cosmological tie-break bias: a genuine directional bias would not track a single imaging leg. Decisively, this entire borderline population is \emph{already included} in the primary real-space Catalog~C dipole null ($+0.41\sigmaunit$, $p_{\rm 2\text{-}sided}\!=\!0.62$, Sec.~\ref{sec:dipole}), so any spatially-coherent argmax tie-break term is bounded below that null in the real-space estimator and cannot reintroduce a dipole. The confidence-cut sweep is consistent: raising the cut to $p_{\rm eq}\!>\!0.8$ removes the borderline population and leaves the real-space dipole null ($z\!=\!+0.51$), so any residual argmax-driven directional term is smaller than the $|z|\!<\!1.2$ high-confidence-regime scatter. The edge-on-\emph{isolated} single-number variant (the same tie-break dipole restricted to $b/a\!<\!0.30$ argmax-flips) is now measured directly: joining \texttt{catalog\_production} (\texttt{class\_eq}, RA/Dec) to the committed \texttt{spiral\_morphology\_dr8} $b/a$ on \texttt{dr8\_id}, the edge-on ($b/a\!<\!0.30$) borderline tie-break population $N\!=\!295{,}170$ is \emph{spatially isotropic}: the family-wise joint significance over the three imaging legs is $p\!=\!0.49$ (observed $\max|z|\!=\!1.17$ @ DES against a null $99$th percentile of $3.54$), with every per-leg $\ell\!=\!1$ dipole $|z|\!<\!1.4$ (BASS$+$MzLS $-0.23$, DECaLS $+0.71$, DES $+1.17$). Isolating to edge-on systems therefore removes the leg-selective coherence seen in the full borderline band (DECaLS $z\!=\!+4.72\!\to\!+0.71$): the argmax tie-break introduces \emph{no} directional dipole even in the edge-on-isolated slice, the strongest possible answer to the App~E coherence concern (\artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/edgeon\_isolated\_tiebreak\_coherence.json}, \artifact{pipelines/p2\_chirality/scripts/edgeon\_isolated\_tiebreak\_coherence.py}). The residual directional systematics are instead the depth/morphology-correlated channels of Appendix~D, which act through the survey-depth weight rather than through edge-on inclination.
