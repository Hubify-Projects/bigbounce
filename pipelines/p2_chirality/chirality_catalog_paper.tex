...
\newcommand{\paperVersion}{v1.0.206}
...
T5: Metadata leakage\footnote{The tabulated $|r|\!<\!0.04$ is a linear Pearson correlation against the raw RA coordinate, which is circular ($0^\circ\!\equiv\!360^\circ$); a linear $r$ can understate azimuthal coupling, so T5 as tabulated is \emph{not} counted as an independent pass criterion for directional coupling. The circular-coordinate-appropriate replacement is the map-level low-$\ell$ real-$Y_{\ell m}$ regression reported in the Appendix~B text (all three $\ell\!=\!1$ coefficients $|z|\!\leq\!1.25$), which supersedes this row for directional-leakage purposes. The tabulated result is the RA correlation $r\!=\!-0.038$ (Dec: $+0.008$; committed values, \artifact{pipelines/p2\_chirality/BIAS\_AUDIT\_REPORT.md}).} & $<0.04$ & $-0.038$ \\
