...
\newcommand{\paperVersion}{v1.0.225}
...
\begin{table}[t]
\caption{\textbf{Primary Results callout.} The two load-bearing claims of this paper (rows P1--P2) and, for reference, the non-primary harmonic diagnostics (rows D1--D2) that are \emph{systematics-attributed, not detections}. Each row names the estimator and the specific null procedure its $\sigma$/$z$ is computed against; the values in different rows come from \emph{incommensurable} null constructions and must \emph{not} be read as comparable detection significances. Only rows P1--P2 carry the paper's scientific conclusion.\label{tab:primary_callout}}
\begin{ruledtabular}
\footnotesize
\begin{tabular}{lll}
