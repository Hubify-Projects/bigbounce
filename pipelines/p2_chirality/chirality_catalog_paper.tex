...
\newcommand{\paperVersion}{v1.0.256}
...
\newcommand{\paperTimestamp}{July 15, 2026, 22:28 PDT}
...
the HC ($p_{\rm eq}\!>\!0.6$) real-space dipole is reduced to $+0.71\sigmaunit$ under the current fixed-occupancy primary null. Generation counts and the projected streaming read are recorded in \artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/fig7\_raw\_vs\_eq\_manifest.json}. This visual
...
CW$\leftrightarrow$CCW error asymmetry consistent with zero in every leg
and confidence stratum (leg asymmetries bounded to $\lesssim\!1.4$~pp,
...
The single global confusion matrix above (Table~\ref{tab:gz1_confusion}) does not by itself establish the spatial transfer needed by the dipole analysis. A spatially varying differential mislabelling rate can inject a spurious $A_p$ dipole, but it is not the only channel: Eq.~\eqref{eq:spatial_confusion_transfer} shows that a spatially varying symmetric rate also changes the observed fraction wherever the local selected CW fraction differs from one half. We therefore stratify the GZ1-overlap confusion matrix by the two P4 imaging legs (BASS+MzLS, $\mathrm{dec}\!>\!+32^\circ$; DECaLS, $\mathrm{dec}\!<\!+32^\circ$) and by the primary confidence cut, and in each stratum measure the CW$\leftrightarrow$CCW error asymmetry with its two-proportion $95\%$ CI (Table~\ref{tab:gz1_stratified}) as well as the net induced $\Delta f_{\rm CW}$. The analysis restricts to the $N\!=\!40{,}987$ GZ1-confident spirals ($\max(P_{\rm CW},P_{\rm ACW})\!>\!0.6$) that the classifier also labels CW or CCW, drawn from the $N\!=\!46{,}017$ GZ1--catalog cross-matches reproduced byte-identically ($1''$ KDTree match) from Table~\ref{tab:gz1_confusion}'s parent match; the $5{,}030$ GZ1-confident spirals assigned to \NS{} are reported separately as an unquantified completeness/selection channel, not as chirality error. The generator script and full stratified JSON are committed (\artifact{pipelines/p2\_chirality/scripts/gz1\_stratified\_confusion.py}, \artifact{pipelines/p2\_chirality/outputs/gz1\_stratified\_confusion.json}).
...
\emph{Conclusion this licenses (stated precisely).} CW$\leftrightarrow$CCW error asymmetry is consistent with zero in every tested stratum: overall $+0.0001\,[-0.0054,+0.0056]$, BASS+MzLS $+0.0042\,[-0.0056,+0.0140]$, DECaLS $-0.0020\,[-0.0086,+0.0046]$, and science cut $-0.0019\,[-0.0061,+0.0023]$. Thus no directional error asymmetry is detected at the tested two-leg resolution, with sensitivity bounded by these intervals. The net induced-fraction point estimates are nevertheless 0.00142 (BASS+MzLS) and 0.00331 (DECaLS); their difference has no retained covariance-aware interval. The test does not resolve RA variation within a leg or joint depth/PSF/morphology dependence, so it cannot exclude every sub-percent spatially varying differential or symmetric-error pattern. The finer conditional model remains open. Stratified accuracies apply to a nested confident-spiral subset and do not validate the illustrative scalar transfer, especially because the historical training overlap was not removed from the GZ1 agreement calculation.
