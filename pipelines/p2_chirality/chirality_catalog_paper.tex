...
\newcommand{\paperVersion}{v1.0.238}
...
(\texttt{class\_eq}/$p_{\rm eq}$) is \emph{exactly parity-antisymmetric at
the image level}: the mirror-flip transfer under TTA is $T_{\rm eq}=0.9997$
with maximum antisymmetry deviation $0.0$, verified numerically image-by-image
on all $8.47\times10^{6}$ galaxies (artifact
