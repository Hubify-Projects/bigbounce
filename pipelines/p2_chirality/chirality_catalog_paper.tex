...
\newcommand{\paperVersion}{v1.0.250}
...
\newcommand{\paperTimestamp}{July 15, 2026, 05:36 PDT}
...
\begin{abstract}
We release observed chirality labels for $8{,}474{,}531$ DESI Legacy DR8 galaxies and test their large-scale dipole. The content-addressed public bundle contains the science catalog, an unsafe-row quarantine, the retained primary-null array, schema, checksums, and a reproducer. Our single primary estimator is a real-space dipole on the high-confidence selection ($p_{\rm eq}>0.6$; $N_{\rm selected}=949{,}584$), with $N_{\rm support}=947{,}326$ galaxies entering the supported-pixel fit and fixed-occupancy label-randomization null. It is consistent with zero at $+0.71\sigmaunit$ (one-sided upper-tail rank $p=0.225$). The release omits raw and reconstructed flip-pass scores from the science product and quarantines 249,066 reconstruction violators (59,515 in the high-confidence selection) as uncalibrated diagnostics. A paired-image-to-field surrogate over 20 deterministic axes is reported only as a pilot grid; it is neither a calibrated recovery curve nor a physical upper bound. WLS and post-MASTER harmonic results use different fields, supports, or nulls and remain systematics diagnostics. Spatially resolved transfer calibration, a matched-footprint independent estimator, joint real-space--harmonic covariance, a DOI-backed source archive, and independent preregistration or blinding remain open. Because the $\ell=1$ morphology observable is parity-even, we claim no primordial parity bound.
\end{abstract}
...
(ii)~WLS template excl. & $3{,}201{,}160$ & --- & $0.49005$ & \FSC{} & block-boot.\ ($10^3$) & $z\!\approx\!-7.6$ \\
...
$2a-1=s_{\rm CW}+s_{\rm CCW}-1$ identically. This algebra is conditional
...
The single global confusion matrix above (Table~\ref{tab:gz1_confusion}) does not by itself establish the spatial transfer needed by the dipole analysis. A spatially varying differential mislabelling rate can inject a spurious $A_p$ dipole, but it is not the only channel: Eq.~\eqref{eq:spatial_confusion_transfer} shows that a spatially varying symmetric rate also changes the observed fraction wherever the local selected CW fraction differs from one half. We therefore stratify the GZ1-overlap confusion matrix by the two P4 imaging legs (BASS+MzLS, $\mathrm{dec}\!>\!+32^\circ$; DECaLS, $\mathrm{dec}\!<\!+32^\circ$) and by the primary confidence cut, and in each stratum measure the CW$\leftrightarrow$CCW error asymmetry with its two-proportion $95\%$ CI (Table~\ref{tab:gz1_stratified}) as well as the net induced $\Delta f_{\rm CW}$. The analysis restricts to the $N\!=\!40{,}987$ GZ1-confident spirals ($\max(P_{\rm CW},P_{\rm ACW})\!>\!0.6$) that the classifier also labels CW or CCW, drawn from the $N\!=\!46{,}017$ GZ1--catalog cross-matches reproduced byte-identically ($1''$ KDTree match) from Table~\ref{tab:gz1_confusion}'s parent match; the $5{,}030$ GZ1-confident spirals the classifier calls \NS{} are a completeness effect (chirality-neutral in $A_p$, which has a spirals-only denominator) and are reported separately, not as chirality error. The generator script and full stratified JSON are committed (\artifact{pipelines/p2\_chirality/scripts/gz1\_stratified\_confusion.py}, \artifact{pipelines/p2\_chirality/outputs/gz1\_stratified\_confusion.json}).

The parenthetical ``chirality-neutral'' description in the historical sentence above is withdrawn. The 5,030 omitted GZ1-confirmed spirals have not been composition- and position-audited here, so their exclusion is an unquantified completeness/selection channel. All confusion-matrix conclusions are restricted to the 40,987 objects classified CW or CCW.
...
This appendix audits eight diagnostics historically associated with the \FSC{} residual. Only apodized-mask robustness and multipole-spectrum coherence are bound to the declared 24,087-pixel \FSC{} base support. Quality-quartile stratification constructs separate per-quartile masks; the leg proxy uses 24,270 pixels; boundary-distance variance uses 35,438 latitude-mask pixels; the density null and WLS/bootstrap use broader latitude-cut supports; and the direct cross-spectrum artifact does not record enough mask provenance to establish identity. Those six calculations are retained only as explicitly out-of-scope provenance and do not support the strict \FSC{} synthesis.
...
\paragraph{WLS mask audit.}
The committed bootstrap artifact explicitly records 24,187 pixels under $(|b_{\rm gal}|>15^\circ)\land(N_{\rm total}>0)$, whereas \FSC{} contains 24,087 pixels under $N_{\rm spiral}\geq10$. The former mask-equivalence assertion is therefore withdrawn; a new \FSC{} bootstrap would be required before this diagnostic could re-enter the synthesis.
...
\begin{itemize}
\item \textbf{Catalog:} \url{https://huggingface.co/datasets/bamfai/galaxy-chirality-catalog} (CC-BY-4.0, Parquet). The original source is pinned to revision \texttt{a21eb596fd10edb9af9e7a1bcefb04f87327a724}, 952,115,239 bytes, and SHA-256 \texttt{e8525ba5c98576f6...3563}. The current ApJS bundle is public at immutable HF data revision \href{https://huggingface.co/datasets/bamfai/galaxy-chirality-catalog/tree/db11023306ab4eed1d7727670bd78e127b7af17a/apjs-release/v1.0.244}{\texttt{db110233}} with provider receipt \href{https://huggingface.co/datasets/bamfai/galaxy-chirality-catalog/blob/e535b26247c892971963be6029435544cf29d19b/apjs-release/v1.0.244/PROVIDER_RECEIPT.json}{\texttt{e535b262}} and manifest SHA prefix \texttt{c2e5404a}. The primary fixed-occupancy array (SHA prefix \texttt{f6360f4b}) and pixel-permutation diagnostic (\texttt{62bb1c01}) have distinct filenames; full hashes are in the public manifest. The superseded initial binding was data commit \texttt{58ecc795a0aa8dda566a28a5adda76a47f3c8942} with receipt commit \texttt{5a322faaed865ae35dae181fc0ff3560ee56383e}. The science-facing Parquet omits raw/reconstructed columns and includes Boolean \texttt{raw\_flip\_qc\_unsafe}; a separate provenance-only quarantine contains all 249,066 catalog-wide violators (59,515 in HC), each marked \texttt{do\_not\_use\_for\_science=True}. Safe filters are \texttt{primary\_hc == True} for the declared hard-label primary and \texttt{primary\_hc == True and raw\_flip\_qc\_unsafe == False} for the strict diagnostic. The machine contract is \artifact{pipelines/p2\_chirality/apjs\_release\_schema\_v1\_0\_244.json}; the human dictionary, builder, tests, and minimal primary-null reproducer are \artifact{pipelines/p2\_chirality/CATALOG\_SCHEMA.md}, \artifact{pipelines/p2\_chirality/build\_apjs\_release\_v1\_0\_244.py}, \artifact{pipelines/p2\_chirality/tests/test\_apjs\_release\_v1\_0\_244.py}, and \artifact{pipelines/p2\_chirality/reproduce\_p4\_primary\_null\_v1\_0\_244.py}. The public bundle also contains the retained 10,000-draw primary-null array, checksums, validation report, schema, and minimal reproducer. A DOI-backed immutable archive of the paper, source, figures, and catalog-release metadata remains open; no placeholder DOI is claimed.
