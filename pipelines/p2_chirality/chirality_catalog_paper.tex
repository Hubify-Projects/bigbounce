...
\newcommand{\paperVersion}{v1.0.249}
...
\newcommand{\paperTimestamp}{July 15, 2026, 05:17 PDT}
...
\begin{abstract}
We publish an ApJS-oriented catalog release for $8{,}474{,}531$ DESI Legacy DR8 galaxies and test the observed-label chirality field. The content-addressed public bundle contains the science-facing catalog, an explicit unsafe-row quarantine, the retained primary-null array, schema, checksums, and reproducer. The single primary estimator is the real-space dipole of the declared high-confidence selection ($p_{\rm eq}>0.6$, $N_{\rm selected}=949{,}584$), of which $N_{\rm support}=947{,}326$ galaxies enter the supported-pixel fit and fixed-occupancy galaxy-label-randomization null. That estimator is null-consistent at $+0.71\sigmaunit$ (one-sided upper-tail rank $p=0.225$). The release contract removes raw-pass and reconstructed flip-pass scores from the science-facing product and quarantines all 249,066 catalog-wide reconstruction violators (59,515 in HC) as explicitly uncalibrated diagnostics. A paired-image-to-field surrogate over 20 deterministic axes is reported only as a pilot grid: the HC score-crossing counts are $1/20$, $16/20$, $16/20$, $20/20$, and $20/20$ at full amplitudes $A=A_p=0.5\%$, $0.75\%$, $1.0\%$, $1.5\%$, and $2.0\%$. These fractions are not a calibrated population recovery curve or physical upper bound; $A=1.5\%$ corresponds to a $0.75\%$ peak deviation of $f_{\rm CW}$ from one half. WLS and post-MASTER harmonic results are diagnostics on different fields and samples. A spatially resolved confusion/transfer model, matched-footprint independent-estimator comparison, joint real-space$\times$harmonic covariance, a DOI-backed paper/source archive, and independent external preregistration or blinding remain open. The $\ell=1$ observable is parity-even; without a transfer function we claim no primordial parity bound.
\end{abstract}
