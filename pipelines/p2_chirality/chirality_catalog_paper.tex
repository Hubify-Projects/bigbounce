...
\newcommand{\paperVersion}{v1.0.257}
...
\newcommand{\paperTimestamp}{July 15, 2026, 23:48 PDT}
...
In this paper we present an 8.47-million-object public chirality catalog release whose novelty relative to CE-ResNet is scale, a dedicated \NS{} class, a machine-readable release contract, and a multi-axis bias-hardening audit. This catalog-and-methods emphasis, including a compact schema, executable selection, checksummed null, and explicit quarantine, is the intended ApJS venue fit. Committed historical training records conflict on exact composition: one reconstruction gives 26,616 rows and 826 CE-ResNet-selected non-spirals (67.5\% CE-derived), whereas \texttt{BENCHMARK\_REPORT.md} gives 26,626 and 846; both accounts establish substantial CE-ResNet dependence, so the catalogs are not fully independent and model-output permutation nulls cannot test inherited survey-correlated structure by themselves. Our sole primary result is the HC real-space observed-label null ($N_{\rm selected}=949{,}584$, $N_{\rm support}=947{,}326$, $z=+0.71$, one-sided upper-tail $p=0.225$). The WLS and MASTER results are systematics-sensitive diagnostics, not independent detections or physical exclusions. The GZ1 human-only test uses human votes rather than learned chirality labels and corroborates the null at lower statistical power; it does not tighten the HC amplitude constraint.
...
\item \emph{Moment-$z$ / rank-$p$}: the real-space dipole pair ($z_{\rm mom}=+0.71$, one-sided upper-tail rank $p=0.225$) are distinct summary statistics of the $A_{\rm dip}$ distribution against the fixed-occupancy galaxy-label-randomization null; they do not follow the Gaussian $z\!\to\!p$ mapping.
...
We assign stable names to the three distinct supports used below: \texttt{HC-REALSPACE-INCLUSIVE} (\HCRI{}; HC rows with $N_{\rm spiral}(p)\!\geq\!10$, 23,682 pixels), \texttt{FULL-SPIRAL-CANONICAL} (\FSC{}; all spirals with $N_{\rm spiral}(p)\!\geq\!10$, 24,087 pixels), and \texttt{MASTER-ALL-GALAXY-FOOTPRINT} (\MAGF{}; $N_{\rm all}(p)\!\geq\!1$, 24,297 pixels); Table~\ref{tab:fsky_summary} consolidates their mask and weighting conventions. These names prevent the primary HC support from being conflated with either full-sample diagnostic footprint. We declare the estimator hierarchy used throughout. This hierarchy --- with the real-space dipole as the row-(i) primary cosmological estimator --- is documented in Appendix~A; the primacy of the real-space estimator is established independently of the harmonic-channel diagnostics. The primary dipole measurement uses the high-confidence selection (Catalog~C with winning-class confidence $p_{\rm eq}\!>\!0.6$; $N_{\rm selected}\!=\!949{,}584$ spirals), of which $N_{\rm support}\!=\!947{,}326$ enter the 23,682 supported pixels used by the estimator and null. This cut suppresses the depth-correlated low-confidence-tail systematic identified in Sec.~\ref{sec:dipole}; all estimator-hierarchy and sensitivity statements below are made on this HC selection unless explicitly noted. We declare the HC $p_{\rm eq}\!>\!0.6$ subsample as the \emph{single primary science sample} for all primary cosmological statements. The historical WLS/bootstrap fit used a different latitude-cut support and is retained only as provenance, not as an \FSC{} or physical exclusion. This is an internal hierarchy declaration, not a claim of formal preregistration or independent blinding. See Appendix~D for the support-audited diagnostic table.
\begin{itemize}\setlength{\itemsep}{0pt}
\item \textbf{Estimator hierarchy:} the primary cosmological estimator is the real-space CW-fraction dipole fit on Catalog~C at NSIDE$=$64 (moment-ratio $z_{\rm mom}=+0.71$ against the fixed-occupancy galaxy-label-randomization null, one-sided upper-tail empirical-rank $p=0.225$). The block-bootstrap WLS fit is a diagnostic of a clean observed-label template ($z\!\approx\!-7.6$ at $A_p=1.7\%$), not a physical $1.7\%$ exclusion; with the illustrative $g=0.398$ transfer the physical comparison is only $1.36\sigma_{\rm boot}$ (Appendix~D).
\item \textbf{Secondary diagnostic estimators:} (iii)~canonical-$N$ direct-MC NaMaster at $\ell=1$ on the patchy \FSC{} support ($f_{\rm sky}=0.49005$, $+3.64\sigmaunit$); (iv)~apodized-footprint MASTER at $\ell=1$ on \MAGF{} ($f_{\rm sky}=0.494$, $C^2$ $2^\circ$; $+7.28\sigmaunit$ vs.\ global label-shuffle and $+7.13\sigmaunit$ vs.\ depth-stratified null, $W_p\!=\!N_{\rm all}$); and (v)~hemisphere maximum-asymmetry ($3.05\sigmaunit$ local maximum; zero exceedances in $10^4$ max-statistic simulations give the add-one estimate $1/10001$ and a $95\%$ binomial upper limit $\simeq3.0\!\times\!10^{-4}$, systematics-attributed; Appendix~C). These characterize how a non-zero global monopole and coherent low-$\ell$ systematics couple through the patchy weighted footprint.
\item \textbf{Generative monopole-only null:} (vi)~$N=500$ binomial-monopole realizations demonstrating that the raw pre-MASTER pseudo-$C_\ell^{(\ell=1)}$ is dominated by monopole-mask leakage; the post-MASTER $+3.64\sigmaunit$ \FSC{} residual is non-primary and requires additional coherent systematics beyond the monopole-only channel (Sec.~\ref{sec:monopole_mask_null}).
...
\hline
\dtc{Real-space chirality dipole is consistent with zero} & \dte{(i) HC real-space dipole} & \dts{HC, $p_{\rm eq}\!>\!0.6$ ($949{,}584$)} & \dtn{fixed-occupancy label randomization ($10^4$)} & \textsc{primary} \\
...
\hline
(i)~real-space dipole (HC) & $949{,}584$ & --- & $0.4818$\footnote{$f_{\rm sky}=0.4818115$ is \HCRI{}, the inclusive $N_{\rm spiral}(p)\!\ge\!10$ support re-evaluated on the high-confidence subsample (23{,}682 of 49{,}152 pixels). The superseded strict $N_{\rm spiral}>10$ execution used 23{,}600 pixels and is retained only as historical mask-sensitivity context.} & \HCRI{} & fixed-occ.\ label rand.\ ($10^4$) & $+0.71$ (moment $z$) \\
(ii)~WLS template diag. & $3{,}201{,}160$ & --- & $0.49209$ & latitude cut (24,187 px) & block-boot.\ ($10^3$) & $z\!\approx\!-7.6$ \\
(iii)~canonical MASTER (diag.) & $3{,}201{,}160$ & --- & $0.49005$ & \FSC{} & pp-shuffle & $+3.64$ \\
(iv)~apod.\ MASTER (diag.) & $3{,}201{,}160$ & $8{,}474{,}531$ & $0.494$\footnote{Geometric $N_{\rm all}\!\ge\!1$ footprint pixel fraction; the corresponding weighted/apodized \emph{effective} sky fraction for this row's $W_p\!=\!N_{\rm all}$, $C^2$ $2^\circ$ configuration is $f_{\rm sky}^{\rm eff}=0.452$ (Appendix~A, Table~\ref{tab:fsky_summary}).} & \MAGF{} & pp-sh./d-str. & $+7.28/{+}7.13$ \\
(v)~hemisphere LEE (MC) & $3{,}201{,}160$ & --- & $0.49005$ & \FSC{} & max-stat MC & $p_{\rm LEE}=1/10001; 95\% upper limit \simeq3.0\times10^{-4}$ (syst.-attr.) \\
(vi)~monopole+mask null & $3{,}201{,}160$ & --- & $0.49005$ & \FSC{} & monopole-only & $+1.69$ \\
...
\hline
\textbf{P1} real-space HC dipole & fixed-occupancy & $+0.71\sigmaunit$ \\
\quad ($p_{\rm eq}\!>\!0.6$), \emph{primary} & \quad label randomization & \quad (upper-tail $p\!=\!0.225$) \\
...
\emph{Raw/flip quarantine.} Reconstructing the flip pass from mismatched raw/equivariant pipeline products identifies 249,066 unsafe rows catalog-wide (2.939\%) whose reconstructed score exits $[0,1]$ by more than $10^{-3}$, with maximum excursion 0.0901. Exactly 59,515 of those rows are in the 949,584-row HC selection; the strict HC diagnostic has 890,069 rows. These diagnostic quantities are not calibrated probabilities. The public v1.0.244 science-facing release omits every raw-pass and reconstructed flip-pass score column, retains only a Boolean \texttt{raw\_flip\_qc\_unsafe} marker, and places all 249,066 offending diagnostic rows in a provenance-only quarantine with \texttt{do\_not\_use\_for\_science=True}. The existing flagged-row-excluded rerun remains null-consistent; it is a robustness diagnostic, not a replacement primary null. The executable schema and filters are given in Data Availability.
...
We pixelize the sky at HEALPix resolution $\nside = 64$ (49,152~pixels, $\sim\!0.84$~deg$^2$ per pixel). In each supported pixel we compute the asymmetry below. The threshold $N_{\rm spiral}(p)\!\ge\!10$ defines two sample-dependent supports: \HCRI{} for the HC primary (23,682 pixels) and \FSC{} for the full-spiral diagnostics (24,087 pixels); Appendix~A records the exact data-vector mapping.
\begin{equation}
...
The primary real-space estimator fits $A_p=m+\bm{a}\cdot\hat n_p$ on the HC observed-label map and compares its amplitude with $10{,}000$ fixed-occupancy galaxy-label randomizations. Each draw conditions on the observed \HCRI{} pixel occupancies and the global CW total among the 947,326 support galaxies, so position, support, and heteroscedastic count structure remain fixed. The declared $p_{\rm eq}>0.6$ cut is encoded in committed generator revision \texttt{94113e5} (2026-06-09), but no externally timestamped preregistration, frozen pre-unblinding tag, or independent blinding record exists; we therefore do not claim formal preregistration or elimination of all researcher degrees of freedom. The HC selection contains $N_{\rm selected}=949{,}584$ spirals; $N_{\rm support}=947{,}326$ of them enter the \HCRI{} fit across 23,682 pixels, with 2,258 excluded by the support rule. The supported fit is null-consistent at $z_{\rm mom}=+0.705$ with one-sided upper-tail rank $p=(2246+1)/(10000+1)=0.22468$. The fitted amplitude is $A_{\rm dip}=0.00459707$ toward equatorial $(\mathrm{RA},\mathrm{Dec})=(294.31^\circ,16.03^\circ)$; the null mean and population standard deviation are $0.00348994$ and $0.00156969$, respectively, for seed 20260715. The legacy strict-mask confidence-cut sweep ($N_{\rm spiral}>10$) gave $z=+0.41,+1.14,+0.51$ at cuts $0.6,0.7,0.8$ and is retained only as historical sensitivity context; the primary $p_{\rm eq}>0.6$ value is the inclusive result above.
...
\begin{table}[tbp]
\caption{Primary HC estimator audit. The empirical rank value is the one-sided upper tail. The exact fixed-occupancy null-array SHA-256 is \texttt{f6360f4bec226690...152c0}; full precision and the complete hash are in the release manifest.\label{tab:primary_audit}}
\scriptsize
\begin{tabular}{lc}
Quantity & Exact or reported value \\
\hline
$N_{\rm selected}$ / $N_{\rm support}$ & $949{,}584$ / $947{,}326$ \\
Excluded / valid pixels & $2{,}258$ / $23{,}682$ \\
Support / seed / null draws & \HCRI{} / $20260715$ / $10{,}000$ \\
$A_{\rm dip}$ / monopole & $0.0045970743$ / $-0.0077688925$ \\
Equatorial direction & $(294.3054^\circ,16.0291^\circ)$ \\
Null mean / std. (ddof $=0$) & $0.0034899420$ / $0.0015696946$ \\
$z_{\rm mom}$ & $+0.7053170$ \\
Upper-tail $k/N$ / rank $p$ & $2246/10000$ / $0.2246775$ \\
\end{tabular}
\end{table}
...
the HC ($p_{\rm eq}\!>\!0.6$) real-space dipole is reduced to $+0.71\sigmaunit$ under the current fixed-occupancy primary null. Generation counts and the projected streaming read are recorded in \artifact{pipelines/p2\_chirality/outputs/canonical\_provenance/fig7\_raw\_vs\_eq\_manifest.json}. This visual diagnostic shows that the equivariant pipeline is associated with a strong reduction of the measured directional bias and illustrates why equivariant post-processing is an important mitigation in future chirality studies. Because the two products have different inference-pass provenance, memberships, and quoted null conventions, the comparison does not by itself isolate TTA as the unique causal explanation.\label{fig:raw_vs_eq}}
\end{figure*}

\paragraph{Angular power spectrum.}
We perform MASTER mode-coupling deconvolution using \textsc{NaMaster}~\cite{Alonso:2019} at $N_{\rm side}=64$ on the \MAGF{} ($N_{\rm all}\!\geq\!1$; $24{,}297$ pixels, $f_{\rm sky}=0.494$, $C^2$ $2^\circ$ apodization, depth weight $W_p$). The MASTER-deconvolved single-mode $\ell\!=\!1$ value (computed on the monopole-subtracted $A_p$ field; declared data vector, Appendix~A) is $C_1^{\rm meas}=2.348\!\times\!10^{-5}$ against a 500-MC per-galaxy label-shuffle null with mean $1.71\!\times\!10^{-6}$ and $\sigma_{\rm null}=2.99\!\times\!10^{-6}$, i.e.\ $+7.28\sigmaunit$ for $W_p\!=\!N_{\rm all}$ ($+9.78\sigmaunit$ for $W_p\!=\!N_{\rm spiral}$); the $10^4$-permutation recompute (Table~\ref{tab:multipole}) reproduces this diagnostic channel at $z=+7.31$ with empirical rank $p=6.0\times10^{-4}$ (one-sided, computed as $(k+1)/(N+1)$ with $k\!=\!5$ of $N\!=\!10^4$ null draws exceeding the data; Table~\ref{tab:multipole} caption), and the Gaussian-equivalent $\sigma$ is quoted from the null moments. A depth-stratified null (labels permuted within 10 $N_{\rm all}(p)$ deciles; this preserves the marginal depth distribution but not joint spatial--depth structure, so it bounds only depth-sampling effects) leaves the excess essentially unchanged ($+7.13\sigmaunit$ / $+9.06\sigmaunit$), excluding pixel-depth sampling alone as the driver. A weight-map sweep gives the same verdict: the excess persists under all three weightings ($W_p\!=\!N_{\rm all}$ $+6.9\sigmaunit$, $W_p\!=\!N_{\rm spiral}$ $+8.5\sigmaunit$, uniform binary $+6.7\sigmaunit$; independent 500-MC null streams, artifact c9c; in each variant the mask-mean subtraction uses the same variant weight map, so the field-consistent $N_{\rm spiral}$-weighted subtraction is included in the sweep), so it is not an artifact of the depth-weight choice, although its magnitude is weight-dependent at the $\pm1\sigmaunit$ level. This channel is therefore a \emph{systematics diagnostic} on the patchy weighted footprint --- the same coherent low-$\ell$ family as the \FSC{} residual (Sec.~\ref{sec:monopole_mask_null}, Appendix~D) --- and is not used as a cosmological null; the cosmological statement rests on the real-space dipole; the WLS observed-label template fit is a diagnostic whose physical-amplitude reinterpretation is given in Appendix~D. NaMaster configuration details and the \FSC{} support-declaration + depth-stratification audit are in Appendix~A. We emphasize that rows~(i) and (iv) of Table~\ref{tab:headline_summary} are not on the same statistical footing --- different fields, masks, weights, and null procedures --- so the apparent $+0.71\sigmaunit$ vs.\ $+7.28\sigmaunit$ gap is not a $17\times$ discrepancy on a common axis; the harmonic-completeness check (Sec.~\ref{sec:conclusions}, artifact c9b) bounds what a clean Shamir-class real-space dipole would have produced in the MASTER channel --- it would register at $z\approx68$--$218$ there, versus the observed $+7.28\sigmaunit$ --- but it does not establish statistical consistency of the two estimators on a common axis.

\begin{table*}[tbp]
\caption{MASTER-decoupled angular power of the equivariant Catalog~C asymmetry map on two diagnostic footprints, each with its own field and null convention (Appendix~A). Values are rounded; full arrays are retained in the release. The apodized \MAGF{} block uses $W_p=N_{\rm all}$, whereas the unapodized \FSC{} block uses binary weight and $N_{\rm spiral}$-weighted mean subtraction, so $C_b$ amplitudes and $z$ values are not comparable across blocks or with the primary real-space estimator. Band~1 is the single $\ell=1$ mode within the full coupling matrix. All entries are systematics diagnostics, not cosmological measurements. Empirical ranks use $(k+1)/(N+1)$; the separate 500-draw direct-MC diagnostic has add-one rank $p_{\rm MC}=0.03194$ (Sec.~\ref{sec:monopole_mask_null}).\label{tab:multipole}}
...
\begin{table*}[tbp]
\caption{Support audit of diagnostics associated with the $+3.64\sigmaunit$ \FSC{} $\ell\!=\!1$ residual. Only rows explicitly bound to the \FSC{} base support enter the synthesis.\label{tab:eightanchor_summary}}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llc}
...
(the $+0.71\sigmaunit$ real-space null and finite observed-label pilot
...
We publish an 8,474,531-object observed-label catalog release and report one primary result: the HC real-space dipole is null-consistent ($N_{\rm selected}=949{,}584$, $N_{\rm support}=947{,}326$, $z=+0.71$, one-sided upper-tail $p=0.225$ under the declared null convention). The exact safe catalog, quarantine, retained null array, manifest, schema, and reproducer are public at the commit-pinned release identified in Data Availability. All harmonic, WLS, injection, and transfer calculations are diagnostics. Our main conclusions are:
...
\end{tabular}
\tablecomments{\textsuperscript{a}Observed harmonic channel; systematics-attributed (Appendix~D).}
\end{table*}
...
A direct single-mode NaMaster execution on the \FSC{} sample ($N_{\rm spiral}\!=\!3{,}201{,}160$, $f_{\rm sky}\!=\!0.49005$, 500 per-pixel random-label permutation nulls, seed~42) yields $\sigma_{\rm canonical}^{\rm direct}\!=\!+3.64\sigmaunit$ ($k=15$ of $N=500$ null draws; add-one rank $p_{\rm MC}=(15+1)/(500+1)=0.03194$; Gaussian-equivalent $\approx\!1.9\sigmaunit$); the $10^4$-permutation recompute of the same \FSC{} unapodized field in Table~\ref{tab:multipole} gives $z\!=\!+7.93\sigmaunit$ --- the $500$-MC $+3.64\sigmaunit$ direct single-mode value is retained for continuity with the leakage analysis; the $10^4$-permutation Table~\ref{tab:multipole} \FSC{} row is the current high-statistics diagnostic under its committed field convention (see Sec.~\ref{sec:notation} and Table~\ref{tab:multipole} caption). \emph{Both the $+3.64\sigmaunit$ and $+7.93\sigmaunit$ values are systematics-attributed diagnostics from different null-run sizes and mask/weight conventions; they are not cosmological detection significances, are not directly comparable to each other, and are not comparable to the primary real-space estimator.} The HC ($p_{\rm eq}>0.6$) real-space dipole at $+0.71\sigmaunit$ under the fixed-occupancy primary null anchors the null verdict. The block-bootstrap WLS fit is a correlated full-sample diagnostic of a clean observed-label template, not an independent physical exclusion; a joint covariance likelihood with the harmonic channel remains open.

\paragraph{Bias hardening is essential.}
Our raw (Catalog~A) analysis produces a $2.31\sigmaunit$ real-space dipole and a $+6.48\sigmaunit$ pre-MASTER pseudo-$C_\ell$ from a classifier CW excess of only $0.79\%$, modulated by non-uniform survey coverage. Equivariant post-processing reduces the real-space dipole to $+0.71\sigmaunit$ under the current fixed-occupancy primary null; MASTER mode-coupling deconvolution substantially reduces the monopole-mask leakage channel that sources the $+6.48\sigmaunit$ pre-MASTER pseudo-$C_\ell$ (the post-MASTER residuals are systematics-attributed; Sec.~\ref{sec:monopole_mask_null}). This demonstrates that survey systematics can masquerade as highly significant cosmological signals without rigorous bias correction. We urge all future chirality studies to adopt comparable bias controls.
...
The \FSC{} $+3.64\sigmaunit$ residual is \emph{not} a positive detection of a primordial chirality dipole. On the exact base support, persistence under apodization and $\ell=2>\ell=1$ broadband structure disfavor a clean dipole. The other six historical diagnostics are different-support or support-unproven and do not strengthen that synthesis. The primary scientific result remains the HC real-space $+0.71\sigma$ observed-label null.
...
Using the HC-broad-0.6 ($p_{\rm eq}\!>\!0.6$, $N\!=\!949{,}584$) and HC-strict ($p_{\rm eq}\!>\!0.8$, $N\!=\!624{,}660$) cuts as high-confidence morphology-selected subsamples (\emph{not} validated inclination proxies --- the axis-ratio cross-match below remains the canonical inclination test): the Catalog~C-full $+4.31\sigmaunit$ \emph{monopole-preserving} pre-MASTER pseudo-$C_\ell^{(\ell=1)}$ estimator collapses to $+0.62\sigmaunit$ (HC-broad-0.6) and $+0.87\sigmaunit$ (HC-strict). The HC-cut collapse (factor $\sim\!7$ in the \FSC{} leakage-contaminated estimator) is the characteristic signature of a classifier label-noise systematic rather than a primordial dipole; it is consistent with the leakage-channel interpretation of the canonical $+3.64\sigmaunit$, not with a $+4.31\sigmaunit$ standalone detection. The primary real-space null at $+0.71\sigmaunit$ (equivariant HC, $p_{\rm eq}>0.6$, $N=949{,}584$) remains the load-bearing result; the WLS clean-template statistic is diagnostic and must not be read as a physical $1.7\%$ exclusion.

\paragraph{Monopole-preserving estimator definition.} The ``monopole-preserving'' Catalog-C-full $+4.31\sigmaunit$ is the single-mode \texttt{pymaster} pseudo-$C_\ell^{(\ell=1)}$ evaluated on the equivariant Catalog~C full-footprint $f_{\rm CW}^{\rm eq}$ field on the \FSC{} support \emph{before} mean-pixel subtraction and \emph{before} MASTER mode-coupling deconvolution; the null is the same per-pixel random-label permutation null used for the canonical $+3.64\sigmaunit$ result (Sec.~\ref{sec:app_systematic}, $N_{\rm MC}\!=\!500$, seed~$42$). It is therefore the \emph{same} estimator family as the canonical $+3.64\sigmaunit$, with the additional choice of not subtracting the spatially-uniform $f_{\rm CW}^{\rm eq}\!=\!0.4974$ monopole prior to single-mode pseudo-$C_\ell$ measurement. The $+4.31\sigmaunit$ vs.\ the primary $+0.71\sigmaunit$ real-space dipole are therefore \emph{not directly comparable} — they are different estimators measuring different observables on the same sample. The $\sim\!10\times$ amplitude gap is sourced by exactly the monopole-mask leakage channel quantified in Sec.~\ref{sec:monopole_mask_null} (where the monopole-only null reproduces $99.32\%$ of the pre-MASTER $C_1$ power on the \FSC{} support): without monopole subtraction the leakage contribution dominates; with monopole subtraction the canonical estimator retains the $+3.64\sigmaunit$ residual, and under MASTER deconvolution the monopole-only null reproduces only ${\sim}12\%$ of the observed power, leaving a $+4.84\sigmaunit$ non-null residual (Sec.~\ref{sec:monopole_mask_null}, Table~\ref{tab:monopole_mask_null}) --- the leakage channel accounts for the bulk of the pre-MASTER power but not for the post-subtraction or post-MASTER residuals, which are systematics-attributed. The HC-cut robustness test below uses the same monopole-preserving variant on the high-confidence subsamples for a like-for-like comparison; it is the \emph{cut-dependence within this estimator}, not consistency with the real-space dipole, that is the substantive finding.
...
Repository state: the analysis artifacts linked throughout this paper resolve against the live \texttt{main} branch of the repository, which always reflects the current version-stamped content (HC $N_{\rm selected}=949{,}584$, $N_{\rm support}=947{,}326$, $p_{\rm eq}>0.6$; real-space dipole $+0.71\sigmaunit$, one-sided upper-tail $p=0.225$). An immutable archival snapshot (PDF + \texttt{.tex} source + figures + canonical-provenance artifacts) will be deposited on Zenodo at journal submission with: (i) a frozen immutable release tag pinning the exact git commit hash of the analysis codebase; (ii) exact commit hashes for the catalog, all scripts, and all canonical-provenance artifacts (\href{https://github.com/Hubify-Projects/bigbounce/tree/main/pipelines/p2_chirality/outputs/canonical_provenance}{\nolinkurl{pipelines/p2_chirality/outputs/canonical_provenance/}}); and (iii) a minted Zenodo DOI that will constitute the single citable archival reproducibility handle for the published version. The DOI and commit hashes will be inserted here in place of this sentence at submission time.

% ============================================================

\begin{itemize}
\item \textbf{Machine-readable release manifest:} the exact public manifest is directly downloadable at immutable data revision \href{https://huggingface.co/datasets/bamfai/galaxy-chirality-catalog/resolve/db11023306ab4eed1d7727670bd78e127b7af17a/apjs-release/v1.0.244/MANIFEST.json}{\texttt{db110233/MANIFEST.json}}.
