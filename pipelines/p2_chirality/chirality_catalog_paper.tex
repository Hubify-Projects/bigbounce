...
\newcommand{\paperVersion}{v1.0.193}
...
\begin{itemize}\setlength{\itemsep}{0pt}
\item \textbf{Primary cosmological estimators:} (i)~real-space CW-fraction dipole fit on Catalog~C at NSIDE$=$64 (moment-ratio $z_{\rm mom}=+0.41$ against the isotropic (pixel-)permutation null, empirical-rank $p=0.31$; these are independent numbers, not Gaussian $(z,p)$ partners --- see Sec.~\ref{sec:dipole}); and (ii)~block-bootstrap WLS template-fit exclusion of a clean $1.7\%$ dipole on the canonical-mask $A_p$ field ($z\!\approx\!-18$ under the adopted NSIDE$\!=\!8$ block-bootstrap error model; Appendix~D).
...
Repository state for this release: commit \texttt{53b41d12} (June 2026); primary sample is HC-broad $N=949{,}584$ ($p_{\rm eq}>0.6$) spirals at this snapshot, with real-space dipole $+0.41\sigmaunit$ ($p=0.31$). An immutable archival snapshot (PDF + \texttt{.tex} source + figures + canonical-provenance artifacts) will be deposited to Zenodo at journal submission; the resulting DOI will be the single citable reproducibility handle for this release. The cited commit hash pins the version-stamp commit; subsequent same-day metadata and figure commits that do not alter analysis artifacts are not reflected in this pin by design (the hash advances only at explicit paper-version restamps).
