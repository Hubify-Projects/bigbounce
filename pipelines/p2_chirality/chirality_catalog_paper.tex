...
\newcommand{\paperVersion}{v1.0.260}
...
\newcommand{\paperTimestamp}{July 16, 2026, 02:05 PDT}
...
We report one primary result from the 8,474,531-object observed-label catalog: the strict release-safe HC real-space dipole is null-consistent ($N_{\rm selected}=890{,}069$, $N_{\rm support}=887{,}472$, $z_{\rm mom}=+0.635$, one-sided add-one rank $p=0.23768$). The repository candidate retains the safe catalog, quarantine, exact strict null array, receipt, and generator. The currently identified public dataset release still documents the earlier unsafe-inclusive primary realization; rebuilding and publishing a synchronized immutable release is therefore an explicit pre-submission gate. All harmonic, WLS, injection, and transfer calculations are diagnostics. Our main conclusions are:
