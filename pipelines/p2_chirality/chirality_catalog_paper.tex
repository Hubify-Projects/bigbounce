...
\newcommand{\paperVersion}{v1.0.177}
...
Key open analyses that would strengthen the canonical-mask interpretation without affecting the primary null: (1)~full physical-template regression against per-galaxy DR8 sweep morphology fields (b/a, fracdev, shape\_r\_eff, PSF FWHM, depth); (2)~a Gaussian-process spatial likelihood to upgrade the WLS+bootstrap template-model disfavor to a formal exclusion of interpretation~(i); (3)~cross-matching with the DESI spectroscopic survey ($\sim\!500{,}000$ spectroscopic spirals in the Year~1 footprint) for redshift-binned dipole analysis. None of these change the primary real-space null, which is anchored on estimators that bypass the canonical-mask leakage channel.
...
\paragraph{Primary finding: a quantifiable monopole-mask leakage channel.}
