...
\newcommand{\paperVersion}{v1.0.172}
...
$\vit$ classifier resolves via the test-time equivariant
...
\item \emph{MASTER $\ell\!=\!1$ moment-$z$}: $\sigma\!=\!C_1^{\rm data}/\sigma_{\rm null}$ vs.\ the label-shuffle null mean and width; $z=+3.64$ (500-MC canonical direct run), $z=+7.93$ ($10^4$-permutation canonical unapodized), $z=+7.28$ ($10^4$-permutation apodized). These three values are from different null-run sizes and mask/weight conventions and are not mutually comparable.
...
A direct single-mode NaMaster execution on the canonical Catalog~C sample ($N_{\rm spiral}\!=\!3{,}201{,}160$, $f_{\rm sky}\!=\!0.49005$, 500 per-pixel random-label permutation nulls, seed~42) yields $\sigma_{\rm canonical}^{\rm direct}\!=\!+3.64\sigmaunit$ ($p_{\rm MC}\!=\!15/500\!=\!0.030$). Two independent estimators on the same Catalog~C anchor the null verdict: the real-space dipole at $0.41\sigmaunit$ and the block-bootstrap WLS template-fit exclusion of a clean $1.7\%$ dipole ($z\!\approx\!-18$, Appendix~D). The no-dipole-at-$\ell\!=\!1$ verdict stands, anchored on these two estimators that bypass the canonical-mask leakage channel.
...
Repository state for this version: commit \texttt{96e78ad5} (v1.0.171, June 2026); headline sample is HC-broad $N=949{,}584$ ($p_{\rm eq}>0.6$) spirals at this snapshot, with real-space dipole $+0.41\sigmaunit$ ($p=0.31$); a Zenodo DOI snapshot will accompany the journal submission.
