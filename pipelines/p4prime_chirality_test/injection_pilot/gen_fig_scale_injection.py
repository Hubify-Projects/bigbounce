#!/usr/bin/env python3
"""Row 13 Part A at scale: figure comparing the N=5000 pixel-level
production-equivariant-TTA injection-recovery curve (spiral-classified
A statistic, 5 seeds/fraction with per-seed std as the noise floor)
...
d = json.loads((HERE / "scale_injection_results.json").read_text())
...
ax.set_title("Row 13 Part A at scale: pixel-level parity injection\nthrough the production equivariant pipeline (N=5000)", fontsize=10.5)
ax.legend(loc="best", fontsize=7.5, frameon=True)
ax.grid(alpha=0.25)
fig.tight_layout()
out = HERE / "fig_scale_injection_recovery.png"
