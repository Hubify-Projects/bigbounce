...
\newcommand{\paperVersion}{v4P.0.12}
...
Poplawski's rotating black-hole-universe model, in which Einstein--Cartan
torsion halts a collapsing black hole's singularity, bouncing into a
daughter universe that inherits a preferred axis from the parent's spin, toward
which galaxies are predicted to align. We report the largest test of that
claim to date: an 8,474,531-galaxy DESI Legacy DR8 chirality catalog and two
independent null tests. The primary, quality-controlled real-space dipole
($N_{\rm support}=887{,}472$ of $890{,}069$ rows) is null-consistent
($z_{\rm mom}=+0.635$, one-sided $p=0.238$), with a $95\%$ sensitivity floor
$A_{95}^{\rm obs}\simeq0.98\%$, the more conservative of two limits and the
floor used below (Neyman $95\%$ CL limit $A_{95}^{\rm CL}\simeq0.75\%$). A companion DESIVAST
void/non-void contrast on $145{,}766$ classifier-labelled galaxies likewise
finds no significant effect ($\Delta f_{\rm CW}=+0.00145$, two-sided
...
floor disfavors a dipole above $\sim\!1\%$ at $\geq\!95\%$ power,
$2$--$20\times$ below the $\sim\!2$--$33\%$ literature amplitudes, though
under an illustrative, not-adopted-for-strengthening bridge the two largest
comparison samples drop below this floor. This null is measured on
$887{,}472$ primary-channel spirals, $4$--$3{,}400\times$ larger than every
comparison catalog in Table~\ref{tab:bh_exclusion} except Shamir's DESI
Legacy sample ($N=1.3$ million), and confirms independent reanalyses by
Iye \etal\ and Patel \& Desmond finding no significant galaxy-spin
anisotropy.
\end{abstract}
...
$512\times512$~px $grz$ cutouts at $0.262''$/pixel, a $134''$ field, resampled
to the $224\times224$ network input); the parent-sample
...
\begin{table}[t]
\caption{Catalog Parquet column schema (catalog release v1.0.244, documented in the v1.0.274 archived release paper~\cite{Golden:P4v274}). One row per
...
\texttt{raw\_flip\_qc\_unsafe} & Release-safety QC flag: raw/eq pipeline-pass mismatch (boolean) \\
\bottomrule
\end{tabular}
\end{table}
...
($454{,}968/887{,}472$, monopole $f_{\rm CW}-\tfrac12=+1.2656\%$, equivalently
$A_p=2(f_{\rm CW}-\tfrac12)=+2.53\%$ in the amplitude convention of
Fig.~\ref{fig:skymap} and Eqs.~\ref{eq:a95_obs}--\ref{eq:a95_cl}). The
catalog-wide and HC-with-unsafe monopoles are likewise
$A_p=-0.53\%$ and $A_p=-0.79\%$ in this same convention. The quarantined rows
...
are tabulated. T5, a linear-Pearson RA/Dec metadata-leakage test, is
omitted: a linear Pearson correlation is inappropriate for the circular RA
coordinate $0^\circ\equiv360^\circ$, so the archived release instead reports
a map-level low-$\ell$ real-$Y_{\ell m}$ regression against RA/Dec, finding
all three $\ell=1$ coefficients consistent with zero, $|z|\leq1.25$. All
seven tabulated tests pass; these are
...
critical value $0.669\%$. Because $A_{\rm dip}$ is a low realization of a
positive-definite estimator whose no-signal distribution has mean
$0.362\%$, this classical one-sided limit falls below the $95\%$-detection
sensitivity $A_{95}^{\rm obs}$. The limit retains exact frequentist
coverage, but it is a statement about this realization, not about the
experiment's reach; a downward fluctuation can make a classical limit
stronger than the amplitude the experiment is guaranteed to detect (the
standard motivation for unified/$\mathrm{CL_s}$ constructions). We
therefore confront the model in Sec.~\ref{sec:bh} with the
realization-independent floor $A_{95}^{\rm obs}=0.98\%$, the more
conservative of the two. $A_{95}^{\rm obs}$ and $A_{95}^{\rm CL}$ are
...
confronted. Sec.~\ref{sec:robustness_disclosure} reports two results that
bear on this floor and this null and are not otherwise referenced from
this section: (i) a per-imaging-leg partition of this same
$887{,}472$-galaxy primary sample is not uniformly clean under leg
removal -- two single-leg-removal variants and the DES-only fit
individually exceed $A_{95}^{\rm obs}$, even though the full four-leg fit
reported here is null-consistent; and (ii) a direct measurement of the
observed-to-physical transfer function, via a PA-restoring
handedness-reversal test, on the classifier's own \texttt{Smith42/galaxies}
imaging gives $D \le 0.600 \pm 0.007$ ($A_{95}^{\rm phys} \ge 1.63\%$),
superseding an earlier bound measured on out-of-domain viewer cutouts and
withdrawn (Sec.~\ref{sec:robustness_disclosure}). (i) remains unresolved and
is stated explicitly, not adopted as a correction; (ii) is now a
measurement, in the conservative direction, adopted below, in
Sec.~\ref{sec:robustness_disclosure}.
...
scatter ($z=-6.57$); the other three channels are all non-significant on
this same shared bootstrap ($z=+2.21$ real-space dipole, $+0.81$ WLS
dipole, $-0.61$ this bootstrap's own MASTER $\ell=1$ channel on the
pre-support-cut $N=949{,}584$ sample -- a different sample, support, and
null than the \FSC{} $\ell=1$ diagnostic of Sec.~\ref{sec:dipole}
($z=+6.923$, 24,087-pixel support), and not to be identified with it),
consistent with the primary null. The block-bootstrap
...
uncorrelated with the other three ($|\rho|\leq0.06$); the real-space
dipole and its own MASTER $\ell=1$ channel are the most correlated pair in
this bootstrap ($0.79$), and the real-space dipole and its WLS variant are
only weakly correlated ($0.16$) -- both patterns consistent with these
four channels answering related but non-identical questions on different
supports.
...
void catalog~\cite{DESIVAST:2025} intersected with the DESI DR1 spectroscopic TARGETID cross-match of the DR8 chirality catalog, as constructed in~\cite{Golden:P5v147} (joined on TARGETID), hereafter the DR1 companion of the
...
strengthen this statement. Sec.~\ref{sec:robustness_disclosure} reports an
architectural argument for why that transfer is not unity, together with a
direct measurement of it.
The production post-processing averages over the horizontal mirror only,
so it is exactly antisymmetric under a detector-aligned reflection but
carries no invariance under rotation; a ViT-S/16 backbone supplies none. A
genuine handedness reversal leaves a galaxy at its observed position angle
and is therefore not the detector-aligned mirror, so the assumption that
it is transmitted into the catalogue label with unit efficiency is not
supported by the architecture. We quantify the resulting dilution in a
companion calibration and quote the observed-label bound here without
asserting $D = 1$. A first attempt at that companion calibration presented
the released pipeline with a genuine, PA-restoring handedness reversal on
Legacy Survey display cutouts rather than the classifier's own
\texttt{Smith42/galaxies} inference images; a pre-registered positive
control caught the domain mismatch (re-running the released preprocessing
and checkpoint on those cutouts reproduced the released catalog's own
labels for only $43$--$45\%$ of galaxies across three independent draws,
against $99.99\%$ agreement with an in-domain forward pass), and the
resulting bound was withdrawn rather than adopted
(Sec.~\ref{sec:robustness_disclosure}). Sec.~\ref{sec:robustness_disclosure}
reports a direct measurement of that transfer function, re-run on the
correct imaging. Because handedness does not depend on the orientation at
which a galaxy is presented, the rate at which the released pipeline
contradicts its own label between two lossless rotations of the same
cutout bounds the dilution $D$ of any true population asymmetry from
above, with no human truth labels entering the argument. Measured on the
catalogue's own imaging -- the $512\times512$ DESI Legacy DR8 cutouts the
released inference ran on -- and on the catalogue's own high-confidence
selection, $D \le 1 - \max_\theta P\big(L(0) \ne L(\theta)\big) = 0.600 \pm
0.007$, with $D$ excluded from unity at $61\sigma$; the
$\theta = 180^\circ$ row, which requires no assumption about
position-angle uniformity, gives $D \le 0.660 \pm 0.008$. The
observed-label floor $A_{95}^{\rm obs} = 0.98\%$ therefore corresponds to a
physical-parity floor $A_{95}^{\rm phys} = A_{95}^{\rm obs}/D \ge 1.63\%$.
This is consistent with the illustrative $g = 0.398$ used above (which
would give $2.46\%$) and supersedes it as the quantity we can defend: it
is an upper bound on $D$, so it can only weaken the exclusion, and we
adopt it in that direction only.
...
Sec.~\ref{sec:robustness_disclosure} discloses one further result that
qualifies this exclusion and is not adopted as a correction to it: the
primary sample's own per-imaging-leg partition is not uniformly clean. A
direct measurement of the observed-to-physical transfer function, also
reported there, gives $D \le 0.600 \pm 0.007$
($A_{95}^{\rm phys} \ge 1.63\%$) on the classifier's own imaging, in the
conservative direction relative to an earlier, out-of-domain measurement
that this manuscript withdraws.
...
from the same torsion mechanism. One disclosure result
(Sec.~\ref{sec:robustness_disclosure}) qualifies this exclusion and
remains unresolved -- the primary channel's own leg-removal stability --
and is not adopted as a correction here. A second, a direct measurement of
the observed-to-physical transfer function on the classifier's own imaging,
gives $D \le 0.600 \pm 0.007$ ($A_{95}^{\rm phys} \ge 1.63\%$), superseding
an earlier out-of-domain measurement that this manuscript withdraws, and is
...
\subsection{Robustness and disclosure: pixel-level calibration, full-parent behavior, and structure cross-correlations}
\label{sec:robustness_disclosure}

\textbf{Pixel-level calibration of the injection--recovery curve.} The
label-level injection--recovery curve used to set $A_{95}^{\rm obs}$
(Sec.~\ref{sec:dipole}) is a catalog-level identity check; a companion
image-level pixel-parity injection through the production equivariant
classifier ($N=20{,}000$ Legacy DR9 cutouts, 10 seeds, true mirror-flip of
raw pixels rather than relabeling) recovers a pixel-level slope
$dA/df=+0.0297\pm0.0238$ (hard, spiral-classified statistic). The
pixel-level mirror injection is exactly the symmetry of the production
...
image, independent of the network's weights. The injected operation
\emph{is} $M$, so it transfers with unit efficiency by construction and
the recovered curve obeys $E[A(f)] = A_0(1-2f)$ at every $N$; we report it
as a consistency check of the post-processing implementation, not as a
measurement of the morphology transfer function. Propagating the
injection-realisation variance that the original analysis omitted, the
fitted slope $dA/df = +0.0297 \pm 0.0238$ is consistent with the exact
label identity at $1.8\sigma$, and the design would first reach $3\sigma$
power at $N \approx 7.3\times10^5$. Table~\ref{tab:pixel_calib} records
this comparison; the injection baseline itself is $A_0=+0.59\%$ for
the spiral-classified population probed here, a different quantity from
(and opposite in sign to) the catalog-wide monopole reported in
Sec.~\ref{sec:catalog_classifier} ($f_{\rm CW}-\tfrac12=-0.265\%$,
equivalently $A_p=-0.53\%$ in this paper's amplitude convention -- not the
HC-selected primary channel's own monopole, which is positive), not the
same baseline residual.

The transfer function is measured directly, on the catalogue's own
imaging. Handedness is a property of the galaxy, not of the orientation at
which it is presented, so whenever the pipeline returns different
chiralities for two lossless $90^\circ$ rotations of the same cutout at
least one of the two labels is wrong; the measured disagreement rate
therefore bounds the per-orientation error rate and hence the dilution
$D = 1 - 2e$ of any true population asymmetry from above. On the
catalogue's high-confidence selection ($N=4{,}579$ \texttt{primary\_hc}
spirals, streamed directly from \texttt{Smith42/galaxies}, $320{,}000$
forward passes, no image bytes written to disk) the pipeline contradicts
itself for $34.0\%$ of spirals handed a lossless $180^\circ$ rotation and
for $40.0\%$ over the full set of right-angle rotations, giving
$D \le 0.600 \pm 0.007$ ($61\sigma$ below unity; $D \le 0.660 \pm 0.008$
on the assumption-free $180^\circ$ row alone). The deficit is a
rotation-equivariance failure rather than a parity failure: the two-fold
horizontal-flip test-time average renders the parity operation exact
wherever it commutes with the rotation, which we verify as a positive
control (the flip identity residual is exactly zero at $0^\circ$ and
$180^\circ$). The implied dilution raises the physical-parity sensitivity
floor to $A_{95}^{\rm phys} \ge 1.63\%$, which is the figure we quote.

\textbf{An earlier version of this measurement is withdrawn.} Manuscript
version v4P.0.10 and before reported $D \le 0.717 \pm 0.005$
($A_{95}^{\rm phys} \ge 1.37\%$) from the same PA-restoring test run on
Legacy Survey display cutouts. A pre-registered positive control caught
the domain mismatch: the released preprocessing, run at $\theta=0^\circ$
(the identity transform) on those cutouts, reproduced the released
catalog's own labels for only $43.9\%$ of $19{,}800$ galaxies ($42.4\%$
and $44.7\%$ on two further independent draws), while an identical check
against this program's own previously committed forward passes on those
same images agreed at $99.99\%$ -- showing the classifier code and
preprocessing were correct and the images were the wrong ones: Legacy
Survey display cutouts (a $39.3''$ field upsampled to $224$~px) rather
than the $512\times512$~px, $0.262''$/pixel \texttt{Smith42/galaxies}
images the released classifier actually ran inference on -- a $134''$
field (median pixel correlation $0.992$ against Legacy Survey cutouts at
that scale, $\le 0.25$ at every other grid point tested; a $3.41\times$
field-of-view mismatch; Sec.~\ref{sec:catalog_schema}). An out-of-domain
instability bounds an in-domain one in neither direction, so that
measurement, and the dilution bound and physical-parity floor it implied,
are \textbf{withdrawn}, not averaged with the in-domain result above. The
in-domain classifier is, if anything, \emph{less} rotation-stable than the
withdrawn out-of-domain measurement suggested -- the $\theta=180^\circ$
self-disagreement rose from $21.1\%$ to $34.0\%$ -- so this correction
moves the physical-parity floor upward, in the conservative direction, not
downward.

The classifier's parity-transfer efficiency is not uniform on the sky: it
carries a dipole of fractional amplitude $0.090 \pm 0.010$
(permutation-null rank $p \le 0.001$), whose axis lies $31.6^\circ$ from
the dipole axis of the high-confidence subset. Because the observed label
asymmetry of that subset has a non-zero monopole ($+2.55\%$), a dilution
modulated in proportion to the measured efficiency map imprints a spurious
dipole of $0.23\%$ in the observed labels --- $23\%$ of the
$A_{95} = 0.98\%$ limit quoted here. This amplitude was measured on the
same out-of-domain viewer cutouts as the withdrawn dilution bound above
and has not been re-measured on the classifier's own imaging; it is
reported as a systematic floor on the interpretation of the limit, not a
detected contamination, and its amplitude should be read as preliminary
pending an in-domain re-measurement. It assumes the dilution tracks the
measured efficiency map, and a true asymmetry of zero produces no dipole
under any dilution. Two further out-of-domain quantities from an earlier
version -- the rotation test-time-average recovery fractions and the
position-angle-restoring transfer efficiency $\bar\epsilon$ -- are omitted
here rather than re-quoted: since the in-domain rotation instability
measured above is larger than the out-of-domain one, a recovery fraction
measured out-of-domain would most likely overstate what test-time
averaging recovers in-domain, and quoting it would be optimistic in the
wrong direction.

\begin{table}[t]
\centering
\caption{Pixel-level injection calibration, and the position-angle-restoring
dilution bound measured in-domain on the classifier's own
\texttt{Smith42/galaxies} imaging ($N=4{,}579$ \texttt{primary\_hc}
spirals, lossless rotations, streamed directly from the dataset). An
earlier bound measured on out-of-domain Legacy Survey display cutouts
($D \le 0.717 \pm 0.005$, $A_{95}^{\rm phys} \ge 1.37\%$) is withdrawn,
not averaged with the values below (Sec.~\ref{sec:robustness_disclosure}).}
\label{tab:pixel_calib}
\begin{ruledtabular}
\begin{tabular}{lc}
Quantity & Value \\
\hline
Pixel-level injection slope $dA/df$ (hard, measured) & $+0.0297\pm0.0238$ \\
$\sigma$ from exact label identity ($-0.011791$) & $1.8\sigma$ \\
Mixture-corrected identity, soft statistic ($-0.0093$) & consistent at $2.3\sigma$ \\
$D \le 1-\max_\theta d(\theta)$ (in-domain) & $0.600\pm0.007$ \\
$\Rightarrow A_{95}^{\rm phys}$ & $\ge 1.63\%$ \\
\end{tabular}
\end{ruledtabular}
\end{table}

\textbf{Full-parent selection behavior.} The primary channel's
$887{,}472$-row \texttt{primary\_hc \&\& !raw\_flip\_qc\_unsafe} selection
exists to exclude rows flagged by post-review raw-flip QC and
low-confidence classification. Re-running the same fixed-occupancy null on
the full $3{,}200{,}420$-galaxy parent (the $3{,}201{,}160$-spiral
classified sample of Sec.~\ref{sec:catalog_classifier}, restricted to the
$N_{\rm spiral}(p)\geq10$ pixel-support cut used by this null; no
QC/confidence cut) gives a
dipole $A=0.566\%$ at $z=+4.44$, axis RA~$278.6^\circ$/Dec~$+25.3^\circ$. A
graded QC sweep localizes this to a single cut: removing only the raw-flip
cut (retaining \texttt{primary\_hc}) restores the null ($z=+0.68$), while
removing only the \texttt{primary\_hc} confidence cut does not ($z=+9.13$,
at a distinct axis); a per-imaging-leg partition shows the excess is also
removed by dropping the DES leg alone ($z=+0.48$) but not by any Galactic
cut. The four selections' best-fit axes are not uniformly scattered: the
full-parent axis and the raw-flip-relaxed (null) axis agree to
$17.3^\circ$, and the \texttt{primary\_hc}-relaxed axis agrees with the
primary channel's own axis to $19.3^\circ$; the two pairs are separated
from each other by $107$--$120^\circ$, well short of antipodal
($180^\circ$). We report the full pairwise pattern rather than a single
separation, since the \texttt{primary\_hc}-relaxed excess sitting near the
primary channel's own axis is itself part of what this disclosure reports.
The monopole-plus-mask-leakage floor in this
population is $0.19\%$. This pattern -- signal present only in the
population the primary QC cuts are designed to exclude, removed by a
single confidence or footprint-leg cut, with an unstable axis -- is
consistent with a confidence-cut/footprint systematic rather than a shared
physical signal, and is reported here as a systematic, not subtracted from
or used to adjust the primary channel's null result. The same per-leg
partition applied instead to the primary $887{,}472$-galaxy channel is
less clean: five of the six single-leg fits exceed $A_{95}^{\rm obs}=0.98\%$
(only BASS+MzLS, $z=+2.00$, $A=3.92\%$; only DECaLS, $z=+2.20$, $A=1.66\%$;
dropping DECaLS, $z=+4.19$, $A=2.21\%$; only DES, $z=+2.49$, $A=5.83\%$;
dropping DES, $z=+4.26$, $A=1.39\%$), and only dropping BASS+MzLS
($z=+1.02$, $A=0.66\%$) does not, versus $z_{\rm mom}=+0.635$
for the full four-leg primary sample. We report this rather than omit it:
it indicates the primary channel's own footprint-stability nulls are not
uniformly clean under leg removal, a residual not resolved by this
disclosure and not subtracted from the primary result.

\textbf{Structure cross-correlations.} On the same $949{,}584$-galaxy
\texttt{primary\_hc}-labelled sample used for the full-parent test above
(pre-dating the \texttt{raw\_flip\_qc\_unsafe} exclusion; not the
$887{,}472$-galaxy primary channel), a pre-registered battery of 16 of 17
declared chirality$\times$structure statistics was executed (the
Shamir-axis statistic below was unavailable; the full $\times17$
Bonferroni correction is retained rather than loosened to $\times16$):
local spiral surface-density quartiles; angular cross-correlation with
1,244 spectroscopic anomaly positions; five spectroscopic-redshift bins;
and dipole projections along the CMB dipole and CMB quadrupole--octopole
axes plus the catalog's own free best-fit direction. Each is tested
against a label-shuffle null where that null has non-zero variance for the
statistic in question, or else (for the pixel-level axis/dipole
projections, where within-pixel label-shuffle is degenerate) against a
1,000-realization random-axis-rotation null and a permutation-of-per-pixel-parity-means
null, with a $3\sigma$ look-elsewhere-corrected threshold declared in
advance. The battery returns no detection: environment ($\chi^2$
$z=-0.38$, trend $z=+0.17$), the anomaly cross-correlation (all eight
angular bins and the nearest-neighbor excess null; the largest raw
excursion falls from $-2.37\sigma$ to $-1.05\sigma$ under a rigid-rotation
null), redshift (trend $z=-1.63$), and the two fixed astrophysical axes
($z=+0.21$, $-0.13$) are all null, and the free best-fit dipole ($0.44\%$)
is below its own look-elsewhere-corrected null ($0.38\%\pm0.16\%$,
$z=+0.32$). Table~\ref{tab:structure_null} summarizes. Two data limits
apply: the environment test uses a parity-blind projected surface-density
proxy in place of a 3D void/filament \emph{membership} product, since the
DESI DR1 LSS products on disk at the time of this run were QSO-only at
$z=0.8$--$2.1$ with no overlap with the $z\lesssim0.3$ spiral sample and no
low-$z$ 3D density field was available on disk (the DESIVAST void
\emph{catalog} used in Sec.~\ref{sec:void} provides hole membership,
not a continuous density field, and does not substitute for one here); and
Shamir's claimed axis is not tested, because this manuscript draws its
quoted Shamir/Longo amplitudes from citation only and did not extract an
explicit RA/Dec axis from those sources for this battery -- not because
the cited works report no axis.

\begin{table}[t]
\centering
\caption{Chirality$\times$structure cross-correlation nulls (representative
statistics; full battery of 15 in the archived analysis).}
\label{tab:structure_null}
\begin{ruledtabular}
\begin{tabular}{lcl}
Statistic & $z$ & Result \\
\hline
Environment density, $\chi^2$ & $-0.38$ & null \\
Environment density, trend & $+0.17$ & null \\
Anomaly cross-corr.\ (rotation null) & $-1.05$ & null \\
Redshift trend & $-1.63$ & null \\
CMB dipole axis & $+0.21$ & null \\
CMB quadrupole--octopole axis & $-0.13$ & null \\
Free best-fit direction & $+0.32$ & null \\
\end{tabular}
\end{ruledtabular}
\end{table}

\textbf{External environment: DESI DR1 BGS clustering catalog.} A
density-based environment split against an external tracer field --
$k$=10-NN density contrast measured on the public DESI DR1
\texttt{BGS\_BRIGHT-21.5} clustering catalog and 4 randoms/cap downloaded
and uniformly sub-sampled to $6.0\times10^6$ total (not a void finder) --
finds no dependence of the parity fraction on environment in either subset
it can be tested on. Table~\ref{tab:bgs_environment} reports a
density-quintile split (void-like = lowest quintile, wall/filament =
middle three quintiles, node-like = top quintile, by relative rank; every
bin's median $\delta_k$ is in fact overdense relative to the randoms in
both subsets, so the split spans relative, not absolute, density and does
not reach genuinely underdense environments)
for the 121,417-spiral DESI spectroscopic-redshift subset (three-dimensional
comoving density; cut to the $0.1<z<0.4$ BGS tracer volume from a
$0<z<0.6$, tracer-matched 231,549-spiral parent) and the same-selection
$949{,}584$-galaxy \texttt{primary\_hc}
sample used for the structure battery above (projected density; this
projected subset is not disjoint from the spec-z subset -- the 121,417
spec-z rows are a nested subset of the 949,584 -- and it is not the
$887{,}472$-galaxy primary channel, since it retains the
\texttt{raw\_flip\_qc\_unsafe} rows excluded from that channel).

\begin{table}[t]
\centering
\caption{Parity fraction by environment bin against the DESI DR1
BGS\_BRIGHT-21.5 clustering catalog (density contrast $\delta_k$,
$k$=10-NN vs.\ randoms). $z$ is against the 1,000-realization
label-shuffle null.}
\label{tab:bgs_environment}
\begin{ruledtabular}
\begin{tabular}{lccc}
Environment & $N$ & $f_{\rm CW}$ & $z$ \\
\hline
\multicolumn{4}{l}{Spec-z, 3D comoving ($N=121{,}417$)} \\
Void-like & 24,284 & $0.48999\pm0.00321$ & $-0.61$ \\
Wall/filament & 72,849 & $0.49489\pm0.00185$ & $+1.11$ \\
Node-like & 24,284 & $0.49238\pm0.00321$ & $-0.84$ \\
\multicolumn{4}{l}{Projected, photo-z/no-$z$ ($N=949{,}584$)} \\
Void-like & 189,917 & $0.49602\pm0.00115$ & $+0.64$ \\
Wall/filament & 569,750 & $0.49680\pm0.00066$ & $-1.06$ \\
Node-like & 189,917 & $0.49388\pm0.00115$ & $+2.96$ \\
\end{tabular}
\end{ruledtabular}
\end{table}

The $\chi^2$ trend across bins is null in both subsets (spec-z: obs
$1.872$ vs.\ null $1.984\pm1.841$, $z=-0.06$, $p=0.952$, confirmed by a
sky-rotation null at $z=-0.12$, $p=0.917$; projected: $z=-1.51$,
$p=0.083$ two-sided, from the same 1,000-realization label-shuffle null --
the projected subset's own sky-rotation cross-check used only 10
realizations for wall-clock reasons and is not used as evidence, only as a
non-quantitative consistency check). Per-bin dipole amplitudes are
likewise consistent with their nulls in every bin ($|z|\le2.15$). The
largest excursion in the full 14-statistic battery (2 subsets $\times$ [3
$f_{\rm CW}$ + 1 $\chi^2$ + 3 dipole]) is the projected node-like bin's
parity fraction, $z=+2.96$ ($p_{\rm local}=0.0060$, $p=0.084$ after the
pre-registered $\times14$ Bonferroni look-elsewhere correction) -- below
the pre-declared $3\sigma$ post-look-elsewhere detection threshold
($|z|\gtrsim3.7$). We report this channel as \textbf{null}: splitting the
chirality sample by local environment, measured against a genuine
external tracer population rather than the chirality catalog's own
projected proxy, shows no environment dependence of handedness. The
spec-z 3D result is the more constraining of the two, since the
projected proxy line-of-sight-integrates true 3D structure.

\textit{Data.} DESI DR1 LSS \texttt{BGS\_BRIGHT-21.5} clustering
catalog, NGC+SGC, $0.1<z<0.4$ (300,043 tracers, 4 randoms/cap); the
environment proxy is a $k$=10 nearest-neighbor density-contrast estimate
against the randoms, not output from a void finder or void/filament
classifier.
...
diagnostic paths (author VoidFinder, Tempel, ASTRA -- the T-Web
diagnostic is reproduced in this manuscript, Sec.~\ref{sec:void}) are not
...
\texttt{object\_id} identifier, sky coordinates, the observed chirality label
...
10.5281/zenodo.21461899 above. The robustness/disclosure results of
Sec.~\ref{sec:robustness_disclosure}, plus the out-of-domain diagnostic that
led to the in-domain re-measurement and the sky-dependence systematic, are
individually reproducible from their registered manifests, all registered
in
\artifact{reproducibility/manifests/programs/galaxy-chirality.json}:
\begin{itemize}
\item pixel-level calibration ($N=20{,}000$ production run): \\
\artifact{reproducibility/manifests/experiments/row16-image-level-injection-n20k.json}
\item full-parent selection behavior (dipole and graded QC/leg sweep): \\
\artifact{reproducibility/manifests/experiments/p4p-row16i-full-parent-dipole.json} \\
\artifact{reproducibility/manifests/experiments/p4p-row16ib-axis-shift.json}
\item structure cross-correlations: \\
\artifact{reproducibility/manifests/experiments/row16iv-chirality-structure.json}
\item external environment (DESI DR1 BGS clustering catalog): \\
\artifact{reproducibility/manifests/experiments/row16ivb-bgs-environment.json}
\item direct dilution measurement, PA-restoring reversal on out-of-domain
Legacy Survey display cutouts -- \textbf{withdrawn}, superseded below: \\
\href{https://github.com/Hubify-Projects/bigbounce/tree/main/pipelines/p4prime_chirality_test/row16_pa_parity_transfer}{\texttt{pipelines/p4prime\_chirality\_test/row16\_pa\_parity\_transfer}} \\
\artifact{reproducibility/manifests/experiments/row16iib-pa-parity-transfer.json}
\item out-of-domain diagnostic that caught the domain mismatch above, the
largest-feasible-sample $d(180^\circ)$ recalculation, the rotation
test-time-average recovery bound, and the sky-dependence dipole systematic
(the last two remain out-of-domain and not re-measured;
Sec.~\ref{sec:robustness_disclosure}): \\
\href{https://github.com/Hubify-Projects/bigbounce/tree/main/pipelines/p4prime_chirality_test/row16_tta_2026_09_21}{\texttt{pipelines/p4prime\_chirality\_test/row16\_tta\_2026\_09\_21}} \\
\artifact{reproducibility/manifests/experiments/row16-tta-recovery-bound.json} \\
\artifact{reproducibility/manifests/experiments/row16-d180-largest-feasible-subset.json} \\
\artifact{reproducibility/manifests/experiments/row16-s6-dipole-projection.json}
\item in-domain dilution measurement (adopted): direct PA-equivalent
rotation test on the classifier's own \texttt{Smith42/galaxies} imaging,
$N=40{,}000$ galaxies streamed by HTTP range read, $320{,}000$ forward
passes, plus the pixel-scale diagnostic establishing the $134''$ in-domain
field: \\
\href{https://github.com/Hubify-Projects/bigbounce/tree/main/pipelines/p4prime_chirality_test/row16_indomain_d180_2026_09_22}{\texttt{pipelines/p4prime\_chirality\_test/row16\_indomain\_d180\_2026\_09\_22}} \\
\artifact{reproducibility/manifests/experiments/row16-indomain-d180.json}
\end{itemize}
