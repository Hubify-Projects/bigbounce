...
\newcommand{\paperVersion}{v4P.0.7}
...
$A_{95}^{\rm obs}\simeq0.98\%$ (Neyman $95\%$ CL limit
$A_{95}^{\rm CL}\simeq0.75\%$, the floor used below). A companion DESIVAST
...
$dA/df=+0.0167\pm0.0089$, far below both the naive label-identity slope
($+0.434$, $47\sigma$ from the pixel result) and the mixture-corrected
label identity ($-0.0093$, $2.9\sigma$). Table~\ref{tab:pixel_calib}
records this comparison. The production pipeline suppresses pixel-level
parity leakage well below what the label-level injection model alone would
predict, consistent with the baseline residual already disclosed in
Sec.~\ref{sec:catalog_classifier}.
...
the full $3{,}200{,}420$-galaxy parent (no QC/confidence cuts) gives a
dipole $A=0.566\%$ at $z=+4.44$, axis RA~$278.6^\circ$/Dec~$+25.3^\circ$. A
graded QC sweep localizes this to a single cut: removing only the
\texttt{primary\_hc} confidence cut restores the null ($z=+0.68$), while
removing only the raw-flip cut does not ($z=+9.13$, at a third,
near-antipodal axis); a per-imaging-leg partition shows the excess is also
removed by dropping the DES leg alone ($z=+0.48$) but not by any Galactic
cut. The axis is inconsistent across selections (shifting by
$107.5^\circ$), and the monopole-plus-mask-leakage floor in this
...
or used to adjust the primary channel's null result.

\textbf{Structure cross-correlations.} A pre-registered battery of 15
chirality$\times$structure statistics (local spiral surface-density
quartiles; angular cross-correlation with 1,244 spectroscopic anomaly
positions; five spectroscopic-redshift bins; and dipole projections along
the CMB dipole and CMB quadrupole--octopole axes plus the catalog's own
free best-fit direction), each tested against 1,000-realization
label-shuffle and sky-rotation nulls with a $3\sigma$
look-elsewhere-corrected threshold declared in advance, returns no
detection: environment ($\chi^2$ $z=-0.38$, trend $z=+0.17$), the anomaly
cross-correlation (all eight angular bins and the nearest-neighbor excess
null; the largest raw excursion falls from $-2.37\sigma$ to $-1.05\sigma$
under a rigid-rotation null), redshift (trend $z=-1.63$), and the two fixed
astrophysical axes ($z=+0.21$, $-0.13$) are all null, and the free
best-fit dipole ($0.44\%$) is below its own look-elsewhere-corrected null
($0.38\%\pm0.16\%$, $z=+0.32$). Table~\ref{tab:structure_null} summarizes.
Two data limits apply: the environment test uses a parity-blind projected
surface-density proxy in place of a 3D void/filament catalog, since the
DESI DR1 LSS products on disk are QSO-only at $z=0.8$--$2.1$ with no
overlap with the $z\lesssim0.3$ spiral sample and no low-$z$ void catalog
was available; and Shamir's claimed axis is not tested, as the sources this
paper draws its quoted amplitudes from quote no explicit RA/Dec for it.
...
\texttt{BGS\_BRIGHT-21.5} clustering catalog and its randoms
(300,043 tracers, 4 randoms per cap; not a void finder) -- finds no
dependence of the parity fraction on environment in either subset it can
be tested on. Table~\ref{tab:bgs_environment} reports the three-way
void-like/wall-filament/node-like split for the 121,417-spiral DESI
spectroscopic-redshift subset (three-dimensional comoving density) and the
949,584-spiral photometric/no-redshift subset (projected density).
...
$p=0.084$ two-sided). Per-bin dipole amplitudes are likewise consistent
with their nulls in every bin ($|z|\le2.15$). The largest excursion in
the full 14-statistic battery (2 subsets $\times$ [3 $f_{\rm CW}$ + 1
$\chi^2$ + 3 dipole]) is the projected node-like bin's parity fraction,
$z=+2.96$ ($p_{\rm local}=0.0060$, $p=0.084$ after the pre-registered
$\times14$ Bonferroni look-elsewhere correction) -- below the
pre-declared $3\sigma$ post-look-elsewhere detection threshold
...
their registered manifests (the latter two registered in
\artifact{reproducibility/manifests/programs/galaxy-chirality.json}):
\begin{itemize}
\item pixel-level calibration: \\
\artifact{reproducibility/manifests/experiments/row13-image-level-injection-pilot.json}
\item full-parent selection behavior: \\
