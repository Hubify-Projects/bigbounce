...
\newcommand{\paperVersion}{v4P.0.10}
...
observed-to-physical transfer function bounds its dilution at
$D \le 0.717 \pm 0.005$, raising the physical-parity sensitivity floor to
$A_{95}^{\rm phys} \ge 1.37\%$ -- consistent with the illustrative bridge
value $g=0.398$ used in Sec.~\ref{sec:bh}. The leg-removal result, (i),
remains unresolved and is stated explicitly, not adopted as a correction,
in Sec.~\ref{sec:robustness_disclosure}.
...
strengthen this statement. Sec.~\ref{sec:robustness_disclosure} reports a
direct measurement of that transfer function. Presenting the released
pipeline with a genuine handedness reversal --- the mirror about each
galaxy's own major axis, which preserves its observed position angle ---
and marginalising over position angle, the reversal is registered in
$\bar\epsilon = 0.754 \pm 0.005$ of high-confidence spirals, $51\sigma$
below unity. Because handedness does not depend on the orientation at
which a galaxy is presented, the rate at which the pipeline contradicts
itself between two orientations of the same image bounds the dilution $D$
of any true population asymmetry from above, $D \le 1 - \max_\theta
P\big(L(0) \ne L(\theta)\big) = 0.717 \pm 0.005$, with no human truth
labels entering the argument. The observed-label floor $A_{95}^{\rm
obs} = 0.98\%$ therefore corresponds to a physical-parity floor
$A_{95}^{\rm phys} = A_{95}^{\rm obs}/D \ge 1.37\%$. This is consistent
with the illustrative $g = 0.398$ used above (which would give $2.46\%$)
and supersedes it as the quantity we can defend: it is an upper bound on
$D$, so it can only weaken the exclusion, and we adopt it in that
direction only.
...
direct measurement of the observed-to-physical transfer function is also
reported there; it bounds the dilution at $D \le 0.717 \pm 0.005$ and is
consistent with, not in tension with, the illustrative bridge $g=0.398$
used above.
...
the observed-to-physical transfer function bounding its dilution at
$D \le 0.717 \pm 0.005$ ($A_{95}^{\rm phys} \ge 1.37\%$), is consistent
with the illustrative bridge used above and is reported in the same
section.
...
The transfer function is instead measured directly, by presenting the
pipeline with a genuine handedness reversal at the galaxy's observed
...
high-confidence spirals; the deficit is a rotation-equivariance failure
rather than a parity failure, since the flip test-time average renders the
parity operation exact wherever it commutes with the rotation
($\bar\epsilon = 1$ at $0^\circ$ and $180^\circ$, the positive control)
and $\epsilon(\varphi)$ tracks the rotation-only class stability
elsewhere. The implied dilution bound $D \le 0.717 \pm 0.005$ raises the
physical-parity sensitivity floor to $A_{95}^{\rm phys} \ge 1.37\%$, which
is the figure we quote. Transfer efficiency is not uniform on the sky ---
it differs by $0.036 \pm 0.005$ between the hemispheres of the
strict-subset dipole axis --- and we flag that position-dependent
dilution as an open systematic rather than correcting for it here.

\begin{table}[t]
\centering
\caption{Pixel-level injection calibration and the direct dilution
measurement (image-level parity injection, $N=20{,}000$, 10 seeds; PA-restoring
reversal, released preprocessing).}
...
$\bar\epsilon$ (PA-restoring reversal, \texttt{primary\_hc}) & $0.754\pm0.005$ \\
$D \le 1-\max_\theta d(\theta)$ & $0.717\pm0.005$ \\
$\Rightarrow A_{95}^{\rm phys}$ & $\ge 1.37\%$ \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
Sec.~\ref{sec:robustness_disclosure} are individually reproducible from
their registered manifests, all registered in
