...
\newcommand{\paperVersion}{v4P.0.9}
...
confronted. Two results in Sec.~\ref{sec:robustness_disclosure} directly
qualify this floor and this null and are not otherwise referenced from
...
reported here is null-consistent; and (ii) a first direct empirical
measurement of the observed-to-physical transfer function implies a
response roughly an order of magnitude smaller than the illustrative
bridge value $g=0.398$ used in Sec.~\ref{sec:bh}, which if propagated at
face value would substantially raise the physical-amplitude floor implied
by $A_{95}^{\rm obs}$. Neither is resolved here; both are stated
explicitly, not adopted as corrections, in
...
first direct empirical probe of that transfer function whose implied
response is roughly an order of magnitude smaller than $g=0.398$; taken at
face value this would weaken, not strengthen, the present exclusion, and is
not resolved here.
...
Sec.~\ref{sec:robustness_disclosure} discloses two results that qualify
this exclusion and are not adopted as corrections to it: the primary
sample's own per-imaging-leg partition is not uniformly clean, and a new
pixel-level transfer measurement is in tension, by roughly an order of
magnitude, with the illustrative observed-to-physical bridge used above.
...
from the same torsion mechanism. Two disclosure results
(Sec.~\ref{sec:robustness_disclosure}) qualify this exclusion and remain
unresolved: the primary channel's own leg-removal stability, and a direct
pixel-level measurement of the observed-to-physical transfer function
roughly an order of magnitude smaller than the illustrative bridge used
above; neither is adopted as a correction here.
...
$dA/df=+0.0167\pm0.0089$, an order of magnitude below the naive
label-identity slope ($+0.434$, $47\sigma$ from the pixel result) and, against
the mixture-corrected label identity ($-0.0093$), consistent in magnitude
but sign-flipped at $2.9\sigma$ -- an unresolved discrepancy, not a
consistency, between the two label-level identities. Table~\ref{tab:pixel_calib}
records this comparison; the injection baseline itself is $A_0=+0.59\%$ for
...
same baseline residual. The production pipeline suppresses pixel-level
parity leakage well below what the label-level injection model alone would
predict.

This pixel-level slope is also the first direct empirical measurement
bearing on the observed-to-physical bridge factor $g$ carried, on an
explicitly illustrative and not-adopted-for-strengthening basis, in
Assumption~2 of Sec.~\ref{sec:bh} ($g=0.398$, from the committed exclusion
script, not an established calibration). Taken at face value, the implied response
ratio $dA/df$ (naive identity) $=0.0167/0.434\approx0.038$ is roughly an
order of magnitude smaller than $g=0.398$; propagated naively, it would
move the observed-label floor $A_{95}^{\rm obs}=0.98\%$ to a physical-amplitude
floor of order $0.98\%/0.038\approx26\%$, under which every literature
amplitude's low-end value in Table~\ref{tab:bh_exclusion} would fall below
the floor rather than above it (only Shamir (2025)'s range, $20$--$33\%$,
straddles this floor). We do not adopt this propagation: the
pixel-level slope is a response to an injected mirror-flip fraction at a
fixed operating point, not a validated dipole-amplitude transfer function on
the sky, and its sign relative to the mixture-corrected identity is itself
unresolved (above). We state this tension explicitly rather than leave it
implicit: Sec.~\ref{sec:bh}'s exclusion rests on $g=0.398$ as an
illustrative, non-adopted bridge, and this new direct measurement of the
pipeline's own pixel-to-label transfer is not consistent with that bridge
value at face value. Resolving this -- deriving a validated
observed-to-physical transfer function, rather than assuming or measuring an
uncalibrated proxy for one -- is necessary before the Sec.~\ref{sec:bh}
exclusion can be treated as more than illustrative, and is not attempted in
this manuscript.

\begin{table}[t]
\centering
\caption{Pixel-level injection calibration vs.\ label-identity models
(image-level parity injection, $N=20{,}000$, 10 seeds).}
\label{tab:pixel_calib}
\begin{ruledtabular}
\begin{tabular}{lcc}
Model & $dA/df$ & $\sigma$ from pixel result \\
\hline
Pixel-level injection (measured) & $+0.0167\pm0.0089$ & --- \\
Naive label identity & $+0.434$ & $47\sigma$ \\
Mixture-corrected label identity & $-0.0093$ & $2.9\sigma$ \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
10.5281/zenodo.21461899 above. The four robustness/disclosure results of
