...
\newcommand{\paperVersion}{v4P.0.11}
...
observed-to-physical transfer function was attempted, via a PA-restoring
handedness-reversal test, but the resulting dilution bound is
\textbf{on hold}: a positive-control check found the measurement was run
on images outside the classifier's own domain, so no dilution bound or
physical-parity floor is adopted here pending an in-domain re-measurement
(Sec.~\ref{sec:robustness_disclosure}). Both (i) and (ii) remain
unresolved and are stated explicitly, not adopted as corrections, in
...
architectural argument for why that transfer is not unity, together with
an attempted direct measurement of it that is currently \textbf{on hold}.
...
asserting $D = 1$. That companion calibration presented the released
pipeline with a genuine, PA-restoring handedness reversal -- the mirror
about each galaxy's own major axis, which preserves its observed position
angle -- but a pre-registered positive control found the measurement was
run on images outside the classifier's own domain: re-running the released
preprocessing and checkpoint on those images reproduces the released
catalog's own labels for only $43$--$45\%$ of galaxies across three
independent draws, against $99.99\%$ agreement with an in-domain forward
pass, showing the images were Legacy Survey display cutouts rather than
the classifier's own $224\times224$ DR8 inference images
(Sec.~\ref{sec:robustness_disclosure}). The dilution bound that
measurement produced is therefore \textbf{withdrawn pending an in-domain
re-measurement} on the correct, pinned \texttt{Smith42/galaxies} images;
we quote the observed-label floor $A_{95}^{\rm obs} = 0.98\%$ here without
adopting a numerical dilution bound or physical-parity floor.
...
direct measurement of the observed-to-physical transfer function was also
attempted there; it is currently \textbf{on hold}, having been run on
images outside the classifier's own domain, and no dilution bound is
adopted pending an in-domain re-measurement.
...
and is not adopted as a correction here. A second, an attempted direct
measurement of the observed-to-physical transfer function, is currently
\textbf{on hold} -- the measurement was run on images outside the
classifier's own domain and is withdrawn pending an in-domain
re-measurement -- and is reported in the same section.
...
The transfer function was targeted for direct measurement, by presenting
the pipeline with a genuine handedness reversal at the galaxy's observed
position angle (Assumption~2). Marginalised over position angle, a real
reversal is registered in $\bar\epsilon = 0.754 \pm 0.005$ of
high-confidence spirals on the images that measurement used; the deficit
pattern is a rotation-equivariance failure rather than a parity failure,
since the flip test-time average renders the parity operation exact
wherever it commutes with the rotation ($\bar\epsilon = 1$ at $0^\circ$
and $180^\circ$, the positive control) and $\epsilon(\varphi)$ tracks the
rotation-only class stability elsewhere.

\textbf{This measurement, and the dilution bound $D \le 0.717 \pm 0.005$
($A_{95}^{\rm phys} \ge 1.37\%$) it implied, are on hold.} A pre-registered
positive control required the released preprocessing, run at
$\theta = 0^\circ$ (the identity transform), to reproduce the released
catalog's own labels on $\geq 99.9\%$ of matched galaxies. On the images
this measurement used it reproduces them for only $43.9\%$ of $19{,}800$
galaxies ($42.4\%$ and $44.7\%$ on two further independent draws), while
an identical check against this program's own previously committed
forward passes on those same images agrees at $99.99\%$ -- showing the
classifier code and preprocessing are correct and the images are the
wrong ones: Legacy Survey display cutouts (a $39.3''$ field upsampled to
$224$~px) rather than the \texttt{Smith42/galaxies} images the released
classifier was actually run on for inference ($224\times224$~px, $58.7''$
native DR8 field, Sec.~\ref{sec:catalog_classifier}). An out-of-domain
instability bounds an in-domain one in neither direction, so no dilution
bound or physical-parity floor is adopted from this measurement until it
is re-run on the correct, pinned \texttt{Smith42/galaxies} images. The
in-domain re-measurement is bounded, disk-only work and does not require a
GPU.
...
$A_{95} = 0.98\%$ limit quoted here. This is a systematic floor on the
interpretation of the limit, not a detected contamination: it assumes the
dilution tracks the measured efficiency map, and a true asymmetry of zero
produces no dipole under any dilution.

\begin{table}[t]
\centering
\caption{Pixel-level injection calibration, and the status of the direct
dilution measurement (image-level parity injection, $N=20{,}000$, 10 seeds;
PA-restoring reversal, released preprocessing). The PA-restoring-reversal
rows are \textbf{on hold}: measured on images outside the classifier's own
domain, withdrawn pending an in-domain re-measurement
(Sec.~\ref{sec:robustness_disclosure}).}
...
$\bar\epsilon$ (PA-restoring reversal, \texttt{primary\_hc}) & on hold \\
$D \le 1-\max_\theta d(\theta)$ & on hold \\
$\Rightarrow A_{95}^{\rm phys}$ & on hold \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
10.5281/zenodo.21461899 above. The five robustness/disclosure results of
Sec.~\ref{sec:robustness_disclosure}, plus the out-of-domain diagnostic and
sky-dependence systematic that put the fifth result on hold, are
...
\item direct dilution measurement (PA-restoring reversal, released
preprocessing) -- \textbf{on hold}, see below: \\
...
\item out-of-domain diagnostic that placed the above on hold, the
