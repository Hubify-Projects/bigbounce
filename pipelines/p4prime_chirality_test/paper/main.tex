...
\newcommand{\paperVersion}{v4P.0.2}
...
literature that motivates the model. This null is measured on
...
astrophysical claim rather than a detached data product.
...
purity. We exclude 59,515 HC rows flagged \texttt{raw\_flip\_qc\_unsafe} (a
...
and are the single primary contract this paper's tests draw on; each row
carries \texttt{dr8\_id}, sky coordinates, the observed chirality label,
per-class probabilities, and quality-control flags including
\texttt{raw\_flip\_qc\_unsafe}, in a single Parquet file with a committed
schema sidecar. Full architecture, per-region calibration, and the complete
systematics appendix are described in the archived release
paper~\cite{Golden:P4v274} and are not reproduced here beyond what
Sec.~\ref{sec:bh} needs.
...
verbatim~\cite{Golden:P4v274}. All seven tabulated tests pass; these are
...
confidence-level bound on the measured value. A genuine $95\%$
confidence-level statement is available directly from the same committed
null distribution used for $z_{\rm mom}$ and rank $p$ above: its $95$th
percentile is $0.669\%$
(\artifact{research/bh\_universe\_dipole/a95\_null\_cl\_2026\_09\_02.py}), so
values of $A_{\rm dip}$ at or above $0.669\%$ occur in fewer than $5\%$ of
null draws. The observed $A_{\rm dip}=0.467\%$ lies below both this $95\%$
CL null threshold and the detection-power floor $A_{95}^{\rm obs}=0.98\%$.
$A_{95}^{\rm obs}$ is additionally an \emph{observed-label} sensitivity
floor on the classifier's chirality labels, not a physical parity-amplitude
bound on deprojected spin: the physical bound additionally requires a
...
($N_{\rm boot}=2{,}000$ resamples, all valid, seed 42, primary HC sample
$N=949{,}584$). The resulting correlation matrix, computed directly from
...
$0.79$ correlation to the real-space channel here) and is nearly
...
declared, post-review $13$-column linear nuisance basis (void indicator,
...
illustrative $g=0.398$ bridge; \S\ref{sec:bh_exclusion}). The four rows pool
non-commensurable statistics -- Longo's dipole amplitude, Shamir (2012)'s
...
both files at the Zenodo DOI and Hugging Face mirror above.
