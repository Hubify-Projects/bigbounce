# Reproducibility manifest — anomaly flagship manuscript (vAF.0.1)
...
| Property | Value |
|---|---|
| Scripts | `pipelines/p1_highz_tracers/flagship_assembly_2026-09-18/scripts/assemble_flagship_evidence.py` then `.../make_tables.py` |
| Inputs | only the ten hashed artifacts above (no network access) |
| Outputs | `flagship_assembly_2026-09-18/outputs/*` (validation contract, family evidence, follow-up set, provenance re-check, zcatalog re-join table) and this directory's `tables/*.tex`, `figures/*.pdf` |
...
the release carries openly), then a 7-page PDF with 0 undefined references and
0 overfull hboxes.
...
| Property | Value |
|---|---|
| Source | `pipelines/p1_highz_tracers/anomaly_flagship_draft/main.tex` (`\paperVersion` = vAF.0.1) |
...
| Tables | machine-generated into `tables/` by `make_tables.py`; never hand-edited |
| Figures | `figures/fig_score_vs_quality.pdf`, `figures/fig_family_evidence.pdf` (generated), `figures/sky_fraction_by_score.png` (from the landed run) |
| Bibliography | inline `thebibliography`; no BibTeX pass required |
| Build | 4 × `pdflatex`; last verified 2026-09-18: 7 pages, 0 undefined references, 0 overfull hboxes |
