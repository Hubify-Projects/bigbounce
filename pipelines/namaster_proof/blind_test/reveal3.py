"""Unseal batch 3 and score the frozen verifier's blind calls.
...
def main() -> int:
    ...
    ok = (seal3.digest(seal3.derive(key)) == committed["assignment_sha256"]
          == seal3.digest(assignment))
    ...
    per_arm = defaultdict(lambda: {"n": 0, "flagged": 0, "rules": defaultdict(int),
                                   ...
                                   "r6_crossrun_disjunct": 0, "r7_residuals": []})
    for run_id, spec in assignment.items():
        ...
        if verdict.get("r7_relative_residual") is not None:
            arm["r7_residuals"].append(verdict["r7_relative_residual"])
        ...
        if "R6_result_reuse_across_inputs" in verdict["rules"]:
            ...
            key_name = ("r6_reference_disjunct"
                        if verify3.band_hash(payload["bandpowers"])
    ...
    report = {
        ...
        "rules": "frozen, RULES_v3_FROZEN.md (committed before the seal)",
        "detection_structural_S1_S4b": cp_bounds(tp, n_struct),
        ...
        "S6_detection": cp_bounds(per_arm["S6_effective_multipole"]["flagged"],
                                  per_arm["S6_effective_multipole"]["n"]),
        "S4b_crossrun_sources": json.loads((SEALED / "crossrun_sources.json").read_text())
        if (SEALED / "crossrun_sources.json").exists() else {},
        "independence_caveat": ("R1-R5 compare seed-independent quantities against a fixed "
                                ...
                                "R7 is seed-dependent (p and the spot-row draw both vary), so its "
                                "replicates are distinct evaluations of one deterministic variant "
                                "-- class-level evidence, not a run-level interval."),
        "per_arm": {a: {"n": v["n"], "flagged": v["flagged"], "rules": dict(v["rules"]),
                        ...
                        "r7_residual_max": (max(v["r7_residuals"]) if v["r7_residuals"] else None),
                        "r7_residual_min": (min(v["r7_residuals"]) if v["r7_residuals"] else None)}
