...
- **P5** v0.1.12-2026-05-22 — **separate companion**: Environmental dependence of spiral chirality across DESI LSS. Paper fully drafted (23 pp / 757 KB) covering Phase 1 V-Web + Phase 2 sensitivity sweep (9 grid configs) + Tempel+2014 FoF cross-validation (filament concordance 0.026pp) + LEE correction + 6 figures + RSD-robustness Limitations item + **§sec:tweb_compare three-VAC concurrent-literature cross-validation + direct DESIVAST per-galaxy cross-match** (T-Web DR1 arXiv:2604.02463 sheet/filament agree to ~5pp, ASTRA EDR arXiv:2604.01456 probabilistic-classifier complementary framing, DESIVAST DR1 BGS arXiv:2411.00148 / ApJ 982 38 + 0/6 V-Web void spirals inside any of 101,863 DESIVAST hole spheres at z≤0.24) + **§sec:results_within_class_density cluster-class density-quartile decomposition** showing the −4.7σ cluster headline is boundary-misclassification leakage from filaments NOT a clean density-dependent effect (Q3-typical-density null at σ=−0.37). PDF 0 overfull / 0 undef refs / 0 undef cites. Pipeline at `pipelines/p5_desi_chirality/`. SSOT: `project-context/SSOT/paper-5/status.md`. Readiness 80%. **First R-round still blocked on OpenRouter per-key weekly cap.**
