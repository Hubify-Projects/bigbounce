...
- **P5** v0.1.15-2026-05-22 — **separate companion**: Environmental dependence of spiral chirality across DESI LSS. Paper fully drafted (24 pp / 774 KB) covering Phase 1 V-Web + Phase 2 sensitivity sweep (9 grid configs) + Tempel+2014 FoF cross-validation + LEE correction + 6 figures + RSD-robustness + **§sec:tweb_compare three-VAC concurrent-literature cross-validation + direct DESIVAST per-galaxy cross-match + DESIVAST-anchored void classifier** (T-Web DR1 arXiv:2604.02463, ASTRA EDR arXiv:2604.01456, DESIVAST DR1 BGS arXiv:2411.00148 / ApJ 982 38; per-galaxy 0/6 V-Web void spirals inside DESIVAST holes; **DESIVAST-anchored void classifier at n=56,981 = 133× V-Web sample → cw_fraction 0.4964 vs non-void 0.4971, Δ=0.0007 statistically indistinguishable, strongest single positive evidence for headline environment-independence**) + **§sec:results_within_class_density cluster-class quadruply-decomposed boundary-leakage** (density Q3-null + z uniform + tracer-program bright-vs-dark sign-flip + DESIVAST-anchored re-analysis). PDF 0 overfull / 0 undef refs / 0 undef cites. Pipeline at `pipelines/p5_desi_chirality/`. SSOT: `project-context/SSOT/paper-5/status.md`. Readiness 85%. **First R-round still blocked on OpenRouter per-key weekly cap.**
