...
| 6 | **P1A** ECH no-go | `submissions/P1A/arxiv_p1a_v1A.0.111.tar.gz` | v1A.0.111 | 37 | v1A.0.111 D-round final polish: disclaimer consolidation + abstract tightening, presentation + disclosure only, no science number changed; tarball rebuilt + standalone-verified (0 undef-refs, 37pp), stale v1A.0.110/.104 removed. Fill `TODO-SUBMISSION` markers in references.bib with wave-1 IDs (see `SUBMISSION_NOTE`) → recompile → rebuild tarball → submit. Then insert P1A's ID back into P1B v2 (reciprocal note). |
