...
|---|-------|--------|-----|----|-------|
| 1 | **P4** chirality catalog | `submissions/P4/arxiv_p4_v1.0.227.tar.gz` (md5 4d384c16) | v1.0.227 | 33 | astro-ph.CO, x-list GA. Metadata in `ARXIV_METADATA.txt`. Bundle rebuilt from current source 2026-07-09 (the .224 bundle was one version stale); standalone re-extract+compile 0 err / 0 undef / 33pp. **Submit FIRST — P5 needs its ID.** |
| 2 | **P3** anomaly catalog | `submissions/P3/arxiv_p3_v3.1.148.tar.gz` (md5 1fa084f7) | v3.1.148 | 36 | Title: "A Multi-Survey Autoencoder Anomaly-Candidate Catalog: 268,519 Reconstruction-Outlier Sources from a Native-Trained Scan of 37.3 Million Spectra and Map Patches". v3.1.148 R9 EXT closure; standalone re-extract+compile 0 err / 0 undef / 36pp. HF dataset `bamfai/bigbounce-anomaly-catalog` already PUBLIC (no flip needed); Zenodo DOI mints at submit (optional). |
| 3 | **P2** f_NL forecast + Cai/Li resolution | `submissions/P2/arxiv_p2_v1.7.106.tar.gz` (md5 07f2c6c3) | v1.7.106 | 37 | Positioned as honest forecast + the −35/16 literature resolution. v1.7.106 W11 EXT presentation-closure (calibrated the −35/16 claim to the exact vertex-sum+Li evidentiary framing, made the SPHEREx additive-quadrature budget an explicit displayed equation, labeled MegaMapper decisively as uncalibrated high-z, fixed stale f_NL≈−4); no science number changed, −35/16 unchanged; standalone re-extract+compile 0 err / 0 undef / 37pp. Zenodo DOI at submit (optional). |
...
|---|-------|--------|-----|----|------------------------------|
| 4 | **P5** DESI environmental chirality | `submissions/P5/arxiv_p5_v0.1.110.tar.gz` | v0.1.110 | 40 | Set the single `\paperIVarxiv` macro to P4's real ID (see `submissions/P5/SUBMISSION_NOTE.txt`) → recompile → rebuild bundle → submit. NOTE: latest committed P5 bundle is `arxiv_p5_v0.1.110.tar.gz`; source is v0.1.110, so the v0.1.110 bundle is produced by the wave-2 ID-insertion rebuild (or rebuild now from source if submitting before wave-1 IDs exist). |
| 5 | **P1U** unified Paper 1 (theory + reproducibility) | `submissions/P1A/arxiv_p1_unified_v1U.0.5.tar.gz` | v1U.0.5 | 60 | P1B is MERGED into P1U — no standalone P1B submission, no reciprocal note. Fill the four companion arXiv IDs in `arxiv/references.bib` `TODO-SUBMISSION` notes (`Golden2026P2/P3/P4` + P5) with wave-1 IDs; the `Golden2026P1a` self-ref stays `[arXiv:XXXX.XXXXX]` (P1U's own ID does not exist yet) → recompile `arxiv/paper1_unified.tex` (regenerates .bbl) → rebuild tarball → submit. |
...
## Verified state per paper (finals: P4 v1.0.227 · P3 v3.1.148 · P2 v1.7.106 · P5 v0.1.110 · P1U v1U.0.5)
- **P4** (v1.0.227): condensed title + reader-first abstract; Grok+Gemini MINOR, null "robustly supported"; every number reproduces.
- **P3** (v3.1.148): byte-identical reproduction; anomalies proven real high-z QSOs (p~1e-103); eROSITA tier excised from every count.
- **P2** (v1.7.106): −35/16 3-way certified (App A Table VII per-vertex derivation); independent+RSD Fisher; Grok MINOR.
- **P5** (v0.1.110): all numbers exact; Δf_CW monopole-invariant on public data; Paper-IV = citation timing.
- **P1U** (v1U.0.5): P1A theory + P1B reproducibility companion MERGED; all tiers derived/grounded/NDA-bounded; f_NL −35/16 propagated; Fierz lemma closed; companion numbers artifact-cited; 0 genuinely-new Grok/Gemini findings.
...
> **⚠ SUPERSEDED for the merged layout (2026-07-09).** Wave-2 is now **P5 v0.1.110
> + the unified Paper 1 (P1U v1U.0.5)** — P1B is merged into P1U, so there is **no
