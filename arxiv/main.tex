...
\item \textit{ALP birefringence consistency check (secondary)}: A spectator ALP with natural parameters ($f_a \sim \MPl$, $m \sim H_0$) accommodates the observed $\beta = 0.242^\circ \pm 0.061^\circ$ ($3.9\sigma$) without fine-tuning. This ALP model is a spectator field mechanism independent of the gravitational theory: the same $\beta \approx 0.27^\circ$ arises in standard GR with an identical ALP, so the result is not a distinctive ECH prediction. The model class was previously studied by Fujita \etal~\cite{Fujita2021}; our contribution is the ECH-motivated parameter identification and independent MCMC inference, not the model itself. The result is a consistency check of the parity-violation signal, not a prediction of minimal ECH (which does not derive the photon-torsion coupling; see Sec.~\ref{sec:birefringence_check}).
\end{enumerate}

\textit{Scope note.}---This paper intentionally spans three threads: the ECH dark-energy framework, the systematic barrier catalog, and the surviving matter-bounce observational predictions. The hybrid scope is not a presentational accident but a scientific necessity: the negative results (14 barriers) are precisely what redirect the science case toward the positive results ($f_{\rm NL}$, birefringence). Presenting the barriers in isolation would leave the reader without the payoff; presenting the predictions without the barriers would lack the motivation for why these specific predictions survive when the ECH dark-energy program does not. The companion papers each focus on a single thread: Paper~2 provides full forecast details for the $f_{\rm NL} = -35/8$ SPHEREx test, Paper~3 presents the multi-survey anomaly catalog, and Paper~4 reports the galaxy chirality catalog.
