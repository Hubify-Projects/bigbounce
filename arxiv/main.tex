...
\item \textit{MCMC verification recovering $\Lambda$CDM}: Independent Cobaya verification across two frozen dataset combinations demonstrates that $\Delta\Neff$ is consistent with zero and $H_0$ recovers the standard Planck $\Lambda$CDM value of $67.68 \pm 1.06$, confirming $\Lambda$CDM stability in the spin-torsion extended parameter space.
...
\textit{Scope note.}---This paper intentionally spans three threads: the ECH dark-energy framework, the systematic constraint catalog, and the matter-bounce observational predictions that survive the constraint analysis. The hybrid scope is a scientific necessity: the structural constraints (14 mechanism-specific results) define which observables remain testable, and the surviving testable predictions ($f_{\rm NL}$, ALP birefringence, primordial gravitational waves) are the focus of independent cosmological tests. Presenting the constraints in isolation would leave the reader without the testable payoff; presenting the predictions without the constraints would lack the motivation for why these specific channels are the right places to look in this class of models. This paper is intended to stand alone as a self-contained framework reference.
