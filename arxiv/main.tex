...
\newcommand{\paperVersion}{v2.3.18}
...
\date{May 5, 2026, 01:30 PDT --- \paperVersion}
...
The $\ln B$ above is biased at $r=-0.89$ correlation and is reported as indicative only. The primary model-selection metrics for this proxy comparison are $\Delta$AIC and $\Delta$BIC, shown below. For the $\Lambda$CDM$+\Delta\Neff$ proxy versus $\Lambda$CDM null on the full-tension dataset (Table~\ref{tab:modelcomp} entries):
\begin{align}
...
The headline observational constraint adopted in this work is the published Eskilt~\etal\ joint Planck+ACT analysis $\beta = 0.342^\circ \pm 0.094^\circ$ ($3.6\sigma$ from zero), which accounts for shared calibration systematics between Planck and ACT. The simplified inverse-variance combination computed below ($\beta = 0.241^\circ \pm 0.061^\circ$, $3.9\sigma$) is retained as an auxiliary cross-check only; it neglects inter-experiment correlations and inflates the apparent significance.\\
...
\textit{NANOGrav spectral fit.}---The matter-bounce induced GW spectrum has a universal $f^2$ infrared scaling, corresponding to spectral index $\gamma = 3$ in the characteristic-strain parameterization $h_c \propto f^{(3-\gamma)/2}$. [SUPERSEDED 2026-06-09 --- provenance audit C7.] An earlier revision of this section reported a Savage-Dickey Bayes factor $B(\text{bounce}/\text{SMBHB}) = 34.0$ built on a spectral-index posterior $\gamma = 3.20 \pm 0.42$ that was described as an ``independent GPU MCMC reanalysis'' of the NANOGrav 15-year free spectrum. A provenance audit found that input was a synthetic-from-power-law, single-PTA CPU fit (no generating script committed; see \texttt{reproducibility/p3\_pta\_mcmc/README.md}), and the claim is withdrawn. The provenanced replacement analysis --- a fit to the NANOGrav 15-yr HD-correlated Ceffyl KDE free-spectrum posteriors --- yields $\gamma = 2.567 \pm 0.382$ and a Savage-Dickey $B_{\rm MB/SMBHB} = 7.14\times 10^{3}$ ($\log_{10}B = +3.85$), and is reported with full methodology in the companion anomaly-catalog paper (\texttt{pipelines/p3\_anomaly\_engine/paper3\_draft.tex}, \S NANOGrav), which supersedes this document for all PTA claims. Earlier draft revisions of this section had also reported template-level Bayes factors derived from synthetic data points reconstructed from the published power-law fit; that comparison was methodologically circular ($\chi^2/\text{dof} = 0.012$) and was removed previously.
...
$\beta = 0.342^\circ \pm 0.094^\circ$ & Literature (headline) & Sec.~\ref{sec:birefringence_check} & Eskilt~\etal\ joint Planck+ACT ($3.6\sigma$); IVW $0.241^\circ$ ($3.9\sigma$) cited as auxiliary only \\
