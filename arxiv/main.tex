...
\textit{Important caveat on the NaMaster analysis.}---The Monte Carlo error estimate above was originally based on only 50 noise realizations, giving ${\sim}14\%$ uncertainty on the standard deviation estimate ($1/\sqrt{2N_{\rm MC}}$). The production-quality analysis has now been re-run with 500 Monte Carlo realizations, reducing this calibration uncertainty to ${\sim}5\%$ (N$_{\rm side}=512$, $\ell_{\rm max}=1024$, $f_{\rm sky}=0.32$, ACT-noise level $10\,\mu\rm{K\cdot arcmin}$); injecting the ALP prediction $\beta=0.27^\circ$ recovers $\hat\beta=0.238^\circ$ (recovery bias $0.032^\circ$, SNR$=20.32$ at ACT sensitivity), and injecting the joint Planck+ACT central value $\beta=0.342^\circ$ recovers $\hat\beta=0.302^\circ$ (SNR$=25.71$); the consistency between prediction and observation at this Monte Carlo precision is $0.77\sigma$ (canonical source: \texttt{pipelines/h200\_results/pod1\_namaster\_umap\_2026-04-29/results/namaster-birefringence/summary.json}). The systematic variation in $\beta$ from $0.167^\circ$ to $0.322^\circ$ across analysis choices (frequency combination, mask threshold, $N_{\rm side}$, $\ell_{\rm max}$) exceeds the quoted statistical uncertainty of $0.065^\circ$ and should be treated as the dominant error budget. This analysis is therefore a preliminary cross-check of the published birefringence detections, not a competitive independent measurement; the pipeline-level recovery test confirms that no systematic bias from the pseudo-$C_\ell$ deconvolution exceeds $0.04^\circ$.
...
\item ALP model prediction ($C_{a\gamma} = 8$, $\theta_i = 1$, $m \approx 2\,H_0$): $\beta \approx 0.29^\circ$
