...
\label{eq:Az}
\end{equation}
with $A_0$ the present-day amplitude. The parity-odd operator introduces systematic chirality into the tidal field, but the framework's coupling $\alpha/M \sim 10^{-21}\;\text{GeV}^{-1}$ underpredicts any plausible spin asymmetry by $>100$ orders of magnitude. Our chirality catalog of $8.47$~million galaxies~\cite{Golden:2026chirality} confirms the null: $\fcw^{\rm eq} = 0.5012 \pm 0.0006$ ($0.4\sigma$ from parity conservation). Galaxy spin asymmetry is not a prediction of the theory, and no significant signal exists to explain.
...
\item \textit{Spin-torsion (this work)}: Isotropic birefringence across all $\ell$ (rotation angle not derived from the torsion background alone; requires a photon--torsion coupling such as the spectator-ALP channel of Sec.~\ref{sec:birefringence_check}), plus an anisotropic low-$\ell$ component whose amplitude and angular spectrum are not derived in this work and are flagged as an open direction in Sec.~\ref{sec:birefringence_check}.
\end{itemize}
...
Several groups have reported a dipole asymmetry in galaxy spin handedness ($A_0 \sim 0.003$, axis near the CMB kinematic dipole)~\cite{Shamir2012,Shamir2022,Longo2011}, but independent reanalyses~\cite{PatelDesmond2024,Philcox2025} find results consistent with isotropy. Our own chirality catalog of $8{,}474{,}531$ galaxies~\cite{Golden:2026chirality}---the largest bias-audited catalog to date, $4.3\times$ CE-ResNet and $6.5\times$ Shamir---finds $\fcw^{\rm eq} = 0.5012 \pm 0.0006$ ($0.4\sigma$ from parity conservation), with a null all-sky dipole ($0.43\sigma$). Shamir's claimed $3\%$ asymmetry is refuted (our maximum regional asymmetry is $0.47\%$, $7\times$ smaller).

The galaxy spin channel is therefore a \textit{confirmed null result}, not a contested anomaly. The minimal ECH framework's underprediction of $A_0$ by $>100$ orders of magnitude (Sec.~\ref{sec:spinmech}) is consistent with the observed null. Our framework's surviving parity-violation evidence comes entirely from CMB birefringence ($4.1\sigma$ from our independent NaMaster analysis of the Planck Commander map, Sec.~\ref{sec:data_cmb}; $2.4$--$2.9\sigma$ from published Planck and ACT~DR6 analyses~\cite{Minami2020,Eskilt2022,DiegoPalazuelos2025}), which is independent of galaxy morphology.\label{fig:galaxy_spin}\label{tab:surveys}
...
Independent verification using Cobaya~v3.6.1 with Planck NPIPE CamSpec TTTEEE + lowl TT/EE + lensing has produced two frozen dataset combinations with publication-quality convergence, plus an ongoing Planck-only run (114{,}992 raw samples, 436{,}799 weighted) that also finds $\Delta\Neff$ consistent with zero. Total MCMC program: 424{,}181 raw samples across 3 dataset combinations.\footnote{\label{fn:sample_stratification}Sample-count stratification across the verification program. \textbf{424{,}181} (abstract) is the total raw-accepted count summed across all three dataset combinations---the two frozen combinations (full-tension and Planck+BAO+SN) plus the ongoing Planck-only run. \textbf{309{,}789} (cited in Sec.~\ref{sec:loophole}) is the sum of the two frozen combinations only: $176{,}840 + 132{,}949$. \textbf{176{,}840} (Table~\ref{tab:verification}) is the raw-accepted count for the full-tension combination alone (six chains). After removing the first 30\% of each chain as burn-in, \textbf{123{,}129} post-burnin samples remain on disk (\texttt{reproducibility/cosmology/frozen/full\_tension\_20260311\_1728/diagnostics/convergence\_summary.json}, key \texttt{total\_samples\_post\_burn}); the \textbf{119{,}617} figure cited in Fig.~\ref{fig:corner_full_tension} and the corresponding bullet in Sec.~\ref{sec:limitations} reflects an additional \texttt{getdist} weight-based thinning applied during corner-plot generation. All four numbers correspond to the same underlying chains.}
...
\textit{Our chirality catalog (Paper~4).}---We independently addressed this question by constructing the largest bias-audited galaxy chirality catalog to date: $8{,}474{,}531$ galaxies from the DESI Legacy Imaging Surveys classified by a Vision Transformer (ViT-Small, $93.7\%$ validation accuracy, $8/8$ bias tests passed) with a three-class output (CW/CCW/not-spiral) and test-time equivariant averaging~\cite{Golden:2026chirality}.
The equivariant CW fraction is $\fcw^{\rm eq} = 0.5012 \pm 0.0006$, consistent with parity conservation at the $0.4\sigma$ level. The all-sky dipole is $0.43\sigma$ (null). Shamir's claimed $3\%$ asymmetry is not reproduced: our maximum regional asymmetry is $0.47\%$ ($7\times$ smaller). The catalog is $4.3\times$ larger than CE-ResNet and $6.5\times$ larger than Shamir's largest catalog, and agrees with CE-ResNet's equivariant result ($\fcw = 0.5013$) to within $0.01$ percentage points.

This definitively establishes that the minimal ECH framework's underprediction of galaxy spin asymmetry ($>100$ orders of magnitude below Shamir's claimed $A_0 \sim 0.003$) is moot: there is no significant galaxy spin asymmetry to explain. The framework's surviving parity-violation evidence comes entirely from CMB birefringence (Sec.~\ref{sec:data_cmb}), which is independent of galaxy morphology.
...
\textit{Independent verification (April 2026).}---We performed an independent NaMaster pseudo-$C_\ell$ analysis on the Planck Commander CMB polarization map at $N_{\rm side} = 2048$ ($50{,}331{,}648$ pixels) with a galactic mask ($|b| > 20^\circ$, $f_{\rm sky} = 0.658$) and point-source masking. Using the $EB$ cross-spectrum in the multipole range $50 < \ell < 1500$ with 50 Monte Carlo noise realizations for error estimation, we measure:
\begin{equation}
  \beta_{\rm NaMaster} = 0.264^\circ \pm 0.065^\circ \quad (\text{SNR} = 4.1).
  \label{eq:beta_namaster}
\end{equation}
This is $0.09\sigma$ from the ALP prediction ($\beta = 0.27^\circ$) and $0.68\sigma$ from the published Planck+ACT combined value ($0.342^\circ \pm 0.094^\circ$), consistent with both. The null hypothesis ($\beta = 0$) is rejected at $4.1\sigma$. This constitutes the first independent map-level birefringence measurement from our pipeline, confirming the robustness of the signal.

\textit{Resolution and mask systematics.}---We verified resolution independence by repeating the analysis at $N_{\rm side} = 256$, $512$, and $1024$ with the same galactic mask: $\beta = 0.321^\circ$, $0.275^\circ$, and $0.322^\circ$, respectively---all consistent with the $N_{\rm side} = 2048$ result and the ALP prediction. We also tested the dedicated Planck galactic mask ($f_{\rm sky} = 0.598$, stricter than our $|b| > 20^\circ$ cut), obtaining $\beta = 0.227^\circ$ ($N_{\rm side} = 512$) and $\beta = 0.167^\circ$ ($N_{\rm side} = 1024$). The lower values with the stricter mask suggest mild galactic contamination in the simple latitude cut. The signal persists across all configurations, with $\beta$ ranging from $0.17^\circ$ to $0.32^\circ$---bracketing the ALP prediction of $0.27^\circ$.
...
\end{cases}
\end{align}
With Planck+BAO alone, the Occam penalty slightly disfavors the model; the preference emerges when tension data are included.\label{tab:ic}\label{tab:fullcomp}\label{fig:naturalness}\label{tab:finetuning}\label{fig:sensitivity}\label{sec:finetuning} The fine-tuning comparison ($10^5$ vs.\ $10^{120}$ for $\Lambda$CDM) is a reparameterization as sensitivity to $N_{\rm tot}$, not a resolution (Sec.~\ref{sec:gdp}). The decisive test is detection of CMB $E$-$B$ parity-odd signatures (confirmed at $4.1\sigma$ by our NaMaster analysis, Sec.~\ref{sec:data_cmb}), not tension resolution alone. The galaxy spin channel is a confirmed null~\cite{Golden:2026chirality}.
...
Combined detection significance is estimated using inverse-variance weighting of the CMB birefringence measurements (Sec.~\ref{sec:birefringence_check}). The galaxy spin channel is a confirmed null ($\fcw^{\rm eq} = 0.5012 \pm 0.0006$, Sec.~\ref{sec:spinobs},~\cite{Golden:2026chirality}); the CMB birefringence channel provides the surviving parity-violation evidence. For the $f_{\rm NL}$ channel, systematic uncertainties including GR projection effects, $b_\phi$ bias, and photo-$z$ degradation are analyzed in the companion paper~\cite{Golden:2026forecast}. The primary residual systematic for the MCMC verification is the dataset-dependent $\Delta\Neff$: the full-tension dataset (including SH0ES) pulls $H_0$ upward, while the Planck+BAO+SN dataset recovers standard values (Sec.~\ref{sec:verification}).
...
\item \textit{Posterior distributions}: The original analysis reported Fisher-matrix error estimates; independent verification with full MCMC chains (Sec.~\ref{sec:verification}) now provides posterior distributions for two frozen dataset combinations. Corner plots showing parameter degeneracies (particularly the strong $r(\alpha/M,\Dinf)=-0.89$ anticorrelation) are shown in Fig.~\ref{fig:corner_full_tension} for the full-tension combination (119,617 post-burnin samples), yielding $H_0 = 67.69 \pm 1.06$, $\Omega_m = 0.308 \pm 0.006$, $\sigma_8 = 0.803 \pm 0.008$, $S_8 = 0.814 \pm 0.009$, and $\Delta N_{\rm eff} = -0.019 \pm 0.169$---the latter consistent with zero to well within $1\sigma$, confirming the framework's compatibility with standard cosmological constraints.

\begin{figure}
\centering
\includegraphics[width=\columnwidth]{figures/paper1_corner_full_tension.pdf}
\caption{Full-tension MCMC corner plot (119,617 post-burnin samples) over the joint Planck+BAO+SN+H0+$S_8$ combination. Marginal posteriors for $H_0$, $\Omega_m$, $\sigma_8$, $S_8$, and $\Delta N_{\rm eff}$ are shown on the diagonal; 2D contours at $1\sigma$ and $2\sigma$ levels off-diagonal. The $\Delta N_{\rm eff}$ posterior is consistent with zero ($-0.019 \pm 0.169$), confirming that the spin-torsion framework does not inject additional relativistic species at recombination.}
...
The galaxy spin asymmetry signal has been resolved by our chirality catalog~\cite{Golden:2026chirality}: $8.47$~million galaxies yield $\fcw^{\rm eq} = 0.5012 \pm 0.0006$, a definitive null ($0.4\sigma$ from parity conservation). This null result is \textit{consistent} with the framework, which underpredicts spin asymmetry by $>100$ orders of magnitude. The framework is \textit{not} falsified:
...
Our chirality catalog has ruled out spin asymmetry at the $A_0 \sim 0.003$ level: the maximum observed regional asymmetry is $0.47\%$ ($7\times$ smaller than Shamir's claim). The parity-violation case therefore rests entirely on CMB birefringence.
...
\textit{NANOGrav spectral fit.}---The matter-bounce induced GW spectrum has a universal $f^2$ infrared scaling, corresponding to spectral index $\gamma = 3$ in the characteristic-strain parameterization $h_c \propto f^{(3-\gamma)/2}$. A spectral template fit to the NANOGrav 15-year free-spectrum data points yields a Bayes factor $B(\text{bounce}/\text{SMBH}) \approx 302$ in favor of the matter-bounce prediction ($\gamma = 3$) over the standard SMBH binary merger spectrum ($\gamma = 13/3$), and $B(\text{bounce}/\text{strings}) \approx 8.1 \times 10^4$ over cosmic strings ($\gamma = 5/3$). The reduced chi-squared values are $\chi^2/\text{dof} = 0.012$ (bounce), $1.04$ (SMBH), and $1.90$ (strings), with $\Delta\text{BIC} = +10.7$ and $+21.9$ for SMBH and strings respectively relative to the bounce model. The best-fit peak frequency $f_{\rm peak} = 45.6$~nHz lies above the PTA band, placing the observed frequencies in the pure $f^2$ regime where the prediction is parameter-free. \emph{Caveat}: this analysis uses synthetic data points reconstructed from the published NANOGrav power-law fit, not the raw free-spectrum posteriors; a proper analysis using the full posterior distributions would provide a more realistic model comparison.
...
The galaxy spin dipole amplitude $A_0$ was previously an open empirical question (Sec.~\ref{sec:spinobs}). Our chirality catalog~\cite{Golden:2026chirality} has resolved this: $8.47$~million galaxies from the DESI Legacy Surveys yield $\fcw^{\rm eq} = 0.5012 \pm 0.0006$, with a null all-sky dipole ($0.43\sigma$) and maximum regional asymmetry of $0.47\%$. This constrains $|\delta| < 0.24\%$ at $95\%$~CL---the tightest galaxy chirality constraint to date, and $7\times$ smaller than Shamir's claimed $3\%$ excess. A first-principles derivation of $A_0$ from the $\varepsilon^{abcd}K_{ab}R_{cd}$ operator is no longer needed: the framework's $>100$~order-of-magnitude underprediction is consistent with the observed null. Future work on this channel shifts to redshift-dependent parity tests and cross-correlation with spectroscopic surveys.
