...
\end{cases}
\end{align}
With Planck+BAO alone, the Occam penalty slightly disfavors the model; the preference emerges when tension data are included.\label{fig:sensitivity} The fine-tuning comparison ($10^5$ vs.\ $10^{120}$ for $\Lambda$CDM) is a reparameterization as sensitivity to $N_{\rm tot}$, not a resolution (Sec.~\ref{sec:gdp}). The decisive test is detection of CMB $E$-$B$ parity-odd signatures (confirmed at $4.1\sigma$ by our NaMaster analysis, Sec.~\ref{sec:data_cmb}), not tension resolution alone. The galaxy spin channel is a confirmed null~\cite{Golden:2026chirality}.
...
\textit{NANOGrav spectral fit.}---The matter-bounce induced GW spectrum has a universal $f^2$ infrared scaling, corresponding to spectral index $\gamma = 3$ in the characteristic-strain parameterization $h_c \propto f^{(3-\gamma)/2}$. \emph{Caveat}: the comparison that follows uses synthetic data points reconstructed from the published NANOGrav 15-year power-law fit, not the raw free-spectrum posteriors; a proper analysis using the full posterior distributions would provide a more realistic model comparison, and the Bayes factors quoted below should be read as illustrative template-level preferences rather than posterior-level model evidence. With that caveat, a spectral template fit to the NANOGrav 15-year synthetic data points yields an illustrative Bayes factor $B(\text{bounce}/\text{SMBH}) \approx 302$ in favor of the matter-bounce prediction ($\gamma = 3$) over the standard SMBH binary merger spectrum ($\gamma = 13/3$), and $B(\text{bounce}/\text{strings}) \approx 8.1 \times 10^4$ over cosmic strings ($\gamma = 5/3$). The reduced chi-squared values are $\chi^2/\text{dof} = 0.012$ (bounce), $1.04$ (SMBH), and $1.90$ (strings), with $\Delta\text{BIC} = +10.7$ and $+21.9$ for SMBH and strings respectively relative to the bounce model. The best-fit peak frequency $f_{\rm peak} = 45.6$~nHz lies above the PTA band, placing the observed frequencies in the pure $f^2$ regime where the prediction is parameter-free. Within this caveat, the best-fit is consistent with matter-bounce $\gamma = 3$; a posterior-level analysis using the raw NANOGrav 15-year free-spectrum data is deferred to future work (see also the independent GPU MCMC γ = 3.20 ± 0.42 result reported in the companion anomaly-catalog paper~\cite{Golden:2026anomaly}).
