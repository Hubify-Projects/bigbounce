...
\newcommand{\paperVersion}{v2.1.0}
\newcommand{\paperTimestamp}{2026-03-21}
...
\date{March 21, 2026 --- \paperVersion}
...
Dark energy fine-tuning & Scaling ansatz$^c$ & $10^5$ vs $10^{120}$ (reparameterized) \\
Singularity problem & LQC bounce & $\rho_c \simeq 0.27$--$0.41\,\rhoPl$ \\
...
\end{equation}
\begin{align}\label{eq:rhoLambda}
\rho_\Lambda &= \left[\frac{\alpha}{M}\,\MPl\right]\times\Dinf\times\MPl^4\nonumber\\
&\sim 10^{-2}\times\Dinf\times\MPl^4 \approx (2.3\;\text{meV})^4,
\end{align}
...
\begin{figure*}[tb]
\centering
\includegraphics[width=0.95\textwidth]{figures/figure2_galaxy_spin_comprehensive.png}
...
These extensions are motivated by the bounce scenario but are treated as phenomenological parameters fit to data. MCMC parameter estimation using Planck~2018 + BAO + SNIa data within our extended parameter space (Sec.~\ref{sec:cosmo_fits}) yields the following best-fit values from the \textit{original} analysis (which included the SH0ES $H_0$ prior; see Sec.~\ref{sec:verification} for the independent verification that supersedes these values):
\begin{align}
...
\begin{figure*}[tb]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_dneff_viability_two_frozen.pdf}
...
\begin{figure*}[tb]
\centering
\includegraphics[width=0.95\textwidth]{figures/cosmology_dataset_comparison_two_frozen.pdf}
...
\begin{figure*}[tb]
\centering
\includegraphics[width=0.95\textwidth]{figures/figure3b_tensions_resolution_comprehensive.png}
...
\begin{align}
&\int \frac{d^dk}{(2\pi)^d}\frac{1}{(k^2-M^2)k^2}\nonumber\\
&\quad = \frac{i}{16\pi^2}\left[\frac{2}{\epsilon}-\ln\frac{M^2}{\mu^2}+\text{finite}\right].
\end{align}
...
\textit{Note:} This section presents a re-analysis of published galaxy spin catalogs using a hierarchical Bayesian framework. The galaxy spin dipole is \emph{not} a prediction of the minimal ECH framework: the parity-odd coupling $\alpha/M$ underpredicts the observed amplitude by $>100$ orders of magnitude (Sec.~\ref{sec:spinmech}). We include this analysis to document the phenomenological $A(z)$ fit and to provide a statistical framework that could be applied if a first-principles mechanism is found in future work. The signal itself remains contested, with null results from independent reanalyses~\cite{PatelDesmond2024,Philcox2025}.
...
\begin{table*}[tb]
\caption{\label{tab:modelcomp} Bayesian model comparison. The $\chi^2$ values here are effective $\chi^2_{\rm eff} \equiv -2\ln\mathcal{L}_{\rm max}$ for the \textit{full tension dataset} (Planck TT,TE,EE+lowE + BAO + Pantheon SNIa + SH0ES $H_0$ prior + DES $S_8$). Bayes factors $\ln B$ are estimated via the Savage-Dickey density ratio from MCMC posteriors. See Eq.~(\ref{eq:Zcomb2}) for dataset-dependent breakdown. Note: $\chi^2_{\rm eff}$ here uses the compressed likelihood; see Table~\ref{tab:fullcomp} for full-multipole $\chi^2$ values ($\sim 2768$), which are larger because they include the full Planck $\ell=2$--$2508$ likelihood.}
\begin{ruledtabular}
...
\begin{table*}[t]
\caption{\label{tab:fullcomp} Comparison of dark energy frameworks. Fine-tuning values are order-of-magnitude estimates on different footings; the spin-torsion $10^5$ is a reparameterization (sensitivity to $N_{\rm tot}$), not a resolution of the cosmological constant problem (see Sec.~\ref{sec:gdp}).}
\begin{ruledtabular}
\begin{tabular}{lcccc}
...
This residual asks: ``why did inflation last $\sim$92 total $e$-folds?''---an initial-condition question with potential dynamical answers through the bounce-to-inflation transition (Sec.~\ref{sec:theory_program}), slow-roll potential shape, or even anthropic constraints. This \emph{reparameterizes} the fine-tuning problem from a fundamental-constant question (``why is $\Lambda_{\rm bare}$ tuned to 120 decimal places?'') into an initial-condition question (``why did inflation last $\sim$92 $e$-folds?''). We emphasize that this is a reparameterization, not a resolution: the $10^5$ residual sensitivity to $N_{\rm tot}$ persists, and no known dynamical attractor mechanism forces inflation to terminate at exactly the required duration.

\begin{figure*}[tb]
\centering
\includegraphics[width=0.95\textwidth]{figures/vacuum_scale_sensitivity.pdf}
...
\subsection{Cosmic Birefringence: Spectator ALP Analysis}\label{sec:birefringence_check}

Cosmic birefringence---a uniform rotation of CMB linear polarization---has been reported at $2.5$--$2.9\sigma$ by independent analyses of Planck~\cite{Minami2020,Eskilt2022} and ACT~DR6~\cite{DiegoPalazuelos2025} data. While the minimal ECH framework does not predict birefringence without an explicit photon-sector coupling, the parity-odd structure of the Holst action provides heuristic motivation for a spectator axion-like particle (ALP) with Planck-scale decay constant $f_a \sim \MPl$ and Hubble-scale mass $m \sim H_0$.
...
\textit{Caveat:} This appendix documents a phenomenological fit to published galaxy spin catalogs. The fitted amplitude $A_0 \sim 0.003$ is not a prediction of the minimal ECH framework (the coupling $\alpha/M$ underpredicts by $>100$ orders of magnitude). The fitting framework is provided for reproducibility and as a statistical tool for future investigations.
...
\mathbf{C}&=\begin{pmatrix}\sigma_1^2 & \rho\sigma_1\sigma_2 \\ \rho\sigma_1\sigma_2 & \sigma_2^2\end{pmatrix},
\end{align}
...
\begin{widetext}
\begin{align}
\frac{\delta H_0}{H_0} &= \sqrt{\left(\frac{\partial \ln H_0}{\partial \Omega_m}\right)^{\!2} \delta\Omega_m^2 + \left(\frac{\partial \ln H_0}{\partial \Delta\Neff}\right)^{\!2} \delta(\Delta\Neff)^2 + \cdots} = 1.2\%.
\end{align}
\end{widetext}
...
\begin{align}
\rho_\Lambda^{\rm bounce} &\sim \frac{\alpha}{M}\cdot\MPl^3 \sim \frac{\alpha\,\MPl^3}{M}\nonumber\\
&= [(\alpha/M)\,\MPl]\cdot\MPl^4 \sim 10^{-2}\,\MPl^4.
\end{align}
\end{enumerate}
...
\begin{align}
\rho_\Lambda^{\rm today} &= \rho_\Lambda^{\rm bounce} \times \Dinf\nonumber\\
&= [(\alpha/M)\,\MPl]\times\Dinf\times\MPl^4 = \Xi\,\MPl^4,
\end{align}
