...
\newcommand{\paperTimestamp}{2026-04-29}
...
\textit{Independent verification (April 2026, production 500-realization run).}---We performed an independent NaMaster pseudo-$C_\ell$ analysis on the Planck Commander CMB polarization map ($N_{\rm side}=512$, $\ell_{\rm max}=1024$, $f_{\rm sky}=0.32$) with 500 Monte Carlo noise realizations at ACT-noise level ($10\,\mu\mathrm{K\cdot arcmin}$). Injecting the ALP prediction $\beta=0.27^\circ$ recovers
\begin{equation}
  \hat\beta_{\rm NaMaster} = 0.238^\circ \quad (\text{SNR}=20.32),
  \label{eq:beta_namaster}
\end{equation}
with recovery bias $0.032^\circ$, while injecting the joint Planck+ACT central value $\beta=0.342^\circ$ recovers $\hat\beta = 0.302^\circ$ at SNR$=25.71$ and the null hypothesis $\beta=0$ recovers SNR$=0.0$. The pipeline-level consistency between prediction and observation at this Monte Carlo precision is $0.77\sigma$. Canonical source: \texttt{pipelines/h200\_results/pod1\_namaster\_umap\_2026-04-29/results/namaster-birefringence/summary.json}. The pipeline-level recovery test confirms that no systematic bias from the pseudo-$C_\ell$ deconvolution exceeds $0.04^\circ$. This constitutes the first map-level NaMaster recovery of the ALP birefringence signal at our pipeline's production precision and rejects the null at high significance.
...
\end{cases}
\end{align}
With Planck+BAO alone, the Occam penalty slightly disfavors the model; the preference emerges when tension data are included. The fine-tuning comparison ($10^5$ vs.\ $10^{120}$ for $\Lambda$CDM) is a reparameterization as sensitivity to $N_{\rm tot}$, not a resolution (Sec.~\ref{sec:gdp}). The decisive test is detection of CMB $E$-$B$ parity-odd signatures (the production NaMaster pipeline delivers SNR$=20.32$ on a 500-MC ACT-sensitivity simulation recovering the $\beta=0.27^\circ$ ALP prediction as $0.238^\circ$, with the earlier 50-MC Planck Commander pilot at $4.1\sigma$ statistical-only; both are cross-checks of the published detections, not standalone confirmations, until the systematic budget of Sec.~\ref{sec:data_cmb} is propagated), not tension resolution alone. (The galaxy spin channel is a confirmed null; see Sec.~\ref{sec:spinobs}.)
