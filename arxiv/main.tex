...
Does the framework resolve $\sigma_8$/$S_8$ tension? & $\sigma_8 = 0.803$; $S_8 = 0.814$$^b$ & \textbf{No.} Below Planck at ${\sim}1\sigma$. \\
...
\end{itemize}
The spread from ${\sim}0.17^\circ$ to ${\sim}0.34^\circ$ across these measurements reflects genuine differences in methodology (different data, masks, galactic contamination levels, and analysis pipelines), not internal inconsistency. All values are mutually consistent within $1$--$2\sigma$. The ALP model's natural parameter range comfortably brackets all measurements. We adopt the inverse-variance combination $\beta = 0.241^\circ \pm 0.061^\circ$ ($3.9\sigma$) as the headline observational constraint throughout this paper, noting that this simplified combination neglects inter-experiment correlations (shared calibration systematics between Planck and ACT); the published joint single-analysis by Eskilt~\etal\ gives $\beta = 0.342^\circ \pm 0.094^\circ$ ($3.6\sigma$), which accounts for these correlations but is less statistically significant. All quantitative comparisons in this work use the $0.241^\circ$ value for consistency with the abstract.
...
$\beta = 0.241^\circ \pm 0.061^\circ$ & Literature combination & Sec.~\ref{sec:birefringence_check} & Summary likelihood of Planck + ACT \\
