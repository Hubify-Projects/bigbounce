...
Dark energy fine-tuning & Geometric origin & $10^5$ vs $10^{120}$$^c$ \\
Singularity problem & Torsion bounce & $\rho_c \simeq 0.27\rhoPl$ \\
Cosmic parity violation & Rotating BH origin & $2.4$--$2.9\sigma$ individual; $3.9\sigma$ combined \\
\end{tabular}
\end{ruledtabular}
\vspace{2pt}
...
\item \textit{Inflationary suppression mechanism}: Introduction of the dimensionless factor $\Xi\equiv[(\alpha/M)\,\MPl]\times\Dinf$ encoding how inflationary expansion suppresses the primordial parity-odd coefficient, converting a Planck-scale effect into the observed dark energy scale. This reparameterizes the fine-tuning from $10^{120}$ to $\sim 10^5$ (as sensitivity to $N_{\rm tot}$, not a resolution; see Sec.~\ref{sec:gdp}).
\end{enumerate}


\subsection{Paper Organization}

This paper is organized as follows. Sections~\ref{sec:theory}--\ref{sec:derivations} develop the theoretical framework, observational signatures, and enhanced derivations. Sections~\ref{sec:data_galaxy}--\ref{sec:cosmo_fits} present data analysis methods and cosmological fits. Sections~\ref{sec:systematics}--\ref{sec:falsification} contain systematic analysis and falsification criteria. Section~\ref{sec:related} positions our work in context. Section~\ref{sec:barriers} presents the 14 structural barriers that close all dark energy derivation routes. Section~\ref{sec:transparency} proves the perturbation-transparency theorem. Section~\ref{sec:loophole} documents the hybrid dark-energy loophole rejection. Section~\ref{sec:discussion} discusses theoretical implications. Section~\ref{sec:limitations} addresses limitations. Section~\ref{sec:conclusions} summarizes conclusions. Appendices provide technical details including a claims classification table (Appendix~\ref{app:claims}).
...
In Einstein-Cartan theory, gravitational collapse is modified by torsion pressure at extreme densities. The Friedmann equation receives a quantum correction\footnote{We note that the effective Friedmann equation Eq.~\eqref{eq:modfriedmann} is the standard LQC result from holonomy corrections~\cite{Ashtekar2011}, which is physically distinct from the Einstein-Cartan torsion bounce of Pop\l{}awski. Both produce a qualitatively similar $\rho^2$ repulsion at high densities but through different mechanisms (quantum geometry vs.\ classical spin-torsion coupling). In this paper, we adopt the LQC effective equation for its well-studied perturbation dynamics, while the EC framework provides the broader theoretical context including the Holst term and parity structure.}:
...
\dot{\Theta}+\frac{1}{3}\Theta^2=-\frac{1}{2\MPl^2}(\rho+3p)+\Lambda+2(\omega^2-\sigma^2)+\nabla_a\dot{u}^a.
\end{equation}
...
\textbf{Spin-Torsion} & $\mathbf{69.2 \pm 0.8}$\textsuperscript{a} & \textbf{MCMC fit} & This work \\
...
\end{tabular}
\end{ruledtabular}
{\small \textsuperscript{a}Superseded by independent verification: $H_0 = 67.68 \pm 1.06$ (Sec.~\ref{sec:verification}, Table~\ref{tab:verification}). The original value was driven by the SH0ES prior included in the tension dataset.}
\end{table}
...
\end{tabular}
\end{ruledtabular}
{\small $^\ddagger$Values shown were obtained from chains that included $\Omega_k$ as a free parameter. Since the model's 92 $e$-folds of inflation mandate $\Omega_k = 0$, the corresponding $\Omega_k = 0$ analysis is expected to yield similar or improved $\chi^2$ values. The qualitative conclusions are unchanged: the model is competitive with $\Lambda$CDM when tension data are included.}
\end{table}

The Bayesian evidence shows:\footnote{These evidence ratios were estimated from the MCMC chain posteriors via the Savage-Dickey density ratio, not from dedicated nested sampling. The values should be treated as approximate; a PolyChord nested-sampling analysis would provide more robust evidence estimates.}
...
potentially detectable by LISA; (ii)~local-type non-Gaussianity $f_{\rm NL}^{\rm local} = -35/8$~\cite{Cai:2009fn}, testable by SPHEREx at $4$--$6\sigma$ significance~\cite{Golden:2026forecast}.
...
\begin{center}
\url{https://github.com/Hubify-Projects/bigbounce/tree/v2.0.0/reproducibility}
\end{center}
\noindent (pinned to tag \texttt{v2.0.0}).
