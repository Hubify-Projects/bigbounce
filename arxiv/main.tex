...
\item \textit{Spin-torsion (this work)}: Isotropic birefringence across all $\ell$ (rotation angle not derived from the torsion background alone; requires a photon--torsion coupling such as the spectator-ALP channel of Sec.~\ref{sec:alp-birefringence}), plus an anisotropic low-$\ell$ component whose amplitude and angular spectrum are not derived in this work and are flagged as an open direction in Sec.~\ref{sec:futuredirections}.
\end{itemize}
