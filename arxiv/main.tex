...
The framework's surviving predictions are testable: (1)~LiteBIRD ($\sigma(\beta) \approx 0.03^\circ$, early 2030s) will confirm or exclude the ALP birefringence at $\sim 9\sigma$; (2)~SPHEREx (first science data $\sim$2028) will test $f_{\rm NL} = -35/8$ at $4$--$6\sigma$ via the galaxy bispectrum; (3)~MCMC parameter values ($H_0$, $\sigma_8$, $\Delta\Neff$) are already consistent with standard $\Lambda$CDM (no tension resolution claimed). The framework has already self-falsified several earlier claims through its own MCMC verification (Sec.~\ref{sec:verification}).
