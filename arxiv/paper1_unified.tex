...
\newcommand{\paperVersion}{v1U.0.20}
...
channels as candidate sources of late-time dark energy and find each
constrained under stated assumptions: R1 (NJL contact) is amplitude-suppressed
and, in vacuum, condensate-excluded by a regulated gap-equation analysis;
R2--R3 (one-loop effective action, Immirzi running) are amplitude-suppressed
under explicitly-labeled scaling ans\"atze; and R4 (parity-CMB coupling) is
closed not by amplitude mismatch but by a naturalness / explanatory-deficit
objection, relocating rather than solving the cosmological-constant problem.
Within minimal ECH these four routes are shown basis-complete at the level of
$M_{\rm Pl}$-power-counting classes: two previously-omitted parity-odd operators
(a Jackiw--Pi gravitational Chern--Simons term and a vector--axial four-fermion
partner) are closed explicitly, and the dark-energy mapping is a single-scale
naive-dimensional-analysis no-go rather than a derived positive amplitude. The
result is a channel-level, assumption-conditional statement, \emph{not} an
operator-level theorem. We further establish a perturbation-transparency
result: for canonical scalar matter the Holst sector decouples from all
scalar/tensor perturbation equations of motion. The two phenomenological
predictors that survive---matter-bounce $\fnl$ and spectator-ALP
birefringence---are ECH-independent class tests shared with other bounce/ALP
completions, and a structural tension we derive makes the ECH dark-energy
mechanism and an observable matter-bounce signature mutually exclusive. The
precise scope, the full constraint catalog, and all load-bearing numerical
values are fixed in Sec.~\ref{sec:intro} and Sec.~\ref{sec:derivations};
