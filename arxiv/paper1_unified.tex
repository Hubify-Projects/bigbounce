...
\newcommand{\paperVersion}{v1U.0.9}
...
1.22\times 10^{19}\,$GeV is the \emph{unreduced} Planck mass, with
$\kappa \equiv 8\pi G = \MPl^{-2}$; the companion gravitational coupling
is $\kappa^2 = 16\pi G$ in the one-loop renormalisation conventions of
Shapiro~\&~Teixeira~\cite{ShapiroTeixeira2014}.\footnote{Planck-mass and
$\kappa$ conventions: main-text analyses use unreduced $\MPl=G^{-1/2}
\approx 1.22\times10^{19}\,$GeV with $\kappa=8\pi G=\MPl^{-2}$.
...
column ``prefactor'' so that $[\mathcal{O}_n^{[4]}]=+4$ throughout.
