...
\newcommand{\paperVersion}{v1U.0.13}
...
amplitude estimate of leg (i). We stress the scope precisely: what is
established here is the finite-density, mean-field bound of leg~(i) together
with the absence of a coherent $w=-1$ structure of leg~(ii); we do
\emph{not} claim to exclude a fully regulated NJL vacuum condensate
$\langle\psi\bar\psi\rangle\neq0$ via a gap equation and effective potential
--- that self-consistent strong-coupling analysis is beyond the present
mean-field treatment and is noted as an out-of-scope open item
(Sec.~\ref{sec:barriers}), not as a solved case. What we do claim, and what
leg~(i) delivers, is that at the $\kappa=M_{\rm Pl}^{-2}$ Planck-suppressed
coefficient no such condensate can reach the $(\text{meV})^4$ scale without a
new light scale, which is the tuning the mechanism is meant to explain.
