...
\newcommand{\paperVersion}{v1U.0.5}
...
of which it is the leading representative is the closed local basis
...
\end{equation}
the six local dimension-$4$ parity-odd densities that minimal-ECH field
content admits (Holst dual, Nieh--Yan, Pontryagin, torsion-square,
axial--torsion, single-curvature dual), enumerated with their fate in
Appendix~\ref{app:dim4_completion} (Table~\ref{tab:dim4_parityodd}). Each
Wilson coefficient $c_n$ carries genuine mass dimension $0$ so that every
$c_n\mathcal{O}_n^{[4]}$ is a bona-fide dimension-$4$ density; the
algebraic Cartan constraint $T=\kappa S$, the first (algebraic) Bianchi
identity, and Chern--Weil exactness collapse this basis to (i) topological
total derivatives (O2, O3), (ii) the $\MPl^{-2}$-suppressed Fierz-closed
four-fermion sector (O4, O5), or (iii) Bianchi-vanishing single-curvature
densities (O1, O6). Passing to the on-shell branch (algebraic torsion
eliminated, one curvature factor stripped by the Bianchi identity) reduces
this closed dimension-$4$ set to the single dimension-$+1$ representative
displayed in Eq.~\eqref{eq:Seff_comp}: the $+1$-vs-$+4$ counting is a
property of that on-shell reduction, not of an ill-defined action.
