...
\newcommand{\paperVersion}{v1U.0.19}
\newcommand{\paperTimestamp}{July 12, 2026}
% v1U.0.19 (2026-07-12 — DP1U presentation overhaul (directive M): (a) abstract
%   repetition purge — the "channel-level not operator-level" caveat, restated
%   ~4x, now stated once with a pointer to the Sec.-IV scope paragraph, and the
%   duplicate 13-barrier caveat collapsed to a bare cross-ref; (b) barrier
%   catalog consolidated — the 14 verbose per-barrier \subsection blocks folded
%   into one compact description-list (\sec:barrier_details) with an explicit
%   barrier→route bracket map (Grok's exact ask), both equations (B1,B12)
%   preserved inline; (c) appendix E/F/G reframed — bespoke ECH-sector ΔNeff
%   derivation + all referenced result tables/figures KEPT byte-identical, the
%   stock-CAMB MCMC / NaMaster pipeline / ALP-chain mechanics signposted as
%   supplementary (companion + \cite{BigBounceRepro} + App reproducibility),
%   H0/σ8 tension narrative tightened. Zero numeric/physics change; every
%   headline number byte-preserved. Addresses the "excessively repetitive /
%   excessive caveats / condense appendices" complaints common to every ChatGPT
%   REJECT raw + Grok/Gemini MAJOR. Prior:
% v1U.0.18 (2026-07-12 — NJ5 INT-Claude MINOR closed: the single residual "far
%   sub-critical" left by the v1U.0.17 tempering pass at the App. njl_gap leg-(B)
%   intro (was internally inconsistent with the tempered abstract/main-text/verdict
%   wording "comfortably"/"sub-critical") is now "comfortably sub-critical". Purely
%   editorial internal-consistency fix; no numeric or physics change. Wave-2/2
%   recompute-verified all NJL numbers vs njl_gap_equation_route1_results.json.)
% v1U.0.17 (2026-07-12 — NJ4 INT-Claude MINOR-REVISIONS closed: (1) leg-(B) magnitude
%   bound now explicitly covers the attractive AA channel — worst scanned
%   |G_AA|/G_crit = 2x0.156 = 0.31 (exact factor G_AA/|G_scalar|=2) stated in App.
%   njl_gap + main-text mirror, no longer relying on the PP-only |G_PP|=|G_scalar|
%   equality; (2) added the physical point that AA (Lorentz-violating axial-vector)
%   and PP (parity-breaking) vacua are not the scalar sigma-condensate at issue, so
%   the scalar sign leg is already decisive; (3) blanket "far sub-critical" in
%   abstract + discussion aligned to plain "sub-critical" to match the tempered
%   appendix worst-case (0.156/0.31). Conclusion unchanged; presentation-only.)
% v1U.0.16 (2026-07-12 — INT-Claude genuinely-new MINOR closed: Route-1 NJL leg-(A)
%   sign exclusion scoped to the scalar chiSB channel; attractive AA/PP channels
%   (G_AA=+3/32kappa, G_PP=+3/64kappa per eq:AAdecomp) excluded by magnitude leg (B);
%   "far sub-critical" tempered to "comfortably sub-critical". Conclusion unchanged.)
% v1U.0.15 (2026-07-12 — NJ1 wave: Holst NJL suppression factor corrected 30x->14x
%   per the paper's own committed script arxiv/scripts/njl_gap_equation_route1.py
%   (+_results.json): holst_factor = gamma^2/(gamma^2+1) = 0.0698 with gamma_BI=0.274,
%   i.e. EC-vs-Holst rows differ by 1/0.0698 = 14.3x ~ ~14x, NOT ~30x (every
%   G_eff/G_crit EC-vs-Holst-dressed JSON pair confirms 14.3x). INT-Claude catch
%   (H17, intwave_P1U_claude_2344.md); two body sentences (Route-1 discussion +
%   App. njl_gap "Two independent exclusions") + header comment updated. Conservative
%   direction preserved (Holst only strengthens the sub-critical exclusion), so the
%   conclusion is unchanged — straight numeric-value fix, nothing else touched.)
% v1U.0.14 (2026-07-11 — directive-L: Route-1 vacuum condensate EXCLUDED by a
%   regulated NJL gap equation (closes DP1U-05/-19, was out-of-scope open item).
%   New Appendix app:njl_gap (leg iii of sec:r1_njl) presents the gap equation on
%   the paper's own operator -(3/16)kappa(J5.J5): (A) Fierz-projecting to the scalar
%   condensate channel via eq:AAdecomp gives G_scalar=-(3/64)kappa<0 = REPULSIVE, so
%   M=0 is the only effective-potential minimum at any coupling/cutoff (no condensate);
%   (B) even |G_eff| is far sub-critical vs the DERIVED G_crit=pi^2/(N_f N_c Lambda^2)
%   — |G_eff|/G_crit=(3/64)N_f N_c/pi^2 ~ 4.3e-2 (Nf Nc=9) at Lambda=M_Pl, worst case
%   0.156 scanned, Holst ~14x smaller. Abstract/intro/conclusion R1 claims upgraded
%   from "finite-density bound + disclosed vacuum gap" to the two-pronged exclusion
%   (mean-field NJL framework assumption stated explicitly). Every number verified
%   against arxiv/scripts/njl_gap_equation_route1.py (+_results.json; self-checks PASS:
%   G_crit symbolic match, sign repulsive, subcritical all cases). Klevansky1992 +
%   HatsudaKunihiro1994 + Buballa2005 bib entries added. Nothing fabricated — all
%   coefficients are the paper's own; G_crit derived symbolically.)
% v1U.0.13 (2026-07-11 — restamp + recompile at program-exit state; no content change.)
% v1U.0.11 (2026-07-10 — H17 EXT re-test closure, ChatGPT "Check D" MAJOR (genuinely-new internal
%   inconsistency, editable): Check D (~L4870) and the O4/O5 reduction (~L1944) printed
%   S_abc S^abc = 6 (J5.J5) using eps_abcd eps^abce = +3! delta AND dropping the (1/4)^2=1/16 from
%   S^abc = (1/4) eps^abcd J5_d, contradicting the once-fixed footnote below Eq.(torsion) (~L1796)
%   which correctly derives S_abc S^abc = (1/16) eps_abcd eps^abce J5^d J5_e = -3/8 (J5.J5) with the
%   Lorentzian mostly-plus eps_abcd eps^abce = -3! delta (eps^0123=+1). FIXED both sites to the
%   footnote's carefully-derived -3/8 (J5.J5) + the correct -3! sign. Structural conclusion UNCHANGED
%   (O4->kappa^2 J5.J5, O5->kappa J5.J5, same M_Pl^-2 power, Fierz-closed basis) — the sign/constant
%   is an intermediate, not a load-bearing coefficient. No fabrication; the correct value was already
%   derived in the paper's own footnote, this just propagates it to the two restatements. Every other
%   ChatGPT/Grok H17 re-test finding is a source-cited RE-FLAG of prior-audit-dispositioned content or
%   a disclosure-backfire (Grok MAJOR #7 punishes the honest "channel-level not operator-level" scope
%   hedging as a weakness). \date stays July 10, 2026. Directive-G: recompile, re-mirror, Convex bump.)
% v1U.0.7 (2026-07-09 — PROCESS-LEAK SCRUB. Two body-text sentences that referenced the
%   internal review process were rewritten as standalone scientific prose (NO number changed,
%   nothing fabricated). (1) Sec II A 2 (~L1876): "This is the point ChatGPT's W12 referee pass
%   correctly raised: ... was a bookkeeping slip" -> "This bookkeeping is essential: labelling the
%   bare invariants as 'dimension-4 with dimensionless c_n' would misstate their mass dimension".
%   (2) sec:p1b_omega_a_def (~L6206): "two independent reviewers flagged the absence of an explicit
%   derivation" -> "an explicit derivation is required to make the classification reproducible".
%   Corpus grep of all 5 .tex sources confirmed no other body-text process leaks (all remaining
%   ChatGPT/Gemini/Grok/agent mentions are in % comments or the legit AI-methods disclosure para).
% v1U.0.6 (2026-07-09 — W12 EXT closure, ChatGPT dim-4 dimensional-bookkeeping MAJOR + unanimous
%   full-promotion ask. VERDICT ON DIM CHECK: ChatGPT RIGHT. Under the paper's own conventions
%   [e]=0,[R]=+2, physical torsion [T]=[kappa S]=+1, [J^5]=[S]=+3, [kappa]=[M_Pl^-2]=-2, the BARE
%   Table-VII invariants have naive dim: O1 (eeR)=+2, O2 (NY)=+2, O3 (R^R)=+4, O4 (T^2)=+2,
%   O5 (TeJ^5)=+4, O6 (single-curvature)=+2. So O1/O2/O4/O6 are NOT dim-4 densities with a bare
%   dimensionless c_n as previously displayed — only O3 (Pontryagin) + O5 (axial-torsion) are.
%   The APPENDIX derivation (INT-verified) was correct (it carries the kappa/M_Pl powers); the error
%   was a MAIN-TEXT/Table-VII transcription slip. FIX: new displayed Eq.dim4_defs gives each
%   O_n^[4] with its explicit M_Pl^2 prefactor (O1,O2,O4,O6 * M_Pl^2; O3,O5 bare) so every
%   c_n O_n^[4] is a genuine dim-4 density with dimensionless c_n. Table VII gains dim + prefactor
%   columns + coefficient-dimension note. NO physics conclusion changed (closure survives at dim 4).
%   Symbolic checks (dim4_parityodd_enumeration.py) re-run: CHECK A + CHECK D both PASS.
%   Second half of W12: FULLY promoted the completeness argument (not just Eq.(7)) into the main
%   text — new "Main-text completeness argument" block after Eq.dim4_defs states the (a) Bianchi-
%   vanishing, (b) Cartan->Fierz collapse, (c) topological-total-derivative closure inline; appendix
%   keeps the derivations. Version+date bump, recompile 0 undef-refs, PDF re-mirror all served paths,
%   Convex paperVersions:bump in same bundle (directive G).
% v1U.0.4->0.5 (2026-07-09): W11 EXT presentation-closure wave (NO number changed, nothing
%   fabricated, no math altered). THE ask (ChatGPT B2 + Gemini Blocker-1, identical): brought
%   the formally dimension-4 OFF-SHELL parity-odd operator into the MAIN TEXT at Eq.(Seff_comp):
%   new displayed Eq.(Seff_dim4) S_eff^(4)=int sqrt(-g) sum_n c_n O_n^[4] over the closed O1-O6
%   basis (Wilson coeffs c_n dim 0 => bona-fide dim-4 densities), with the explicit statement
%   that the dim-+1 form of Eq.(Seff_comp) is an ON-SHELL PRESENTATION SHORTHAND for that closed
%   dim-4 set (algebraic torsion eliminated + one curvature stripped by Bianchi), cross-ref to
%   App B.1 app:dim4_completion + Table tab:dim4_parityodd where the closure was already DERIVED.
%   The +1-vs-+4 counting is now framed as a property of the on-shell reduction, not an ill-defined
%   action. (2) Grok "light polishing" list closed: R4 quintessence/f(R) contrast sentence added
%   (the m_theta~H0 tuning is FORCED by one coupling sourcing both beta_obs and rho_Lambda, not an
%   added handle); Route-2 margin robustness parenthetical (inflating the one-loop prefactor by 10
%   OOM still leaves >=48 orders of suppression margin); perturbation-transparency exclusions
%   rationale sentence (the four excluded sectors are exactly the only ways to reintroduce T!=0).
%   Abstract/intro evasion-condition + N_tot=92+-2 order-of-magnitude framing already present
%   (v1A.0.84/107, App B L4503) — verified, no re-edit needed. (3) Gemini/ChatGPT editorial
%   cross-talk minor: struck the "not the 10^-2 that an earlier draft mis-stated" internal-note
%   phrasing at Sec IV.E, stating the corrected Delta gamma/gamma~0.3 directly. Version+date bump,
%   served-PDF re-mirror, Convex paperVersions:bump in same bundle (directive G).
...
\emph{amplitude-suppressed} by a standard torsion-elimination derivation and,
in the vacuum case, \emph{condensate-excluded} by a regulated NJL gap-equation
analysis (the scalar condensate channel is Fierz-projected repulsive and the
coupling is sub-critical; mean-field NJL framework),
...
This is a
channel-level assessment, \emph{not} an operator-level theorem (the precise
scope --- what is and is not claimed --- is fixed once in
Sec.~\ref{sec:derivations}): the four enumerated routes (NJL, one-loop EA,
Immirzi running, parity-CMB) are not proven to be a complete
diffeomorphism-invariant operator basis for the minimal-ECH effective action.
Two previously-omitted parity-odd operators are nonetheless closed explicitly at
the operator level (Sec.~\ref{sec:jackiwpi_cs}, Sec.~\ref{sec:r1_parityodd_partner}):
the Jackiw--Pi gravitational Chern--Simons $R\!\wedge\!\tilde R$ (a total
derivative for constant coupling, R4-class otherwise) and the vector--axial
four-fermion partner (which inherits R1's $M_{\rm Pl}^{-2}$ suppression, and
per Freidel--Minic--Takeuchi arises only under non-minimal fermion coupling).
...
scope is the non-minimal completion (a new light scale or non-minimal torsion
coupling). The dark-energy mapping is a single-scale NDA dimensional no-go: the
parity-odd operator has off-shell mass dimension $+1$, and single-scale power
counting ($\Lambda\sim\MPl$, no cancellation) forces its natural density to
$\rho_\Lambda^{\rm ECH}\sim\MPl^4$, never $(\text{meV})^4$
(Appendix~\ref{app:dimensions}); the genuine local dimension-$4$ parity-odd
completion is enumerated in Appendix~\ref{app:dim4_completion} (every admissible
density is topological, Fierz-reducible, or Bianchi-vanishing), so single-scale
closure survives at dimension~4 without the heuristic on-shell dressing. No
amplitude is derived, so the argument is not circular; the only evasion is a
non-minimal light scale or exact cancellation that is itself the tuning being
explained.
...
and is constrained at the channel-amplitude level by the mechanism-class
constraints above (Sec.~\ref{sec:barriers}).
...
with the vacuum condensate additionally excluded by a regulated NJL
gap equation (repulsive scalar channel, sub-critical coupling;
...
\textit{Observational appendices.}---$\Lambda$CDM$+\Delta\Neff$ MCMC baseline
envelope check (Cobaya~v3.6.1, $\mathbf{309{,}189}$ raw accepted samples across two
...
parameter, and $T^{abc}$ is the torsion tensor. To forestall any ambiguity in
the variational principle, we read Eq.~\eqref{eq:ECH} as a \emph{two-step}
object: (step~1) the fundamental, off-shell first-order
Einstein--Cartan--Holst$+$Dirac action is varied with respect to the
independent fields $\{e^a_\mu,\omega^{ab}_\mu,\psi\}$ --- the displayed
$\tfrac14 T^{abc}T_{abc}$ is \emph{not} present as an independent term at this
stage and is \emph{not} varied; (step~2) the algebraic Cartan equation
Eq.~\eqref{eq:torsion} is solved pointwise and back-substituted, and only the
resulting effective four-fermion contact term Eq.~\eqref{eq:4fermi} appears.
The $\tfrac14 T^{abc}T_{abc}$ written above is purely the on-shell Hehl--Datta
shorthand for that step-2 effective term; Eqs.~\eqref{eq:torsion}--\eqref{eq:4fermi}
are therefore derived from the step-1 off-shell action, not from varying the
displayed shorthand.\footnote{Two convention
...
with its dimension-$+1$ integrand is the on-shell reduction of the
dimension-$4$ basis after the algebraic Bianchi identity strips one curvature
factor; it is a convenient dimensional-analysis shorthand, \emph{not} the
...
represents the same physics on the torsion-free branch after the Bianchi
identity reduces $[\mathcal{F}_{IJ\rho\sigma}]$ from $+2$ to $+1$; the
apparent dimension-$+1$ deficit is an artifact of that on-shell reduction,
not of any ill-defined operator. The Case~II on-shell curvature dressing
...
\emph{Main-text completeness argument.}---The collapse is closed by one
finite enumeration plus two tensor identities, both verified symbolically
in the released script
\texttt{arxiv/scripts/dim4\_parityodd\_enumeration.py}. (a) The single-curvature
...
Lorentzian $\varepsilon_{abcd}\varepsilon^{abce}=-3!\,\delta^e_d$; footnote below Eq.~\eqref{eq:torsion}), to the parity-odd
...
(Table~\ref{tab:dim4_parityodd}). Passing to the on-shell branch (algebraic
torsion eliminated, one curvature factor stripped by the Bianchi identity)
reduces this closed dimension-$4$ set to the single dimension-$+1$
representative displayed in Eq.~\eqref{eq:Seff_comp}: the $+1$-vs-$+4$ counting
is a property of that on-shell reduction, not of an ill-defined action.
...
1.22\times 10^{19}\,$GeV is the \emph{unreduced} Planck mass, so that
$\kappa \equiv 8\pi G = 8\pi\,\MPl^{-2} = \bar M_{\rm Pl}^{-2}$ with
$\bar M_{\rm Pl}=(8\pi G)^{-1/2}=\MPl/\sqrt{8\pi}$ the reduced Planck mass;
the compact identity $\kappa=M_{\rm Pl}^{-2}$ used as shorthand in the
order-of-magnitude estimates below is understood in this reduced-mass sense,
the $8\pi\approx25$ factor being immaterial to every such estimate
(the reduced-mass distinction is a factor $8\pi$ in $M_{\rm Pl}^2$, below the
order-of-magnitude resolution of all contact-term and amplitude budgets). The
companion one-loop coupling is $\kappa^2 = 16\pi G$ in the renormalisation
conventions of Shapiro~\&~Teixeira~\cite{ShapiroTeixeira2014}.\footnote{Planck-mass and
$\kappa$ conventions: main-text analyses use unreduced $\MPl=G^{-1/2}
\approx1.22\times10^{19}\,$GeV; $\kappa=8\pi G=8\pi\MPl^{-2}=\bar M_{\rm Pl}^{-2}$, and
$\kappa=M_{\rm Pl}^{-2}$ appears only as a reduced-mass shorthand in order-of-magnitude budgets.
Appendix~\ref{sec:p1b_torsion_neff} (the bespoke ECH-sector $\Delta\Neff$
calculation) uses the \emph{reduced} Planck mass
$\bar M_{\rm Pl}=(8\pi G_N)^{-1/2}\approx2.44\times10^{18}$\,GeV for
convenient comparison with standard CMB references; numerical results in
that appendix are self-consistent and differ from the main-text convention
by $\sqrt{8\pi}\approx5$ in mass or $8\pi\approx25$ in $M_{\rm Pl}^2$,
which is below the order-of-magnitude resolution of all contact-term and
amplitude-budget estimates.}
The reduced-mass distinction ($\bar M_{\rm Pl} = \MPl/\sqrt{8\pi}$, a factor
...
inflationary initial conditions. Unlike a rolling quintessence or $f(R)$
scalar, whose potential is engineered by hand to yield $\rho_\Lambda$, the
$m_\theta\!\sim\!H_0$ tuning here is \emph{forced} by the requirement that a
single coupling source both the birefringence angle $\beta_{\rm obs}$ and
$\rho_\Lambda$: the tuning is not an added handle but the residue of demanding
a single-scale minimal-ECH origin for two observables at once.
...
\textit{MCMC baseline envelope check and cosmological fits.}---The $\Lambda$CDM$+\Delta\Neff$
stock-CAMB run (upper-bound observational envelope on any generic extra radiation
degree of freedom, not a test of the ECH spin-torsion module) finds $\Delta\Neff\approx 0$ and recovers a Hubble
...
operator---minimal coupling yields only the axial--axial R1 projection
[Freidel--Minic--Takeuchi~\cite{Freidel2005}], while the vector--axial
``Holst partner'' arises only under non-minimal coupling; either way any Fierz
...
by $H_0/\MPl \sim 10^{-61}$ in the dimensionful form, or the equivalent
...
amplitude estimate of leg (i). \emph{(iii)~The self-consistent vacuum
condensate is excluded by a regulated gap equation.} Beyond the finite-density
mean-field bound of leg (i), the possibility that the four-fermion contact
term dynamically generates a nonzero chiral condensate
$\langle\bar\psi\psi\rangle\neq0$ in vacuum---a strong-coupling effect that
$\langle J^5\rangle\approx0$ does not by itself preclude---is closed
explicitly, not deferred. The standard Nambu--Jona-Lasinio analysis
(Appendix~\ref{app:njl_gap}, computed in
\artifact{arxiv/scripts/njl\_gap\_equation\_route1.py}) yields two
independent exclusions. First, the sign: Fierz-projecting the generated
operator $-\tfrac{3}{16}\kappa\,(J^5\!\cdot\!J^5)$ onto the scalar
(condensate) channel via the paper's own lemma
(Eq.~\eqref{eq:AAdecomp}, $(J^5\!\cdot\!J^5)\to+\tfrac14 SS+\dots$) gives
$G_{\rm scalar}=-\tfrac{3}{64}\kappa<0$, a \emph{repulsive} scalar coupling,
whereas NJL condensation requires an \emph{attractive} channel ($G_s>0$); a
repulsive channel admits no non-trivial gap-equation solution, leaving $M=0$
as the only $\chi$SB minimum of the effective potential at \emph{any} coupling
strength or cutoff \emph{in the scalar channel}, so no scalar condensate forms.
(By the same Fierz lemma the axial and pseudoscalar channels are
\emph{attractive}, $G_{\rm AA}=+\tfrac{3}{32}\kappa$ and
$G_{\rm PP}=+\tfrac{3}{64}\kappa>0$; these are not the scalar condensate at
issue---$AA$ is a Lorentz-violating axial-vector vacuum and $PP$ is
parity-breaking, neither sourcing a coherent $w=-1$ term---and are in any case
sub-critical in magnitude ($|G_{\rm PP}|=|G_{\rm scalar}|$; the $AA$ ratio is
larger by the exact factor $2$, worst scanned $\approx0.31$).) Second,
the magnitude: even taking $|G_{\rm eff}|$, the coupling is comfortably
sub-critical
relative to the (derived, not quoted) critical value
$G_{\rm crit}=\pi^2/(N_f N_c\Lambda^2)$; at $\Lambda=M_{\rm Pl}$ the ratio is
cutoff-independent, $|G_{\rm eff}|/G_{\rm crit}=(3/64)\,N_f N_c/\pi^2
\approx 4.3\times10^{-2}$ (QCD-like $N_f N_c=9$; $\approx 4.7\times10^{-3}$
single-species), with a worst case of $0.156$ across all cutoffs/flavors
scanned and Holst-dressed values ${\sim}14\times$ smaller again. Under the
standard mean-field NJL framework this closes the vacuum case: at the
$\kappa=M_{\rm Pl}^{-2}$ Planck-suppressed coefficient no dynamical chiral
condensate reaches the $(\text{meV})^4$ scale---the tuning the mechanism is
meant to explain---and, more strongly, no condensate forms at all because the
condensate channel is repulsive.
...
\emph{Closure: amplitude-suppressed, parity-even, and---in the vacuum
case---condensate-excluded by a regulated NJL gap equation
(repulsive scalar channel, sub-critical coupling; Appendix~\ref{app:njl_gap}).}
...
action~\cite{Freidel2005,Mercuri2009}. We stress the minimal-coupling
status of this projection explicitly, following the effective-Lagrangian
analysis of Freidel, Minic \& Takeuchi~\cite{Freidel2005}: with the
\emph{minimally} coupled Dirac action the spin current is totally
antisymmetric (structural fact F2, Sec.~\ref{sec:fourroute}), the effective
four-fermion interaction contains \emph{only} the parity-even axial--axial
$J^5\!\cdot\!J^5$ term, and there is no parity violation --- exactly
Eq.~\eqref{eq:4fermi}. A parity-\emph{odd} vector--axial cross term arises in
that same analysis \emph{only} once a \emph{non-minimal} fermion--torsion
coupling is switched on; it is therefore \emph{not} a minimal-ECH operator but
a non-minimal ``Holst partner,''
\begin{equation}\label{eq:4fermi_partner}
\mathcal{L}^{\rm VA}_{\rm int}
\;\propto\; \frac{\gamma_{\rm BI}}{\gamma_{\rm BI}^2+1}\,8\pi G\;
J_\mu\,J^{5\mu},
\end{equation}
present only under non-minimal coupling. Its
coefficient carries a single power of $\gamma_{\rm BI}$ in the numerator
(parity-odd), versus the $\gamma_{\rm BI}^2$ of the parity-even R1 term
Eq.~\eqref{eq:4fermi}. We include it here for completeness --- to show that
\emph{even if} the minimal-coupling restriction is relaxed to admit this
non-minimal partner, the route stays closed --- not as a claim that it is
present in the minimal theory. This operator does \emph{not} open a new dark-energy
...
torsion-squared piece, both of which vanish at $T=0$ as shown in
Sec.~\ref{sec:transparency}) and reduces to the Nieh--Yan density on shell
once torsion is integrated out~\cite{Holst1996,Freidel2005}. Quantum
...
residual $\mathcal{O}(1)$ ambiguity in $\beta(\gamma)$.
...
\emph{What is ECH-specific versus generic here.} We state this
distinction honestly, since it is easy to misread R4 as a generic
quintessence complaint dressed up as an ECH barrier. The
$m_\theta\!\sim\!H_0$ ultralight-mass tuning is \emph{not}
ECH-specific: it is the same tuning that afflicts \emph{every}
ultralight-axion / rolling-quintessence dark-energy model whose potential
is engineered to freeze at $\rho_\Lambda$ today, and by itself it is a
generic naturalness feature, not a distinctive ECH failure. Two things
\emph{are} ECH-specific and are what actually close the route. (i) Under
the \emph{rigid} one-loop matching $\alpha/M\!\sim\!10^{-21}\,$GeV$^{-1}$
that minimal ECH predicts (Eq.~\eqref{eq:oneloop}; not a free handle),
R4 is \emph{not} merely a naturalness deficit but a hard amplitude
\emph{overshoot} of $\rho_\theta$ over $\rho_\Lambda$ by $22$--$36$ orders
of magnitude across the natural ALP-mass range --- a quantitative no-go
that a generic quintessence model, with its coupling free, does not face.
(ii) Once $\alpha/M$ is floated to escape that overshoot, the residual
obstruction collapses \emph{exactly} to the generic ultralight-DE tuning
--- and this is the honest point: at that stage minimal ECH supplies
\emph{no} first-principles reason for $m_\theta\!\sim\!H_0$ (no ECH
symmetry, scale, or dynamics fixes it), so the route provides no
\emph{predictive} advantage over an arbitrary quintessence field and is
therefore closed \emph{as an ECH-distinctive dark-energy mechanism}, not
because generic quintessence tuning is itself novel. In short: the
distinctive ECH content is the rigid-coupling overshoot (a genuine
amplitude no-go); the generic content is the floated-coupling
$m_\theta\!\sim\!H_0$ tuning, which we do not claim as a novel ECH
barrier but note closes the \emph{predictive}-route claim because minimal
ECH cannot derive it. Either way the minimal-ECH route to dark energy
via R4 does not survive.
...
below; the route each barrier constrains is given in brackets (the one-to-one
barrier$\to$route map is drawn in Fig.~\ref{fig:barrier_map}). Two entries carry
the only equations in the catalog (B1, B12); the remainder are one-line
mechanism statements.

\begin{description}[leftmargin=1.6em,itemsep=3pt]

\item[B1 --- Mass-Coupling Lock (Found.\ A) {[R1]}.] In Poincar\'{e} gauge
...
\item[B2 --- Topological-Shift Duality (Found.\ B) {[R1]}.] In metric-affine
...
\item[B3 --- Scalar-Tensor Universality (Found.\ C) {[R2]}.] On an FRW
...
\item[B4 --- Planck Suppression (Found.\ D) {[R2]}.] Disformal torsion couplings
...
\item[B5 --- Scale Separation (Found.\ E) {[R3]}.] The global vacuum integral
...
across $\sim 92$ $e$-folds of inflation; none exists within minimal ECH.
(General naturalness/classification argument, not an ECH-specific calculation.)

\item[B6 --- Attractor-Sensitivity Dilemma (Found.\ F) {[R3]}.] If post-bounce
...
\item[B7 --- Parameter Immunity (Found.\ G) {[R4]}.] Cyclic vacuum selection
...
\item[B8 --- Parity-Even Interaction (Branch H).] \emph{[R1; subsumed by B14.]}
...
\item[B9 --- Liouville Conservation (Branch J) {[R2]}.] Phase-space volume
...
\item[B10 --- UV$\to$IR Specificity Dilemma (Branch L) {[R3]}.] Any bridge from
...
\item[B11 --- Decoupling Universality (Branches L/M) {[R3]}.] At low energies all
...
\item[B12 --- Vacuum Amplification Ceiling (Branch M) {[R3]}.] Gravitational-wave
...
\item[B13 --- Gravitational Democracy (Branches N/O) {[R4]}.] Torsion couples
...
\item[B14 --- Perturbation Transparency (ECH gates).] \emph{[R1--R4.]} For
canonical scalar matter, torsion vanishes at all perturbation orders and the Holst sector
decouples from all scalar/tensor perturbation observables; elaborated in
Sec.~\ref{sec:transparency} (scalar-sector proof, Sec.~\ref{sec:transp_proof};
explicit all-order Holst-term verification, Sec.~\ref{sec:transp_holst}). This is
...
all perturbation orders. The decoupling is \emph{exact and non-perturbative on the
torsion-free branch}: it does not rely on a truncated perturbative series but
follows from two algebraic identities (the Cartan constraint $T=\kappa S$ with
$S=0$, and the first Bianchi identity $R_{\mu[\nu\rho\sigma]}=0$) that hold
pointwise for any field configuration in the scalar-matter sector. The background
is FLRW or any near-FLRW metric for which the torsion-free Levi-Civita connection
is operative; no slow-roll or adiabaticity assumption beyond canonical scalar
matter is needed. The restriction to the torsion-free ($T=0$) branch is
...
domain rather than a gap in it. These four are exactly the exclusions relevant
to the late-time dark-energy claim: each is the only way to reintroduce a
non-vanishing $T$ (a nonzero spin source, a torsion kinetic term, a running
$\gamma$, or a spin-carrying matter coupling) once canonical scalar matter has
set the classical spin density to zero, so ruling them out is what makes the
$T=0$ branch the operative one for a scalar-dominated late universe.
...
orders below $\rho_\Lambda$) and parity-even; the vacuum condensate is
excluded outright by a regulated NJL gap equation---the scalar channel is
Fierz-projected repulsive and the coupling sub-critical
(Appendix~\ref{app:njl_gap}). The one-loop route is
...
consistent with the once-fixed normalization of the footnote below Eq.~\eqref{eq:torsion}.
Every $\varepsilon$-contracted
...
%% --- NJL gap-equation exclusion of the Route-1 vacuum condensate ---
\section{Regulated NJL Gap Equation: Exclusion of the Route-1 Vacuum
Condensate}\label{app:njl_gap}
This appendix closes, by explicit computation rather than scope concession,
the possibility raised in Sec.~\ref{sec:r1_njl} that the torsion-induced
four-fermion contact term dynamically generates a chiral condensate
$\langle\bar\psi\psi\rangle\neq0$ in vacuum. That the mean axial current
vanishes, $\langle J^5\rangle\approx0$, does not by itself preclude a
self-consistent condensate; a genuine exclusion requires a regulated gap
equation and effective potential. We supply exactly that. The analysis is a
standard mean-field Nambu--Jona-Lasinio treatment~\cite{Klevansky1992,%
HatsudaKunihiro1994} of the paper's \emph{own} contact operator; all
coefficients are taken from the manuscript (Eqs.~\eqref{eq:NJL_torsion},
\eqref{eq:AAdecomp}) and every step is machine-checked in
\artifact{arxiv/scripts/njl\_gap\_equation\_route1.py} (sympy symbolic
$+$ numeric, self-verifying).

\emph{Scalar-channel coupling.}---The torsion-elimination operator is
$\mathcal{L}^{\rm NJL}_{\rm tor}=-\tfrac{3}{16}\,\kappa\,(J^5\!\cdot\!J^5)$
(Eq.~\eqref{eq:NJL_torsion}; the pure Einstein--Cartan $\gamma\to\infty$
value, so its magnitude is the \emph{maximal} coupling---using it is
conservative, since the Holst prefactor $\gamma^2/(\gamma^2+1)\in(0,1)$ only
reduces it). The chiral condensate lives in the scalar ($SS$) channel;
projecting with the paper's Fierz lemma (Eq.~\eqref{eq:AAdecomp},
$(J^5\!\cdot\!J^5)\to+\tfrac14 SS+\tfrac12 VV-\tfrac12 AA-\tfrac14 PP$) gives
\begin{equation}\label{eq:gscalar}
G_{\rm scalar}=\Bigl(-\tfrac{3}{16}\Bigr)\cdot\Bigl(+\tfrac14\Bigr)\,\kappa
=-\tfrac{3}{64}\,\kappa=-\tfrac{3}{64}\,M_{\rm Pl}^{-2}.
\end{equation}

\emph{Gap equation and derived critical coupling.}---For a scalar contact
Lagrangian $G_s(\bar\psi\psi)^2$ with $N_f$ flavors, $N_c$ colors, hard
$4$-momentum cutoff $\Lambda$, the mean-field gap equation is
$M=m_0+2G_s N_f N_c\,(M/2\pi^2)[\Lambda^2-M^2\ln(1+\Lambda^2/M^2)]$.
Requiring a nontrivial $M\neq0$ solution to bifurcate from $M=0$ in the
chiral limit (slope of the right-hand side over $M$ set to unity as
$M\to0$) \emph{derives}---not quotes---the critical coupling
\begin{equation}\label{eq:gcrit}
G_{\rm crit}=\frac{\pi^2}{N_f N_c\,\Lambda^2},
\end{equation}
matching the textbook value~\cite{Klevansky1992,Buballa2005}; the symbolic
derivation and this match are verified in the committed script.

\emph{Two independent exclusions.}---\textbf{(A) Sign (decisive).} By
Eq.~\eqref{eq:gscalar} the scalar-channel coupling is
$G_{\rm scalar}=-\tfrac{3}{64}\kappa<0$: \emph{repulsive}. Dynamical
chiral-symmetry breaking in NJL requires an \emph{attractive} scalar channel
($G_s>0$); with $G_s<0$ the gap equation admits no non-trivial solution and
$M=0$ is the only $\chi$SB minimum of the effective potential---\emph{no
scalar condensate forms at any coupling strength or cutoff}. This sign
exclusion is specific to the scalar ($SS$) channel: by
Eq.~\eqref{eq:AAdecomp} the axial ($AA$) and pseudoscalar ($PP$) channels are
instead \emph{attractive}, $G_{\rm AA}=(-\tfrac{3}{16})(-\tfrac12)\kappa
=+\tfrac{3}{32}\kappa$ and $G_{\rm PP}=(-\tfrac{3}{16})(-\tfrac14)\kappa
=+\tfrac{3}{64}\kappa>0$, so leg (A) does not by itself exclude them. They are,
however, not the dark-energy condensate at issue: leg (A) kills the scalar
$\sigma$-condensate $\langle\bar\psi\psi\rangle$, whereas an $AA$ ``condensate''
$\langle\bar\psi\gamma^\mu\gamma_5\psi\rangle$ is a Lorentz-violating
axial-vector vacuum and $PP$ is $\langle\bar\psi i\gamma_5\psi\rangle$
(parity-breaking); neither is a scalar condensate that could source a coherent
$w=-1$ term, so the scalar sign leg is already decisive for the object under
exclusion. As a magnitude check they are sub-critical as well: the $PP$ ratio
equals the scalar ratio ($|G_{\rm PP}|=|G_{\rm scalar}|$), and the $AA$ ratio,
larger by the exact factor $G_{\rm AA}/|G_{\rm scalar}|=2$, reaches a worst
scanned value $|G_{\rm AA}|/G_{\rm crit}\approx 2\times0.156=0.31$---still
sub-critical. \textbf{(B) Magnitude (belt-and-suspenders).}
Even discarding the sign and taking $|G_{\rm eff}|$, the coupling is comfortably
sub-critical. At $\Lambda=M_{\rm Pl}$ the ratio is cutoff-independent
($M_{\rm Pl}$ cancels),
\begin{equation}\label{eq:ratio}
\frac{|G_{\rm eff}|}{G_{\rm crit}}=\frac{3}{64}\,\frac{N_f N_c}{\pi^2}
=\begin{cases}
4.7\times10^{-3} & (N_f N_c=1),\\
4.3\times10^{-2} & (N_f N_c=9,\ \text{QCD-like}).
\end{cases}
\end{equation}
Across all cutoffs and flavor counts scanned the worst case is
$|G_{\rm eff}|/G_{\rm crit}=0.156$ (at the strong-EFT scale
$\Lambda_{\rm strong}\sim M_{\rm Pl}/\sqrt{\gamma_{\rm BI}}$, $N_f N_c=9$),
still comfortably sub-critical; the Holst-dressed values are ${\sim}14\times$
smaller again.

\emph{Verdict.}---The Route-1 vacuum condensate is \emph{excluded} for two
independent reasons: the condensate channel is repulsive (no condensate at
any coupling), and $|G_{\rm eff}|$ is comfortably sub-critical in every case
scanned (worst scanned ratio $0.156$).
This closes the vacuum case with the regulated gap equation and effective
potential that the scope statement of Sec.~\ref{sec:r1_njl} previously
deferred---within the standard mean-field NJL framework, and using only the
manuscript's own operator coefficients; nothing is fabricated. (Strong-coupling
effects genuinely beyond mean field, or non-minimal completions introducing a
new light scale, remain outside this treatment, consistent with the paper's
...
The load-bearing physics of this appendix is the first-principles bespoke
ECH-sector $\Delta\Neff$ bound (Sec.~\ref{sec:p1b_torsion_neff}). The
accompanying stock-CAMB $\Lambda$CDM$+\Delta\Neff$ MCMC proxy run and its full
cosmological fits and model comparison are numerical consistency cross-checks
that contextualize the main-text no-go and test no spin-torsion physics; their
complete likelihood configurations, per-chain convergence diagnostics, and
posterior tables are supplementary, documented in the MCMC companion
(\texttt{paper1b\_mcmc\_companion.tex}) and reproducible from the committed
artifacts~\cite{BigBounceRepro} (Appendix~\ref{app:reproducibility}). Summary
result tables (Tables~\ref{tab:verification}, \ref{tab:chain_datasets}) are
retained here.
...
production at the bounce) as a phenomenological proxy; $(\omega/H)_0$ and
$\Omega_k$ are fixed to zero in the sampled configuration
(Sec.~\ref{sec:p1b_fullcomp}). In the full-tension combination, which includes
the SH0ES~\cite{Riess2022} $M_B$-anchor likelihood (\texttt{H0.riess2020Mb}, via
the supernova absolute magnitude, not a direct $H_0$ prior), the Planck NPIPE
(PR4) CamSpec + Planck~2018 low-$\ell$ + lensing likelihoods carry sufficient
inverse-variance weight that the posterior is Planck-dominated at
$H_0 = 67.68 \pm 1.06$ (not the equal-weight $\sim 70$), with $\Delta\Neff$
consistent with zero. Adding $\Delta\Neff$ in stock CAMB therefore neither
resolves nor moves the SH0ES tension; the spin-torsion framework alone does not
resolve cosmological tensions at present data precision.


%% ============================================================
%%  3. STOCK-CAMB LCDM+DNEFF MCMC: GENERIC RADIATION-PROXY TEST
%% ============================================================
\subsection{Stock-CAMB $\Lambda$CDM$+\Delta\Neff$ MCMC: Upper-Bound Baseline Envelope Check\\
(Not a Spin-Torsion Theory Module)}\label{sec:p1b_verification}

\textit{Scope statement.}---Every use of ``MCMC'' or ``MCMC baseline envelope check'' throughout this
appendix refers to a stock CAMB run of $\Lambda$CDM extended by $\Delta\Neff$ as a free parameter.
\emph{No custom CAMB modifications are used; no torsion-modified Boltzmann
equations are solved.} This is an \emph{upper-bound baseline envelope check}:
it tests whether the data prefer a generic extra radiation-like degree of freedom
(a phenomenological proxy for any possible effective radiation contribution from the
spin-torsion sector), and establishes a conservative observational ceiling on
$\Delta\Neff$.  It does \emph{not} test the ECH spin-torsion module itself,
simulate ECH dynamics, or constitute an independent verification of ECH physics;
that would require a bespoke modified Boltzmann code incorporating torsion-modified
perturbation equations. What the MCMC envelope \emph{does} anchor,
via the bespoke ECH-sector calculation in Sec.~\ref{sec:p1b_torsion_neff} below,
is a conservative observational ceiling on the actual radiation contribution
...
competitive sky-detection significance claim. The full pipeline mechanics
(mask/apodization construction, mode-coupling-matrix inversion, band-power
binning, noise model, software versions, and convergence diagnostics) are
supplementary and are documented in the companion~\cite{Golden2026P1B} and
reproducible from the committed artifacts~\cite{BigBounceRepro}
(Appendix~\ref{app:reproducibility});
the load-bearing recovered-bias results are retained below.
...
distinctive ECH prediction. The full ALP-MCMC chain mechanics --- the
sampled-parameter list, per-parameter priors, the dataset-likelihood stack,
sampler settings, and per-chain convergence diagnostics --- are supplementary:
the priors/likelihood table is in Appendix~\ref{app:alp_priors}, and the chains
and reductions are committed as reproducibility
artifacts~\cite{BigBounceRepro} (Appendix~\ref{app:reproducibility}). This
appendix retains the load-bearing benchmark result and a compact summary of the
chain outputs.
...
and an explicit derivation is required to make the classification reproducible.
