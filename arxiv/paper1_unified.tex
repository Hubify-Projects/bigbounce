...
\newcommand{\paperVersion}{v1U.0.14}
...
%   0.156 scanned, Holst ~30x smaller. Abstract/intro/conclusion R1 claims upgraded
...
repulsive channel leaves $M=0$ as the only minimum of the effective potential
at \emph{any} coupling strength or cutoff, so no condensate forms. Second,
...
scanned and Holst-dressed values ${\sim}30\times$ smaller again. Under the
...
($G_s>0$); with $G_s<0$ the effective potential is monotone-increasing from
$M=0$, so $M=0$ is the only minimum---\emph{no condensate forms at any
coupling strength or cutoff}. \textbf{(B) Magnitude (belt-and-suspenders).}
...
still comfortably sub-critical; the Holst-dressed values are ${\sim}30\times$
smaller again.
