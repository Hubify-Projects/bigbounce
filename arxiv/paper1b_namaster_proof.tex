...
\newcommand{\paperVersion}{v2B.0.9}
\newcommand{\paperTimestamp}{2026-07-16 17:22 PT}
...
CPU. The optional PyMaster integration example and the retained physical
validation campaign (\(N_{\rm side}=512\), \(\ell_{\max}=1024\)) depend on the
user-supplied PyMaster and healpy stacks and correspondingly more memory, but
those workloads lie outside the self-contained packaged code path.
