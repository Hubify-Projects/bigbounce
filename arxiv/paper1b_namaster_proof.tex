...
\newcommand{\Cl}{C_\ell}
\newcommand{\paperVersion}{v2B.0.1}
\newcommand{\paperTimestamp}{2026-07-16 10:52 PT}
...
content-addressed sidecar receipts with atomic per-file replacement and fails
...
changed receipts, or asserted-contract mismatches raise an exception.  The two
...
Version 0.1.1 contains 23 automated tests covering numerical behavior, invalid
...
double-precision arrays and the package's zero-tolerance compatibility test;
the original workspace tensor was not retained, so the scalar is not a
...
The package is operating-system independent where Python and NumPy are
available; CI covers Linux with Python 3.10--3.13.

\paragraph{Programming language and dependencies.}
Version 0.1.1 requires Python 3.10 or later and NumPy 1.24 or later. PyMaster
is optional and is supplied by the user for physical NaMaster workspaces;
the retained physical validation used PyMaster 2.6.

\paragraph{Code repository.}
The MIT-licensed source, issue tracker, installation instructions, API
documentation, tests, and examples are available directly at
\href{https://github.com/Hubify-Projects/bigbounce/tree/main/packages/namaster-proof}
{the \software{} package directory}. Citation metadata are supplied in
\texttt{CITATION.cff}.
...
must bind version 0.1.1 to an immutable archive before journal submission.

\paragraph{Validation artifacts.}
The corrected physical example is bound to
\nolinkurl{results/physical_spectrum_v2/summary.json} (SHA-256
\texttt{\seqsplit{745b0a2f060773ce69c005ea84b74b305ec26a85f6aaafe58f0b3244b7f39914}})
and \nolinkurl{bandpowers.npz} (SHA-256
...
sharded JSON calculations outside cosmology. Extensions can add other
