...
\newcommand{\paperVersion}{v1B.0.71}
...
shifts $\Omega_a$ by $\lesssim 1\%$ ($\Omega_a \propto H_0^{-2}$), well below
...
\end{equation}
with $\rho_{\rm crit,0} = 3H_0^2\MPl^2 \approx 8.1\times10^{-11}$\,eV$^4$
and $\MPl \approx 2.44\times10^{18}$\,GeV. For $f_a = \MPl$ and small $\theta_i$
this gives $\Omega_a \approx m_a^2\theta_i^2/(2H_0^2(1+z_{\rm osc})^3)$,
...
lensing without a release-consistency (pairing-swap) control run; a
dedicated PR4-vs-2018-only consistency run is planned to verify that the
quoted $\Delta\Neff$ and $H_0$ posteriors are stable under the
release-pairing choice (caveat (e), Sec.~\ref{sec:verification}).
Current data
