...
\newcommand{\paperVersion}{v1B.0.3}
...
\date{May 8, 2026, 21:30 PDT --- \paperVersion}
...
(Cobaya~v3.6.1, 424{,}781 samples across three dataset combinations): this
...
convergence, plus an ongoing Planck-only run (114{,}992 raw samples). Total
MCMC program: 424{,}781 raw samples across 3 dataset combinations.
...
stratification: \textbf{424{,}781} (abstract) is the total raw-accepted count
across all three combinations. \textbf{309{,}789} is the sum of the two frozen
combinations only ($176{,}840 + 132{,}949$). After removing the first 30\% of
...
weight-based thinning. All four numbers correspond to the same underlying chains.}
...
constrained an EC torsion model using DESI~DR2 + PantheonPlus + DES~Y5 +
Planck~2018, finding torsion preferred by AIC ($\Delta\text{AIC}=-5.7$ to $-6.6$).
...
\noindent\textbf{Pipeline configuration.}---\emph{Beam and pixel window.}---The
...
\textbf{(i)} The three frozen MCMC dataset combinations of this companion paper
(``Full-tension'', ``Planck+BAO+SN'', ``Planck-only''; Table~\ref{tab:mcmc_inventory}
rows~1--3, $176{,}840 + 132{,}949 + 114{,}992 = 424{,}781$ accepted samples)
\emph{do not include free $w_0 w_a$ as a sampled parameter}. Those chains hold
$w_0 = -1$, $w_a = 0$ at the $\Lambda$CDM fiducial throughout. Quoting any of
these frozen posteriors as a posterior-preference verdict on the
...
row~4) is currently running on Pod~3 H200 with $\sim 109$ samples accepted as of
2026-05-08 18:27 PT and $\hat R - 1 \approx 0.076$. Publication-quality convergence
($\hat R - 1 < 0.01$) is still 1--3 days out. Until that chain converges, this
program has \emph{no $w_0 w_a$ posterior at all}, neither for the matter-bounce
candidate row nor for any other model.
...
\textit{Forward.}---A new DESI DR2 + Planck NPIPE + Pantheon+ cobaya chain
(4 chains, Pod~3 H200) is currently running (109 accepted samples as of
2026-05-05, $\sim$3-day ETA to convergence). When converged, GetDist posteriors
on $w_0$-$w_a$ will be incorporated into the Paper~I(a) §~Structural Tension
section as an empirical test of the quintom-B scenario~\cite{DESI2025DR2}.
