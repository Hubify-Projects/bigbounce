...
\newcommand{\paperVersion}{v1B.0.10}
...
auditable script is on-record-deferred to v1B.0.10 (the v1B.0.8 marker was carried over from the prior version-numbering scheme; v1B.0.9 closes R4 propagation-tail items but the model-comparison recompute requires a fresh MCMC pull and is queued for v1B.0.10+).
...
P1(b) & v1B.0.9 & 66\% & Model-comparison recompute (deferred v1B.0.10) \\
...
\begin{table}[tb]
\caption{\label{tab:mcmc_inventory} MCMC program inventory. The cobaya DESI DR2 $w_0 w_a$ iter2 chain (16-chain MPI, OMP=6 isolation) is, as of 2026-05-17 01:29~UTC, at $101{,}979$ accepted samples with $\hat{R}-1 = 0.01176$ (last flushed 01:29~UTC; \emph{near the $\hat R\!-\!1\!<\!10^{-2}$ standard publication target} --- the chain has progressed from the v1B.0.7 snapshot of $59{,}832$/$0.01945$ at 2026-05-14 22:53~UTC by ${\sim}42{,}000$ samples and $\hat R - 1$ has dropped from $0.01945$ to $0.01176$). The $\dagger$ entry above marks values updated v1B.0.8 from the live chain state. The 2 frozen chains below ($\Lambda$CDM+$\Delta\Neff$ proxy, $w_0 w_a$ not sampled) suffice for the P1(b) headline conclusions of this companion paper; the DESI DR2 $w_0 w_a$ chain is forward work to be incorporated into Paper~I(a)'s \S~Structural Tension as a future quintom-B empirical test once it reaches the standard publication target $\hat{R}-1 < 10^{-2}$.}
\begin{ruledtabular}
\begin{tabular}{lccc}
...
DESI DR2 w0wa (iter2) & 101{,}979\,\textsuperscript{$\dagger$} & 0.0118\,\textsuperscript{$\dagger$} & Near target \\
...
2026-05-17 01:29~UTC the chain has accumulated $101{,}979$ accepted
samples across the 16 chains and reports $\hat R - 1 = 0.01176$ (within ${\sim}0.002$ of the standard $\hat R\!-\!1\!<\!10^{-2}$ publication target; see Table~\ref{tab:mcmc_inventory} caption); we
deliberately do not commit to a specific calendar date for convergence
to $\hat R - 1 < 10^{-2}$ in this subsection. Until that chain
...
\textit{Forward.}---A new DESI~DR2 + Planck~NPIPE + Pantheon+~\cite{Brout2022PantheonPlus} + DES-SN5YR~\cite{DES2024SN5YR} cobaya chain with the $w_0 w_a$ free-parameter extension is running on a dedicated MPI pod (16 chains, OMP threads tuned to suppress BLAS oversubscription, GetDist-built posterior covmat from a preliminary 4-chain iter1 with $\sim\!9{,}500$ accepted samples). \textbf{Current status (as of 2026-05-17 01:29~UTC)}: the chain has accumulated $101{,}979$ accepted samples across the 16 chains and reports $\hat R - 1 = 0.01176$, within ${\sim}0.002$ of the standard $\hat R\!-\!1\!<\!10^{-2}$ publication target. The chain has progressed from the v1B.0.7 snapshot of $59{,}832$/$0.01945$ at 2026-05-14 22:53~UTC by ${\sim}42{,}000$ samples; $\hat R - 1$ has dropped from $0.01945$ to $0.01176$, a $\sim\!40\%$ reduction consistent with continued mixing. The 16-rank mpirun process remains alive on the pod, and the sampler is slow-mode-dominated rather than stalled or crashed. We deliberately do not commit to a calendar-date convergence ETA in this manuscript. When converged ($\hat R - 1 < 10^{-2}$), GetDist posteriors on $w_0 w_a$ will be incorporated into the Paper~I(a) §~Structural Tension section as an empirical test of the quintom-B scenario~\cite{DESI2025DR2}; in the present manuscript, all DESI DR2 $w_0 w_a$ conclusions in P1(a) remain \emph{not tested} until convergence is reached.
...
Model-comparison $\Delta$AIC/BIC/$\ln B$               & Numerical & Deferred & Removed v1B.0.7; recompute v1B.0.10 \\
