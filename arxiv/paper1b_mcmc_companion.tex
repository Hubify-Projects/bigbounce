...
\newcommand{\paperVersion}{v1B.0.75}
\newcommand{\paperTimestamp}{June 19, 2026}
...
$w_0 + w_a$   & $-1.4788 \pm 0.1485$ & phantom-crossing indicated (caveated; fn.~\ref{fn:wcaveat})\\
...
observed NaMaster pipeline bias (Sec.~\ref{sec:data_cmb}). Underlying data
...
pipeline bias. This is a methodology cross-check, not a competitive
...
$f_{\rm sky}=0.85$ (Planck-like Galactic-cut mask: $|b|>5^\circ$,
all declinations, apodized at $2^\circ$ FWHM, giving $f_{\rm sky}\approx 0.85$;
...
and $f_{\rm sky}=0.65$ (ACT-DR6-like Galactic-cut mask: $|b|>15^\circ$,
declination cut ${\rm dec}\in[-60^\circ,+25^\circ]$, apodized at $2^\circ$ FWHM)
...
multiplicative correction and a $0.040^\circ$ observed pipeline bias, none of
...
(40,349 raw rows, 28,245 post-burn-in at 30\%, 107,853 effective weight)
...
$\sim 25\times$ fine-tuning of the misalignment initial condition
...
across the spectator-consistent posterior.
...
\item MCMC chain diagnostics and convergence CSV files:
  \url{https://huggingface.co/datasets/Hubify/p1b-mcmc-diagnostics}.
\item NaMaster pipeline artifacts (mask, MC seeds, output spectra):
  \url{https://huggingface.co/datasets/Hubify/p1b-namaster-artifacts}.
\item ALP parameter MCMC chains:
  \url{https://huggingface.co/datasets/Hubify/p1b-alp-chains}.
\end{enumerate}
