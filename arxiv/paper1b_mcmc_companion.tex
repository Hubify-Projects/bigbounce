...
\newcommand{\paperVersion}{v1B.0.52}
...
above (${\approx}5$ percentage points of the $12\%$). Restricting the fit
...
trajectories; same artifact), a span that brackets the observed value but
...
spectra themselves), settles at $\theta_i = 1.32 \pm 0.41$ and
...
with the field-displacement range $\Delta\phi/f_a \in [0.06, 1.19]$ from the
...
${\approx}160$ (smallest displacement: $\theta_i=0.5$, $m=H_0$); at the
...
benchmarks and requires non-minimal model building. The lower end
...
$\Delta\phi/f_a \in [0.06,1.19]$ over the natural box giving required
