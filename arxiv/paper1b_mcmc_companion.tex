...
\newcommand{\paperVersion}{v1B.0.50}
\newcommand{\paperTimestamp}{June 9, 2026 PDT}
...
$\hat\beta-\beta_{\rm inj}=-0.032^\circ$). \emph{Scope of the validation}: the test confirms the algebraic pseudo-$C_\ell$ $E\!\to\!B$ deconvolution under MASTER mode coupling, NOT the physical separation of the cosmic-rotation angle $\beta$ from the instrumental-miscalibration angle $\alpha$ which strictly requires unrotated galactic foregrounds (the synthetic CMB-only skies contain no galactic foregrounds, so the very component that breaks the $\beta$--$\alpha$ degeneracy in published Planck/ACT~DR6 measurements is absent by construction). The MC recovery is therefore a pipeline-validation figure, not a sky-detection significance claim. The primary sky detection significance is the
published Planck/ACT DR6 $2.7$--$2.9\sigma$~\cite{Eskilt2022,DiegoPalazuelos2025};\footnote{\label{fn:eskilt_pr3_pr4}Eskilt
...
$w_0 + w_a \approx -1.48$. At the decorrelation pivot
($z_p = 0.27$; fn.~\ref{fn:wpivot}), the effective equation of state is
...
($M_B = -19.263$, $H_0 = 67.68$), the constant is
$-19.263 - 5\log_{10}(67.68) = -28.415$, i.e.\ a $0.156$ mag offset from
...
terms in the $H_0$ axis). The chain posterior is therefore the correct compromise of three
...
($|67.79 - 68.41|/\sqrt{1.09^2 + 0.32^2}$) and $1.0\sigma$ in $S_8$
($|0.831 - 0.812|/\sqrt{0.018^2 + 0.006^2}$); their torsion-consistent-with-zero
result parallels our $\Delta\Neff$-consistent-with-zero null finding.
...
the canonical $f_{\rm sky}=0.32$ artifact, so that point carries the mean
only. The worst-case $|{\rm bias}| = 0.040^\circ$ is the NaMaster systematic
floor adopted in
Eq.~\ref{eq:beta_namaster}--fn.~\ref{fn:snr_definition}. Underlying data
...
to $\sin(2\beta)\cos(2\beta)\,C_\ell^{EE}$ on a $\beta$ grid. Only
non-negative injections were run; a negative-$\beta$ injection
(sign-symmetry/linearity check) is not part of the present MC suite and is
an acknowledged validation gap.
...
$\Delta\hat\beta \equiv \hat\beta - \beta_{\rm inj}$, is $-0.032^\circ$
(consistent with the apodized-mask power suppression expected from
...
$f_{\rm sky}=0.85$ and $0.65$ equal $20.32\sqrt{f_{\rm sky}/0.32}$. The
...
($\sigma_\beta = 0.033^\circ$, $f_{\rm sky}=0.65$); $\sigma_\beta$ was not
recorded for the canonical $f_{\rm sky}=0.32$ run, but the
$\sqrt{f_{\rm sky}}$ scaling validated between the two measured points
implies $\sigma_\beta(0.32) \approx 0.029^\circ\sqrt{0.85/0.32} \approx
0.047^\circ$ and hence $\hat\beta/\sigma_\beta \approx 0.238/0.047 \approx
5$ per realization at the canonical mask. The standard error of the 500-MC
...
bias of $-0.032^\circ$ (the $0.001^\circ$ difference is $0.8\times$ the
standard error of the 500-realization mean,
$\sigma_\beta/\sqrt{500} \approx 0.029^\circ/22.4 \approx 0.0013^\circ$).
The $\sim\!12\%$ multiplicative under-recovery is
therefore a sky-fraction-independent property of the apodized pipeline
(consistent with apodization-induced power suppression), not an artifact of
the small canonical mask, and remains well below the published observational
uncertainty $\sigma_\beta^{\rm obs}=0.094^\circ$ at every sky fraction tested.
Artifact: \path{reproducibility/p1_namaster_500mc/results/c1_fsky_sweep.json}.
...
five chains are listed in Table~\ref{tab:chain_datasets}. Parameter estimation
...
\begin{equation}
\beta \approx \frac{\alpha_{\rm EM}\times 8}{4\pi}\times 1.07
= 4.97\times 10^{-3}\,\text{rad} \times \frac{180^\circ}{\pi} \approx 0.29^\circ.
\end{equation}
...
spectra themselves), pulls $(\theta_i, m/H_0)$ toward the
...
consistent with the model-independent fit $\beta_{\rm free} = 0.344^\circ\pm 0.096^\circ$ (our internal model-independent MCMC fit, with $\beta$ as a free parameter, to a Gaussian summary likelihood on the published Eskilt--Komatsu joint WMAP+Planck birefringence measurement $\beta_{\rm obs}=0.342^\circ\pm0.094^\circ$~\cite{Eskilt2022} --- not a re-analysis of the $EB$ spectra themselves; 9{,}720 accepted samples across the 3 ALP-MCMC configurations described in Sec.~\ref{sec:birefringence_check} (configurations $C_{a\gamma}=4,8,12$; full priors and likelihood details in Appendix~\ref{app:alp_priors}); $\beta_{\rm free}$ denotes the unconstrained-amplitude fit distinct from $\beta_{\rm ALP}$ which has $C_{a\gamma}=8$ fixed)
...
Because the EOM-required coupling range $[{\sim}9, {\sim}51]$ exceeds the
$C_{a\gamma}\in[1,30]$ prior of the original extended configuration (which
truncated ${\sim}28\%$ of the posterior mass above $C_{a\gamma}=30$), we reran
the $C_{a\gamma}$-free fit with a continuous uniform prior
$C_{a\gamma}\in[4,60]$ covering the full required band (4 MPI chains,
$\hat{R}-1=0.0095$, 8{,}955 accepted samples). The resulting posterior is
broad, as expected for a single-amplitude constraint on a three-parameter
degeneracy: median $C_{a\gamma}=20.7$ with $16$--$84\%$ range $[7.3, 45.6]$,
and $69\%$ of the posterior mass falls inside the EOM-required band
$[9,51]$ ($\approx 10.3/1.1$ to $10.3/0.2$, the
$C_{a\gamma}\,\Delta\phi/f_a \approx 10.3$ product divided across the
$\Delta\phi/f_a\in[0.2,1.1]$ envelope; 22\% of the mass lies below
...
the full required coupling range rather than at fixed benchmarks.
...
$0.4\%$ of the posterior mass (too few samples for a stable marginal),
quantifying the misalignment tuning of the spectator-consistent corner.
Convergence:
...
sampled freely (flat priors $C_{a\gamma}\in[4,60]$ --- widened from the
earlier $[1,30]$ to cover the full EOM-required band $[9,51]$ ---
...
far broader than the benchmark $m/H_0\in[1,3]$),
broadening the three fixed-$C_{a\gamma}$ benchmark configurations of
...
0.107^\circ$ and the observed $\beta_{\rm obs}=0.342^\circ\pm 0.094^\circ$
...
between $\sim 9$ and $\sim 51$; see \S\ref{sec:birefringence_check} for the
...
disclosure: see fn.~\ref{fn:theta_backreaction} in
Sec.~\ref{sec:birefringence_check}. In brief, the $\theta_i\in[0.5,2]$
prior is an envelope-completeness choice, not the spectator-consistent
sub-range ($\theta_i\sim 0.1$, a $\sim 25\times$ tuning relative to the
prior midpoint); posterior samples at $\theta_i\gtrsim 0.5$ belong to the
...
threshold $\hat R - 1 < 0.01$ across all 3 benchmark configurations (achieved
at $N_{\rm tot}=9{,}720$ accepted samples post burn-in) and for the
continuous-prior $C_{a\gamma}\in[4,60]$ configuration ($\hat R-1=0.0095$,
8{,}955 accepted samples).
\textit{Chain-to-number map}: $\beta_{\rm ALP}=0.336^\circ\pm0.107^\circ$
--- the three fixed-$C_{a\gamma}\in\{4,8,12\}$ benchmark chains (9{,}720
samples); $\beta_{\rm free}=0.344^\circ\pm0.096^\circ$ --- the
model-independent single-parameter chain; median $C_{a\gamma}=20.7$,
