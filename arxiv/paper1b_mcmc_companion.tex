...
\newcommand{\paperVersion}{v1B.0.85}
...
\subsection{$w_0 w_a$ cross-check with stated SN-overlap systematic}\label{sec:w0wa_crosscheck}
...
and no model-selection statement is claimed. This cross-check is
retained in the main text---rather than relegated to an appendix---
deliberately: every significance, $\sigma$-distance, and model-selection
statistic that an overlap-uncorrected likelihood could spuriously inflate
...
$\Lambda$CDM point flagged as unsampled), so the surviving content is a
transparent, fully-caveated diagnostic whose only role is to motivate the
deferred overlap-controlled nested-sampling analysis. Hiding a result whose
caveats are this prominent would reduce, not improve, the reader's ability
to judge its scope. The goodness-of-fit decomposition ($\chi^2_{\rm BAO}$,
...
\textit{$w_0 w_a$ cross-check.}---The converged DESI~DR2 + Planck~NPIPE + Pantheon+~\cite{Brout2022PantheonPlus} + DES-SN5YR~\cite{DES2024SN5YR} Cobaya chain with the $w_0 w_a$ free-parameter extension ($128{,}385$ accepted samples across 16 MPI chains; $\hat R - 1 = 0.00820$, below the standard $\hat R\!-\!1\!<\!10^{-2}$ publication target) supplies the GetDist $w_0 w_a$ posteriors of Table~\ref{tab:iter2_posterior}, a caveated overlap-uncorrected diagnostic of the quintom-B scenario~\cite{Cai2010quintomReview} (tested against DESI~DR2~\cite{DESI2025DR2}) under an overlap-uncorrected SN product likelihood (Sec.~\ref{sec:verification}, caveat~(e)); the SN-overlap systematic is stated as a known limitation and no model-selection claim is made.
...
Table~I, Table~III, and the relevant Table~V claim-classification entries
of this paper; the ALP $c5$ continuous chain backs Table~IV separately:
