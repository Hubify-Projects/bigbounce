...
\newcommand{\paperVersion}{v1B.0.80}
...
$w_0$         & $-0.8122 \pm 0.0436$ & (overlap-inflated; not quotable)\footnote{\label{fn:wcaveat}No valid $\sigma$-distance from $\Lambda$CDM can be quoted from this chain. The SN likelihood combines DES-Y5 and Pantheon$+$ as a product without a joint cross-covariance despite ${\sim}20\%$ shared supernovae, which double-counts the common events and artificially narrows the posterior; any departure-from-$\Lambda$CDM significance computed from these widths is an overlap-inflated product-likelihood artifact. Separately, $\Lambda$CDM is unsampled by the present Metropolis-Hastings run (no $w_0\!=\!-1, w_a\!=\!0$ samples), so any nominal departure would in any case be a posterior-tail extrapolation, \emph{not} a Bayes-factor or $\ln B$ exclusion and \emph{not} a frequentist tension. A Savage-Dickey readout is not viable; robust $\ln B$ is left to a follow-up overlap-controlled nested-sampling analysis (see \S~Headline-result discussion).}\\
$w_a$         & $-0.6666 \pm 0.1864$ & (overlap-inflated; not quotable; fn.~\ref{fn:wcaveat})\\
$w_0 + w_a$   & $-1.4788 \pm 0.1485$ & $<-1$ (phantom-crossing region; fn.~\ref{fn:wcaveat})\\
$w_{\rm pivot}$ & $-0.952 \pm 0.019$ & (overlap-inflated; not quotable)\footnote{\label{fn:wpivot}$w_{\rm pivot} \equiv w_0 + (1-a_p)\,w_a$ with the pivot $a_p$ chosen so that $w_{\rm pivot}$ and $w_a$ are uncorrelated in the posterior covariance, i.e.\ $\mathrm{Cov}(w_0,w_a) + (1-a_p)\,\mathrm{Var}(w_a) = 0$, giving $1-a_p = -\mathrm{Cov}(w_0, w_a) / \mathrm{Var}(w_a)$. Direct readout of the converged iter2 chain ($30\%$ burn-in): $\mathrm{Cov}(w_0,w_a) = -0.00729$ (correlation coefficient $\rho = -0.90$), so $1-a_p = 0.210$, $a_p = 0.790$, and $z_p = 1/a_p - 1 = 0.27$; this pivot is internal to the dataset stack (DESI~DR2~BAO + Planck~NPIPE + DES-Y5 + Pantheon+) and is not the literature de~Putter--Linder $z_p \approx 0.4$ value. The decorrelated combination has $\sigma_{w_{\rm pivot}}^2 = \sigma_{w_0}^2 - \mathrm{Cov}^2(w_0,w_a)/\sigma_{w_a}^2 = (0.0436)^2 - (0.00729/0.1864)^2 = (0.0193)^2$, reproducing the $\pm 0.019$ value above, with mean $w_{\rm pivot} = -0.8122 + 0.210\times(-0.6666) = -0.952$. The choice of $a_p$ depends on the dataset stack; rerunning the chain with SH0ES or DESI~DR2~BAO alone would shift $z_p$ and $w_{\rm pivot}$ at the linear-Fisher level.}\\
\hline
