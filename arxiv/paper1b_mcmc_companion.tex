...
\newcommand{\paperVersion}{v1B.0.7}
...
\begin{table}[tb]
\caption{\label{tab:mcmc_inventory} MCMC program inventory. The cobaya DESI DR2 $w_0 w_a$ iter2 chain (16-chain MPI, OMP=6 isolation) is, as of 2026-05-14 22:53~UTC, at $59{,}832$ accepted samples with $\hat{R}-1 = 0.01945$ (last flushed 22:53~UTC; \emph{slow-mode-dominated, not stalled} --- the chain has progressed from the v1B.0.6 snapshot of $53{,}736$/$0.01775$ at 15:43~UTC by ${\sim}6{,}000$ samples). The slight $\hat{R}-1$ uptick from $0.01775$ to $0.01945$ is expected behavior for a slow-mixing chain exploring newly opened posterior volume and is not a divergence. The 2 frozen chains below ($\Lambda$CDM+$\Delta\Neff$ proxy, $w_0 w_a$ not sampled) suffice for the P1(b) headline conclusions of this companion paper; the DESI DR2 $w_0 w_a$ chain is forward work to be incorporated into Paper~I(a)'s \S~Structural Tension as a future quintom-B empirical test once it reaches the standard publication target $\hat{R}-1 < 10^{-2}$.}
\begin{ruledtabular}
\begin{tabular}{lccc}
...
DESI DR2 w0wa (iter2) & 59{,}832 & 0.0195 & Slow-mode \\
...
\textit{Forward.}---A new DESI~DR2 + Planck~NPIPE + Pantheon+~\cite{Brout2022PantheonPlus} + DES-SN5YR~\cite{DES2024SN5YR} cobaya chain with the $w_0 w_a$ free-parameter extension is running on a dedicated MPI pod (16 chains, OMP threads tuned to suppress BLAS oversubscription, GetDist-built posterior covmat from a preliminary 4-chain iter1 with $\sim\!9{,}500$ accepted samples). \textbf{Current status (as of 2026-05-14 22:53~UTC)}: the chain has accumulated $59{,}832$ accepted samples across the 16 chains and reports $\hat R - 1 = 0.01945$. The chain has progressed from the v1B.0.6 snapshot of $53{,}736$/$0.01775$ at 15:43~UTC by $\sim 6{,}000$ samples; the slight $\hat R - 1$ uptick is expected behavior for a slow-mixing chain exploring newly opened posterior volume and is not a divergence. The 16-rank mpirun process remains alive on the pod, and the sampler is slow-mode-dominated rather than stalled or crashed. We deliberately do not commit to a calendar-date convergence ETA in this manuscript. When converged ($\hat R - 1 < 10^{-2}$), GetDist posteriors on $w_0 w_a$ will be incorporated into the Paper~I(a) §~Structural Tension section as an empirical test of the quintom-B scenario~\cite{DESI2025DR2}; in the present manuscript, all DESI DR2 $w_0 w_a$ conclusions in P1(a) remain \emph{not tested} until convergence is reached.
