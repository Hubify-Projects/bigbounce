...
\newcommand{\paperVersion}{v1B.0.48}
...
Convergence: $\hat{R}-1 < 0.01$ for all runs. The joint posterior structure
of the extended ($C_{a\gamma}$-free) cross-check configuration is shown in
Fig.~\ref{fig:alp_triangle}.

\begin{figure}
\centering
\includegraphics[width=\columnwidth]{figures/alp_triangle_plot.png}
\caption{Spectator-ALP joint posterior triangle from the extended
cross-check configuration in which the photon anomaly coefficient is
sampled freely (flat priors $C_{a\gamma}\in[1,30]$,
