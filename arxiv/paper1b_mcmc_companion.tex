...
\newcommand{\paperVersion}{v1B.0.4}
...
\date{May 13, 2026 PDT --- \paperVersion}
...
stratification: \textbf{309{,}789} is the sum of the two frozen
combinations ($176{,}840 + 132{,}949$). After removing the first 30\% of
each chain as burn-in, \textbf{123{,}129} post-burnin samples remain
(\texttt{convergence\_summary.json}); the \textbf{119{,}617} figure in
Fig.~\ref{fig:corner_full_tension} reflects additional \texttt{getdist}
weight-based thinning. All three numbers correspond to the same underlying
two frozen chains; the third (Planck-only) dataset combination is still
accumulating samples and is reported separately in Table~\ref{tab:convergence}
with $\hat R - 1\sim 0.05$ as ongoing, not yet aggregated into the
$309{,}789$-sample headline.}
...
run (424{,}781 samples) finds $\Delta\Neff$ consistent with zero in both frozen
