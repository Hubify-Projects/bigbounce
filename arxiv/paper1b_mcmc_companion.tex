...
\newcommand{\paperVersion}{v1B.0.102}
\newcommand{\paperTimestamp}{July 7, 2026}
...
That said, the present paper does contribute one genuinely new first-principles
result of its own --- the derivation (Sec.~\ref{sec:torsion_neff}) that the
minimal ECH spin-torsion sector's radiation-era $\Delta\Neff$ is
Planck-suppressed to $\sim\!10^{-44}$ at BBN, some forty orders of magnitude
below any foreseeable sensitivity --- alongside a validated $E\!\to\!B$ recovery
pipeline, a quantified spectator-ALP accommodation cost, and a complete public
reproducibility manifest, all self-contained within this manuscript.
...
via the bespoke ECH-sector calculation in Sec.~\ref{sec:torsion_neff} below,
...
\subsection{Bespoke ECH-sector $\Delta\Neff$ from the torsion-induced
four-fermion interaction}\label{sec:torsion_neff}
...
coefficients dropped rather than a rigorous NJL finite-$T$ computation), so that
\begin{equation}
...
non-zero torsion $\Delta\Neff$; the honest, first-principles outcome is the
...
(Sec.~\ref{sec:cosmo_fits}) exceeds the theoretical ECH value
...
AI research pipeline (Anthropic's Claude, operated under the author's
direction) used for MCMC configuration and convergence checking, pipeline
