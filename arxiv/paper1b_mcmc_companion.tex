...
\newcommand{\paperVersion}{v1B.0.56}
...
(Cobaya~v3.6.1, $\mathbf{309{,}189}$ frozen samples across two converged dataset combinations; a third Planck-only combination is excluded from all tables and the headline count): this
...
subset.  The $106{,}361$ figure at $20\%$ is the GetDist-reported value; the
...
\hline
$H_0$ [km\,s$^{-1}$\,Mpc$^{-1}$] & $67.68 \pm 1.06$ & $67.79 \pm 1.09$ \\
...
$67.79 \pm 1.09$) is therefore the no-SH0ES result. The interaction with the SH0ES prior is the subject of
...
combined Planck+BAO constraint; the qualitative quintom-B finding ($w_0 + w_a < -1$)
is therefore unlikely to be reversed by a rigorous joint-covariance treatment,
but the precise tension significance should be interpreted with this caveat.
Users requiring a rigorous SN combination should refer to the DES Collaboration
comparison analysis~\cite{DES2024SN5YR} and construct a joint covariance.
\par
...
the spin-torsion sector; CMB-S4 ($\sigma(\Neff)\sim 0.03$~\cite{CMBS4_ScienceBook})
...
($|67.79 - 68.41|/\sqrt{1.09^2 + 0.32^2}$) and $1.3\sigma$ in $S_8$
...
CMB-S4 ($\sigma(\Neff)\sim 0.03$~\cite{CMBS4_ScienceBook}) will sharpen
...
\url{https://github.com/Hubify-Projects/bigbounce/tree/main/reproducibility}
\end{center}
The version of the repository corresponding to this paper is tagged
\texttt{\paperVersion}; the exact commit SHA is recorded in the repository's
\texttt{CHANGELOG.md} under that tag.
...
in the repository README at the \texttt{\paperVersion} tag; links are preserved
against future README changes via the tagged commit.
...
\textit{What is NOT included.}---The $\Lambda$CDM$+\Delta\Neff$ proxy
chains are not pre-computed (regenerate
via \texttt{reproduce\_cosmology.sh}, $\sim$4--12\,h per config on 4 CPU cores);
the converged iter2 $w_0w_a$ chain bundle \emph{is} committed
(\path{reproducibility/cosmology/iter2_converged_2026-05-18/}), as are the
ALP-MCMC chains
...
this work (DOI links are pinned to the \texttt{\paperVersion} commit in the
repository README; the DOIs are preserved against future README changes):
\begin{enumerate}[leftmargin=*]
...
$\beta_{\rm ALP} = 0.336^\circ\pm 0.10^\circ$         & MCMC      & Verified & ALP MCMC \\
...
\begin{itemize}[leftmargin=*]
\item $C_{a\gamma}$: uniform prior on $[4, 60]$ (covering the full
EOM-required band $[9, 51]$).
