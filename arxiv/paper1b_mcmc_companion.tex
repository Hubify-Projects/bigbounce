...
\newcommand{\paperVersion}{v1B.0.67}
...
as a published cross-check of the quintom-B direction on the existing
converged Cobaya chain. Its SN likelihood combines DES-SN5YR and
Pantheon+ without a joint overlap covariance (caveat~(e) of
Sec.~\ref{sec:verification}), so the SN-overlap systematic is stated
...
that only the $\Omega_a < 0.01$ spectator-safe subset
...
\textit{$w_0 w_a$ cross-check.}---The converged DESI~DR2 + Planck~NPIPE + Pantheon+~\cite{Brout2022PantheonPlus} + DES-SN5YR~\cite{DES2024SN5YR} Cobaya chain with the $w_0 w_a$ free-parameter extension ($128{,}385$ accepted samples across 16 MPI chains; $\hat R - 1 = 0.00820$, below the standard $\hat R\!-\!1\!<\!10^{-2}$ publication target) supplies the GetDist $w_0 w_a$ posteriors of Table~\ref{tab:iter2_posterior}, a published cross-check of the quintom-B scenario~\cite{Cai2010quintomReview} (tested against DESI~DR2~\cite{DESI2025DR2}) under an overlap-uncorrected SN product likelihood (Sec.~\ref{sec:verification}, caveat~(e)); the SN-overlap systematic is stated as a known limitation and no model-selection claim is made.
