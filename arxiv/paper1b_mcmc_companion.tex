...
\newcommand{\paperVersion}{v1B.0.63}
\newcommand{\paperTimestamp}{June 12, 2026}
...
The $\chi^2$ goodness-of-fit decomposition (BAO, CMB, SN, and total contributions) is reported in Table~\ref{tab:iter2_posterior}; the AIC, BIC, and $\ln B$ evidence metrics are \emph{not} reported there. The central marginal-tail result is $w_0 = -0.812 \pm 0.044$ (departing from the $\Lambda$CDM point $w_0=-1$ at $+4.3\sigma$; fn.~\ref{fn:wcaveat}) and $w_a = -0.667 \pm 0.186$ (departing from $w_a=0$ at $-3.6\sigma$), with $w_0+w_a = -1.48 \pm 0.15$ requiring phantom crossing (the canonical quintom signature under the overlap-uncorrected likelihood; see §\ref{sec:cosmo_fits} caveat). Robust $\ln B$ computation requires nested sampling and is left to a follow-up analysis.
