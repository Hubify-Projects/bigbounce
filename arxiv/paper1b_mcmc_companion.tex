...
\newcommand{\paperVersion}{v1B.0.53}
...
samples --- too few for a stable marginal, so the following band is
indicative only): within that sliver the required coupling piles against
...
dropped $[1,4)$ interval lies entirely below the EOM-required minimum
${\approx}8.6$ ---
