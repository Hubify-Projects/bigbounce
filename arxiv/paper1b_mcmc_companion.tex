...
\newcommand{\paperVersion}{v1B.0.93}
...
they have mixed only to $R\!-\!1\approx0.05$ (down from $\approx0.06$ and
still running), which remains above the $R\!-\!1<0.01$ Gelman--Rubin
threshold used for the load-bearing $\Lambda$CDM$+\Delta\Neff$ chains of
Sec.~\ref{sec:cosmo_fits}. The convergence status is therefore
\emph{disclosed, not hidden}: the final $R\!-\!1<0.01$ posteriors are
pending a live re-run that is presently in progress. At the current
mixing level the chains are well-mixed for the \emph{sign} of the crossing
but the per-chain $\sigma$-distances quoted above
($2.1\sigma$--$3.4\sigma$) are \emph{provisional point estimates} that
will shift as sampling continues toward $R\!-\!1<0.01$; they must not be
read as final significances, and no significance, model-selection, or
$\sigma$-distance statement from these control chains enters the main-text
conclusions or the P1A companion. The claim these chains support is
strictly \emph{directional} --- the phantom-divide crossing recovers with
the same sign in both disjoint supernova samples --- and that
sign-consistency is robust to residual $R\!-\!1$ mixing noise in a way the
numerical $\sigma$-distances are not. A full joint
