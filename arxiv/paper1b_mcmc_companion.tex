...
\newcommand{\paperVersion}{v1B.0.65}
...
$w_0 w_a$ posterior reported below is \emph{provisional} under an
overlap-uncorrected DES-SN5YR~$\times$~Pantheon+ product likelihood, with
the $\sim 20\%$ shared-event Malmquist-correction issue and the queued
SN-overlap control chains discussed in caveat~(e) and
Sec.~\ref{sec:verification}; \emph{no} model-selection statement is
claimed pending those controls, and the tail-distance significances
...
(provisional tail-distances $+4.3\sigma$ in $w_0$, $-3.6\sigma$ in
...
$z>z_\times$, quintessence-like today). This provisional posterior sits
in the phantom-crossing region under an overlap-uncorrected product
likelihood and, if reproduced under a joint-covariance treatment of the
overlapping SN events, would be a phenomenological match to a quintom-B
type evolution; whether the present posterior survives the SN-overlap
control chains is left for the queued verification analysis. In the
converged chain there are zero free-$w_0 w_a$ samples at the
$\Lambda$CDM point; the chain is centered at $w_0 + w_a \approx -1.48$,
again provisional under the same overlap caveat. To be explicit about what is and is not
...
\subsection{Exploratory cross-check: $w_0 w_a$ posterior under uncorrected SN overlap}\label{sec:w0wa_exploratory}

The DESI~DR2 $w_0 w_a$ chain (Table~\ref{tab:iter2_posterior}) is an
\emph{exploratory} cross-check only: its SN likelihood combines DES-SN5YR and
Pantheon+ without a joint overlap covariance (caveat~(e) of
Sec.~\ref{sec:verification}), so no model-selection statement is claimed pending
the queued control chains. The goodness-of-fit decomposition ($\chi^2_{\rm BAO}$,
...
$w_0+w_a = -1.48 \pm 0.15$ requiring phantom crossing; these figures are
provisional under the overlap-uncorrected likelihood and are discussed in detail
...
\textit{Exploratory $w_0 w_a$ cross-check.}---The converged DESI~DR2 + Planck~NPIPE + Pantheon+~\cite{Brout2022PantheonPlus} + DES-SN5YR~\cite{DES2024SN5YR} Cobaya chain with the $w_0 w_a$ free-parameter extension ($128{,}385$ accepted samples across 16 MPI chains; $\hat R - 1 = 0.00820$, below the standard $\hat R\!-\!1\!<\!10^{-2}$ publication target) supplies the GetDist $w_0 w_a$ posteriors of Table~\ref{tab:iter2_posterior}, an exploratory, overlap-uncorrected test of the quintom-B scenario~\cite{Cai2010quintomReview} (tested against DESI~DR2~\cite{DESI2025DR2}) that remains provisional pending the queued SN-overlap control chains (Sec.~\ref{sec:verification}, caveat~(e)).
