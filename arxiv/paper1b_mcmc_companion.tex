...
\newcommand{\paperVersion}{v1B.0.88}
...
overlap-uncorrected (caveat~(e)), so no significance, directional robustness, or
model comparison is claimed; the full posterior, scope statement, and the
deferred overlap-controlled control chains are given in Appendix~\ref{app:w0wa}.
...
DES-SN5YR only, each followed by a joint cross-covariance comparison of the
shifts in $w_0$, $w_a$, $w_0+w_a$, $w_{\rm pivot}$, and $\chi^2$ --- are
\emph{deferred to a dedicated follow-up} and are currently computing as
fresh MPI MCMC runs; until they converge no robustness of the quintom-B
direction against the overlap systematic is claimed, and their results will
be folded into a subsequent revision when available. The goodness-of-fit decomposition ($\chi^2_{\rm BAO}$,
