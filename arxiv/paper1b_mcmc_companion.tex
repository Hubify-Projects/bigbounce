...
\& Komatsu 2022 disambiguation (per R-upgraded-round4 GEM-m2 and to
preempt the recurring PR3-vs-PR4 reviewer reflag): the published PRD
...
YAML omission.
\par
\textit{M$_B$--H$_0$ joint-posterior offset check.} An earlier truth-audit
falsification argued that the joint posterior mean
...
queued; we report only the parameter posteriors below, which are unaffected
...
disclosure (per R-upgraded-round4 GEM-B1 spectator-prose / numerical-scan
reconciliation): the ALP backreaction fraction scales as $\Omega_a \sim
...
\begin{table*}[tb]
\caption{\label{tab:crosspaper} Cross-paper status table --- frozen Mid-May 2026
snapshot (Wave 14). Versions and readiness percentages are intentionally pinned
to the wave-14 cross-paper snapshot used by the companion-program audit so the
P1(b) text remains a reproducible record of the cross-paper state at the time
of submission of this companion; the live program continues to advance the
per-paper versions (e.g.\ P1(b) is now post-v1B.0.40 in development), tracked
on the project site rather than re-bumped inside this table.
DESI DR2 cobaya chains: see Table~\ref{tab:mcmc_inventory} for current
slow-mode-dominated state.}
\begin{ruledtabular}
\begin{tabular}{llcl}
...
disclosure (per R-upgraded-round4 GEM-B1, cross-link to
footnote~\ref{fn:theta_backreaction} in Sec.~\ref{sec:birefringence_check}):
