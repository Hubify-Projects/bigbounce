...
\newcommand{\paperVersion}{v1B.0.27}
\newcommand{\paperTimestamp}{2026-05-24 PDT}
...
stratification (real cross-vendor R-round v1B.0.5 reconciliation): \textbf{309{,}189} is
...
%% ============================================================
\begin{table*}[!htb]
\caption{\label{tab:iter2_posterior} Converged iter2 DESI~DR2 $w_0 w_a$ posterior summary (extracted v1B.0.13 via GetDist on the live converged chain). Chain state at extraction: $N_{\rm total}=128{,}385$ accepted samples across 16 chains; $N_{\rm effective}=89{,}871$ after 30\% burn-in discard; $\hat R - 1 = 0.00820$ (sustained across two consecutive flushes 2026-05-18 01:34 and 07:53 UTC); mean acceptance 0.2828; $\hat R_{\rm cl} = 0.0705$ chain-length diagnostic. Likelihood stack: DESI~DR2~BAO + Planck~2018 NPIPE lowl.EE + lowl.TT + highl.CamSpec.TTTEEE + lensing.native + DES-Y5 + Pantheon+. Auditable on-record at \href{https://github.com/Hubify-Projects/bigbounce/blob/main/reproducibility/cosmology/iter2_converged_2026-05-18/posterior_summary.txt}{\texttt{reproducibility/cosmology/iter2\_\-converged\_\-2026-05-18/posterior\_\-summary.txt}}. Sampled parameter space: 8 cosmological ($\log A$, $n_s$, $\omega_b h^2$, $\omega_c h^2$, $\tau$, $100\theta_{\rm MC}$, $w$, $w_a$) + 9 nuisance ($A_{\rm planck}$, three CamSpec foreground amplitudes, three CamSpec spectral indices, calTE, calEE) $=17$ total --- coincidentally the same total dimension as the $k=7+10=17$ frozen $\Lambda$CDM+$\Delta\Neff$ chains in Table~\ref{tab:verification}, but with a different parameter set (the iter2 cosmological block trades the frozen-chain $\Delta N_{\rm eff}$ parametrization $n_{\nu}$ for the extended-parameter pair $(w, w_a)$, yielding 8 cosmological parameters in iter2 vs 7 in the frozen chain; the iter2 nuisance block drops the frozen-chain $M_b$ SH0ES SN-Ia absolute-magnitude nuisance (because the iter2 SN-Ia stack DES-Y5+Pantheon+ uses internal absolute-magnitude marginalization rather than the frozen-chain SH0ES external calibrator), yielding 9 nuisance parameters in iter2 vs 10 in the frozen chain; the 9 Planck-likelihood CamSpec foreground/calibration nuisance parameters $\{A_{\rm planck},\, \text{amp}_{143},\, \text{amp}_{217},\, \text{amp}_{143\times 217},\, n_{143},\, n_{217},\, n_{143\times 217},\, \text{calTE},\, \text{calEE}\}$ are identical between the two chains; the totals match by construction at $k_{\rm sampled}=17$ but the parameter spaces are otherwise distinct; R25c-MIN-1 corrected v1B.0.25 from prior "$k=7+7=14$" stale carry, R25d-MAJ-1 corrected v1B.0.26 "distinct from" to "coincidentally same-total" framing, R25g-MAJ-1 corrected v1B.0.27 the previously-confabulated ``different foreground-amplitude/spectral-index split'' to the actual $n_{\nu}$-to-$(w, w_a)$ cosmological-block trade plus the $M_b$-drop nuisance-block diff verified against the on-disk iter2 chain header).}
\begin{ruledtabular}
\begin{tabular}{lcc}
Parameter & Mean $\pm \sigma$ & vs LCDM\\
\hline
\multicolumn{3}{l}{\textit{Dark-energy extension}}\\
$w_0$         & $-0.8122 \pm 0.0436$ & $+4.3\sigma$ from $-1$\footnote{\label{fn:wcaveat}Marginal-tail departure: LCDM is unsampled by this chain (no $w_0\!=\!-1, w_a\!=\!0$ samples in the present Metropolis-Hastings run), so the $+4.3\sigma$ figure is a posterior-tail extrapolation, not a Bayes-factor or $\ln B$ exclusion. A Savage-Dickey readout is not viable at this tail depth; the nested-sampling $\ln B$ recompute is queued (see \S~Headline-result discussion; R10 GEM-M1 / R7 GPT-B3 / R25d-MAJ-2 caveat).}\\
...
along ALP trajectories rather than an independent variable (R25b-BLK-1
clarification, v1B.0.24).
