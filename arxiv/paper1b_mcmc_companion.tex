...
\newcommand{\paperVersion}{v1B.0.79}
\newcommand{\paperTimestamp}{June 27, 2026}
...
\caption{\label{tab:iter2_posterior} DESI~DR2 $w_0 w_a$ posterior summary ($N=128{,}385$ accepted samples across 16 chains, $\hat R-1=0.00820$; 8 cosmological + 9 nuisance parameters). Likelihood stack: DESI~DR2~BAO + Planck NPIPE (PR4) CamSpec high-$\ell$ TTTEEE + Planck~2018 low-$\ell$ TT/EE + Planck~2018 lensing (\texttt{native}) + DES-Y5 + Pantheon+. Full parameter list and chain diagnostics are available in the companion data repository. Derived-parameter widths are GetDist readouts of the full chain covariance: the chain's $\sigma_8$--$\Omega_m$ correlation is $\rho=-0.27$ (the standard CMB acoustic-scale/lensing degeneracy), which is why $\sigma_{S_8}=0.0089$ is smaller than the uncorrelated propagation through $S_8=\sigma_8(\Omega_m/0.3)^{0.5}$ ($\approx 0.010$). \textbf{Overlap-uncorrected DES-Y5$\times$Pantheon$+$ product likelihood; the $\sigma$-distances are marginal-tail posterior-extrapolation distances, diagnostic only --- not detection significances and not suitable for model selection (caveat~(e), fn.~\ref{fn:wcaveat}).}}
\begin{ruledtabular}
\begin{tabular}{lcc}
...
$w_0 + w_a$   & $-1.4788 \pm 0.1485$ & $<-1$ (phantom-crossing region; marg.-tail, fn.~\ref{fn:wcaveat})\\
...
(overlap-inflated marginal-tail upper bounds, \emph{not} detection
significances: $+4.3\sigma$ in $w_0$, $-3.6\sigma$ in
$w_a$; fn.~\ref{fn:wcaveat}) and
...
$w_a$ term ($-3.6\sigma$). The CPL trajectory's effect on the expansion
...
(the $\Lambda$CDM point sits at $>4\sigma$ in the joint marginal tails and is
unsampled by the Metropolis-Hastings chain; see fn.~\ref{fn:wcaveat}).
The central marginal-tail posterior-extrapolation distances are
$w_0 = -0.812 \pm 0.044$ ($+4.3\sigma$ from $-1$; fn.~\ref{fn:wcaveat})
and $w_a = -0.667 \pm 0.186$ ($-3.6\sigma$ from $0$), with
$w_0+w_a = -1.48 \pm 0.15$ indicating phantom crossing
\textbf{(warning: these $\sigma$-distances are inflated by the
uncorrected DES-SN5YR\,$\times$\,Pantheon+ ${\sim}20\%$ shared-event
sample overlap, which double-weights the shared supernovae and narrows
the posterior; they are an \emph{upper bound} on the apparent departure
from $\Lambda$CDM --- an uncorrected product-likelihood artifact, not a
detection significance --- and must be read together with caveat~(e),
fn.~\ref{fn:wcaveat})}; these figures
...
The $+4.3\sigma$ departure of $w_0$ from $-1$ is a posterior-tail
extrapolation upper bound, not a detection significance: $\Lambda$CDM is
unsampled in the present Metropolis-Hastings run, so the figure measures
the distance from the chain center to the $\Lambda$CDM point in units of
the marginal posterior width (fn.~\ref{fn:wcaveat}); a robust exclusion
bound requires the deferred nested-sampling analysis.
