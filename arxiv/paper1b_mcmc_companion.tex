...
\newcommand{\paperVersion}{v1B.0.54}
...
(Cobaya~v3.6.1, $\mathbf{309{,}189}$ frozen samples across two converged dataset combinations, plus a third Planck-only combination still accumulating): this
...
BBN-consistent default (no explicit override in any of the four YAMLs).\footnote{\label{fn:sample_stratification}Sample-count
...
(\texttt{cobaya\_full\_tension.yaml}). [Correction note: an earlier
version quoted the Planck+BAO+SN $S_8$ marginal as $0.831\pm 0.018$; the
$\pm 0.018$ width could not be traced to any committed analysis, and a
direct GetDist pass over the frozen chains gives $0.827\pm 0.010$
(132{,}949 samples), which replaces it in the table.] With the
...
departure from LCDM persists, at reduced significance, even in the
...
controlled posterior density (with zero free-$w_0 w_a$ samples at the LCDM
...
($0.238/0.27 = 0.302/0.342 = 0.88$). The deconvolution is therefore unbiased at the
$|\Delta\hat\beta| \le 0.040^\circ$ level in the worst-case injection, which we carry forward as the
...
$(\theta_i\approx 1.4,\ m\approx 3\,H_0)$. [Correction note: an earlier
draft paired $\Delta\phi/f_a\approx 1.0$--$1.07$ with $m\approx
1.8$--$2\,H_0$; the committed EOM integration gives $\Delta\phi/f_a =
0.35$--$0.42$ at those masses ($\theta_i=1$), and the mass pairings are
corrected throughout against the released grid scan.] Across the box
...
(\path{research/branch_R_alp_birefringence/phase2_mcmc/chains/}).
[Correction note: an earlier draft described the model-dependent fits as
``three benchmark configurations $C_{a\gamma}\in\{4,8,12\}$, 3{,}240
samples each, $\theta_i\in[0.5,2]$, $m/H_0\in[1,3]$''; no archived chain
matches that description, and the configuration list below replaces it
with the committed truth. The headline posteriors are unaffected: they
are recomputed here directly from the archived chains.]
