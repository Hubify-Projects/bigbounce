...
\newcommand{\paperVersion}{v1B.0.69}
...
(same likelihood stack: Planck NPIPE high-$\ell$ CamSpec TTTEEE +
\texttt{planck\_2018\_lowl.TT} + \texttt{planck\_2020\_lollipop.lowlE} +
\texttt{planckpr4lensing} + full SDSS DR16 BAO block +
\texttt{sn.pantheonplus}; identical prior block) converged on an
...
\emph{Release-pairing note (E3/E4):} both the primary frozen chains and the
c15 verification chain use Planck NPIPE (PR4) CamSpec high-$\ell$ TTTEEE
paired with Planck~2018 (not PR4) low-$\ell$ TT/EE and Planck~2018 lensing;
the low-$\ell$ and lensing release years therefore differ from the high-$\ell$
component. A dedicated PR4-consistent low-$\ell$/lensing swap test has not been
run; any pairing-induced bias on $\Delta\Neff$, $H_0$, and $S_8$ at the
quoted precision is unquantified (see Sec.~\ref{sec:verification}, limitation note).
...
$w_0+w_a = -1.48 \pm 0.15$ requiring phantom crossing; these figures
...
\emph{H$_0$ note (E8):} $\Omega_a$ at each MCMC step uses the $\Lambda$CDM background
...
$\Omega_a < 0.01$), an ALP with $f_a\sim\MPl$, $m\sim H_0$
