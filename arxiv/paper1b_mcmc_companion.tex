...
\newcommand{\paperVersion}{v1B.0.55}
\newcommand{\paperTimestamp}{June 10, 2026 PDT}
% v1B.0.55 (2026-06-10 — EXT1-P1B closure wave):
...
$\Delta\Neff < 0.31$ (read from the posterior CDF at the $95^{\rm th}$
percentile of the $\Delta\Neff > 0$ half; the planck+BAO+SN chain gives
$\Delta\Neff < 0.39$).  These bounds are consistent with the two-sided
...
$\int\!\min(p_1,p_2)\,dS_8 = 0.05$); the full-tension posterior sits at
$2.0\sigma$ from DES-Y3 (overlap $0.12$); overlay artifact
...
\begin{equation}
\beta \approx \frac{\alpha_{\rm EM}\times 8}{4\pi}\times 1.06
= 4.93\times 10^{-3}\,\text{rad} \times \frac{180^\circ}{\pi} \approx 0.28^\circ.
\end{equation}
...
\emph{Unit warning for JSON artifacts:} the frozen diagnostic file
\path{reproducibility/cosmology/frozen/full\_tension\_20260311\_1728/diagnostics/parameter\_summary.json}
stores raw Cobaya-normalised parameter values (not physical units); a
physically-converted companion file
\path{parameter\_summary\_CORRECTED.json} and a
\path{parameter\_summary\_units\_README.md} in the same directory document
...
this work (DOI links are pinned to the \texttt{v1B.0.54} commit in the
