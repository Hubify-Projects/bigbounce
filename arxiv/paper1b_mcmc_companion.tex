...
\newcommand{\paperVersion}{v1B.0.61}
...
\textit{Physics interpretation (Table~\ref{tab:iter2_posterior}).}---We
front-load the interpretive caveat before stating the numbers: the
converged $w_0 w_a$ posterior below is a \emph{provisional} readout in
the phantom-crossing region under an overlap-uncorrected
DES-SN5YR~$\times$~Pantheon+ product likelihood (see caveat~(e) below for
the $\sim 20\%$ shared-event Malmquist-correction issue, and
Sec.~\ref{sec:verification}, caveat~(e), for the queued SN-overlap
control chains); the quoted tail-distance significances refer to the
marginalized posterior of the present product-likelihood chain and
should not be read as a model-selection statement. With that caveat
front-loaded: the converged $w_0w_a$ posterior \emph{disfavors} (in the
marginal-tail sense; see fn.~\ref{fn:wcaveat}) the $\Lambda$CDM point
