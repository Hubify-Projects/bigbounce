...
\newcommand{\paperVersion}{v1B.0.94}
...
observed NaMaster pipeline bias (deconvolution-algebra bias on foreground-free skies; \emph{estimator-specific} --- this figure is tied to the unweighted $\chi^2$ template fit chosen to match published scripts, and the inverse-variance-weighted estimator removes ${\approx}80\%$ of it (Sec.~\ref{sec:data_cmb}); it is \emph{not} a real-sky systematics bound, which additionally requires foreground cleaning and breaking of the $\beta$--$\alpha$ degeneracy by unrotated galactic foregrounds). Underlying data
...
consistently in \emph{both} independent supernova samples (sign only; no
$\sigma$-distance is claimed, as the control chains inherit the same
overlap-uncorrected likelihood), so the crossing is robust to the overlap
systematic in sign; the full posteriors,
...
exploratory DESI~DR2 + Planck~NPIPE + Pantheon+~\cite{Brout2022PantheonPlus} + DES-SN5YR~\cite{DES2024SN5YR} Cobaya chain with the $w_0 w_a$ free-parameter extension ($128{,}385$ accepted samples across 16 MPI chains; $\hat R - 1 = 0.00820$) was run; its overlap-uncorrected posteriors (Table~\ref{tab:iter2_posterior}) are reported only in Appendix~\ref{app:w0wa} as a non-load-bearing exploratory diagnostic. Because the SN likelihood combines DES-Y5 and Pantheon$+$ as a product without a joint cross-covariance despite their ${\sim}20\%$ shared events (disclosed as caveat~(e) of Sec.~\ref{sec:verification}), no $\sigma$-distance, significance, or model-selection statement is claimed from this headline chain. The one property of this chain that \emph{is} tested against the overlap systematic is the \emph{sign} of the quintom-B direction: two disjoint-sample overlap-controlled control chains (Pantheon$+$-only and DES-SN5YR-only, each dropping the other, overlapping supernova sample) have now been run (Appendix~\ref{app:w0wa}) and recover the same phantom-divide crossing ($w_0>-1$, $w_0+w_a<-1$) in \emph{both} independent samples ($w_0+w_a=-1.40$ and $-1.57$), demonstrating that the directional finding is robust to the double-counting rather than an artifact of the uncorrected joint covariance; a fully joint-covariance / nested-sampling significance remains a dedicated follow-up.
...
cosmological constraint;} the overlap-controlled control chains needed to test
the \emph{directional} robustness of this diagnostic (specified below) have now
been run, and confirm the quintom-B direction survives in both disjoint
supernova samples.
...
that are \emph{still actively sampling} (Gelman--Rubin currently
$R\!-\!1\approx0.05$, tightening from an initial $\approx0.06$ and not yet
at the $R\!-\!1<0.01$ threshold; the run is live and continuing toward
full convergence), and \textbf{the quintom-B direction survives in both
independent supernova samples}. The Pantheon$+$-only chain gives central
values $w_0=-0.874$, $w_a=-0.530$, $w_0+w_a=-1.404$ ($w_0>-1$ \emph{and}
$w_0+w_a<-1$); the DES-SN5YR-only chain gives $w_0=-0.787$, $w_a=-0.785$,
$w_0+w_a=-1.572$ ($w_0>-1$ \emph{and} $w_0+w_a<-1$). We quote these as
\emph{central values that fix the sign} of each dark-energy parameter, not
as $\pm$ measurements: because these control chains inherit the same
overlap-uncorrected product-likelihood construction as the headline stack
(each simply drops the \emph{other} overlapping supernova sample rather
than supplying a joint cross-covariance), their marginal widths are not
valid uncertainties and \textbf{no $\sigma$-distance, significance, or
model-selection statistic is computed from them}. Because the phantom-divide crossing ($w_0>-1$ \emph{and}
...
\artifact{reproducibility/cosmology}). We caution explicitly that these
control chains are \emph{not yet fully converged}: at the time of writing
they have mixed only to $R\!-\!1\approx0.045$ (Pantheon$+$-only) and
$R\!-\!1\approx0.048$ (DES-SN5YR-only), both still running and still above
the $R\!-\!1<0.01$ Gelman--Rubin threshold used for the load-bearing
$\Lambda$CDM$+\Delta\Neff$ chains of Sec.~\ref{sec:cosmo_fits}. The
convergence status is therefore \emph{disclosed, not hidden}: the final
$R\!-\!1<0.01$ posteriors are pending a live re-run that is presently in
progress. \textbf{We deliberately claim no $\sigma$-distance from these
control chains --- not even a provisional one.} A $\sigma$-distance read
off an overlap-uncorrected product likelihood is not a valid significance
regardless of convergence, because the double-counted ${\sim}20\%$ shared
supernovae narrow the marginal widths by an unquantified amount; converging
the chains to $R\!-\!1<0.01$ would only sharpen an \emph{invalid} width, not
make it valid. The only property of these chains we report is the
\emph{sign} of the crossing: at the current mixing level the chains are
well-mixed for the sign of the phantom-divide crossing, and that sign is
recovered identically in both disjoint supernova samples. The claim these
chains support is therefore strictly \emph{directional} --- the
phantom-divide crossing ($w_0>-1$ \emph{and} $w_0+w_a<-1$) recovers with the
same sign whether the fit uses Pantheon$+$ alone or DES-SN5YR alone --- and
establishes \emph{direction only, not significance}. A full joint
DES$\times$Pantheon$+$ overlap-covariance cross-comparison, run to
$R\!-\!1<0.01$, remains the definitive follow-up; the directional
robustness claim rests only on the consistency of the crossing across the
two disjoint-sample fits, which neither that covariance nor further
mixing can overturn. The goodness-of-fit decomposition ($\chi^2_{\rm BAO}$,
