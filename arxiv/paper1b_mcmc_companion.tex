...
\newcommand{\paperVersion}{v1B.0.13}
\newcommand{\paperTimestamp}{2026-05-18 PDT}
% Internal version-history note (not rendered in PDF; per feedback_pdf_visual_formatting,
% long round-closure log MUST NOT live inside \date{}):
...
$\chi^2_{\rm total}$ & $14037.4 \pm 5.6$ & ---\\
...
$\ln B$ against LCDM is NOT reported in this v1B.0.13; standard posterior
MCMC samples do not give a robust $\ln B$, and Savage-Dickey requires
explicit prior density and nuisance-marginalized posterior density at the
LCDM point. The deterministic Savage-Dickey ratio on the converged 2D
$(w, w_a)$ marginal is queued for v1B.0.13+. (b)~The chain uses the
Planck~2018 prior on $\tau$ (Gaussian centered at the Planck best-fit
value) rather than a free $\tau$ from a low-$\ell$ likelihood; in
combination with the standard CMB likelihood stack this is the canonical
choice but readers should be aware. (c)~The SH0ES $H_0$ prior is
\emph{not} included in this chain (the chain is BAO + CMB + SN-only, no
local-distance ladder); the joint posterior $H_0 = 67.185 \pm 0.455$
km/s/Mpc is therefore the no-SH0ES result. The interaction with the
SH0ES prior (which would pull $H_0$ upward) is the subject of the
separate full-tension chain reported in Table~\ref{tab:verification}.
...
auditable script was extracted in v1B.0.13 via GetDist on the converged chain ($\hat R\!-\!1\!=\!0.00820$ at $N=128{,}385$, 2026-05-18 07:53~UTC; mean acceptance 0.2828; $\hat R_{\rm cl}=0.0705$ as chain-length diagnostic) and is reported in Table~\ref{tab:iter2_posterior}. The headline result is $w_0 = -0.812 \pm 0.044$ (departing from the $\Lambda$CDM point $w_0=-1$ at $+4.3\sigma$) and $w_a = -0.667 \pm 0.186$ (departing from $w_a=0$ at $-3.6\sigma$), with $w_0+w_a = -1.48 \pm 0.15$ requiring phantom crossing (the canonical quintom signature). A robust Bayesian evidence / Bayes factor against $\Lambda$CDM requires either a separate $\Lambda$CDM Cobaya run on the identical likelihood stack with nested sampling (PolyChord/MultiNest), or a documented Savage-Dickey density ratio at the LCDM point with explicit prior measure and nuisance-marginalized posterior density --- ordinary posterior MCMC samples alone do not yield a robust $\ln B$ (R7 GPT-B3 caveat). The Savage-Dickey $\ln B$ recompute is queued for v1B.0.13+ (the v1B.0.8 / v1B.0.9 / v1B.0.10 / v1B.0.12 markers were carried over from prior version-numbering schemes when the chain was still mixing).
...
Savage-Dickey $\ln B$ recompute against the LCDM point, queued for
v1B.0.13+.
...
Model-comparison $\Delta$AIC/BIC/$\ln B$               & Numerical & Tab.~\ref{tab:iter2_posterior} & v1B.0.13+ Savage-Dickey \\
