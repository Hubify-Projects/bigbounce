...
\newcommand{\paperVersion}{v1C.0.8}
...
arrows to all four routes because B14 (perturbation transparency)
constrains R1--R4, while its co-resident B8 constrains R1 only --- the
in-figure labels record this per-barrier attribution (the upper Branch-H
arrow to R1 carries both B8 and B14; the lower three-route fan is B14
alone), which is otherwise not resolved at box granularity. The branch
...
\end{align}
where $\alpha_{\rm em}/(4\pi) \approx 6 \times 10^{-4}$ (more precisely
$5.8\times 10^{-4}$), rounded \emph{up} to $10^{-3}$ in the explicit
...
ambiguity). The role of each denominator factor is stated explicitly so
the contraction is not misread as a double division: the displayed ratio
normalizes the one-loop amplitude \emph{both} by the observed angle
$\beta_{\rm obs}$ \emph{and} by the dimensionless strength
$M_{\rm Pl}(\alpha/M)\sim10^{-2}$ of the R4-fitted coupling that
reproduces that angle --- i.e.\ it states the suppression relative to the
fitted-coupling benchmark, the weaker (more conservative) normalization.
...
the Route-2 amplitude estimate. The algebraic origin of that scale,
...
elevated ISM-like normalization~\cite{Golden2026P1a}.\strut} \\[3pt]
...
content introduces $F\widetilde F$ only once electromagnetic fields ---
...
commit
\href{https://github.com/Hubify-Projects/bigbounce/tree/9b92721d5d7eb6601ebf48c49379318238b7100b}{\texttt{9b92721d5d7e}}\footnote{The
descriptive header text of the first script differs between the pinned
commit and the current repository head; the checks performed and their
pass/fail results are identical at both.}.
...
(CC-BY-4.0)~\cite{Golden2026P1a}, whereas the two
\texttt{theory\_audit} verification artifacts listed above are not part
of that deposit and are available at the pinned repository commit. An
...
$\kappa(J^5\!\cdot\!J^5)$ [$\MPl^2\kappa^2=\kappa$; see text].
...
step) and which the companion's gap-equation bound conservatively omits
