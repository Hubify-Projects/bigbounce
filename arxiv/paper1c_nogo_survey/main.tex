...
\newcommand{\paperVersion}{v1C.0.5}
...
one-loop renormalization-group flow that leaves the derived contribution
$61$--$67$ orders of magnitude below the \emph{observed dark-energy
density}. Neither route is presented as a viable mapping: the
...
script-verified reduction identities; (iii)~the integrated Route-3 bound
...
$\geq 58$-order margins quoted. Third, the symbol pair $(\alpha,M)$
...
is dimensionless, as required. Once the dimension-$(+2)$ background value
...
bounded by a derived integrated renormalization-group flow sitting
$61$--$67$ orders below the observed dark-energy density. An operator-basis-completeness argument
...
The ``Fate'' column records the reduction of the \emph{bare} invariant,
prior to multiplication by the prefactor; restoring the prefactor, the two
genuine dimension-$4$ densities O4 and O5 both land on
...
O4 & $2$ & $\varepsilon^{\mu\nu\rho\sigma}T^{I}{}_{\mu\nu}T_{I\rho\sigma}$ (torsion$^2$) & $\MPl^{2}$ & $\to\kappa^2(J^5\!\cdot\!J^5)$, Fierz basis \\
...
$\kappa=M_{\rm Pl}^{-2}$ prefactor is preserved exactly --- which is all
