...
\newcommand{\paperVersion}{v1A.0.51}
...
$\lesssim\!10^{-22}\,\rho_\Lambda^{\rm obs}$ level --- negligible at all
...
function of $\gamma$. The form is the natural EFT operator built from the
...
to $10^{-60}$ (the factor-of-$\sim$100 ambiguity reflects the
perturbative order at which the quasi-dust $\varepsilon$-correction
enters the loop estimate --- here $\varepsilon = 3(1+w)/2$ is the
matter-bounce contraction equation-of-state parameter and the
``$\varepsilon$-correction'' is the $\mathcal{O}(\varepsilon - 3/2)$
quasi-dust correction defined in the companion $f_{\rm NL}$ forecast
paper; the
...
half the coupling times the field excursion), $\rho_\theta$ is the
...
\caption{\textbf{Naturalness landscape for the four minimal-ECH dark-energy routes}
(R1 NJL, R2 one-loop effective action, R3 Immirzi running, R4 parity-CMB
spectator-ALP). Each route is shown as a point in the (mass$\!\times\!$coupling)
plane required to source $\rho_\Lambda$ at the observed value, with the
naturalness window (gray band) defined by the scale of the underlying physics
($M_{\rm Pl}$ for R3, $H_0$ for R4 spectator-ALP via the
cosmological-constant-class tuning admitted in
\S\ref{sec:r4_birefringence}). All four routes either sit outside the
naturalness window or require a $m_\theta\!\sim\!H_0$ tuning that re-imports
the cosmological-constant problem. The 13 logically-independent barriers
...
$\mathcal{H}\equiv a'/a$ is the conformal Hubble rate; in cosmic time the
...
adjustment (same $\varepsilon$-correction as in the Route-2 loop
estimate above) is $\lesssim 1$ $e$-fold. A rigorous derivation of the prefactor from the
...
(\S\ref{sec:surviving}).\label{fig:detection_forecast}}
\end{figure}
