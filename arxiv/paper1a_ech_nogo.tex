...
\newcommand{\paperVersion}{v1A.0.110}
...
are forecast in a companion~\cite{Golden2026P2}, posted concurrently in the coordinated submission; we quote no
...
routes (Sec.~\ref{sec:fourroute}) at amplitude-budget granularity. Within the
minimal ECH field content the enumeration is basis-complete at the level of
$M_{\rm Pl}$-power-counting classes (Sec.~\ref{sec:rotation}, completeness
lemma): the Jackiw--Pi gravitational Chern--Simons term and the parity-odd
four-fermion partner of R1---previously flagged as omitted---are closed
explicitly in the body, and only the fully explicit Fierz-by-Fierz projection
lemma over the most general diffeomorphism-invariant parity-odd basis remains a
scoped follow-up. The complementary observational programme is hosted in
companion papers posted concurrently in the coordinated
submission~\cite{Golden2026P2,Golden2026P1b}.
$\Lambda$CDM$+\Delta\Neff$ MCMC verification, NaMaster pipeline validation,
and ALP parameter fitting are documented in the computational
companion~\cite{Golden2026P1b}, whose reproducibility artifacts are archived
...
explicitly in \S\ref{sec:rotation}), with only the fully explicit
...
\textbf{(c)}~any claim resting on the companions posted concurrently in the coordinated submission---their
...
(Paper~I(b), II, III, IV; all posted concurrently in the coordinated submission) are
...
A referee unable to access the companions posted concurrently in the coordinated submission can therefore still
...
Table~\ref{tab:companion_inputs}) receive their full narrative treatment in the
named companions; nonetheless the underlying computational artifacts---the
frozen MCMC chains, the NaMaster pseudo-$C_\ell$ validation suite, the
spectator-ALP fit, and the galaxy/anomaly catalogs---are archived with this
submission in the public reproducibility
tree~\cite{BigBounceRepro} and can therefore be inspected and re-derived by a
referee \emph{now}, independently of the companion manuscripts' posting
schedule. The coordinated submission fixes only the citable arXiv identifiers;
the numbers themselves are already reproducible, and in any case bear on none of
...
(companion, posted concurrently in the coordinated submission~\cite{Golden2026P2}) and is \emph{not} quoted here, as it is not a
...
the present paper's logic without the companions posted concurrently in the coordinated submission in hand; these
...
hosted entirely in Paper~I(b) (companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}); this
...
the power-counting-class completeness itself is established here. We stress that
...
(companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}); the present
...
are in Paper~I(b) (companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}).
...
framework rather than falsifying it (details in Paper~I(b), companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}).
...
\end{tabular}
\end{ruledtabular}
$^{\dagger}$Quintom-B can in principle accommodate the DESI $w_0 w_a$ evidence; the MCMC analysis in Paper~I(b) (companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}) was not extended to the $w_0 w_a$ parameter space, so this row is reported as ``consistent at the model level'' rather than a posterior-preference $\checkmark$.\\
...
of the 15-yr free-spectrum data (GPU MCMC, companion, posted concurrently in the coordinated submission~\cite{Golden2026P3};
...
Paper~I(b) (companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}) Table~IV row ``DESI DR2 w0wa (new)'' and has not yet
...
LiteBIRD forecast are in Paper~I(b) (companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}).
...
$f_{\rm NL}$ Fisher landscape (companion, posted concurrently in the coordinated submission~\cite{Golden2026P2}, $1.3$--$2.75\sigma$ projection).
Bottom: spectator-ALP cosmic birefringence in the LiteBIRD $\sigma(\beta)
\!\approx\!0.03^\circ$ window (companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}); the WMAP+Planck
...
convergence diagnostics are in Paper~I(b) (companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}).
\end{itemize}
...
and NaMaster pipeline validation are in Paper~I(b) (companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}).
...
MCMC analysis (companion, posted concurrently in the coordinated submission~\cite{Golden2026P1b}) and are not independently
