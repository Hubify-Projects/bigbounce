...
\newcommand{\paperVersion}{v1A.0.62}
...
mechanism-class constraints that collectively constrain the
...
Levi-Civita connection ($T=0$, metric-compatible) by the first (algebraic) Bianchi identity
$R_{\mu[\nu\rho\sigma]}=0$, and the Holst sector therefore decouples from all
...
pieces vanish at $T=0$, recovering the Bianchi-vanishing result. Earlier
versions of this manuscript erroneously identified the two; the correction
preserves the headline conclusion (Holst sector decouples from scalar/tensor
EOM) by a Bianchi-trivial argument rather than a Pontryagin total-derivative
argument.}
...
observable of this paper. The rotation contribution $c_\omega\omega^2$ is a
\emph{distinct} and negligible term: the CMB isotropy bound
$(\omega/H)_0 < 5\times 10^{-11}$ from Saadeh~\etal~\cite{Saadeh2016}
confines it to $\lesssim\!10^{-21}\,\rho_\Lambda^{\rm obs}$
($(\omega/H)_0^2 < 2.5\times 10^{-21}$; dividing by
$3\Omega_\Lambda \approx 2.1$ gives $\sim\!1.2\times 10^{-21}$ of
$\rho_\Lambda^{\rm obs}$) --- completely invisible on the scale plotted.
The dark-energy mechanism in this paper is therefore the
$\Xi\,\MPl^2$ term sourced by the parity-odd contorsion sector
(\S\ref{sec:parityodd}), \emph{not} the rotation
component.\label{fig:rotation_expansion}}
\end{figure}
...
for $T\!\lesssim\!10^{12}\,$GeV --- so the sphaleron channel does not
itself exceed $H$ at $T_{\rm reh}$, but completes the erasure once the
plasma cools through the $10^{12}\,$GeV regime while remaining in
...
chirality-flipping channels above (the expectation, given the
$\alpha_W^5 M_{\rm Pl}/T\gg 1$ and $y_t^2 M_{\rm Pl}/T\gg 1$ ratios at
the GUT scale), \emph{then} the coherent axial component is
thermally reset. Because the Cartan equation is
...
with the standard published derivation where available (R1, R4) or with explicitly-labeled ansatz-level scoping (R2--R3), constituting the no-go audit for each route.
...
which for late-time / post-recombination Standard-Model number densities
(the relevant regime for the late-time dark-energy claim assessed in
this route; thermal-era fermion densities are addressed separately in
the reheating-thermal-reset paragraph of Sec.~\ref{sec:dilution}) is
many orders of magnitude below the present-day
dark-energy density $\rho_\Lambda \sim (10^{-3}\,\text{eV})^4$:
at recombination the baryon/electron number density is
$n_\psi \sim n_b(z\!\simeq\!1100) \approx 330\,\text{cm}^{-3}
\approx 2.5\times 10^{-12}\,\text{eV}^3$, giving
$\rho_{\rm NJL} \sim n_\psi^2/M_{\rm Pl}^2 \approx
4\times 10^{-80}\,\text{eV}^4 \sim 10^{-69}\,\rho_\Lambda$ ---
nearly $70$ orders of magnitude below the dark-energy density.
...
parity. We retain the section's existing label for continuity with
earlier draft conventions but flag the precise classification here. (A
separate note on the photon-coupling chain: the operator above does not
contain an electromagnetic field strength; the mapping to a CMB
birefringence $\beta$ in the dimensionless ratio below uses the
amplitude-budget translation through the standard chiral-anomaly
$\partial_\mu J^{5\mu} \supset (\alpha_{\rm em}/4\pi)\,F\widetilde F$
chain at the EFT level. We treat the resulting $\beta$ estimate strictly
as an amplitude-budget bound and not a derived prediction.)} The function
...
$|0.342-0.215|/\sqrt{0.094^2+0.074^2} = 0.127/0.120 \approx 1.06$) bounds $\alpha/M$ at
...
documented in
Sec.~\ref{sec:r1_njl} as a quantitative closure rather than a
...
(footnote~\ref{fn:spherex_range}). Both surveys deliver
parameter-independent, ECH-independent class-level discrimination tests; the
joint outcome falsifies the surviving ECH framework or leaves it as the
unique surviving minimal-ECH channel under the stated
ans\"atze.\label{fig:obs_timeline}}
\end{figure}
...
residual tuning): $\Lambda$CDM ($10^{120}$), quintessence ($10^{60}$),
...
metric-compatible connection \emph{vanishes identically} by the first (algebraic) Bianchi identity
...
The vanishing requires both $T=0$ \emph{and} metric compatibility (so that
the first Bianchi identity above holds), and can fail in non-metric
connections even when torsion is zero.
...
Holst dual vanishes pointwise in the torsionless sector. \emph{This must be carefully
...
(with systematics) from SPHEREx, yielding $2.6$--$5\sigma$ model separation.
...
$\gamma_{\rm PTA} = 3.20\pm 0.42$ used in pre-real-KDE drafts; the migration is
...
companion Paper~I(b) Table~IV row ``DESI DR2 w0wa (new)'' and has not yet
...
$f_{\rm NL}$ Fisher landscape (companion Paper~II, $2.6$--$5\sigma$ projection).
Bottom: spectator-ALP cosmic birefringence in the LiteBIRD $\sigma(\beta)
\!\approx\!0.03^\circ$ window (Paper~Ib companion); the WMAP+Planck
$\beta=0.342^\circ\pm 0.094^\circ$ and ACT~DR6 $\beta=0.215^\circ\pm 0.074^\circ$
points are shown for reference. The SPHEREx $f_{\rm NL}$ forecast is
decisive ($\gtrsim 5\sigma$ on Stage~III/IV survey timescales) against
$f_{\rm NL}=0$; the LiteBIRD birefringence forecast targets a
...
combination of Fig.~\ref{fig:obs_timeline}; this panel is the summary
...
annotations.\label{fig:detection_forecast}}
\end{figure}
...
\end{center}
The bundle is labelled \texttt{v1A.0.59-bundle} (resynced 2026-06-10) to
track the present manuscript title and version; the bump from
\texttt{v1A.0.58-bundle} covers the EXT2 external-round textual-closure edits
and a README results-table resync to the frozen-chain values of companion
Paper~I(b) Table~IV only,
no MCMC chain / Cobaya YAML / Stan galaxy-spin code or data input changed,
and the two bundles are byte-identical except for the README and BibTeX
citation metadata; the present manuscript version \texttt{v1A.0.61} uses
the \texttt{v1A.0.59-bundle} unchanged (text-only restamp). The frozen
MCMC chains backing the cosmology tables are committed in the bundle
(under \texttt{reproducibility/cosmology/frozen/}); fresh re-verification
...
Paper~I(b)~\cite{Golden2026P1b}. The $\gamma$ scheme range denotes LQG
area-spectrum scheme dependence, \emph{not} a statistical error.}
\begin{ruledtabular}
\begin{tabular}{llccc}
Parameter & Description & Prior & Verified Value & Notes \\
\hline
\multicolumn{5}{l}{\textit{Fundamental theory parameters}} \\
$\gamma$ & Barbero-Immirzi & Fixed: 0.274 & $0.274$ (scheme range $\sim\!0.020$) & LQG area spectrum (Eq.~\ref{eq:gamma}) \\
...
\textit{Setup.}---After the reduction of Appendix~\ref{app:dimensions},
the parity-odd sector takes the Maxwell--Chern--Simons form
\begin{equation}
...
(the convention block of the companion's \S VI), closing the
normalization chain from the Lagrangian to the numerical prediction;
