...
\newcommand{\paperVersion}{v1A.0.27}
...
\paragraph{Internal cross-vendor review history (v1A.0.21; status updated v1A.0.24 — Route 2 dimensionless reduction is now executed in-line in Sec.~\ref{sec:route2} below, no longer deferred). \textbf{NOTE FOR JOURNAL SUBMISSION (per R6 Grok-M3 + multi-paper convergent):} this paragraph is internal version-history tracking and SHOULD BE REMOVED before submission to a refereed journal; it is retained in the arXiv-tracked source for provenance audit only.}
