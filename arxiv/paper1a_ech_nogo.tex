...
\newcommand{\paperVersion}{v1A.0.24}
...
$e$-folds, and the matter-bounce $\fnl=-35/8$ signature, which would be \emph{definitively} erased at SPHEREx-accessible comoving wavenumbers by that many $e$-folds (a contracting-phase quantity mode with $k_{\rm SPHEREx}\sim 10^{-1}\,h/$Mpc is pushed to $k_{\rm bounce}\sim k_{\rm SPHEREx}\,e^{N_{\rm tot}}\sim e^{30}$ times the observable horizon, i.e.\ a regime carrying purely vacuum-inflationary fluctuations rather than matter-bounce contraction modes); ECH is therefore not internally consistent as both a
...
$\fnl=-35/8$ prediction (\emph{definitively} erased by $N_{\rm tot}\!\gtrsim\!60$ inflationary $e$-folds at SPHEREx-relevant comoving wavenumbers, since the SPHEREx accessible band $k\sim 10^{-4}$--$10^{-1}\,h/$Mpc is pushed to bounce-era comoving scales $k_{\rm bounce} = k_{\rm SPHEREx}\,e^{N_{\rm tot}}$ that lie deep inside the inflationary subhorizon regime where the observable bispectrum is dominated by vacuum-inflationary modes rather than the matter-bounce contraction modes; see Sec.~\ref{sec:structural_tension}), establishing that
...
re-derivation---remains \textbf{on-record deferred} pending a dedicated
photon-Chern-Simons coupling derivation; the channel-level amplitude
