...
\newcommand{\paperVersion}{v1A.0.22}
\newcommand{\paperTimestamp}{2026-05-15 00:30 PDT (1st real cross-vendor R-round closures: 13-barrier propagation; f\_NL scoped to scalar-only $w=0$ class with P2 Assumption (f); dimensional bookkeeping fix in Appendix B; Hehl-Datta parity-even reaffirmed; Route 2 dimensional ratio remains on-record deferred)}
...
(Sec.~\ref{sec:barriers}) close the enumerated minimal-ECH dark-energy routes. The surviving
...
Is there a testable prediction? & $\fnl = -35/8$ (Paper~II forecast $3$--$5\sigma$ post-systematic-budget under Heinrich+2024 $\sigma(\fnl)\approx 0.7$$^b$) & \textbf{Yes} (class-level: scalar-only $w=0$ matter-bounce under Assumption (f) of Paper~II; not a fully mechanism-independent claim across the bouncing-cosmology landscape; not a distinctive ECH prediction). \\
...
bounce densities. Two factors enter explicitly: $K\sim\MPl$ (the curvature scale,
dim $+1$) and $R\sim\MPl^2$ (the Ricci scalar evaluated at the bounce, dim $+2$),
contributing $+3$ to the on-shell action density. The remaining $+2$ powers needed
to bridge $[\mathcal{L}_{\rm odd}] = +1$ (off-shell) to $[\rho_\Lambda] = +4$
(on-shell vacuum energy density) come from the dimensional reduction implicit in
expressing the volume-integrated action density at horizon-crossing scale
$L\sim 1/H \sim 1/\MPl$ during the bounce epoch, which contributes a factor
$\MPl^{2}$ from the spatial volume integration density. Combining all factors:
\begin{equation}
\rho_\Lambda^{\rm bounce}\sim(\alpha/M)\cdot K\cdot R\cdot \MPl^{2}
\sim (\alpha/M)\,\MPl^5 \sim 10^{-2}\,\MPl^4,
\label{eq:onshell_rho}
\end{equation}
where we used $\alpha/M \sim 10^{-21}\,\text{GeV}^{-1} \sim 10^{-2}\,\MPl^{-1}$,
so $(\alpha/M)\,\MPl \sim 10^{-2}$ (dimensionless) and the final $(\alpha/M)\,\MPl^5
\sim 10^{-2}\,\MPl^4$ is the on-shell scaling ansatz with $[\rho_\Lambda]=+4$
\emph{by construction}. Inflationary dilution ($\Dinf\sim e^{-3N_{\rm tot}}$) then
yields $\rho_\Lambda = \Xi\,\MPl^4$ with $\Xi = (\alpha/M)\,\MPl\,\Dinf$. This is a
\textit{scaling ansatz}, not a derivation from a renormalizable off-shell EFT; the
volume-integration factor $\MPl^{2}$ is a placeholder for the missing
dimensional-reduction integral and should be replaced by an explicit calculation
in a future iteration. Importantly, no \emph{quantitative} conclusion in the
main text relies on the precise on-shell dimensional bookkeeping: the 13
logically-independent structural barriers close the bounce-to-dark-energy route on
amplitude-budget and operator-counting grounds that are independent of the
specific power of $\MPl$ in Eq.~\ref{eq:onshell_rho} (the closure depends on the
overall scale separation between $\rho_\Lambda^{\rm bounce}$ and the observed
$\rho_\Lambda \sim (10^{-3}\,\text{eV})^4$, which is $\sim 35$ orders of magnitude
regardless of whether the bounce-scale density is $\MPl^4$ or $10^{-2}\,\MPl^4$).
The corrected dimensional bookkeeping above replaces the v1A.0.21 deferral note
(a) on the $(\alpha/M)\,\MPl^3$ vs.\ $[(\alpha/M)\,\MPl]\,\MPl^4$ inconsistency
flagged by the real-cross-vendor R-round.
