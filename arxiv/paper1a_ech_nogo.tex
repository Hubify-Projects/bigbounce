...
\newcommand{\paperVersion}{v1A.0.33}
...
re-derivation---was \textbf{closed in v1A.0.24 / v1A.0.25}: the dimensionless reduction is now executed in-line in Sec.~\ref{sec:route2} below (Eq.~\ref{eq:route2_ratio}, with the $H_0\to H_0/M_{\rm Pl}$ factor restored in the numerator), no longer deferred to a dedicated photon-Chern-Simons follow-up; the channel-level amplitude
...
\tilde F_{\mu\nu} F^{\mu\nu}$ (the standard ALP-photon Chern-Simons coupling with all indices fully contracted; the integrated-by-parts equivalent $(\alpha/M)\,\partial_\mu\theta\, K^\mu$ where $K^\mu$ is the Chern-Simons 4-current is also valid; v1A.0.28 R7 GPT-M3 closure: prior $(\alpha/M)\,\partial_\mu\theta\, \tilde F^{\mu\nu} F_{\mu\nu}$ left $\mu$ uncontracted on $\partial_\mu\theta$) into the rotation angle
\begin{equation}
...
\end{equation}
where we have used the LQG-bounce critical-density window $\rhocrit/\rhoPl \simeq 0.27\text{--}0.41$ from Sec.~\ref{sec:lqc} (v1A.0.28 R7 GPT-M5 closure: prior $\sim 10^{-2}$ arithmetic mistook the square as ${\sim}(\rhocrit/\rhoPl)^4$ rather than ${\sim}(\rhocrit/\rhoPl)^2$). This total bounce-epoch GW energy-density fraction is not directly comparable to the present-day PTA spectral-density measurement $\Omega_{\rm GW}(f_{\rm nHz}) \sim 10^{-9}$, which differs by both (i)~redshift / cosmological-dilution factors from the bounce epoch to today, and (ii)~the integration over frequency to recover a spectral density at a given band. A quantitative comparison to NANOGrav requires propagating the bounce GW spectrum through the transfer function to the nHz band, which is deferred to a forthcoming bounce-GW dedicated paper (queued); for the present analysis, Barrier~12 closes as a global energy-density-fraction ceiling rather than a direct NANOGrav exclusion.
