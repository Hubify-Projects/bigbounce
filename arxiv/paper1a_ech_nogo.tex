...
\newcommand{\paperVersion}{v1A.0.1}
...
Perturbation Transparency, Inflation--$f_{\rm NL}$ Tension, and\\
Surviving Matter-Bounce Tests}
...
mechanisms. Through 7 foundation studies and 17 research branches we catalog
...
by that many $e$-folds; these are treated as independent observational problems.
Two mechanism-independent predictions survive ECH structural closure:
(i)~$\fnl = -35/8$ from the matter-bounce class~\cite{Cai:2009fn}, testable by
SPHEREx at $3$--$5\sigma$ realistic significance (detailed multi-tracer Fisher
forecast in Paper~II, Ref.~\cite{Golden2026P2}); and (ii)~spectator-ALP
birefringence $\beta\approx 0.27^\circ$ consistent with the published
Planck/ACT~DR6 $3.6\sigma$ signal, arising in any GR+ALP setup with
the same parameters and not derived from the ECH action.
...
Through 7 foundation studies and 17 research branches, we establish 14 independent
...
(a fitted parameter, not predicted). This reparameterizes fine-tuning from
$10^{120}$ to $\sim 10^5$ as sensitivity to $\Delta N_{\rm tot}\approx 4$
$e$-folds, without solving the cosmological constant problem.
...
An independent ViT-Small classifier applied to $8{,}474{,}531$ DESI Legacy DR8
galaxies confirms the null: $\fcw^{\rm eq} = 0.5012 \pm 0.0006$ (all-sky dipole
at $0.43\sigma$). Galaxy spin asymmetry is not a prediction of the theory.
...
An independent ViT-Small chirality classifier on $8{,}474{,}531$ galaxies---the
largest bias-audited catalog to date---finds $\fcw^{\rm eq} = 0.5012 \pm 0.0006$
(benchmark-overlap subset), consistent with null all-sky dipole ($0.43\sigma$).
Shamir's claimed $3\%$ asymmetry is refuted (maximum regional asymmetry $0.32\%$,
${\sim}9\times$ smaller). The minimal ECH framework underpredicts $A_0$ by
$>100$ orders of magnitude, consistent with the observed null.

\textit{MCMC verification and cosmological fits.}---The $\Lambda$CDM$+\Delta\Neff$
proxy MCMC (424{,}781 samples, Cobaya~v3.6.1, two frozen dataset combinations)
finds $\Delta\Neff\approx 0$ and $H_0 = 67.68\pm 1.06\,\text{km\,s}^{-1}\,
\text{Mpc}^{-1}$, recovering standard $\Lambda$CDM. Full details, MCMC
diagnostics, and the AIC/BIC model comparison are in companion
Paper~I(b)~\cite{Golden2026P1b}.
...
The one-loop motivation for the parity-odd coefficient, the vacuum energy from
fermion condensates, and the derivation of cosmological parameters from $\Leff$
are documented in a companion technical note~\cite{Golden2026supplement}. In
summary: $\alpha/M$ is treated as a phenomenological parameter
(Sec.~\ref{sec:parityodd}), the condensate mechanism yields a vacuum energy
many orders of magnitude too large (not a viable DE source), and MCMC-fit
parameters are in companion Paper~I(b)~\cite{Golden2026P1b}.
...
(ViT-Small, $93.7\%$ three-class validation accuracy, $8/8$ bias tests passed)
with test-time equivariant averaging to $8{,}474{,}531$ DESI Legacy Imaging
Survey galaxies. The equivariant CW fraction is $\fcw^{\rm eq} = 0.5012 \pm 0.0006$
(all-sky dipole at $0.43\sigma$). The catalog is $4.3\times$ larger than
CE-ResNet and $6.5\times$ larger than Shamir's largest catalog, and agrees
with CE-ResNet's equivariant result to within $0.01$ percentage points.
This confirms the null result of Sec.~\ref{sec:spinobs}.
...
early 2030s) will confirm or exclude the ALP birefringence at $\sim 9\sigma$;
...
\hline
Matter bounce (ECH-compatible) & $\checkmark$ & (spectator) & $\checkmark$ & $\times$ \\
Slow-roll inflation & $\times$ ($\fnl\approx 0.015$) & (spectator) & $\times$ & $\times$ \\
Quintom-B & $\times$ & (spectator) & --- & $\checkmark$ \\
...
not a resolution of the cosmological constant problem. A 100{,}000-sample Monte
Carlo sensitivity scan (Sec.~\ref{sec:conclusions}) confirms: 2.2\% of the
physically motivated parameter space produces a viable vacuum energy, with
$N_{\rm tot}$ identified as the single controlling parameter
(Spearman $|\rho_s|=0.996$; all other parameters $|\rho_s|<0.08$).
...
phase of any ECH-compatible bounce produces a universal non-Gaussianity
...
\textbf{(2) Spectator-ALP birefringence $\beta\approx 0.27^\circ$.}---An ALP
with $f_a\sim\MPl$, $m\sim H_0$ is motivated by the ECH parity structure but
arises identically in standard GR with the same parameters. It is consistent
with the published $3.6\sigma$ signal without fine-tuning. LiteBIRD
($\sigma(\beta)\approx 0.03^\circ$, early 2030s) will either confirm this at
$\sim 9\sigma$ or cleanly exclude the ALP explanation.
...
\begin{itemize}[leftmargin=*]
\item \textit{Galaxy spin}: Null confirmed ($0.43\sigma$ all-sky dipole), consistent
with the $>100$-orders-of-magnitude underprediction by the ECH coupling.
...
The galaxy spin channel is a confirmed null ($\fcw^{\rm eq}=0.5012\pm 0.0006$,
all-sky dipole at $0.43\sigma$), \textit{consistent} with the framework (which
underpredicts $A_0$ by $>100$ orders of magnitude). The parity-violation case
...
17+ branches established 14 independent structural constraints on the minimal ECH
