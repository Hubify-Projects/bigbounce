...
\newcommand{\paperVersion}{v1A.0.56}
...
\caption{\label{fig:theory_map}
  ...
  phenomenologically free pathways within those channels. The burned-in PTA
  annotation $\gamma = 3.20 \pm 0.42$ is the superseded pre-real-KDE draft
  value; the current real-KDE reanalysis gives $\gamma = 2.567 \pm 0.382$
  (Sec.~\ref{sec:discrimination}), and the figure annotation is pending
  regeneration.}
\end{figure*}
...
the bounce but is \emph{not} derived from the ECH action. The panel's
burned-in dilution waypoint is an \emph{illustrative} $N\approx 55$,
$e^{-3N}\sim 10^{-72}$ schematic; the quantitative bookkeeping of
Sec.~\ref{sec:gdp} and Appendix~\ref{app:dimensions} uses
...
redshifts. (The panel's burned-in annotation quotes the looser
decade-rounded ceiling $|\omega^2/H^2| < 10^{-20}$ for the same
dimensionless ratio; the body's
$(\omega/H)_0^2 < 2.5\times 10^{-21}$ is the sharper statement, and
the figure label simply rounds up to the nearest decade.) The dark-energy mechanism in this paper is therefore the
...
\begin{equation}
\beta = \frac{\alpha}{2M}\,\Delta\theta_{\rm rec\to today}
...
excursion estimate $\Delta\theta_{\rm rec\to today} \sim
...
parameterization of this work ($10^{5}$). The burned-in ``115 orders of
magnitude improvement'' annotation refers to this \emph{score}
difference under the $N_{\rm tot}$ reparameterization; per
Sec.~\ref{sec:gdp} this is a reparameterization of the
cosmological-constant problem as sensitivity to $N_{\rm tot}$,
\emph{not} a resolution, and the residual $10^{5}$ inherits the
order-of-magnitude status discussed there. All four enumerated routes
...
\end{center}
The repository includes Cobaya YAML configurations,
galaxy spin pipeline code, and an implementation map.
...
- \tfrac{1}{4}\,\frac{\alpha}{M}\,\theta\,F_{\mu\nu}\tilde F^{\mu\nu},
\label{eq:mcs_lagrangian}
\end{equation}
with $\tilde F^{\mu\nu} = \tfrac{1}{2}\epsilon^{\mu\nu\rho\sigma}
F_{\rho\sigma}$ and $\theta$ a homogeneous pseudoscalar
(spatial gradients negligible for the cosmological background field
considered here)~\cite{CarrollFieldJackiw1990,HarariSikivie1992}.
...
\begin{equation}
A_\pm'' + \Bigl[k^2 \mp \frac{\alpha}{M}\,\theta'\,k\Bigr] A_\pm = 0,
\label{eq:helicity_dispersion}
\end{equation}
where $' \equiv d/d\eta$ and $k$ is comoving. For
$k \gg (\alpha/M)\theta'$ (satisfied by ${\sim}60$ orders of magnitude
for CMB photons against the cosmological field considered here:
$(\alpha/M)\theta' \sim 10^{-30}\,\text{eV}^{-1}\times
\mathcal{O}(1)\times H_0 \sim 10^{-63}\,$eV for an $\mathcal{O}(1)$
total excursion accumulated over a Hubble time, versus a CMB photon
$k \sim 6\times 10^{-4}\,$eV at 150~GHz, a ratio of
${\sim}10^{59}$) the WKB
phase of each helicity is
$\int\!\omega_\pm\,d\eta$ with
$\omega_\pm \simeq k \mp \tfrac{1}{2}(\alpha/M)\,\theta'$.
...
\theta'\,d\eta \nonumber\\
&= \frac{\alpha}{2M}\,\bigl[\theta(\eta_{\rm obs})
- \theta(\eta_{\rm em})\bigr]
\;\equiv\; \frac{\alpha}{2M}\,\Delta\theta .
\label{eq:beta_derived}
\end{align}
The result is achromatic (no $k$ dependence), independent of the
expansion history except through the endpoint values of $\theta$
(a total derivative along the line of sight --- the well-known
