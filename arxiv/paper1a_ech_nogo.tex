...
\newcommand{\paperVersion}{v1A.0.103}
\newcommand{\paperTimestamp}{July 3, 2026}
% v1A.0.102->103: R2 suppression grounded in verified Shapiro-Teixeira one-loop analysis (arXiv:1402.4854) — one-loop 1/(16π²) × Planck κ² × O(1) Immirzi factor; exact β(γ) still adopted, closure insensitive (≳60 orders margin).
...
$(\mu_{\rm UV}/M_{\rm Pl})^2$. Integrating Eq.~\eqref{eq:bs_beta} from a
GUT scale $\mu_{\rm UV}\sim 10^{16}\,$GeV (with $\gamma$ near the LQG value
$\gamma\!\approx\!0.24$) gives $|\Delta\gamma/\gamma|\sim 10^{-6}$ ---
Planck-suppressed to roughly \emph{five orders of magnitude below} the
chiral-count ansatz Eq.~\eqref{eq:gamma_running}. The precise coefficient
is scheme-dependent (BS work off-shell) and $\gamma^2=1$ lies outside
perturbative control, but the $(\mu/M_{\rm Pl})^2$ suppression is a robust
structural consequence of the gravitational coupling $\kappa^2$. The
derived physical running therefore only \emph{strengthens} Route~3; we
nonetheless retain the far larger chiral-count estimate
Eq.~\eqref{eq:gamma_running}, $\Delta\gamma/\gamma\!\sim\!0.3$, as a
deliberately pessimistic \emph{upper} bound in the budget below. Because the Route-3
