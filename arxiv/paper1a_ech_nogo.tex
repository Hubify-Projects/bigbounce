...
\newcommand{\paperVersion}{v1A.0.31}
...
inflationary subhorizon (bounce-era \emph{physical} scales $k_{\rm bounce}^{\rm phys}\sim k_{\rm SPHEREx}^{\rm phys}\,e^{N_{\rm tot}-N_{\rm exit}}\sim e^{32}\,k_{\rm SPHEREx}^{\rm phys}$ at $N_{\rm tot}\sim 92$, $N_{\rm exit}\sim 60$; comoving wavenumbers $k$ are constant by definition; the absolute scaling is $k\,e^{N_{\rm tot}}\sim
e^{30}\times k_{\rm SPHEREx}$) and the surviving bispectrum signal
