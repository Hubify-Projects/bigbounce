...
\newcommand{\paperVersion}{v1A.0.28}
...
\par
\textbf{Crucially, no quantitative claim in the main text relies on this
dimensional ansatz.} The 13 logically-independent structural barriers
(Sec.~\ref{sec:barriers}) close the bounce-to-dark-energy route on
amplitude-budget and operator-counting grounds that are independent of the
precise power of $\MPl$ in Eq.~(\ref{eq:onshell_rho}); the closure depends
on the overall scale separation between $\rho_\Lambda^{\rm bounce}$ and the
observed $\rho_\Lambda$, which is $\sim 120$ orders of magnitude under
\emph{any} non-cancellation assumption. Whether the bounce-scale density
is $\MPl^4$, $10^{-2}\,\MPl^4$, or $10^{-4}\,\MPl^4$ does not change the
sign or significance of the channel-amplitude closures in Routes 1--4.
\par
