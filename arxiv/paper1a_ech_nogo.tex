...
\newcommand{\paperVersion}{v1A.0.15}
\newcommand{\paperTimestamp}{2026-05-09 21:00 PDT}
...
to $10^{-60}$ (the factor-of-$\sim$100 ambiguity reflects $\varepsilon$-correction
scaling and the eV-vs-GeV convention used to convert $\alpha/M$ between
natural-unit systems; both orderings give the same physical conclusion),
i.e.\ the one-loop induced $\beta$ is suppressed by $\sim 58$--$60$ orders
...
in both directions of the range (because $m_\theta=H_0\approx 1.5\times 10^{-33}$\,eV is the only point where $\rho_\theta=\rho_\Lambda$, and the natural ALP range lies entirely \emph{above} that point): at $m_\theta \sim 10^{-22}$\,eV the
