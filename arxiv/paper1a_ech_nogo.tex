...
\newcommand{\paperVersion}{v1A.0.108}
...
(\S\ref{sec:rotation}), covered by the completeness argument rather than
deferred. What remains for follow-up is only the fully explicit
Fierz-by-Fierz projection lemma, not the closure itself. The dark-energy
...
meant to explain (Appendix~\ref{app:dimensions}). The single residual open item
is the fully explicit Fierz-by-Fierz projection lemma of \S\ref{sec:rotation};
the power-counting-class completeness itself is established here.
...
monotonicity leave no power-counting class unbounded. What remains genuinely
open --- and is the scoped follow-up the abstract already promises --- is only
the fully explicit \emph{Fierz-by-Fierz} form of the projection lemma over the
dimension-6 parity-odd basis (all four-fermion structures $VV,AA,VA,$ tensor,
together with the single gravitational-Chern--Simons invariant), showing every
individual projection inherits the bound term by term; the power-counting-class
statement is established. Genuine operator-level escape therefore requires a
