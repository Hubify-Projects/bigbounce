...
\newcommand{\paperVersion}{v1A.0.57}
...
\caption{\label{fig:theory_map}
  ...
  $\gamma = 2.567 \pm 0.382$ (Sec.~\ref{sec:discrimination}).}
\end{figure*}
...
the on-shell substitution $T^{abc}=\kappa S^{abc}$ gives
$\tfrac{1}{4}T^{abc}T_{abc} = \tfrac{\kappa^2}{4}S^{abc}S_{abc}$,
and the standard Hehl--Datta bookkeeping (the gravitational contact
piece plus the fermionic spin--torsion coupling; the same sign
manipulation quoted at Eq.~\eqref{eq:NJL_torsion}) nets to
$\mathcal{L}_{\rm int} = +\tfrac{\kappa}{2}\,S_{abc}S^{abc}$, so
integrating out torsion yields
$\mathcal{L}_{\rm int} = -\tfrac{3\kappa}{16}\,J^5_\mu J^{5\mu}$ ---
precisely the $\gamma\!\to\!\infty$ (pure Einstein--Cartan) limit of
Eq.~\eqref{eq:4fermi} and the Hehl--Datta coefficient of
Eq.~\eqref{eq:NJL_torsion}, closing the chain with no residual factor.
...
\caption{\textbf{Cosmic rotation $\omega$ vs Hubble expansion $H$.}
Distance-impact of a residual cosmic rotation component on the late-time
Hubble flow. The CMB isotropy bound $(\omega/H)_0 < 5\times 10^{-11}$
from Saadeh~\etal~\cite{Saadeh2016} pins the rotation contribution to
$\Lambda_{\rm eff} = c_\omega\omega^2$ (Eq.~\ref{eq:Leff_full}) at the
$\lesssim\!10^{-21}\,\rho_\Lambda^{\rm obs}$ level
($(\omega/H)_0^2 < 2.5\times 10^{-21}$; dividing by
$3\Omega_\Lambda \approx 2.1$ gives $\sim\!1.2\times 10^{-21}$ of
$\rho_\Lambda^{\rm obs}$) --- negligible at all
redshifts. The dark-energy mechanism in this paper is therefore the
...
three SM channels: (i) electroweak-sphaleron $B\!+\!L$-violation, which
becomes the dominant chirality-flipping process below the EW phase
transition ($\Gamma_{\rm sph}\sim\alpha_W^5 T$ in the symmetric phase,
Kuzmin--Rubakov--Shaposhnikov~\cite{Kuzmin1985}, giving
$\Gamma_{\rm sph}/H\sim\alpha_W^5\,M_{\rm Pl}/T\gg 1$ for
$T\!<\!10^{12}\,$GeV); (ii) SM Yukawa chirality-flipping rates
$\Gamma_y\sim y_f^2 T$ with the top Yukawa $y_t\sim 1$ giving
$\Gamma_t/H\sim y_t^2\,M_{\rm Pl}/T\gg 1$ at $T\!\sim\!T_{\rm reh}$;
(iii) neutrino-oscillation chirality randomization, which is
sub-dominant to (i)/(ii) above the EW scale but completes the erasure
at lower temperatures. A full Boltzmann calculation of
...
via the perturbation-transparency theorem (B14, Sec.~\ref{sec:transparency});
...
with the standard published derivation, constituting the no-go audit for each route.
...
function of $\gamma$ (written with its explicit argument throughout to
...
mechanism-independent ECH falsification paths.} Top: LiteBIRD CMB
...
$\fnl=-35/8$ prediction at $3$--$5\sigma$ realistic significance
(footnote~\ref{fn:spherex_range}). Both surveys deliver
parameter-independent, mechanism-independent discrimination tests; the
joint outcome falsifies the surviving ECH framework or leaves it as the
unique survivor.\label{fig:obs_timeline}}
\end{figure}
...
\item \textbf{Holst term vanishes by Bianchi.} The Holst term evaluated with
the Levi-Civita connection gives $\frac{1}{2}\epsilon^{\mu\nu\rho\sigma}
R_{\mu\nu\rho\sigma}(\mathring{\Gamma})$, which on a torsion-free connection
\emph{vanishes identically} by the first (algebraic) Bianchi identity
...
\caption{\textbf{Detection forecast for the two surviving mechanism-independent
tests.} Top: matter-bounce $f_{\rm NL}\!=\!-35/8$ in the SPHEREx multi-tracer
$f_{\rm NL}$ Fisher landscape (companion Paper~II, $3$--$5\sigma$ projection).
...
points are shown for reference. Both forecasts are decisive ($\gtrsim 5\sigma$
on Stage~III/IV survey timescales); neither is uniquely an ECH prediction
...
``erased by $N_{\rm tot}\!\gtrsim\!60$'' statement is the standard
...
\end{center}
The bundle is labelled \texttt{v1A.0.56-bundle} (resynced 2026-06-10) to
track the present manuscript title and version; MCMC chains, NaMaster
pipeline, and ALP parameter fitting are documented in companion
...
$\beta = (\alpha/2M)\,\Delta\theta$ used in
...
$k \gg (\alpha/M)\phi'$ (satisfied by ${\sim}60$ orders of magnitude
for CMB photons against the cosmological field considered here:
$(\alpha/M)\phi' \sim 10^{-30}\,\text{eV}^{-1}\times
[\mathcal{O}(1) f_a]\times H_0 \sim 10^{-63}\,$eV for an $\mathcal{O}(1)$
...
$m_\theta\sim H_0$ that sets $\dot\theta\sim H_0$, versus a CMB photon
