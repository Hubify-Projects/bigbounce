...
\newcommand{\artifact}[1]{\href{https://github.com/Hubify-Projects/bigbounce/blob/7befce143848b925998a3e6ecc850aa510ab3a94/#1}{repository artifact}}
\newcommand{\artifactnamed}[2]{\href{https://github.com/Hubify-Projects/bigbounce/blob/7befce143848b925998a3e6ecc850aa510ab3a94/#1}{\nolinkurl{#2}}}
\newcommand{\artifactpinned}[2]{\href{https://github.com/Hubify-Projects/bigbounce/blob/7befce143848b925998a3e6ecc850aa510ab3a94/#1}{\nolinkurl{#2}}}
% v1A.0.125->126 (2026-07-22): CONFIRMATION-BOARD CLOSURE (truth audit
%   ROUND_2026-07-22-P1A-v1A.0.125-EXACTPDF-88760604-CLAUDESTACK-CONFIRM).
%   Closed the 3 genuinely-new-real items. (M1) The v1A.0.125 DOI insertion
%   had landed inside the commented-out sec:theory block (tex 1375-1716) and
%   never compiled; moved the archival-deposit DOI sentence into the ACTIVE
%   Data and Code Availability section (after the frozen-commit sentence) and
%   deleted the orphaned commented-out copy, so the DOI now appears exactly
%   once in compiled output.  (m1) Fierz-convention attribution at
%   app:fierz named only "Nieves and Pal" for \cite{ItzyksonZuber,
%   NievesPal2004}; corrected to name both source pairs (Itzykson--Zuber and
%   Nieves--Pal).  (m2) The NJL contact operator is displayed twice
%   (eq:minimal_contact and eq:NJL_torsion) with no note that they are the
%   same operator; added a one-clause cross-reference at the second display.
%   No number, coefficient, or figure changed.
% v1A.0.125 (2026-07-21 19:58 PT): embeds the published archival deposit DOI
%   10.5281/zenodo.21481838 (Houston explicit publish-go; CC-BY-4.0 per D2)
%   in the reproducibility-availability paragraph. No number changed.
% v1A.0.127 (2026-07-24 18:35 PDT): CQG SUBMISSION-GATE CLOSURE. No science,
%   number, coefficient, abstract, or figure change.
%   (1) ACKNOWLEDGMENTS + MANDATORY DECLARATIONS NOW COMPILE. The compiled Note
%       previously contained NO acknowledgements, NO competing-interests
%       statement, NO funding statement, and NO AI-usage statement, because the
%       whole acknowledgments block sat inside the \begin{comment} region that
%       also holds the cut cosmology material. IOP/CQG make COI and funding
%       declarations unconditionally mandatory and generative-AI disclosure
%       mandatory where AI was used, all three in the Acknowledgements section
%       (https://publishingsupport.iopscience.iop.org/ethical-policy-journals/).
%       A NEW, NOTE-SCOPED block was written and placed before Data and Code
%       Availability; the old full-paper block is LEFT COMMENTED because it
%       credits Planck/CMB-S4/LiteBIRD/LSST/DESI, Lior Shamir's galaxy-spin
%       counts, and frozen MCMC chains that this 8-page Note does not use.
%       The rest of the commented region (sec:discrimination, the loophole and
%       surviving-tests sections, the parameter/dimension appendices) stays
%       commented -- restoring it would blow the Note scope.
%   (2) Data and Code Availability promoted \subsection* -> \section* so it
%       reads as its own dedicated section after the acknowledgements, per IOP's
%       stated placement advice.
%   (3) ORCID 0009-0008-5616-5994 added to the author block (\altaffiliation
%       footnote, matching how P1B carries it). Verified public 2026-07-24:
%       pub.orcid.org/v3.0/0009-0008-5616-5994 -> HTTP 200, "Houston Golden".
%   ABSTRACT UNTOUCHED. Verified count against the IOP 300-word cap: 269 words
%   counting each inline math expression as one word (manuscript-native), 285
%   words for the ASCII-ified paste block in the CQG kit, 295 by naive
%   whitespace tokenization of pdftotext output where rendered math fragments
%   split. Under the cap on every counting convention; no cut required.
\newcommand{\paperVersion}{v1A.0.127}
\newcommand{\paperTimestamp}{July 24, 2026, 18:35 PDT}
% v1A.0.123->124 (2026-07-16): CQG NOTE EDITORIAL-CLARITY CLOSURE (truth audit
%   ROUND_2026-07-16-P1A-v1A.0.123-EXACTPDF-4c450a67-CLAUDESTACK).  Closed the 3
%   genuinely-new editorial items; NO science/number/coefficient/readiness change.
%   (m4) Torsion-vanishing lemma (sec:theory): made the 4D contraction coefficients
%   explicit -- shown, not asserted -- from iota_{E_I}e^I=delta^I_I=4 and the
%   p-form number identity e^I ^ iota_{E_I} alpha_(p) = p alpha_(p) (so the first
%   contraction gives T^J+e^J^t=0 and the second gives iota_{E_J}(e^J^t)=(4-1)t=3t
%   => iota_{E_J}T^J=-3t => 4t=0 => t=0); coefficients verified by hand from the
%   manuscript's own e^{[I}^T^{J]}=0, no fabrication.  (M2) sec:r1_njl subsection
%   header "Standard mean-field NJL check" -> "...diagnostic"; softened "has no
%   nonzero solution" -> "admits no nonzero scalar gap solution in this single
%   mean-field convention" and "closes the scalar condensate ..." -> "As a
%   mean-field diagnostic, it finds no nonzero scalar gap solution ...; we make no
%   claim beyond this convention" to match App B's existing diagnostic/no-claim
%   framing.  Every number unchanged.  (M1) Added one consolidated "Relation to
%   prior work" sentence at the intro's end collecting the already-inline
%   attributions (Freidel-Minic-Takeuchi Eqs.6/17/23; Hehl-Datta EC contact; first
%   Bianchi identity) -- consolidation only, no new claim.  0 undef-refs.
% v1A.0.122->123 (2026-07-14): CQG NOTE REPRODUCIBILITY CLOSURE.
%   Aligned the active NJL script/JSON with the paper's M_Pl-only three-row
%   cutoff scope and pinned every reader-facing reproducibility link to the
%   immutable artifact commit containing that correction.  The legacy six-row
%   diagnostic remains preserved in Git/review history.  No derivation,
%   coefficient, regulator result, numerical claim, or readiness value changed.
% v1A.0.121->122 (2026-07-14): CQG NOTE BOUNDED CLARITY CLOSURE.
%   Recast the density illustration as a dimensional coefficient benchmark;
%   separated coefficient-one, contact, Holst, and state-dependent factors;
%   made the Freidel--Minic--Takeuchi source/coefficient bridge explicit;
%   defined standard matched boundary data; cross-referenced the Fierz
%   ordering; repeated the cutoff-ceiling and R_A scope; narrowed the
%   Euclidean-running barrier; expanded TB/EB; removed PACS; and pinned the
%   reproducibility scripts to the immutable v1A.0.121 review commit.  No new
%   regulator result, numerical value, or external verdict is claimed.
% v1A.0.120->121 (2026-07-14): CQG NOTE SURGICAL CLOSURE.
%   Unified the paper around the spin-sourced and zero-spin branches of the
%   algebraic Cartan equation; added the explicit sourced connection equation,
%   invertible-tetrad kernel proof, and the Freidel--Minic--Takeuchi contorsion
%   and back-substitution normalization; removed above-Planck NJL rows and
%   companion-pipeline prose; parameterized and rounded the density example;
%   localized classical/initial/boundary/global/quantum conditions; and added
%   the one-line Fierz matrix-to-G_s bridge. No new external verdict is claimed.
% v1A.0.119->120 (2026-07-14): EXACT-PDF BOUNDED CORRECTION.
%   Recast kappa*n_psi^2 everywhere as a coefficient-one dimensional
%   homogeneous benchmark, not a bound on the renormalized composite
%   <J5 J5>; centralized the metric/epsilon/gamma5/torsion/operator-ordering
%   conventions and connected the Appendix-A exchange row to the displayed
%   G_s(bar psi psi)^2 convention; displayed the Holst Cartan bivector
%   inverse for real finite nonzero gamma and excluded the complex self-dual
%   singular cases gamma=+/-i; labeled the tensor wave equation as an
%   illustrative linear-FRW specialization; and removed the non-load-bearing
%   total-derivative item from the pointwise Bianchi proof.  Gemini's novelty/
%   venue REJECT remains unresolved and is not represented as a correctness
%   closure.
% v1A.0.118->119 (2026-07-14): FIERZ OPERATOR-CONVENTION CORRECTION.
%   Separated the Nieves--Pal c-number spinor matrix from the anticommuting
%   field-operator exchange map; repaired the row/column bug in Eq. (A2) and
%   fierz_lemma_check.py; corrected G_scalar from -3kappa/64 to -3kappa/16 and
%   all six scalar ratios by x4; retained only |G_A|=3kappa/32 as an explicitly
%   convention-bounded magnitude benchmark; removed the invalid eigenspace
%   argument; repaired the missing convention pointer and the gamma=0.274
%   cutoff-only wording.  The repulsive-sign/no-real-gap conclusion survives.
% v1A.0.117->118 (2026-07-14): CLAIM-BOUNDARY SURGERY. Removed the last
%   reader-facing suggestion that <J5>=0 establishes an absent coherent field,
%   vacuum stress, or equation of state. Narrowed the contact result everywhere
%   to (i) the explicit n=100 cm^-3 finite-density coefficient-scale benchmark
%   and (ii) the conditional direct-channel hard-4D-cutoff repulsive-scalar sign
%   result. Updated the conclusion and appendix-table caption accordingly.
% v1A.0.116->117 (2026-07-14): EXACT-CONVENTION CLOSURE. Enforced the
%   unreduced-Planck convention kappa=8pi/M_Pl^2; rederived the declared
%   G_s(psibar psi)^2 hard-4D-cutoff gap equation and
%   G_crit=2pi^2/(N_f N_c Lambda^2); recomputed all six scalar and axial
%   coefficient/threshold ratios; disclosed the formal magnitude-supercritical
%   corner; retained only the separately derived repulsive-scalar sign result;
%   recomputed the late-density bound; and removed the unsupported inference
%   from <J5>=0 to w=-1.
% v1A.0.115->116 (2026-07-14): M44 SPLIT-REVIVAL / CLAIM-SURGERY.
%   Restored P1A and P1B as separate submissions without modifying the historical
%   unified source. P1A is now a narrow theory paper: exact first-order minimal ECH
%   action and algebraic torsion elimination; the induced axial four-fermion channel
%   with a standard mean-field NJL check; and the classical canonical-scalar
%   perturbation-transparency identity. Reader-visible R2/R3 dark-energy mappings,
%   single-scale dark-energy ansatz, spectator-ALP/NaMaster/MCMC payload, barrier
%   catalog, and unrestricted/basis-complete no-go rhetoric are retired from this
%   submission. R2/R3 remain literature context only: their mapping to a cosmological
%   stress tensor or observable is unresolved. No new observational or EFT-amplitude
%   result is claimed. The retained Fierz and NJL statements are explicitly limited
%   to the declared direct-channel, standard mean-field setup.
...
%   zero escape classes, all coefficients dimensionless rationals => kappa=8pi/M_Pl^2
...
%   (superseded by v117 exact convention: kappa*n^2=9.9542e-80 eV^4,
%   3.5571e-69 rho_Lambda, 68.45 orders below),
...
\title{Algebraic Cartan Elimination in Minimal Einstein--Cartan--Holst Gravity:\\
...
\begin{abstract}
We consolidate two standard consequences of the same algebraic Cartan
equation in minimal Einstein--Cartan--Holst (ECH) gravity.  On the
spin-sourced branch, eliminating the non-propagating connection gives the
minimal axial--axial contact interaction
$-(3\kappa/16)[\gamma^2/(1+\gamma^2)]J_5^2$.  A deliberately elevated
homogeneous normalization illustrates only its scale:
$\kappa n_\psi^2/\rho_\Lambda\simeq
3.6\times10^{-69}(n_\psi/100\,\mathrm{cm}^{-3})^2$.
This coefficient-one dimensional benchmark omits the actual contact factor
$3/16$ and the finite-Holst factor $\gamma^2/(1+\gamma^2)$; number density
also does not fix the state-dependent renormalized composite
$\langle J_5^I J_{5I}\rangle$, a vacuum stress tensor, or an equation of
state.  In the declared direct-channel, hard-four-momentum-cutoff, standard
mean-field NJL convention, the scalar Fierz projection is repulsive,
$G_s=-3\kappa/16$, so its real homogeneous scalar gap equation has no nonzero
solution.  This conditional sign result does not exclude other truncations,
species structures, non-minimal couplings, or propagating torsion.

On the zero-spin branch, canonical scalar matter has no Lorentz-connection
source; for an invertible tetrad and real nonsingular constant $\gamma$, the
same algebraic equation gives $C=T=0$.  After solving that equation, the local
...
standard falloff, so the first-order variational surface contribution
vanishes.  Equality of right- and left-helicity solutions additionally
requires matched parity-symmetric initial data.  This on-connection-shell
statement is not an off-shell equality
of the original first-order actions and excludes quantum/anomaly,
non-minimal, propagating-torsion, and nontrivial global/topological sectors.
...
documented in companion papers~\cite{Golden2026P2,Golden2026P1b} whose
reproducibility artifacts are archived with this submission.
\end{abstract}
\end{comment}

\keywords{Einstein--Cartan gravity, Holst action, algebraic torsion, four-fermion interaction, NJL model, perturbation transparency}
...
In Einstein--Cartan gravity the Lorentz connection is independent of the
tetrad, and fermionic spin sources torsion.  For the minimal action the
connection equation is algebraic: torsion does not propagate and may be
eliminated exactly, leaving a local four-fermion interaction
~\cite{Hehl1976,HehlDattaNJL1971,Freidel2005}.  The Holst extension introduces
the dimensionless Barbero--Immirzi parameter but no new propagating degree of
freedom when that parameter is constant.  These elementary facts permit two
useful, auditable questions: how large can the induced contact channel be at
late times, and can the constant-Immirzi Holst term affect classical
perturbations sourced only by canonical scalars?

This Note answers those two questions within an intentionally narrow domain.
The underlying Cartan elimination, axial contact interaction, and torsion-free
Bianchi identity are standard.  Our contribution is to place their
normalizations, operator ordering, logical branches, and observable scope in
one convention-audited derivation.  In relation to prior work, the
finite-$\gamma$ bivector inverse and axial contact coefficient follow Freidel,
Minic, and Takeuchi (their Eqs.~6, 17, 23~\cite{Freidel2005}), the
Einstein--Cartan axial--axial contact is the Hehl--Datta
result~\cite{Hehl1976,HehlDattaNJL1971}, and the vanishing of the Holst density
on the torsion-free scalar branch is the first (algebraic) Bianchi identity;
the element assembled here is their convention-audited consolidation into the
two Cartan branches plus the bounded dimensional benchmark, with no new
dark-energy or birefringence prediction.  Section~\ref{sec:theory} states the
first-order action and the sourced connection equation.
Section~\ref{sec:r1_njl} gives a finite-density scale illustration and a
conditional standard mean-field NJL check.  Appendix~\ref{app:fierz} records
the Fierz convention and Appendix~\ref{app:njl_gap} the regulated gap equation.
Section~\ref{sec:transparency} gives the local classical zero-spin-branch
argument.

The claim boundary is as important as the calculation.  A Fierz identity for
the contact interaction does not enumerate the gravitational EFT.  A
mean-field NJL calculation does not eliminate Fierz ambiguity or
beyond-mean-field strong-coupling completions.  Likewise, vanishing of the
Holst density on the torsion-free scalar branch is not a theorem about
fermionic, quantum, non-minimal, dynamical-Immirzi, or propagating-torsion
sectors.  Earlier exploratory mappings from one-loop Holst/Immirzi running to
late-time dark energy are not retained as results: the cited calculations do
not derive the required cosmological stress tensor and observable matching
~\cite{ShapiroTeixeira2014,BenedettiSpeziale2011run}.  No operator-complete or
unrestricted no-go is claimed.
...
(Paper~I(b), II, III, IV; all posted concurrently in the coordinated submission,
which fixes only the citable arXiv identifiers) are cited only to anchor
...
programme; \emph{none of these companion-imported numbers is load-bearing for
any closure, no-go, or theorem stated here.} A referee unable to access the
companion papers can therefore still audit every primary claim directly from
...
re-derived \emph{now}, independently of the companions' posting schedule.
...
Paper~I(b)~\cite{Golden2026P1b}, coordinated submission), consolidated in
...
\emph{companion; coordinated submission}); they are documented internally rather than as
...
the present paper's logic without the companion papers in hand; these
...
Table~\ref{tab:params}. The underlying computational artifacts are archived with
this submission and are refereeable now, independently of the companions'
posting schedule~\cite{BigBounceRepro}: the
$H_0$/$\Delta\Neff$/$\sigma_8$/$\Omega_m$ posteriors and the spectator-ALP fit
from the frozen Cobaya chains
(\artifactnamed{reproducibility/cosmology/chains}{cosmology/chains}), the
NaMaster $EB$ validation
(\artifactnamed{reproducibility/p1_namaster_500mc}{p1_namaster_500mc}), and the
galaxy-chirality and multi-survey anomaly catalogs
(\artifactnamed{pipelines/p2_chirality}{p2_chirality},
\artifactnamed{pipelines/p3_anomaly_engine}{p3_anomaly_engine}).
External (non-companion) measurements are cited inline at point of use, not
here.}
\begin{ruledtabular}
\begin{tabular}{llll}
...
\parbox[t]{0.36\textwidth}{\raggedright Derived parameter of the same MCMC
chain.\strut} \\[3pt]
SPHEREx $\fnl$ forecast & \parbox[t]{0.16\textwidth}{\raggedright $1.3$--$2.75\sigma$
realistic ($2.6$--$2.75\sigma$ optimistic)\strut} & Paper~II~\cite{Golden2026P2} &
...
\section{Minimal ECH and Algebraic Torsion}\label{sec:theory}

We use the first-order Einstein--Cartan--Holst action with a constant
Barbero--Immirzi parameter $\gamma$ and a minimally coupled Dirac field,
...
+ S_{\rm D}[e,\omega,\psi],
\end{align}
where $\kappa=8\pi G=8\pi/M_{\rm Pl}^2=1/\bar M_{\rm Pl}^2$,
$M_{\rm Pl}\equiv G^{-1/2}=1.22089\times10^{19}\,$GeV is the unreduced Planck
mass, $\bar M_{\rm Pl}\equiv(8\pi G)^{-1/2}$ is the reduced Planck mass,
$R^{IJ}(\omega)=d\omega^{IJ}+\omega^I{}_K\wedge\omega^{KJ}$, and $e^I$ and
$\omega^{IJ}$ are varied independently.  There
is no independent $T^2$ term in Eq.~\eqref{eq:ECH}; any torsion-square or
four-fermion expression below is an on-shell result obtained only after the
connection equation has been solved.  This ordering avoids double counting.

All sign-sensitive statements use the following single convention set.
The Lorentzian internal metric is mostly plus,
$\eta_{IJ}=\mathrm{diag}(-,+,+,+)$; $\epsilon_{0123}=+1$ (hence
$\epsilon^{0123}=-1$); $\{\gamma^I,\gamma^J\}=2\eta^{IJ}$; and
$\gamma^5=i\gamma^0\gamma^1\gamma^2\gamma^3$.  Antisymmetrization carries
unit weight.  The torsion two-form and full-weight component convention are
$T^I=de^I+\omega^I{}_J\wedge e^J
=\tfrac12T^I{}_{\mu\nu}dx^\mu\wedge dx^\nu$ and
$T^\lambda{}_{\mu\nu}=2\Gamma^\lambda{}_{[\mu\nu]}$.  Coincident bilinear
products below are renormalized composite operators with their displayed
field order.  Appendix~\ref{app:fierz} exchanges the middle Grassmann fields
to map the ordering $(12)(34)$ to $(14)(32)$; its one exchange supplies the
minus sign relative to the Nieves--Pal c-number identity.  The resulting
scalar row is inserted into the explicitly declared interaction
$+G_s(\bar\psi\psi)^2$, for which $G_s<0$ is repulsive in the gap equation of
Appendix~\ref{app:njl_gap}.

The Holst-modified Cartan operator is nonsingular on the paper's real
constant-$\gamma$ domain.  Define the internal dual on Lorentz bivectors by
$(\star X)_{IJ}=\tfrac12\epsilon_{IJ}{}^{KL}X_{KL}$, so
$\star^2=-\mathbb 1$.  Acting on an antisymmetric bivector, the tensor in
parentheses in Eq.~\eqref{eq:ECH} is twice
\begin{equation}\label{eq:holst_cartan_inverse}
\mathcal Q_\gamma=\star+\gamma^{-1}\mathbb 1,\qquad
\mathcal Q_\gamma^{-1}
=\frac{\gamma^2}{1+\gamma^2}
\left(\gamma^{-1}\mathbb 1-\star\right),
\end{equation}
as is verified by direct multiplication.  This is the same
$\gamma^2/(1+\gamma^2)$ bivector inverse displayed, in the dualized
convention, by Freidel, Minic, and Takeuchi~\cite{Freidel2005}, Eq.~(6).
It exists for every real finite nonzero $\gamma$; $\gamma=0$ is outside the
action written in Eq.~\eqref{eq:ECH}, while the complex self-dual values
$\gamma=\pm i$ make $1+\gamma^2=0$ and are explicitly excluded.  Therefore,
writing the Lorentz-connection source three-form as
$\mathcal J^{IJ}\propto\delta S_{\rm matter}/\delta\omega_{IJ}$, the
connection variation has the explicit sourced form
\begin{equation}\label{eq:cartan_source}
\frac{\delta S}{\delta\omega_{IJ}}=0:\qquad
\mathcal Q_\gamma\!\left(e^{[I}\wedge T^{J]}\right)=\mathcal J^{IJ}.
\end{equation}
The normalization of $\mathcal J^{IJ}$ is fixed by this equation and will not
be needed below.  A canonical scalar action is independent of
$\omega_{IJ}$, so $\mathcal J^{IJ}=0$ and invertibility of
$\mathcal Q_\gamma$ gives $e^{[I}\wedge T^{J]}=0$.
For completeness, let $E_I$ be the frame dual to $e^I$
($\iota_{E_J}e^I=\delta^I_J$) and define the one-form
$t\equiv\iota_{E_I}T^I$ (summed over $I$).  Contracting the last equation with
$\iota_{E_I}$ in four dimensions, using $\iota_{E_I}e^I=\delta^I_I=4$ together
with the number identity $e^I\wedge\iota_{E_I}\alpha_{(p)}=p\,\alpha_{(p)}$
(so $e^I\wedge\iota_{E_I}T^J=2T^J$), gives
$\iota_{E_I}(e^{[I}\wedge T^{J]})=\tfrac12(T^J+e^J\wedge t)=0$, i.e.\
$T^J+e^J\wedge t=0$.  A second contraction with $\iota_{E_J}$, using
$\iota_{E_J}(e^J\wedge t)=(4-1)\,t=3t$, yields
$t=\iota_{E_J}T^J=-3t$, hence $4t=0$, so $t=0$ and therefore $T^I=0$.  Thus the
kernel is trivial for an invertible tetrad.  The Einstein--Cartan limit
$\gamma\to\infty$ is nonsingular as well.

For minimal fermion coupling the spin current is totally antisymmetric and
dual to $J_5^I=\bar\psi\gamma^I\gamma^5\psi$.  The connection equation is
algebraic.  In the Einstein--Cartan limit, with the spin-current convention
$S^{IJK}=\tfrac14\epsilon^{IJKL}J_{5L}$ used here, it reads
\begin{equation}\label{eq:torsion}
T^{IJK}=\kappa S^{IJK}=\frac{\kappa}{4}\epsilon^{IJKL}J_{5L}.
\end{equation}
At finite $\gamma$, Freidel, Minic, and Takeuchi vary the connection explicitly
and obtain, for minimal coupling and their axial current $A^I=J_5^I$,
\begin{equation}\label{eq:fmt_contorsion}
e_I{}^\mu C_{\mu JK}=4\pi G\frac{\gamma^2}{1+\gamma^2}
\left(\frac12\epsilon_{IJKL}J_5^L
-\frac1\gamma\eta_{I[J}J^5_{K]}\right),
\end{equation}
their Eq.~(17)~\cite{Freidel2005}.  This source-backed solution makes the
zero-current implication immediate: $J_5^I=0$ gives $C_{\mu JK}=0$ and hence
$T^I=0$.  Their direct back-substitution, Eq.~(23), gives
$\mathcal L_{\rm int}=-(3/2)\pi G[\gamma^2/(1+\gamma^2)]J_5^2$ for minimal
coupling.  In the convention of Eqs.~\eqref{eq:cartan_source} and
\eqref{eq:fmt_contorsion}, the source-to-contact normalization bridge is
\begin{equation}\label{eq:fmt_normalization_bridge}
4\pi G=\frac{\kappa}{2},\qquad
-\frac{3}{2}\pi G=-\frac{3\kappa}{16},
\end{equation}
so the source solution and back-substituted coefficient are in the same
Freidel--Minic--Takeuchi normalization~\cite{Freidel2005}.  It gives
\begin{equation}\label{eq:4fermi}
\mathcal L_{4\psi}=-\frac{3\kappa}{16}
\frac{\gamma^2}{1+\gamma^2}\,(J_5^I J_{5I}).
\end{equation}
The pure Einstein--Cartan coefficient is recovered for
$\gamma\rightarrow\infty$.  A vector--axial term is not generated by the
minimal coupling used here; it requires a non-minimal fermion coupling and is
outside this paper's declared field content.

For the late-time dimensional benchmark and gap-equation calculation below we use the
maximal Einstein--Cartan magnitude,
\begin{equation}\label{eq:minimal_contact}
\mathcal L^{\rm NJL}_{\rm tor}=-\frac{3\kappa}{16}(J_5^I J_{5I}),
\end{equation}
because $\gamma^2/(1+\gamma^2)<1$ can only reduce the coupling.  Equations
\eqref{eq:ECH}--\eqref{eq:minimal_contact} are the complete action-level input to
the results that follow.

\begin{comment}
\section{Theoretical Framework}\label{sec:theory-old}
...
1.22089\times 10^{19}\,$GeV is the \emph{unreduced} Planck mass; the
...
hosted entirely in Paper~I(b) (companion~\cite{Golden2026P1b}); this
...
the single $\kappa=8\pi/M_{\rm Pl}^{2}$ prefactor of the same torsion-elimination
...
\section{The Minimal Four-Fermion Contact Channel}\label{sec:fourfermion}

The contact operator in Eq.~\eqref{eq:NJL_torsion} is a specific consequence
of minimal algebraic torsion.  We test its finite-density amplitude and its
standard mean-field scalar-condensate channel.  Neither test is promoted to a
claim about every four-fermion completion or about the complete gravitational
EFT.

\subsection{Finite-density benchmark and equation-of-state boundary}
\label{sec:r1_njl}
...
because the half-weight definition absorbs the factor of two relative to the
full-weight definition used in Eq.~\eqref{eq:torsion}) allows torsion to
...
(identical to the minimal contact operator of Eq.~\ref{eq:minimal_contact};
repeated here in the NJL context)
\begin{equation}
...
\emph{parity-even} in the $C\!P$-conserving Standard Model sector. For a
specified homogeneous number density we define the coefficient-one
dimensional benchmark
$\rho_{4f}^{(\mathrm{dim})}\equiv\kappa n_\psi^2
=8\pi n_\psi^2/M_{\rm Pl}^2$.
The actual maximal axial--axial coefficient in Eq.~\eqref{eq:NJL_torsion}
adds the factor $3/16$.  Since $n_\psi$ has mass dimension $+3$, both
expressions have the required energy-density dimension $+4$.
This definition is not an inequality for
$|\langle J_5^I J_{5I}\rangle|$: number density does not determine that
state-dependent coincident composite without specifying the ensemble, spin
polarization, relativistic normalization, species contractions, and a
renormalized contact prescription.
Using $\hbar c=1.9733\times10^{-5}\,\text{eV}\,\text{cm}$ and
$M_{\rm Pl}=1.2209\times10^{28}\,\text{eV}$ gives the transparent scaling
\begin{align}
...
\end{align}
The normalization $100\,\mathrm{cm}^{-3}$ is deliberately elevated for
illustration; it is neither a cosmological-density estimate nor a preferred
state.  At that normalization the coefficient-one benchmark is about $68.4$
orders below the canonical
$\rho_\Lambda\approx (2.3\,\text{meV})^4 \approx 2.8\times 10^{-11}\,\text{eV}^4$
used throughout this paper.  Including the actual $3/16$ contact coefficient
gives the coefficient-weighted benchmark
$1.9\times10^{-80}\,\text{eV}^4=6.7\times10^{-70}\rho_\Lambda$.
No coherent order parameter, vacuum expectation, stress tensor, or equation of
state is inferred from $\langle J^5\rangle\approx0$: a one-point current does
not determine the state-dependent composite $\langle J^5J^5\rangle$ or its
metric variation.  The only late-density statement made here is the explicit
finite-density coefficient-scale comparison above.
This numerical comparison alone is not a constraint on the actual composite
expectation or on arbitrary torsion and fermion sectors.  It illustrates the
fermionic spin-sourced branch and must not be conflated with the canonical
scalar, zero-spin theorem of Sec.~\ref{sec:transparency}.  The finite-$\gamma$ coefficient
$\gamma^2/(1+\gamma^2)$ in Eq.~\eqref{eq:4fermi} has magnitude below unity for
real finite $\gamma$, so it does not increase this coefficient benchmark above
the Einstein--Cartan limit used in the calculation.

\subsection{Standard mean-field NJL diagnostic}

The finite-density benchmark does not decide whether a vacuum condensate
exists.  As a logically separate conditional check, we Fierz-project the
axial--axial operator using the exchange ordering and Grassmann sign fixed in
Appendix~\ref{app:fierz}, and solve a
hard-cutoff NJL gap equation in Appendix~\ref{app:njl_gap}.  In the declared
direct-channel convention the scalar projection is
$G_{\rm scalar}=-3\kappa/16<0$, hence repulsive.  Substitution in the real
homogeneous scalar gap equation shows that this negative coupling admits no
nonzero scalar gap solution in this single mean-field convention.  The sign
conclusion is logically separate from the
coefficient-magnitude diagnostic: at the retained EFT bookkeeping ceiling
$\Lambda=M_{\rm Pl}$, the three displayed scalar ratios run from $0.239$ to
$2.15$ and therefore are not uniformly subcritical.  We do not evaluate the
contact EFT above $M_{\rm Pl}$.  The magnitude $|G_A|=3\kappa/32$ divided by
the same scalar-channel threshold is one half of the scalar ratio; it is not
an independently derived axial-condensation threshold.  The three-row scan is
reported in Appendix~\ref{app:njl_gap}.
The machine-checkable calculation is
\artifactnamed{arxiv/scripts/njl_gap_equation_route1.py}{njl_gap_equation_route1.py}.

This result is deliberately conditional.  As a mean-field diagnostic, it finds
no nonzero scalar gap solution in the standard mean-field NJL model with the
stated regulator, field content, and projection convention; we make no claim
beyond this convention.  It does not remove the known Fierz ambiguity of
mean-field truncations and does not exclude beyond-mean-field strong coupling,
additional flavor/color exchange structure, or non-minimal torsion couplings.
...
$\kappa = 8\pi G = 8\pi/M_{\rm Pl}^{2}$ prefactor, so its finite-density
coefficient is bounded by the same $\kappa n_\psi^2$ estimate as R1 ---
$3.5571\times10^{-69}\rho_\Lambda$ for the deliberately elevated
$100\,\mathrm{cm}^{-3}$ benchmark; the parity-odd coefficient
$\gamma_{\rm BI}/(\gamma_{\rm BI}^2{+}1)\le\tfrac12$ is $O(1)$ and cannot lift
the amplitude.  No equation of state is inferred from
$\langle J^5\rangle\approx0$: the stress tensor of a composite expectation
value is state-dependent and would have to be calculated explicitly.
\emph{Closure: the finite-density amplitude is Planck-suppressed; no separate
equation-of-state closure is claimed (Tier-III).}
...
Eq.~37, $\kappa^2=16\pi G=16\pi/M_{\rm Pl}^{2}$), with classical coefficient
...
$\kappa^2=16\pi/M_{\rm Pl}^{2}$ on every renormalized charge, i.e.\ the
...
coefficient, since ST's explicit $\kappa^2=16\pi/M_{\rm Pl}^{2}$ confirms the
...
the explicit $\mu^2\kappa^2=16\pi(\mu/M_{\rm Pl})^2$ prefactor: the running is
...
$\mu^2\kappa^2=16\pi(\mu/M_{\rm Pl})^2$ prefactor makes the flow power-suppressed,
...
$\kappa^2=16\pi/M_{\rm Pl}^{2}$ Planck suppression are fixed by the Shapiro
...
dense ISM-like densities $n_\psi\!\sim\!10^2\,\text{cm}^{-3}$ ($68.45$ orders
...
(companion~\cite{Golden2026P1b}); the present
...
($68.45$ orders below $\rho_\Lambda$) is a Tier-III order-of-magnitude
...
are in Paper~I(b) (companion~\cite{Golden2026P1b}).
...
framework rather than falsifying it (details in Paper~I(b), companion~\cite{Golden2026P1b}).
...
\paragraph{Running literature.}
The one-loop external-current calculation of Shapiro and Teixeira and the
truncation-dependent Immirzi running of Benedetti and Speziale are Euclidean
one-loop/RG calculations.  The unresolved bridge is a derivation of a matched
physical Lorentzian cosmological stress tensor and observable from those
results~\cite{ShapiroTeixeira2014,BenedettiSpeziale2011run,Benedetti2011}.  We draw no
dark-energy or birefringence inference from those running calculations.

\section{Classical Scalar-Sector Transparency}\label{sec:transparency}

\subsection{Statement}

The precise local statement is as follows.  Consider the classical ECH action
with an invertible tetrad, canonical scalar matter, and a real finite nonzero
constant $\gamma$, on a local patch with matched background, initial, and
boundary data.  Here standard boundary data means the usual falloff conditions
with the boundary contribution to the first-order variation set to zero.
Solve the algebraic connection equation before expanding in
perturbations.  Then the reduced action on the zero-spin branch is the
Einstein--scalar action, so scalar equations and tensor evolution operators
coincide with GR at every perturbative order for the same background and
boundary data.  Equality of right- and left-helicity \emph{solutions}, rather
than their evolution operators, additionally requires parity-symmetric initial
data.  The complex singular values $\gamma=\pm i$, nontrivial global or
topological sectors, nonstandard boundary terms, quantum loops or anomalies,
fermion sources, non-minimal matter, a dynamical Immirzi field, and
propagating torsion are outside this statement.

This is equality after solving the Cartan equation---an on-connection-shell
equality of local classical reduced actions---not an off-shell equality of the
original first-order ECH and Einstein--scalar actions.  Its all-orders content
does not come from an order-by-order calculation: the Cartan constraint is
algebraic, and the Bianchi identity used below is pointwise.  The displayed
linear tensor equation and second-order Holst expression are checks of those
identities, not their foundation.
...
\item \textbf{Zero torsion.} The full Holst-modified connection equation uses
the algebraic bivector operator $\mathcal Q_\gamma$ in
Eqs.~\eqref{eq:holst_cartan_inverse} and \eqref{eq:cartan_source}.  For real
finite nonzero $\gamma$ its inverse exists because $1+\gamma^2>0$.  The zero
scalar source therefore gives $e^{[I}\wedge T^{J]}=0$, and the explicit
dual-frame contraction below Eq.~\eqref{eq:cartan_source} proves that its
invertible-tetrad kernel is trivial: $T^I=0$.  This holds at every classical
field configuration in the stated local domain and hence before any
perturbative expansion.  The complex singular values $\gamma=\pm i$ are
excluded.
...
density, a separate invariant containing two curvature factors; see the
explicit verification below.

\end{enumerate}

No total-derivative argument is a load-bearing step in this proof.  The
operative statement is the pointwise algebraic Bianchi identity in Step~4.
The Nieh--Yan boundary-form decomposition quoted below is retained only to
distinguish related densities; at nonzero torsion it does not replace the
Cartan equation or make the full Holst density a boundary term.

\subsection{Extension to Tensor Sector}

The same four steps apply to tensor perturbations.  As an illustrative
source-free \emph{linear} specialization on an FRW background, $T=0$ gives
the standard GR tensor equation
\begin{equation}
...
has no parity-dependent modifications.  The left- and right-helicity
evolution operators are identical,
\begin{equation}
\mathcal E_R=\mathcal E_L=\partial_\eta^2+2\mathcal H\partial_\eta+k^2.
\end{equation}
Thus minimal ECH produces no helicity-dependent propagation; $v_R=v_L$ follows
only when the initial data are also parity symmetric.  It generates no GW
birefringence, tensor chirality, or temperature--$B$-mode ($TB$) and
$E$-mode--$B$-mode ($EB$) CMB cross-power spectra from
parity-symmetric initial data.
The all-order statement is not the displayed linear equation: it is equality
of the classical action and equations of motion to their GR forms on the
torsion-free branch, before any perturbative expansion.
...
connection.  The equivalent Nieh--Yan identity
$d(e_I\wedge T^I)=T_I\wedge T^I-e_I\wedge e_J\wedge R^{IJ}$ also gives zero
pointwise when $T^I=0$; it is not a separate boundary assumption.  This
one-curvature Holst contraction must not be confused with the Pontryagin
density, which contains two curvature factors and can be nonzero pointwise.
On $T=0$ the Holst contraction is identically zero by Bianchi, contributing
nothing to the local classical reduced equations of motion at any perturbation order. In
particular, the cubic action for $\zeta$ (which determines the bispectrum)
receives zero contribution from the Holst term in the stated canonical-scalar
domain. The resulting bispectrum equation is therefore the standard GR one
for matched background, initial, and boundary data.

\subsection{What Would Break the Transparency}

The statement need not survive fermionic spin sources, torsion kinetic terms,
non-minimal matter couplings, a dynamical Immirzi field, nonstandard boundary
terms, nontrivial global/topological sectors, or quantum loops and anomalies.
Those cases change either the Cartan source, the algebraic character of the
connection equation, or the local classical action and are not analyzed here.
...
bispectrum): after solving the Cartan equation, their local classical equations
equal the GR equations at every perturbative order under the conditions in
Sec.~\ref{sec:transparency}.  Observable solutions also require the same
background, initial, and boundary data.
\item \textit{Parity}: the two tensor-helicity evolution operators coincide.
Equal helicity solutions require parity-symmetric initial data; any parity
signal must enter through a sector excluded from this theorem.
\end{itemize}

\section{Discussion and Limitations}\label{sec:discussion}

The two results have different logical status.  Algebraic torsion elimination
is an action-level calculation.  The finite-density estimate then follows
from dimensional scaling of that derived contact operator, whereas the NJL
statement additionally assumes a specific mean-field reduction and regulator.
The transparency result is exact within its classical domain because both
load-bearing steps---zero scalar spin current and the algebraic Bianchi
identity---hold pointwise.  Its novelty is not a new curvature identity; its
utility is to make the dimensional benchmark and claim boundary explicit
for the constant-Immirzi, canonical-scalar branch.

Several stronger conclusions are not supported.  This paper does not
enumerate all diffeomorphism-invariant operators, establish vacuum stability
beyond the stated NJL truncation, or constrain non-minimal fermion couplings.
It does not analyze propagating torsion, a dynamical Immirzi field, quantum
loops or anomalies in the scalar sector, or boundary/topological sectors with
nontrivial global data.  Most importantly, it does not derive a late-time
vacuum energy, cosmic birefringence, or a bounce observable from ECH.  These
are residual research problems, not closures hidden by terminology.

\section{Conclusions}\label{sec:conclusions}

Minimal ECH gravity supplies a definite, Planck-suppressed axial contact
interaction after algebraic torsion is integrated out.  At the stated
$n_\psi=100\,\mathrm{cm}^{-3}$ illustrative normalization its coefficient-one
scale is $3.6\times10^{-69}\rho_\Lambda$, scaling as $n_\psi^2$; this is not an
equation-of-state or vacuum-stress calculation.  Separately, its scalar projection has no nonzero
real homogeneous solution in the declared direct-channel, hard-four-momentum-
cutoff, standard mean-field gap equation because its coupling is repulsive.
This conditional conclusion does not use a blanket magnitude-subcriticality
claim: at the retained $\Lambda=M_{\rm Pl}$ bookkeeping ceiling the
$N_fN_c=9$ coefficient magnitude is supercritical, but no above-Planck cutoff
is evaluated.  Separately, canonical
scalar matter sources no torsion, and the constant-Immirzi Holst term vanishes
on the resulting torsion-free branch by the first Bianchi identity, leaving
the local classical scalar equations and tensor evolution operators unchanged
for the matched data and scope stated in Sec.~\ref{sec:transparency}.

These are the complete claims of this paper.  The evaluated finite-density
benchmark is far below the observed dark-energy scale, but no conclusion about
a coherent mean field, vacuum stress tensor, or equation of state is drawn
from $\langle J^5\rangle\approx0$.  Independently, the classical Holst sector
leaves scalar/tensor perturbations unchanged on the stated torsion-free scalar
branch after the algebraic connection has been solved; this is not an off-shell
identity of the original first-order actions.  It does not yield a bounce
observable there.  This does not constitute
an operator-complete no-go or an ECH explanation of dark energy.  The unresolved
step in proposed running-based extensions is the derivation of a matched
Lorentzian cosmological stress tensor and observable from the underlying
one-loop/RG calculation.  Until that step exists, the honest conclusion is a
specific contact-channel benchmark and a narrow classical transparency
identity.

%% ============================================================
%%  ACKNOWLEDGMENTS + MANDATORY DECLARATIONS  (v1A.0.127)
%%  NOTE-SCOPED ON PURPOSE.  IOP/CQG make competing-interests and funding
%%  declarations unconditionally mandatory and generative-AI disclosure
%%  mandatory where AI was used, all three in the Acknowledgements section:
%%    "All authors and co-authors are required to disclose any potential
%%     conflicts of interest ... Any conflicts of interest should be included
%%     in an acknowledgements section."
%%    "IOP Publishing requires all authors to declare any funding they
%%     received related to the research article they are submitting."
%%    "If authors use generative AI tools for any of the tasks listed above,
%%     they must disclose this usage in the Acknowledgements section ...
%%     list the model and version of the generative AI tool and how it was
%%     used in the work."
%%  (https://publishingsupport.iopscience.iop.org/ethical-policy-journals/)
%%  The FULL-PAPER acknowledgments block further down this file is inside the
%%  \begin{comment} region together with the cut cosmology material.  It is
%%  deliberately LEFT COMMENTED: it thanks the Planck/CMB-S4/LiteBIRD/LSST/DESI
%%  collaborations and Lior Shamir for galaxy-spin counts, and cites frozen MCMC
%%  chains -- none of which this 8-page Note uses.  Restoring it would credit
%%  material the Note does not contain.  This block carries only what the Note
%%  actually owes and what IOP actually mandates.
%%  Placement: before Data and Code Availability, per IOP's own advice that the
%%  data statement go "in a dedicated section of the manuscript after the
%%  acknowledgements".
%% ============================================================
\begin{acknowledgments}
The identities consolidated here are standard; the author thanks the authors of
the Einstein--Cartan, Holst-sector, and Fierz-rearrangement literature cited in
this Note, whose derivations this work audits and assembles rather than
replaces, and the developers of the open-source symbolic and numerical tools
used for the committed algebraic checks.

\emph{Funding.}---No external funding was received for the research,
authorship, or publication of this Note. No grant, institutional, or commercial
support was received, and no funder had any role in the work. Computational
costs were borne by the author.

\emph{Competing interests.}---The author declares no competing interests,
financial or non-financial.

% NOTE ON SOURCE FORMATTING: the AI-usage paragraph is kept on a SINGLE long
% source line, exactly as P3/P4/P5 keep theirs. tools/directive_g.sh's leak-gate
% exempts a line only when the model names and the disclosure phrasing sit on
% the SAME line; reflowing this paragraph would trip the gate for the wrong
% reason. Do not hard-wrap it.
\emph{Generative-AI usage.}---This Note was prepared with an agentic AI research pipeline operated under the author's direction, and that assistance was substantive rather than cosmetic. Anthropic Claude (Opus~4 family, 2026 releases) was used for pipeline orchestration, generation of the algebraic check scripts named in the Data and Code Availability section, symbolic and dimensional cross-checking of the Fierz rearrangement and the mean-field gap equation, literature triage, and drafting and revision of the text; xAI Grok (Grok-4 and Grok-4.3), Google Gemini (2.5~Pro and 3.1~Pro), and OpenAI ChatGPT (GPT-5-class models, in review rounds through 2026 July) were used as cross-checking and adversarial referee-style reviewers of the compiled manuscript, each bound to the exact PDF under review. What remained with the author is the scientific content: the author set the scope and the declared conventions, made every physics judgment---including which results are stated as exact and which are labelled conditional---verified each reported coefficient against the committed check scripts, whose printed output any reader can reproduce, and adjudicated every AI-raised finding against the source rather than adopting it on the reviewer's authority. The author takes full responsibility for the contents, including all material produced with AI assistance. No AI system is an author, and none is credited as one.
\end{acknowledgments}

\section*{Data and Code Availability}

The algebraic checks used here are committed at
\artifactpinned{arxiv/scripts/fierz_lemma_check.py}{fierz_lemma_check.py} and
\artifactpinned{arxiv/scripts/njl_gap_equation_route1.py}{njl_gap_equation_route1.py}; the latter writes the
cutoff-ceiling and density record
\artifactpinned{arxiv/scripts/njl_gap_equation_route1_results.json}{njl_gap_equation_route1_results.json}.  These exact files are frozen at immutable
repository commit
\href{https://github.com/Hubify-Projects/bigbounce/tree/7befce143848b925998a3e6ecc850aa510ab3a94}{\texttt{7befce143848}}.
No companion computation is
required for either action-level result in this Note.
...
\end{tabular}
\end{ruledtabular}
$^{\dagger}$Quintom-B can in principle accommodate the DESI $w_0 w_a$ evidence; the MCMC analysis in Paper~I(b) (companion~\cite{Golden2026P1b}) was not extended to the $w_0 w_a$ parameter space, so this row is reported as ``consistent at the model level'' rather than a posterior-preference $\checkmark$.\\
...
Paper~I(b) (companion~\cite{Golden2026P1b}) Table~IV row ``DESI DR2 w0wa (new)'' and has not yet
...
($\kappa n_\psi^2 \approx 9.9542\times10^{-80}\,$eV$^4$, $68.45$
...
LiteBIRD forecast are in Paper~I(b) (companion~\cite{Golden2026P1b}).
...
\!\approx\!0.03^\circ$ window (companion~\cite{Golden2026P1b}); the WMAP+Planck
...
convergence diagnostics are in Paper~I(b) (companion~\cite{Golden2026P1b}).
\end{itemize}
...
and NaMaster pipeline validation are in Paper~I(b) (companion~\cite{Golden2026P1b}).
...
bundle (under
\artifactnamed{reproducibility/cosmology/frozen}{cosmology/frozen}); fresh re-verification
...
MCMC analysis (companion~\cite{Golden2026P1b}) and are not independently
...
\section{Fierz Rearrangement of the Minimal Axial Contact Operator}
\label{app:fierz}

On the $16$-dimensional Clifford basis, grouped into the five Lorentz classes
$(S,V,T,A,P)$, we first state the c-number spinor rearrangement in the
normalized convention of Itzykson--Zuber and Nieves--Pal~\cite{ItzyksonZuber,NievesPal2004}:
$e_I(1234)=\sum_J(F_{\rm c})_{IJ}e_J(1432)$, with rows $I$ the source classes
and columns $J$ the produced classes,
...
\end{equation}
In that normalized basis $\Gamma_A^\mu=i\gamma^\mu\gamma^5$ and
$n_A=-i$, so the two factors combine to make $e_A$ exactly the physical
$(\bar\psi\gamma^\mu\gamma^5\psi)
(\bar\psi\gamma_\mu\gamma^5\psi)$ operator; the phases are already absorbed
in $F_{\rm c}$.  Turning this c-number identity into an anticommuting
field-operator identity requires one Grassmann exchange, so
$F_{\rm op}=-F_{\rm c}$.
The axial source is the fourth
\emph{row}, not the fourth column.  In the resulting declared exchange-channel
operator ordering,
\begin{equation}\label{eq:AAdecomp}
(J^5\!\cdot J^5)\xrightarrow{\rm Fierz}
SS+\tfrac12 VV+\tfrac12 AA-PP.
\end{equation}
The scalar bridge is therefore visible in three steps:
\begin{equation}\label{eq:fierz_scalar_bridge}
\begin{aligned}
(F_{\rm c})_{AS}&=\tfrac14(-4)=-1,\\
(F_{\rm op})_{AS}&=- (F_{\rm c})_{AS}=+1,\\
G_s&=\left(-\tfrac{3\kappa}{16}\right)(+1).
\end{aligned}
\end{equation}
Here the factor $1/4$ is the normalized gamma-trace factor in
Eq.~\eqref{eq:fierzmatrix}, and the second minus sign is the single Grassmann
exchange.  Multiplying the resulting scalar coefficient $+1$ by
the contact prefactor in Eq.~\eqref{eq:minimal_contact} gives
$\mathcal L_{S}=(-3\kappa/16)(\bar\psi\psi)^2
\equiv G_s(\bar\psi\psi)^2$, hence $G_s=-3\kappa/16$ in the gap-equation
convention used below.
The coefficients are dimensionless rationals, so rearrangement preserves the
common $\kappa=8\pi/M_{\rm Pl}^{2}$ prefactor.  The matrix orientation,
Grassmann sign, scalar gamma-trace projection, and channel coefficients are
checked directly by
\artifactnamed{arxiv/scripts/fierz_lemma_check.py}{fierz_lemma_check.py}.

Equation~\eqref{eq:AAdecomp} is an operator-order-specific Dirac-algebra
identity, not an assertion that one mean-field factorization is
basis-independent.  The gap-equation test below uses only its scalar exchange
coefficient in the displayed direct-channel convention.  We report the axial
entry only through $|G_A|$ because its sign is convention-bound and no axial
gap equation is solved.  Nor does this identity enumerate derivative
operators, curvature--current operators, independent flavor/color
contractions, non-minimal torsion irreducible components, or the gravitational
EFT.

\section{Regulated Standard Mean-Field NJL Check}\label{app:njl_gap}

For the maximal Einstein--Cartan contact coefficient in
Eq.~\eqref{eq:NJL_torsion}, the scalar coefficient selected by
Eq.~\eqref{eq:AAdecomp} is
\begin{equation}\label{eq:gscalar}
G_{\rm scalar}=\left(-\frac{3}{16}\right)(+1)\kappa
=-\frac{3}{16}\kappa.
\end{equation}
Consider the standard hard-Euclidean-four-momentum-cutoff NJL model with
$N_f$ flavors, $N_c$ colors, and scalar contact term
$G_s(\bar\psi\psi)^2$.  In the chiral limit the mean-field gap equation is
\begin{align}
M={}&G_sN_fN_c\frac{M}{2\pi^2}
\left[\Lambda^2-M^2\ln\!\left(1+\frac{\Lambda^2}{M^2}\right)\right],
\end{align}
which gives the bifurcation threshold
\begin{equation}\label{eq:gcrit}
G_{\rm crit}=\frac{2\pi^2}{N_fN_c\Lambda^2}
\end{equation}
in this normalization~\cite{Klevansky1992,HatsudaKunihiro1994,Buballa2005}.
Here $M=-2G_s\langle\bar\psi\psi\rangle$ and the hard four-dimensional
Euclidean ball gives
$\langle\bar\psi\psi\rangle=-(N_fN_cM/4\pi^2)
[\Lambda^2-M^2\ln(1+\Lambda^2/M^2)]$, fixing every factor in
Eq.~\eqref{eq:gcrit}.
The scalar coefficient in Eq.~\eqref{eq:gscalar} is negative and therefore
repulsive in this convention.  Discarding that sign as a conservative
magnitude test gives
\begin{equation}\label{eq:ratio}
\frac{|G_{\rm scalar}|}{G_{\rm crit}}
=\frac{3N_fN_c}{4\pi}\,\frac{\Lambda^2}{M_{\rm Pl}^2},
\end{equation}
where the unreduced mass convention $\kappa=8\pi/M_{\rm Pl}^2$ has been used.
The coupling in this diagnostic is deliberately the maximal
Einstein--Cartan limit.  We take $N_fN_c=1,3,9$ at the bookkeeping ceiling
$\Lambda=M_{\rm Pl}$.  No cutoff above $M_{\rm Pl}$ is evaluated because it
would lie outside the controlled contact-EFT domain.

\begin{table}[tb]
\caption{Coefficient-to-threshold diagnostics, with
$R_S=|G_{\rm scalar}|/G_{\rm crit}$ and
$R_A=|G_A|/G_{\rm crit}$, where $|G_A|=3\kappa/32$ and $G_{\rm crit}$ is the
\emph{scalar} threshold.  Thus $R_A$ is a coefficient-magnitude benchmark, not
a derived axial-vector condensation threshold.  Every row uses
$\Lambda=M_{\rm Pl}$ only as a bookkeeping ceiling; no controlled UV extension
above that ceiling is implied.  This table tests neither
coherent axial order nor a cosmological stress tensor or equation of state.}
\begin{ruledtabular}
\begin{tabular}{cccc}
$N_fN_c$ & $\Lambda/M_{\rm Pl}$ & $R_S$ & $R_A$ \\
\hline
1 & $1$ & $0.239$ & $0.119$ \\
3 & $1$ & $0.716$ & $0.358$ \\
9 & $1$ & $2.15$ & $1.07$ \\
...
The scalar magnitude is therefore formally supercritical only in the
$N_fN_c=9$ cutoff-ceiling row, where the axial benchmark also exceeds unity.
The old blanket
magnitude-subcritical statement is false.  The scalar-condensate conclusion
instead follows from the separate sign result: inserting
$G_{\rm scalar}<0$ into the real homogeneous scalar gap equation admits no
nonzero solution.  The pseudoscalar coefficient magnitude equals the scalar
magnitude.

The result is reproducible with
\artifactnamed{arxiv/scripts/njl_gap_equation_route1.py}{njl_gap_equation_route1.py}.  Its interpretation is
strictly standard mean field with this regulator and channel convention.  A
different Fierz-complete truncation, exchange treatment, multi-species model,
or nonperturbative completion can change the mean-field organization, so the
calculation is not presented as a regulator- or basis-independent exclusion
of every condensate mechanism.  No alternate regulator is evaluated here, so
we make no claim about how one would change the stability condition.
...
prefactor $\kappa=8\pi G=8\pi/M_{\rm Pl}^{2}$, the axial--axial contact term
...
\artifactnamed{arxiv/scripts/fierz_lemma_check.py}{fierz_lemma_check.py}), it equals the standard
...
$\kappa=8\pi/M_{\rm Pl}^{2}$ prefactor exactly. This is the explicit Fierz-by-Fierz
