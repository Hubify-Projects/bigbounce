...
\newcommand{\paperVersion}{v1A.0.9}
...
\date{May 9, 2026, 01:30 PDT --- \paperVersion}
...
14 distinct mechanism-class constraints that collectively close every minimal ECH
...
Through 7 foundation studies (Foundations~A--G) and 7 observational research branches (Branches~H--O), we establish 14 distinct mechanism-class
...
substituted, the induced birefringence \emph{rotation rate} is bounded
above by $\dot\beta \lesssim 10^{-21}\,$eV (units of inverse time in
natural units). Converting to the integrated rotation angle accumulated
between recombination and today,
$\Delta\theta_{\rm one\text{-}loop} \sim \dot\beta \cdot t_{\rm rec\to today}
\sim 10^{-21}\,\text{eV} \cdot 4.4\times 10^{17}\,\text{s} \sim
10^{-3}\,\text{rad} \sim 0.06^\circ$, which is comparable in magnitude
to the observed $\beta_{\rm obs} = 0.342^\circ$ and well within the
Planck/ACT~DR6 statistical sensitivity $\sigma(\beta_{\rm obs})\sim 0.094^\circ
\sim 1.6\times 10^{-3}\,\text{rad}$ (see Eq.~\ref{eq:beta_bound} below
for the explicit dimensionless reduction). \emph{The one-loop induced
$\beta$ is therefore at the same order of magnitude as the observed signal
when the parity-odd coupling is set at its one-loop value.} The
distinguishing constraint comes from R4 below, not from the one-loop
amplitude itself: identifying the one-loop sector with the dark-energy
source forces a vacuum energy
$\rho_\theta \sim m_\theta^2\,(\alpha/M)^{-2}\,\beta_{\rm obs}^2 / 2$,
which at $m_\theta\sim H_0$ gives $\rho_\theta \lesssim 10^{-46}\,$eV$^4$,
many orders of magnitude smaller than $\rho_\Lambda \sim 10^{-11}\,$eV$^4$.
The one-loop Holst-sector parity-odd term cannot, therefore, simultaneously
explain the observed birefringence \emph{and} the dark-energy density
at this single operator coupling.
...
amplitude exceeding this bound by approximately
$\log_{10}(\rho_\Lambda/\rho_\theta) = \log_{10}(10^{-11}/10^{-46})
\approx 35$ orders of magnitude in vacuum-energy density,
...
$\rho_\Lambda \sim 10^{-11}\,$eV$^4$ (the ratio of energy densities
is the relevant dimensional comparison; the same constraint is
$\sim 8$ orders of magnitude in operator coupling $\alpha/M$ if the
two are bound separately at their respective fits, but the energy-density
ratio is the load-bearing mismatch between the dark-energy and
birefringence channels). The inequality is rigid because
...
$z>2$, Heinrich \etal~2023~\cite{Heinrich:2023} Sec.~3.4), $b_\phi$ bias-prior uncertainty ($\sigma(b_\phi)/b_\phi\approx 0.2$),
...
is a structural no-go: the 14 distinct mechanism-class constraints (Table~\ref{tab:barriers})
...
7 observational branches established 14 distinct mechanism-class constraints on the minimal ECH
