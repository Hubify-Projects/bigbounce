...
\newcommand{\paperVersion}{v1A.0.13}
...
\date{May 9, 2026, 14:00 PDT --- \paperVersion}
...
mechanisms. Through 7 foundation studies (Foundations~A--G) and 6 observational research branches (Branches~H, J, L, M, N, O) we catalog
14 mechanism-class constraints (one of which, B8, is a special case of B14:
the parity-even-interaction barrier is the observational consequence of the
perturbation-transparency theorem, retained in the catalog for historical
mechanism-class completeness) that collectively close every minimal ECH
...
Through 7 foundation studies (Foundations~A--G) and 6 observational research branches (Branches~H, J, L, M, N, O), we establish 14 distinct mechanism-class
...
i.e.\ the one-loop induced $\beta$ is suppressed by $\sim 58$ orders
of magnitude relative to the observed signal. The R2 amplitude is
...
$\rho_\theta$ either undershoots $\rho_\Lambda$ by 22 orders of
magnitude (at $m_\theta \sim 10^{-22}$ eV) or overshoots it by 14
orders of magnitude (at $m_\theta \sim 10^{-15}$ eV); the
...
$^{\ddagger}$The frozen MCMC posteriors hosted in Paper~I(b) (the three completed dataset combinations enumerated in Paper~I(b) \S~VII Table~III, subsection ``Free-$w_0 w_a$ chain status'') contain zero free-$w_0 w_a$ samples; the $w_0 w_a$ extension was never implemented computationally for any of those frozen runs. A new DESI~DR2 + Planck~NPIPE + Pantheon$+$ + DES-SN5YR cobaya chain with the $w_0 w_a$ free-parameter extension is currently running on Pod~3 H200 ($\sim 109$ samples accepted as of 2026-05-08 18:27 PT, target $\hat R - 1 < 0.01$ still 1--3 days from publication-quality convergence; Paper~I(b) Table~III row ``DESI DR2 w0wa (new)''). Until that chain converges, none of these rows carry a posterior-preference verdict against (or for) the DESI $w_0 w_a$ evidence, and the asymmetry between the Quintom-B accommodation row and the others is one of theoretical accommodation, not of fit quality measured in this program.
\end{table*}
...
$e$-folds; the matter-bounce $\fnl=-35/8$ would plausibly be erased (precise threshold mode-dependent on contracting-phase duration) by $N_{\rm tot}\gg 60$.
