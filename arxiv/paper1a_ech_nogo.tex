...
\newcommand{\paperVersion}{v1A.0.54}
...
T^{abc} = 8\pi G\, S^{abc},
\end{equation}
where $S^{abc} = \frac{1}{4}\bar{\psi}\gamma^{[a}\gamma^{bc]}\psi$.\footnote{Normalization-convention
disclosure: with $\gamma^{bc}\equiv\gamma^{[b}\gamma^{c]}$ this
coefficient ($2\pi G$ on the bilinear
$\bar\psi\gamma^{[a}\gamma^b\gamma^{c]}\psi$) differs by a factor of~2
from the $T^{abc} = (\kappa/2)\,\bar\psi\gamma^{[a}\gamma^b\gamma^{c]}\psi$
($\kappa=8\pi G$) form quoted with the Hehl--Datta term in
Sec.~\ref{sec:r1_njl}; the two reflect different
spin-tensor/antisymmetrization weight conventions in the literature, and
a single-convention re-derivation is left to the operator-level
follow-up. Every amplitude estimate in this paper that descends from the
Cartan equation is an order-of-magnitude statement carrying explicit
$\ge 2$-order conservatism allowances, so an overall factor of~2 in this
normalization does not affect any closure conclusion.}
