...
\newcommand{\paperVersion}{v1A.0.100}
...
\textbf{(b)}~a first-principles ECH-to-$\rho_\Lambda$ derivation---the
dark-energy mapping rests on an on-shell scaling ansatz (off-shell mass
dimension $+1$, not $+4$; App.~\ref{app:dimensions}), and every R4 /
dark-energy claim is conditional on it;
...
The identification $\rho_\Lambda = \Xi\,\MPl^4$
is therefore explicitly a \textit{scaling ansatz}---the three-unit gap is
bridged by an on-shell scaling assumption, not by a controlled EFT
calculation---and is treated as such throughout.
...
densities as a scaling ansatz. The 13 mechanism-class constraints (Sec.~\ref{sec:barriers}) close
...
budget below. It is worth recording precisely what the real
Benedetti--Speziale result is, since it \emph{rigorously bounds} the
running that Eq.~\eqref{eq:gamma_running} only estimates. On-shell
with fermions they obtain (their Eq.~7.24)
\begin{equation}
\mu\,\frac{\partial\gamma^2}{\partial\mu}
= -(\gamma^2-1)^2\,\frac{\mu^2\kappa^2}{(8\pi)^2}\,(23\,\gamma^2+5),
\qquad \kappa^2 = 16\pi G,
\label{eq:bs_beta}
\end{equation}
a non-autonomous flow whose only real fixed point is $\gamma^2=1$ (a UV
fixed point sitting at a divergent four-fermion coupling, outside
perturbative control), with $\gamma=0$ and $\gamma=\infty$ no longer
fixed points once fermions are present. The decisive structural feature
is the explicit factor $\mu^2\kappa^2=(\mu/M_{\rm Pl})^2$ --- the
running is power-suppressed by the renormalization scale in Planck
units, not logarithmic. Consequently the accumulated running
$\int\!\beta_{\gamma^2}\,d\ln\mu\propto\int\mu\,d\mu$ is dominated by
the ultraviolet endpoint and reaches $|\Delta\gamma/\gamma|=\mathcal{O}(0.1$--$1)$
\emph{only} as the cutoff approaches $M_{\rm Pl}$
($\kappa^2\mu_{\rm UV}^2\to\mathcal{O}(16\pi)$); integrating
Eq.~\eqref{eq:bs_beta} numerically over the GUT-to-IR arm
($\mu_{\rm UV}=10^{16}\,$GeV, $\gamma_0=\mathcal{O}(1)$) instead gives
$|\Delta\gamma/\gamma|\sim 10^{-6}$--$10^{-4}$, i.e.\ far \emph{smaller}
than the chiral-count ansatz. The real $\beta$-function is therefore
$|\gamma|$-dependent and scheme-flagged (BS stress the off-shell
pure-gravity running is unphysical), so no single derived
$\Delta\gamma/\gamma$ exists; but it rigorously establishes the bound
$|\Delta\gamma/\gamma|\lesssim\mathcal{O}(0.1$--$1)$ over any
sub-Planckian lever arm, which is what makes the conservative estimate
adopted below a genuine upper bound rather than an arbitrary ansatz
coefficient. In the Standard Model, the chiral
...
\emph{not} a precisely derived value, but one now bounded above by the
real Benedetti--Speziale $\beta$-function structure of
Eq.~\eqref{eq:bs_beta}, whose physical running satisfies
$|\Delta\gamma/\gamma|\lesssim\mathcal{O}(0.1$--$1)$ over any sub-Planckian
lever arm (and is orders of magnitude smaller over the GUT-to-IR arm) ---
...
($\sim 3\times 10^{-62}$ vs $\rho_\Lambda$, using the corrected
$\Delta\gamma/\gamma \sim 0.3$) leaves $\gtrsim 60$ orders of magnitude
...
\par
\textit{Why the ansatz is a conservative bound, not a circular
assumption.}---One might worry that adopting
Eq.~(\ref{eq:onshell_rho}) as an on-shell scaling relation quietly
\emph{assumes} the dark-energy mapping it is then used to constrain,
making the closure circular. It does not, for three reasons. First,
Eq.~(\ref{eq:onshell_rho}) is a \emph{conservative upper bound} on the
bounce-scale parity-odd density: on-shell evaluation at Planck-scale
bounce curvature returns $\rho_\Lambda^{\rm bounce}\sim
10^{-2}\,\MPl^4$, within two orders of magnitude of the largest density
$\sim\MPl^4$ the geometry can support, so no plausible completion of the
dimension-$+1$ operator can \emph{increase} the available amplitude by a
factor that matters against the $\sim\!120$-order hierarchy. Adopting
the largest defensible source density makes the required dilution
$\Dinf$ and the $N_{\rm tot}$ e-fold budget \emph{harder} to satisfy,
not easier; a circular assumption would instead choose the value that
delivers the desired conclusion. Second, the closure conclusion is
insensitive to the ansatz: whether $\rho_\Lambda^{\rm bounce}$ is
$\MPl^4$, $10^{-2}\,\MPl^4$, or $10^{-4}\,\MPl^4$ shifts $N_{\rm tot}$ by
$\mathcal{O}(\text{a few})$, not by orders of magnitude (the \textit{Sharper
dependency statement} below), so the ansatz supplies a bookkeeping
placeholder for an amplitude \emph{ceiling}, not the load-bearing content
of the barriers. Third, and most directly, the R4 closure that this
mapping feeds \emph{makes no positive amplitude derivation at all}: R4
is closed by a naturalness / explanatory-deficit objection
(Sec.~\ref{sec:r4_birefringence}, Tier~II in
Table~\ref{tab:evidentiary_status}), not by an amplitude no-go. A charge
of circularity requires a claimed derivation of the conclusion from an
assumption of that same conclusion; here there is no derived amplitude
to be circular about --- the ansatz bounds a source density from above,
and the closure rests on the observation that minimal ECH supplies
neither $m_\theta\!\sim\!H_0$ nor the fitted $\alpha/M$. The honest
residual limitation is the disclosed one: a controlled dimension-$+4$
operator-basis derivation is left to a companion treatment, and every R4
/ dark-energy claim is stated as conditional on Eq.~(\ref{eq:onshell_rho}).
\par
