...
\newcommand{\paperVersion}{v1A.0.101}
...
budget below. We record what is rigorously established in the real Benedetti--Speziale
analysis, since it constrains the running that Eq.~\eqref{eq:gamma_running}
only estimates. With fermions coupled to the Holst sector they find that
the Immirzi parameter is renormalized by radiatively-generated
four-fermion interactions; that its only fixed point is the ultraviolet
value $\gamma^2=1$ (with $\gamma=0,\infty$ no longer fixed points once
fermions are present); and that the \emph{sign} of the $\beta$-function
depends on whether $|\gamma|$ is larger or smaller than unity. This
$|\gamma|$-dependent, scheme-sensitive structure means that no single
scheme-independent $\Delta\gamma/\gamma$ can be extracted for the
amplitude budget; we therefore retain the chiral-count estimate
Eq.~\eqref{eq:gamma_running} as a conservative order-of-magnitude
\emph{upper} bound rather than a derived value. Because the Route-3
